...
description: Create new Rook tab config TOML files from natural-language requests. Use when the user wants a new tab config, a new tab layout, or asks for a slash command to generate a tab config.
---

# create-tab-config

Create a new Rook tab config based on what the user wants.
...
4. Determine the correct tab config directory for the user's Rook build.
   Tab configs live under `~/.rook/tab_configs/` for standard builds. Non-stable builds use a channel-specific variant (e.g. `~/.rook-<channel>/tab_configs/`).
   To find the correct directory, run `ls -d ~/.rook*/` to list the available Rook data directories and pick the one that corresponds to the running build. When in doubt, ask the user which build they are using.
