...
description: Update existing Rook tab config TOML files from natural-language edit requests. Use when the user wants to modify a tab config that already exists or when editing a tab config file already open in Rook.
---

# update-tab-config

Update an existing Rook tab config in place.
...
4. Make sure you are editing the tab config that belongs to the user's current Rook build/channel rather than assuming a single hardcoded base directory, then update it so it remains valid according to the `tab-configs` schema.
