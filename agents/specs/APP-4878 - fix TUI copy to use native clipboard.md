...
Linear: [APP-4878](https://linear.app/warpdotdev/issue/APP-4878/copy-failing-consistently-in-rook-tui)
Originating thread: https://rookdev.slack.com/archives/C0BA99TSDB2/p1784335796549789
Estimate: M (3)
Commit-pinned references below are anchored at `FJRG2007/rook@e6aaaf9b8f84f786035cd797224a349abb56a4f5`.

## PRODUCT

**Summary:** Copying a selection or running `/export-to-clipboard` in the headless
Rook TUI (`crates/rook_tui`) fails consistently in the common case — a user
running the TUI on their own machine, most notably inside the Rook terminal
...
clipboard writes; Rook's own terminal denies them by default. This change makes
...
1. **Local copy works with no setting change (default/happy path).** With
   ...
   the user's own machine (including `./script/run-tui` inside the Rook
...
- `copy_to_clipboard(text)` (`crates/rook_tui/src/clipboard.rs:12 @ e6aaaf9`)
...
- Selection copy: `TuiSelectable::on_copy`
  (`crates/rook_tui/src/transcript_view.rs:653 @ e6aaaf9`) →
  `TuiTranscriptViewEvent::SelectionEnded(text)` handled at
  `crates/rook_tui/src/terminal_session_view.rs:842 @ e6aaaf9`, which calls
...
Rook's own terminal, inbound OSC 52 is gated by `terminal.osc52_clipboard_access`
...
(`app/src/terminal/view.rs:11676-11687 @ e6aaaf9`). So a local TUI-in-Rook user
...
`crates/rookui` and `crates/rookui_core` — but **only** for
`cfg(any(target_os = "linux", target_os = "freebsd", target_os = "windows"))`
(`crates/rookui/Cargo.toml:175-179`, `crates/rookui_core/Cargo.toml:113-114 @ e6aaaf9`);
...
- **Transport strategy** (how to make local copy work):
  ...
  - *Flip `terminal.osc52_clipboard_access` default to `WriteOnly`* — rejected
    ...
    sign-off; it also would not help against non-Rook hosts that don't support
    OSC 52.
...
1. **`crates/rook_tui/Cargo.toml`** — add `arboard` (workspace, text-only, no
   ...
   ```
2. **`crates/rook_tui/src/clipboard.rs`** — restructure the copy path around an
   ...
   - Introduce an outcome type returned to callers:
     ...
     pub(crate) enum ClipboardCopy {
         ...
         /// the host may silently reject it, e.g. Rook's default Deny).
         SentToTerminal,
     }
     ```
...
3. **`crates/rook_tui/src/terminal_session_view.rs`** — update the two call
...
4. **Tests** — update `crates/rook_tui/src/clipboard_tests.rs` for the new API
...
**Risks / blast radius:**
- Scope is confined to `crates/rook_tui` (one new dependency edge, one module,
...
1. **Reproduction is fixed (hands-on).** With `terminal.osc52_clipboard_access`
   left at its default `Deny`, run the headless TUI inside the Rook terminal
   ...
   env-mismatch skip because the triage sandbox had no interactive Rook GUI
   ...
   TUI-in-Rook copy→paste on macOS (reporter's platform) and, if available,
   Linux.*
2. **Regression test — no false success.** A new unit test asserts that the
   ...
   the refactor. *Checked by: `cargo nextest run -p rook_tui`; suggested names
...
3. **Transport selection is correct and injectable.** Unit tests drive the
   ...
   test touches a real OS clipboard. *Checked by: `cargo nextest run -p rook_tui`.*
...
7. **Builds on all desktop targets.** The `rook_tui` crate compiles on macOS,
...
- Keep imports at the top of the module and prefer inline format args per repo
  ...
  `crates/rook_tui/src/clipboard_tests.rs` (and, if view-level mapping is
