...
Target repository: `FJRG2007/rook`
...
4. **TUI rooking indicator**
   - While a response is in progress, the rooking row includes a right-aligned `▶▶ Auto approve on` or `▶▶ Auto approve off` indicator followed by the existing `Ctrl + C to stop` hint.
...
- The current TUI force-enable behavior is in `crates/rook_tui/src/conversation_selection.rs:43-70,84-101,229-245,258-268`: new conversations pass `true` to `start_new_conversation`, and deferred replacements seed `RunToCompletion`. The existing regression test is `crates/rook_tui/src/conversation_selection_tests.rs:325-354`.
- TUI slash-command dispatch is `crates/rook_tui/src/terminal_session_view.rs:2407-2574`; the command-to-executor mapping must be exhaustive. Existing classifier tests are `app/src/terminal/input/slash_commands/mod_tests.rs`, and TUI menu/render tests are `crates/rook_tui/src/slash_commands_tests.rs`.
...
- **Store display state separately from the conversation override:** rejected. Both the TUI slash-row suffix and rooking indicator derive on/off from `ConversationSelection`; only the rooking indicator's three-second color feedback is transient view state.
...
5. Extend the TUI rooking row with a right-aligned, clickable auto-approve status and register `ctrl+shift+i` for the same toggle. Slash-command and keyboard toggles start or restart a cancelable three-second success-color timer; click toggles clear any active feedback and retain the normal muted/hover style.
...
7. **TUI visual proof:** run the built TUI, open the slash menu, capture `/auto-approve` with its green current-state suffix, execute it, and capture the rooking row's on/off indicator in both its transient green and normal muted states. Verify `ctrl+shift+i` toggles with the same transient feedback and clicking the indicator toggles without it. Attach the screenshot evidence to the task/PR; do not commit media.
