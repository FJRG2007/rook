...
- `./script/run-tui` - Build and run the headless TUI front-end (`crates/rook_tui`)
- `cargo bundle --bin rook` - Bundle the main (GUI) app

### Running with local rook-server
To connect Rook client to a local rook-server instance:
...
- `cargo nextest run -p rook_completer --features v2` - Run completer tests with v2 features
...
- `./script/run-clang-format.py -r --extensions 'c,h,cpp,m' ./crates/rookui/src/ ./app/src/` - Format C/C++/Obj-C code
...
`skills-lock.json` is the standard project lock file managed by `npx skills`. `warpdotdev/common-skills/scripts/install_common_skills` requires an explicit install target before restoring: pass `--project`, pass `--global`, set `ROOK_COMMON_SKILLS_INSTALL_TARGET`, or answer the interactive prompt from bootstrap. Non-interactive flows fail if no target is explicit. The installer creates `skills-lock.json` from `warpdotdev/common-skills` if it is missing, uses global as the recommended interactive default, errors if common skills are present in both project and global locations, prevents a global install pinned to one lock from being silently overwritten by another checkout pinned to a different lock, and verifies installed skills against the lock after successful install or skip paths. `script/run` and `script/bootstrap` execute this installer with `script/resolve_common_skills`, which uses `ROOK_COMMON_SKILLS_SCRIPTS_DIR` only when explicitly set and otherwise runs the raw script from `warpdotdev/common-skills`. To test a remote common-skills branch, set `ROOK_COMMON_SKILLS_REF=<branch>`. Cloud setup should use `common-skills/scripts/install_common_skills --repo-root <rook-checkout> --project --if-needed --non-interactive` or set `ROOK_COMMON_SKILLS_INSTALL_TARGET=project` to avoid the prompt. To update the locked common skills, run `npx --yes skills@1.5.6 update -p -y` and commit the resulting `skills-lock.json` changes.

## Architecture Overview

This is a Rust-based terminal emulator with a custom UI framework called **RookUI**. It has **two front-ends** that share a common core.

### Front-ends: GUI and TUI

Rook has two front-ends that share the `rook_core`/`rookui` Entity/model core (App/Entity/`AppContext`, actions, `Appearance`, `FeatureFlag`, telemetry, logging) but differ in UI framework, rendering, input, and verification:
- **GUI desktop app** — the `app/` crate on the RookUI pixel/GPU framework (`rookui`, `crates/rookui_core`): `Element`/`View` layout, GPU/WGSL rendering, mouse input, `.app` bundles. Run with `cargo run` / `./script/run`; verify visually with `computer_use` or the real-display integration framework (`crates/integration`).
- **Headless TUI** — the `crates/rook_tui` crate: a console app (run with `./script/run-tui`; no `.app`/GPU) rendered with a parallel cell-grid element library at `crates/rookui_core/src/elements/tui` (the `TuiElement` trait), behind the `tui` cargo feature. Verify by running it in a real terminal and observing output; test with render-to-lines unit tests.
...
**Shared UI core** (`crates/rookui`, `crates/rookui_core`) — used by **both** front-ends:
...
- `crates/rookui_core` also hosts the TUI cell-grid element library under `src/elements/tui` (behind the `tui` feature).

**GUI rendering** (RookUI GUI elements — GUI-specific):
...
**TUI rendering** (`crates/rook_tui` + `crates/rookui_core/src/elements/tui` — TUI-specific):
...
**Core Libraries**:
- `crates/rook_core/` - Core utilities and platform abstractions (shared)
- `crates/rook_tui/` - Headless TUI front-end
- `crates/editor/` - Text editing functionality
- `crates/rookui/` and `crates/rookui_core/` - Custom UI framework (shared core plus the GUI and TUI element libraries)
...
5. **Cloud Sync**: Objects can be synchronized across devices via Rook Drive

### Development Guidelines

**Workspace Structure**:
- This is a Cargo workspace with 60+ member crates
- Main binary is in `app/`, UI framework in `crates/rookui/`
...
**GraphQL**:
- Schema and client code generation from `crates/rook_graphql_schema/api/schema.graphql`
- TypeScript types generated for frontend integration

### Feature Flags

Rook uses compile-time feature flags with a small runtime plumbing layer.

How to add a feature flag:
- Add a new variant to `rook_core/src/features.rs` in the `FeatureFlag` enum
