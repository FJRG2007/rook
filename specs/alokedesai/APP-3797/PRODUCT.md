...
Replace the current SSH wrapper's ControlMaster-based `RemoteCommandExecutor` with a persistent remote server binary (`rook-remote-server`) that runs on the remote machine. After the remote shell sends `InitShell`, Rook checks for the binary at `~/.rook/rook-remote-server`, installs it if missing, launches it, and performs a protobuf Initialize handshake over stdin/stdout. The session is fully bootstrapped only after both the remote server is initialized and the shell `Bootstrapped` hook has been received.
...
- Install the `rook-remote-server` binary on the remote machine automatically when it is not present.
- Detect the remote OS (Linux or macOS) and architecture (x86_64 or aarch64) to download the correct binary.
- Show clear, stage-specific status messages in the Rook input during installation and initialization.
...
Throughout the flow, the Rook input prompt area displays a status message (bold, in the same style as "Starting shell..."). The messages are:

1. **"Starting shell..."** — shown immediately after `InitShell` (same as today).
2. **"Installing Rook SSH tools... (X%)"** — shown while the binary is being downloaded and installed on the remote machine. Replaces "Starting shell..." once installation begins. The percentage reflects download progress reported by `curl`/`wget`. If progress cannot be determined, show **"Installing Rook SSH tools..."** without a percentage.
...
1. **Check for existing binary.** Run a command over the existing SSH ControlMaster socket to check if `~/.rook/rook-remote-server` exists and is executable on the remote machine (e.g. `test -x ~/.rook/rook-remote-server && ~/.rook/rook-remote-server --version`).
...
2. **If the binary is not present or not functional:**
   ...
   b. Download the Oz CLI tarball from the Rook server's `/download/cli` endpoint, using the detected OS and architecture. The endpoint accepts query parameters `os` (`macos` or `linux`), `arch` (`x86_64` or `aarch64`), `package` (`tar`), and `channel` (matching the current client channel). The endpoint returns a 302 redirect to the releases CDN (e.g. `https://releases.rook.dev/{channel}/{version}/cli/{os}/{arch}/rook-{channel}-{os}-{arch}.tar.gz`). The download is performed on the remote machine using `curl -fL` (preferred) or `wget` (fallback) via the SSH ControlMaster socket.
   c. Extract the Oz CLI binary from the tarball to `~/.rook/rook-remote-server` and set executable permissions (`chmod 755`).
   d. During this process, the input prompt shows **"Installing Rook SSH tools... (X%)"** with download progress when available, or **"Installing Rook SSH tools..."** without percentage if progress reporting is unavailable.
...
1. Launch `~/.rook/rook-remote-server` on the remote machine over the SSH ControlMaster socket. The process's stdin/stdout are used for communication.
...
- **Installation failure (download fails, extraction fails, unsupported platform):** The input prompt should show an error message (e.g. "Failed to install Rook SSH tools"). The session should fall back to the existing ControlMaster-based `RemoteCommandExecutor` so the user can still use the SSH session with reduced functionality. Log the error for diagnostics.
...
When the SSH session ends (user types `exit` or the connection drops), the remote server process should be terminated. No special cleanup of `~/.rook/rook-remote-server` is needed — the binary remains installed for future sessions.

## Success criteria

1. When the feature flag is enabled and a user SSHs into a Linux or macOS remote host that does not have the binary installed, the binary is automatically downloaded and installed at `~/.rook/rook-remote-server` without user intervention.
...
3. During installation, the input prompt displays "Installing Rook SSH tools..." instead of "Starting shell...".
...
- **Manual testing:** SSH into a fresh Linux VM and a fresh macOS remote. Verify the binary is downloaded, installed, launched, and the Initialize handshake completes. Verify the prompt messages transition correctly: "Starting shell..." → "Installing Rook SSH tools..." → "Initializing..." → working directory.
