...
- `model_snapshot` — three catalog branches: Oz/unset uses the Rook LLM catalog
...
- `host_snapshot` — workspace default (badged `Default`), `rook`, connected worker
...
- Commands: `cargo check -p rook`, `cargo check -p rook --features tui`,
