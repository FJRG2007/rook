...
2. Each log item is a single formatted entry containing a timestamp plus the debug-formatted request or response, matching the format previously written to `rook_network.log`.
...
5. Triggering the `input:insert_network_logging_workflow` keybinding ("Show Rook network log") opens the same pane with the same layout behavior.
...
14. The previously available "Tail Rook network log" workflow is removed from the command palette and workflow list. Users will no longer find a workflow that runs `tail -f` on a network log file.
