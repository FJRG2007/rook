...
## Summary
Mermaid diagrams in rendered markdown surfaces should never leave users staring at an indefinite “Rendering Mermaid diagram…” placeholder. When rendering fails or remains pending too long, Rook replaces the loading state with a clear failure callout while preserving the underlying markdown source.
## Problem
...
## Behavior
1. When a rendered markdown surface contains a fenced Mermaid code block and Mermaid rendering is enabled, Rook initially shows the existing pending state: “Rendering Mermaid diagram…”.
...
3. When a Mermaid render attempt enters the failure state, Rook replaces “Rendering Mermaid diagram…” with a visible callout that says “Failed to render Mermaid diagram”.
...
5. The failure callout is compact enough that a failed diagram does not reserve the large default placeholder height when Rook can determine the render has permanently failed. If the failure is only a UI timeout while the underlying render is still unresolved, the callout must still be visible in the existing placeholder area.
6. A successfully rendered Mermaid diagram continues to display as the rendered SVG, not as a callout.
7. If a render attempt times out but later resolves successfully, Rook replaces the timeout callout with the successfully rendered diagram. Rook must not switch back to the loading placeholder for the same unresolved render attempt.
8. If the underlying Mermaid source changes, Rook treats the new source as a new render attempt: the previous success, failure, or timeout state does not permanently carry over to the changed diagram.
