...
A new modal ("Create your default tab config") appears after onboarding completes, overlayed on the terminal. It collects three inputs — session type, directory, and worktree preference — then creates a persistent tab config TOML in `~/.rook/tab_configs/` and opens it in the current tab.
...
- Always persist the configuration as a reusable tab config TOML in `~/.rook/tab_configs/`.
...
The modal appears once, immediately after the user completes the onboarding slide flow. It only appears when `OpenRookNewSettingsModes` is enabled — this is the new onboarding path. Users on the old onboarding flow never see this modal. It is rendered as a centered overlay on top of the terminal workspace (not as a full-screen onboarding slide). The user cannot interact with the terminal behind it while the modal is open.
...
- **"Get rooking" button:** Primary action button with Enter keyboard shortcut.

### Session type behavior

Each session type determines what happens when the user clicks "Get rooking":
...
Clicking "Get rooking" always saves a tab config and opens it:

1. Writes a new TOML file to `~/.rook/tab_configs/`. The file is named `startup_config.toml` (or `startup_config_1.toml`, `startup_config_2.toml`, etc. if the name is taken).
...
- **Enter:** Activates "Get rooking" (same as clicking the button).
...
- Clicking "Get rooking" (takes action).
...
2. Selecting "Terminal" + a directory + "Get rooking" writes a tab config TOML to `~/.rook/tab_configs/` and replaces the current tab with a session in that directory.
3. Selecting a CLI agent + a directory + "Get rooking" writes a tab config TOML, replaces the current tab, sets the working directory, and auto-runs the agent CLI command.
4. Selecting "Built in agent" + a directory + "Get rooking" writes a tab config TOML, replaces the current tab, and opens Oz agent view in that directory.
