...
- `crates/rook_features/src/lib.rs`: add `QueuedPromptsV2` to the `FeatureFlag` enum, alongside the existing `QueueSlashCommand` entry. Add the variant to `DOGFOOD_FLAGS`.
...
- **§1, §2 (feature gating)**: compile both with and without the cargo feature (`cargo check -p rook` and `cargo check -p rook --features queued_prompts_v2`); unit-test that all new helpers no-op when V2 is off.
