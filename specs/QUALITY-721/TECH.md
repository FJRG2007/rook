...
## Context
Orchestrated child agents need to read parent-created plans when explicitly given a plan ID. Plan availability is backed by Rook Drive rather than inherited launch metadata or copied markdown.
## Architecture
...
- Unbacked plans start Rook Drive creation.
...
`ReadDocumentsExecutor` first reads requested IDs from the local `AIDocumentModel`. If a requested plan is absent, it attempts Rook Drive hydration only when the acting conversation participates in orchestration:
...
Hydration uses `AIDocumentModel::hydrate_saved_plan_from_rook_drive`, then retries the complete read so result ordering and all-or-nothing behavior remain unchanged. Missing IDs in non-orchestrated conversations, or IDs still missing after hydration, return the existing clear `Document(s) not found` error.
...
- saving-plan publication refreshes content edited after Rook Drive creation began;
...
Rook Drive creation or update can be slow or unavailable.
