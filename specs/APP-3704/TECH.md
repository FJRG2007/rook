...
5. `write_tab_config` serializes to TOML and writes to `~/.rook/tab_configs/my_tab_config.toml`.
...
- Build and run Rook, create a split layout, right-click → "Save as new config", verify the TOML, open from `+` menu.
