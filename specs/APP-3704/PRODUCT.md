...
Add a "Save as new config" item to the tab right-click context menu that snapshots the current tab's pane layout (splits, working directories, focus, and color) and writes it as a new tab config TOML file in `~/.rook/tab_configs/`.
...
1. Rook snapshots the tab's pane tree: split directions, each pane's current working directory, focus state, and tab color.
2. Rook writes a new `.toml` file to `~/.rook/tab_configs/` with an auto-generated filename (`my_tab_config.toml`, or `my_tab_config_1.toml` if the name is taken).
3. Rook opens the file in the user's configured editor (same behavior as "Create new tab config...").
