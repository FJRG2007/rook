...
- `script/deploy_remote_server` is the developer escape hatch: it `rsync`s a locally-built binary into `~/.rook-local/remote-server/oz-local` and assumes the client won't try to download on top of it.
...
`rook-server`'s `/download/cli` already honours a `version=` query parameter (pins the redirect to the exact versioned artifact when present; falls back to latest-for-channel when absent), so no server-side change is needed.
...
`script/deploy_remote_server` is not touched; its target path (`~/.rook-local/remote-server/oz-local` on the local channel) is exactly what the `Channel::Local` branch of `remote_server_binary()` returns.
