# PRODUCT.md — Rename Oz to Rook Agent in settings and onboarding

Issue: https://github.com/warpdotdev/warp-external/issues/1063

## Summary

The in-app agent is being renamed from "Oz" to "Rook Agent" in the settings page
...
1. On the settings sidebar, under the "Agents" umbrella, the first subpage entry
   is labeled "Rook Agent" (previously "Oz"). The ordering of subpages, the
   umbrella name ("Agents"), and all sibling subpages ("Profiles", "MCP
   servers", "Knowledge", "Third party CLI agents") are unchanged.

2. Opening that "Rook Agent" subpage renders the same settings content it did
   before this rename. No widgets, toggles, options, defaults, or telemetry
   change behavior as a result of this rename.

3. On the "Rook Agent" subpage, the primary page heading that sits above the
   global AI enable toggle reads "Rook Agent" (previously "Oz"). Typography,
...
5. On the onboarding agent slide (the third onboarding step), the title reads
   "Customize your Rook Agent" (previously "Customize your Agent, Oz"). The
...
6. When the "new settings modes" feature flag is enabled and the slide renders
   the disable checkbox row, the checkbox label reads "Disable Rook Agent"
...
7. Search within settings still finds the in-app agent subpage when the user
   ...
   command", "api keys"). Additionally, typing "rook agent" finds this subpage.
...
8. Deep links and external callers that previously navigated to this subpage by
   ...
   the renamed "Rook Agent" subpage. Navigation requests referencing "Rook
...
11. Accessibility: the settings sidebar item, the page heading, and the
    onboarding checkbox label expose the new visible text ("Rook Agent",
    "Disable Rook Agent") to assistive technologies. No separate aria-only label
...
12. The rename applies consistently across themes, appearances, and the new
    ...
    flag states remains whether the "Disable Rook Agent" row renders at all; its
