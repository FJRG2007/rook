...
The orchestration UI (orchestrate confirmation card and plan-card orchestration block) hosts a row of pickers — model, harness, environment — that drive child agent dispatch. Until now there was no UI to choose the worker host: the dispatched `RunAgents` request always carried `worker_host = "rook"`, which routes to the default Rook cluster. Customers running self-hosted workers had no way to target them from the desktop client; the Oz webapp's host selector is the only existing entry point.
...
**Reference**
- `client/packages/agents/src/components/HostSelector.tsx` (rook-server) — the webapp's host selector.
...
- **List mode** wraps an inner `Dropdown<InternalAction>` styled identically to the other orchestration pickers (`picker_styles()`). The menu is populated with the workspace default (badged "Default"), `rook`, the most-recent custom slug, and a "Custom host…" entry, in that order. Selecting any known item dispatches `InternalAction::SelectKnown(slug)`; selecting "Custom host…" dispatches `InternalAction::EnterCustomMode`.
...
- When the user types `rook` into custom mode, `commit_custom` collapses back to the standard `rook` selection rather than persisting `rook` as a custom value. This avoids the asymmetric case where `current_slug` is a casing variant of `rook` that doesn't match any menu label.
...
`OrchestrationPickerHandles` gets a new `host_picker: Option<ViewHandle<HostPicker>>` field. `sync_picker_selections` is taught to call `picker.set_selected(...)` with the current `worker_host` whenever the edit state changes; this handles both initial population and subsequent changes from other pickers (e.g. mode toggle resetting host to `rook`).

Four new free functions in `orchestration_controls.rs`:

- `populate_host_picker(picker, initial_host, ctx)` — reads the workspace default and recent slug, calls `picker.set_options(...)`, then `picker.set_selected(initial_host)`. Empty input falls back to `rook`. Used by both card views during `ensure_pickers`.

- `resolve_default_host_slug(ctx) -> Option<String>` — returns the workspace default slug, honoring the developer-only `ROOK_CLOUD_MODE_DEFAULT_HOST` env var override, otherwise reading from `UserWorkspaces::default_host_slug()`. Mirrors the single-agent ambient flow.

- `resolve_recent_host_slug(ctx) -> Option<String>` — returns the persisted last-selected custom slug, deduplicated against `rook` and the workspace default (so the menu doesn't show a duplicate row).

- `persist_host_selection(worker_host, ctx)` — writes the slug to `CloudAgentSettings.last_selected_host`. Skipped for empty values and for `rook` so those never become "recent" entries.

Both card views also pre-fill defaults when restoring a Remote config with an empty host: prefer the workspace default over the bare `rook` fallback so self-hosted teams see their default pre-selected, matching the Oz webapp.
...
The `worker_host` field already exists on `OrchestrationEditState` and on `RunAgentsExecutionMode::Remote`, so no downstream changes (dispatch, server marshalling, auto-launch matching) are needed. The previously-hardcoded `"rook"` value flows through the same code paths as any user-selected slug.
...
- `build_menu_items` with no default and no recent → only `rook` + `Custom host…`. (Behavior 4)
- `build_menu_items` with default set → default row first, badged; then `rook`; then `Custom host…`. (Behavior 4)
- `build_menu_items` with recent set → `rook` first; then recent as plain slug; then `Custom host…`. (Behavior 4)
- `build_menu_items` dedups when recent equals default. (Behavior 5)
- `build_menu_items` dedups when recent equals `rook`. (Behavior 5)
- `build_menu_items` rook entry dispatches `SelectKnown("rook")`. (Behavior 6)
...
- `menu_label_for` returns plain slug for `rook`. (Behavior 6)
- `menu_label_for` returns plain slug for an unknown value (custom-mode display). (Behavior 15)
- `normalize_slug` trims whitespace and falls back to `rook` on empty input. (Behavior 14)
...
- **Behavior 4, 5, 6**: With and without `defaultHostSlug` configured (toggle via SQL on the local `organization_settings` table, or via `ROOK_CLOUD_MODE_DEFAULT_HOST`), verify the dropdown contents and ordering, the "Default" badge, and the badge appearing in the collapsed top bar.
- **Behavior 8, 9, 10, 11, 12**: Open custom mode, verify the editor is pre-filled and focused; type a slug, press Enter; reopen the menu and verify the slug now appears as a recent entry. Repeat with Escape and with the cancel button. Try committing an empty buffer and the literal string `rook` / `ROOK`.
...
- **Behavior 15**: Use the developer override (`ROOK_CLOUD_MODE_DEFAULT_HOST=some-unknown-slug`) and verify the picker boots into custom mode pre-filled with the slug.
...
- **Behavior 19**: Pick a non-`rook` slug, dispatch the agents, and verify the worker-host slug reaches the worker. End-to-end smoke against a local Oz stack with a self-hosted worker registered as `local-dev` is the canonical check; worker logs show `task_claimed worker_id:"local-dev"` when the custom slug is routed correctly.
