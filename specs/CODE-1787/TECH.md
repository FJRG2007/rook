...
## Context
Two independent bugs prevent quake mode from working correctly on Windows when triggered while a non-Rook application has foreground focus.

### Bug 1: focus not transferred
`WinitWindow::focus()` in `crates/rookui/src/windowing/winit/window.rs:1080` had two branches: if the window was already visible it called `focus_window()`, otherwise it called `set_visible(true)` and relied on visibility implying focus. On Windows, `set_visible(true)` does not steal foreground focus from another application — an explicit `SetForegroundWindow` (via winit's `focus_window()`) is required. The quake window was hidden via `set_visible(false)`, so re-showing it always took the `set_visible(true)` branch and never called `focus_window()`.

### Bug 2: incorrect window size
All Windows monitor queries in `crates/rookui/src/windowing/winit/window/windows_wm.rs` routed through `get_active_window_handle()`, which requires a focused + visible Rook window. When no Rook window has focus, this fails and `active_display_bounds()` falls back to a hardcoded `DEFAULT_WINDOW_SIZE` (1280×800). The quake window is then sized as a percentage of that default instead of the actual display dimensions.

### Relevant code
- `crates/rookui/src/windowing/winit/window.rs:1080-1092` — `WinitWindow::focus()`
- `crates/rookui/src/windowing/winit/window/windows_wm.rs` — all Windows monitor query methods
- `crates/rookui/src/windowing/winit/window.rs:222-227` — `WindowManager::show_window_and_focus_app` (calls `focus()`)
...
**Active-monitor queries** (need to know which monitor the user is on): `get_active_monitor`, `get_current_monitor_id`, `get_active_monitor_logical_bounds`. When a Rook window has focus, use its `current_monitor()`. When no Rook window has focus, fall back to `get_foreground_monitor()`, which uses Win32 `GetForegroundWindow` + `MonitorFromWindow(hwnd, MONITOR_DEFAULTTONEAREST)` to find the monitor of the app that has keyboard focus. This is the window that triggered the global hotkey.
...
Enable `Win32_Graphics_Gdi` (for `MonitorFromWindow`, `MONITOR_DEFAULTTONEAREST`) and `Win32_UI_WindowsAndMessaging` (for `GetForegroundWindow`) in `crates/rookui/Cargo.toml`.

## Testing and validation
- Manual: configure quake mode with 100% width, focus a non-Rook app, press the hotkey. Verify the quake window receives focus and spans the full display width. (Behavior 1, 3)
- Manual: with a Rook window focused, press the hotkey. Verify existing behavior is preserved. (Behavior 2)
