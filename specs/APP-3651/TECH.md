...
New structure:
1. **Primary row**: `Flex::row` with kind icon (12px) + title text. For code panes, resolve the icon via `crate::code::icon_from_file_path` on the title string (which is the file path), falling back to `RookIcon::Code2.to_rookui_icon(sub_text_color)`. For other types, use `TypedPane::icon().to_rookui_icon(sub_text_color)`.
...
- Change icon from `chevron_icon` (`ChevronDown`/`ChevronRight`) to a constant `RookIcon::X` (or `UiIcon::X`).
