...
Add one editable, keyless-by-default action that advances the active tab through Rook's existing tab colors. The action gives users a keyboard-only alternative to the tab color picker and `/set-tab-color`, including when a terminal-hosted chat tool consumes slash-prefixed input itself.
...
- Adding this GUI tab-color action to Rook's headless TUI.

## Behavior

1. Rook exposes one action named **Cycle current tab color** in Settings → Keyboard Shortcuts and in the Command Palette whenever workspace actions are available.
...
4. Repeated invocations continue around the cycle: after Cyan becomes no color, the next invocation sets Red. Rook does not open a picker or require a second confirmation at any point in the cycle.
...
12. If no active tab exists when the action is dispatched, Rook safely leaves the workspace unchanged and shows no error.

13. The color indicator uses the active Rook theme's existing ANSI tab-color values. The action does not introduce hard-coded RGB values or otherwise change how tab colors are rendered.
