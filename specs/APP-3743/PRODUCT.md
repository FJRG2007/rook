...
- Introduce a default worktree tab config at `~/.rook/default-tab-configs/` that is parameterized by repo and auto-generates the branch name.
...
5. **[User tab configs]** — One item per loaded tab config from `~/.rook/tab_configs/`. Icon: `LayoutAlt01` for non-worktree configs, `Dataflow02` for worktree configs. (Same detection logic as APP-3578.)
6. **Separator**.
...
**Clicking a repo**:
1. The system loads the default worktree tab config from `~/.rook/default-tab-configs/worktree.toml`.
...
A new directory `~/.rook/default-tab-configs/` stores built-in default tab configs that ship with Rook (distinct from user-created configs in `~/.rook/tab_configs/`).

The default worktree config at `~/.rook/default-tab-configs/worktree.toml`:
...
If this file does not exist at `~/.rook/default-tab-configs/worktree.toml`, it is created on first use from an embedded template (similar to how `new_tab_config_template.toml` works). The file is user-editable — users can customize the worktree commands, add additional panes, etc. Rook does not overwrite user modifications on updates.

### New Tab Config menu item

Clicking "New Tab Config" in the menu writes the starter tab-config template to the next unused file under `~/.rook/tab_configs/` and opens it in the user's configured editor. The filesystem watcher then picks it up and it appears in the menu once saved.
...
2. **Default worktree config missing**: If `~/.rook/default-tab-configs/worktree.toml` doesn't exist, it is created from an embedded template on first invocation.
...
6. The default worktree tab config exists at `~/.rook/default-tab-configs/worktree.toml` and is created from a template if missing.
7. "New Tab Config" creates and opens the starter template under `~/.rook/tab_configs/`.
