...
- Expanding this ticket to every image-rendering surface in Rook outside the markdown and notebook viewer.
...
- Rendered markdown files and notebook markdown content that use Rook's existing markdown image blocks.
...
- Rook must not require multi-GB memory usage to display an animated image source in markdown.
- Rook should do only the work required to show a static first-frame preview rather than decode, cache, or resize the full animation.
...
- If Rook can decode enough of an animated image to extract a first visible frame, it should show that static preview.
...
- Preview-only handling is silent in this iteration. Rook does not add a warning label or toast merely because the source asset is animated.
...
- Rook does not rewrite the source markdown. The authored `![](...)` reference remains unchanged.
...
2. The affected markdown document remains readable instead of causing Rook to freeze, crash, or become unusably slow.
...
- Open the Petra repro markdown document associated with `APP-3857` and verify that Rook remains usable while the document is opened and scrolled.
...
- In a future iteration, should Rook expose opt-in playback for animated images in markdown once a memory-safe animation path exists?
