...
- Live shared-session viewer join (from `rook://shared_session/{id}`, management view `OpenAmbientAgentSession`, or conversation list `OpenAmbientAgentSession`).
- Transcript viewer load (from `rook://conversation/{id}` or management / conversation-list `OpenConversationTranscriptViewer`).
...
The `CloudConversationData::CLIAgent` branch maps `AIAgentHarness` → `rook_cli::agent::Harness`, restores the block snapshot, calls `set_harness` to keep the viewer's `AmbientAgentViewModel::harness` in sync with the loaded run, calls `enter_agent_view_for_new_conversation(None, ThirdPartyCloudAgent, ctx)`, and then calls `attach_non_startup_blocks_to_conversation(vehicle_conversation_id)` to retag the snapshot block. All four steps run inside a single `terminal_view.update` closure so the flow reads linearly.
...
- **Claude / Gemini, live, shared session link** — start a run on one client, join from another via `rook://shared_session/{id}`. Confirm the joining client lands in agent view.
...
- **Claude / Gemini, completed, conversation link** — same via `rook://conversation/{id}`.
