...
GitHub issue: https://github.com/FJRG2007/rook/issues/10090
Figma: none provided

## Summary
Rook should notice when Git remote-tracking refs stored on disk change for a repository that Rook is already watching. When the changed remote ref is the upstream ref tracked by a watched repository's current branch, Rook refreshes that repository's Git metadata so code review and Git operations UI reflects the new push/fetch state without requiring a manual reload or another unrelated filesystem change.

The first user-visible outcome is that the unpushed commit computation updates after a user pushes a branch. If a successful push updates `.git/refs/remotes/<remote>/<branch>`, Rook should recompute the current branch metadata and stop showing those commits as unpushed.

## Problem
Rook's repository watcher already reacts to local working-tree changes and selected Git metadata changes such as `HEAD`, local branch refs, and `index.lock`. Remote-tracking refs under `.git/refs/remotes/*` are ignored today. As a result, a push or fetch that updates a loose remote ref can leave Rook's cached code review metadata stale. The user may still see a Push action, an outdated unpushed commit list, or stale branch comparison metadata even though Git has already updated the remote-tracking ref on disk.

Users expect Rook's Git UI to track Git state changes that happen through Rook or through external commands in the same repository. Remote refs are part of that local Git state when they are stored as loose refs.

## Goals
- Detect loose remote-tracking ref changes under `.git/refs/remotes/<remote>/<branch>` for repositories already watched by Rook's repo watcher.
...
- Refreshing metadata for non-current branches that happen to track the changed remote ref. Rook's current Git metadata is scoped to the repository's active branch.
...
## User experience
1. When a watched repository is on a branch with upstream `origin/feature` and Git updates `.git/refs/remotes/origin/feature`, Rook refreshes that repository's Git metadata after the existing watcher debounce and metadata throttling intervals.
...
3. If the branch still has commits ahead of its upstream after the remote ref update, Rook continues to show those commits as unpushed.
4. If the remote-tracking ref update creates the upstream ref for a newly published branch, Rook refreshes metadata for the repository that now tracks that ref and the primary action can advance from Publish/Push to the next appropriate state.
5. If a different remote ref changes, for example `.git/refs/remotes/origin/main` while the current branch tracks `origin/feature`, Rook does not refresh the feature branch's repository metadata solely because of that event.
...
10. Remote refs stored only in packed refs are ignored. If a push or fetch updates only `.git/packed-refs`, Rook may remain stale until another existing refresh trigger occurs.
...
12. Filesystem errors, missing refs, and racing Git updates are handled silently in the same style as existing watcher events. A failed attempt to classify a remote ref should not crash Rook or invalidate unrelated repositories.

## Success criteria
1. In a watched repository on a branch tracking `origin/<branch>`, pushing commits so that `.git/refs/remotes/origin/<branch>` updates causes Rook to refresh metadata and remove those commits from the unpushed commit list.
...
- Manually validate from an external terminal command that updates the same repository while Rook is running.
