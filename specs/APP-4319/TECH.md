...
Feature flags are defined in `crates/rook_features/src/lib.rs`. `CloudModeSetupV2` already exists near the end of the enum at `crates/rook_features/src/lib.rs:827`, and the rollout arrays are defined at `crates/rook_features/src/lib.rs:852`, `crates/rook_features/src/lib.rs:910`, and `crates/rook_features/src/lib.rs:926`. There is currently no `HandoffCloudCloud` flag. The Cargo feature graph in `app/Cargo.toml` is the right place to encode that `handoff_cloud_cloud` depends on `cloud_mode_setup_v2`.
...
The server follow-up API is `POST /api/v1/agent/runs/{runId}/followups` with a JSON body containing `message`. The server route and request type are in the server worktree at `/Users/zachbai/.rook-dev/worktrees/rook-server/cloud-agent-task-name/public_api/openapi.yaml:644`, `/Users/zachbai/.rook-dev/worktrees/rook-server/cloud-agent-task-name/router/handlers/public_api/agent_webhooks.go:181`, `/Users/zachbai/.rook-dev/worktrees/rook-server/cloud-agent-task-name/router/handlers/public_api/agent_webhooks.go:608`, and `/Users/zachbai/.rook-dev/worktrees/rook-server/cloud-agent-task-name/public_api/types/types.gen.go:1225`.
...
Add `HandoffCloudCloud` to `FeatureFlag` in `crates/rook_features/src/lib.rs`, preserving the enum's chronological ordering. Keep the description concise and product-focused, for example gating cloud-to-cloud continuation of cloud mode conversations.
...
`cargo nextest run -p rook server::server_api::ai::tests::build_run_followup_url_routes_to_run_followups server::server_api::ai::tests::serialize_run_followup_request ai::ambient_agents::spawn::tests`
`cargo nextest run -p rook ai::agent_conversations_model::tests ai::conversation_details_panel::tests`
`cargo check -p rook --features handoff_cloud_cloud`
