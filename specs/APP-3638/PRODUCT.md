...
## Problem
When users compose prompts for CLI agents through Rook's rich input (Ctrl-G or the Compose button), they cannot browse or insert saved prompts. The normal Rook agent input supports prompts, but the CLI agent input does not. Users must manually type or paste prompt content.
...
## Non-goals
- Changing how prompts work in the normal Rook agent input.
...
### Trigger
User selects a prompt from the prompts menu, same as in normal Rook input.
