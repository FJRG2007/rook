...
## Summary
Users sharing a live Rook session should be able to show a QR code for the same session URL that appears in the live-session sharing dialog. The QR code makes it easy for someone nearby to join from another device without manually copying or typing the link.
## Problem
...
- Adding QR codes for non-session shareable objects such as notebooks, workflows, Rook Drive objects, or AI conversations.
- Adding analytics, expiration, or tracking parameters to the QR URL beyond what the canonical session URL already contains.
## Behavior
1. When Rook starts a shared terminal session and copies the remote-control link automatically, the toast reads `Remote control link copied.` and includes a `View QR code` link.
2. Activating `View QR code` opens the QR-code view for that newly-started shared session directly in the pane-header sharing overlay. Rook does not require the user to open the access-management panel first. Once that overlay opens, the toast may expire or be dismissed independently, but that must not dismiss, navigate away from, or otherwise change the QR-code view.
...
4. If the live-session sharing overlay is already open when the toast action is activated, Rook transitions that overlay into the QR-code view instead of creating a second overlay.
...
17. If the session ends while the QR-code view is open, the QR view keeps encoding the same URL that the sharing dialog would copy for the session. If Rook can no longer provide a session URL, the QR view closes or shows a non-scannable error state instead of displaying stale or empty data.
...
20. When the QR-code view copy action succeeds, Rook shows the existing link-copy success feedback.
21. The QR-code view copy action does not attempt to write QR image data to the clipboard.
22. The download icon in the QR-code view opens the platform save-file flow for a PNG file. The default filename is recognizable as a Rook session QR code and includes the session id when available.
23. If the user cancels the save-file flow, Rook leaves the QR-code view open and does not show an error.
24. If PNG generation or file writing fails, Rook leaves the QR-code view open and shows a concise failure toast.
