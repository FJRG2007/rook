...
The TUI prompt input ([`crates/rook_tui/src/input/view.rs @ 51145bb7`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/input/view.rs)) is a `TuiInputView` backed by a char-cell `CodeEditorModel`. Keystrokes map to `TuiInputAction`s in `TuiInputElement::dispatch_event` ([view.rs (940-1022)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/input/view.rs#L940-L1022)); Enter emits `TuiInputViewEvent::Submitted` and clears the buffer ([view.rs (492-497)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/input/view.rs#L492-L497)).

`TuiTerminalSessionView` ([`crates/rook_tui/src/terminal_session_view.rs @ 51145bb7`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/terminal_session_view.rs)) routes every `Submitted` to `send_prompt` ([terminal_session_view.rs (129-137)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/terminal_session_view.rs#L129-L137)) — there is no shell path. It already:
- constructs a `BlocklistAIInputModel` ([terminal_session_view.rs (90-98)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/terminal_session_view.rs#L90-L98)) whose `InputConfig` is unused by the TUI today;
- bridges agent-requested commands to PTY execution via `TuiTerminalSessionEvent::ExecuteCommand` with `CommandExecutionSource::AI` ([terminal_session_view.rs (196-246)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/terminal_session_view.rs#L196-L246)), and the transcript renders the resulting command blocks;
- draws the input border in cyan ([terminal_session_view.rs (262-281)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rook_tui/src/terminal_session_view.rs#L262-L281)). There is currently no hint line below the input.

GUI prior art (the semantics we mirror):
- Shell mode state = `InputConfig { input_type: InputType::Shell, is_locked: true }` on `BlocklistAIInputModel` ([`app/src/ai/blocklist/input_model.rs:112 @ 51145bb7`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/ai/blocklist/input_model.rs#L112)).
- Typed-only `!` detection strips the prefix immediately; the buffer never contains `!` ([`app/src/terminal/input.rs (10206-10247) @ 51145bb7`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/terminal/input.rs#L10206-L10247), `TERMINAL_INPUT_PREFIX` at [input.rs:489](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/terminal/input.rs#L489)).
- Shell submissions execute with `CommandExecutionSource::User` via `try_execute_command_from_source` ([input.rs:7150](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/terminal/input.rs#L7150)), gated by `can_execute_command` ([input.rs (6945-6960)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/terminal/input.rs#L6945-L6960)) which blocks when the active block is long-running and not in-band.
- A successful execution cancels any in-progress conversation with `CancellationReason::UserCommandExecuted` ([input.rs (13392-13406)](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/terminal/input.rs#L13392-L13406), reason defined at [`app/src/ai/agent/mod.rs:94`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/ai/agent/mod.rs#L94), `cancel_conversation_progress` at [`app/src/ai/blocklist/controller.rs:2596`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/ai/blocklist/controller.rs#L2596)).
- Visuals use `theme.ansi_fg_blue()` for the prefix glyph and border ([`app/src/terminal/input/agent.rs (206-220)`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/terminal/input/agent.rs#L206-L220), prefix indicator at [input.rs:16243](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/app/src/terminal/input.rs#L16243)).

## Proposed changes

### 0. TUI framework dependency: `TuiFlex` cross-axis sizing

The shell-mode gutter is composed with existing `rookui_core::elements::tui` primitives on top of the stacked `TuiFlex` change (branch `harry/tui-flex-alignment`):
- **Styled text**: `TuiText::with_style(TuiStyle)` applies one style to a whole run ([`crates/rookui_core/src/elements/tui/text.rs:45 @ 51145bb7`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rookui_core/src/elements/tui/text.rs#L45)). The blue `!` glyph and the single-color hint line each render as their own `TuiText`; no rich-text/multi-span element is required.
- **Horizontal composition**: the gutter is a `TuiFlex::row()` — a fixed `!` affordance child followed by the editor element as a flex child filling the leftover width. The row sizes to the editor's height because `TuiFlex` (per the stacked branch) sizes its cross axis to its largest child, clamped to the constraint, matching the GUI `Flex`'s content-sized cross-axis policy; cross-axis positioning/fill uses `TuiFlex::with_cross_axis_alignment(CrossAxisAlignment)` (shared with the GUI), with `Stretch` used by the transcript's full-width input banner in `crates/rook_tui/src/agent_block_sections.rs`.
- **Border color**: the input border already uses `TuiContainer::with_border_style` ([container.rs:120](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rookui_core/src/elements/tui/container.rs#L120)); shell mode just swaps the style's fg. Theme-token → `Color` conversion (`CoreFill::from(ThemeFill::from(...)).into()`) already exists in the session view's render.
- **Esc key**: the runtime already converts `KeyCode::Esc` to keystroke key `"escape"` ([`crates/rookui_core/src/runtime/event_conversion.rs:146`](https://github.com/FJRG2007/rook/blob/51145bb70dc2e461d1152880e8f173dce28ac165/crates/rookui_core/src/runtime/event_conversion.rs#L146)); the input key table just gains a binding for it, gated on a shell-mode keymap-context flag. No new `TuiEvent` variants.
- **Clickable gutter**: the `!` affordance is a `TuiHoverable` (`crates/rookui_core/src/elements/tui/hoverable.rs`), whose clicks use the GUI's press-then-release pairing — an unconsumed mouse-down inside the area arms the click on the shared `MouseState`, and the mouse-up fires the handler only when released inside the area.
- **New surface area is crate-local to `rook_tui`**: two `TuiInputAction` variants (`ExitShellMode`, `SetCursor`), the footer hint slot, and the reusable `TransientHint` notice (`crates/rook_tui/src/transient_hint.rs`) driven by an abortable delayed task via the existing view-context spawn APIs.
...
### 2. Shell-mode state and editing (`crates/rook_tui/src/input/view.rs`)
...
### 3. Submit routing and execution (`crates/rook_tui/src/terminal_session_view.rs`)
...
### 4. Footer hint slot with transient messages (`crates/rook_tui/src/transient_hint.rs`)
...
- **`BlockSpacing`** (`app/src/terminal/terminal_manager.rs`): the spacing baked into `TerminalModel` block heights — per-block padding, reserved Rook-prompt height, and the memory-stats footer row — was previously derived from GUI settings unconditionally inside `compute_block_size` / `create_terminal_model`. It is now a `BlockSpacing` struct passed by the frontend at model creation; settings-driven frontends (local/remote/mock/shared-session-viewer managers) use `BlockSpacing::for_gui`, and `create_tui_model` takes it as a parameter. The TUI passes `TRANSCRIPT_BLOCK_SPACING` (`crates/rook_tui/src/transcript_view.rs`): exactly `BLOCK_TOP_PADDING_ROWS` (1) blank row above each block, no Rook-prompt reserve, and no memory-stats row — the transcript renders whole rows, so the GUI's fractional pixel-derived padding would otherwise ceil into several blank rows per block.
- Agent blocks apply the same one-row top padding (`crates/rook_tui/src/agent_block.rs`) and no longer pad after their last section, so every adjacent block pair — terminal or agent — is separated by exactly one row.
- Terminal-block cells whose background is the theme's default are rendered with the background unset (`crates/rook_tui/src/terminal_block.rs`), so blocks inherit the TUI's own background instead of painting the theme's background color; explicitly-set cell backgrounds still paint.

## Testing and validation

Unit tests follow the repo convention (separate `_tests.rs` files included via `#[cfg(test)] #[path = ...]`), run with `cargo nextest run -p rook_tui`.
...
`crates/rook_tui/src/input/view_tests.rs` (extends the existing `App::test`-based suite):
...
`crates/rook_tui/src/transient_hint_tests.rs`: show/supersede display semantics; expiry timing and superseded-timer aborts ride on the framework's `SpawnedFutureHandle::abort`.

`crates/rook_tui/src/agent_block_tests.rs`: updated to pin the one-row top padding block layout (§5).
...
Manual validation (run the TUI binary from `crates/rook_tui/src/bin` against a real session):
