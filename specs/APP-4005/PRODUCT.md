...
Figma: none provided
## Summary
Add a `/rename-tab` slash command that lets users rename the active Rook tab from the input by providing the desired name inline: `/rename-tab <name>`.
...
## Problem
Renaming a tab currently requires interacting with the tab UI directly: double-clicking a horizontal tab, double-clicking the relevant vertical-tabs row/header, or using the tab context menu. Users who are already typing in Rook's input must switch interaction modes to rename the current tab.
...
- Make the command available in Rook-owned slash-command inputs where renaming the active tab is meaningful, including standard terminal input and Rook Agent / Cloud Agent input surfaces backed by a tab.
...
- Making CLI-agent rich input support general Rook workspace commands unless that input surface already supports them by the time this feature is implemented.
## Figma / design references
Figma: none provided.
This feature should use the existing slash-command menu item styling. No new visual design is required.
## User experience
### Slash command discovery
When the user types `/` in an eligible Rook input, `/rename-tab` appears in the slash-command menu.
...
- The current input can execute Rook static slash commands.
...
- Rook Agent input associated with a terminal-backed tab.
- Cloud Agent input associated with a terminal-backed tab.
Expected unavailable contexts:
- Inputs that do not expose Rook static slash commands.
...
If future work broadens CLI-agent rich input to include general Rook workspace commands, `/rename-tab` should become eligible there as long as executing it is handled by Rook rather than written through to the CLI agent's PTY.
### Executing with an argument
When the user executes `/rename-tab <name>`, Rook sets the active tab's custom name directly without opening the inline rename editor.
...
For Rook Agent and Cloud Agent conversations that live inside a tab with a Rook input, `/rename-tab <name>` should rename that tab, not the conversation itself.
...
If the command is somehow executed when no active tab exists, the active tab cannot be renamed, or the provided name is empty after trimming, Rook should fail gracefully:
...
2. `/rename-tab` appears in Rook Agent and Cloud Agent input surfaces where static slash commands can rename the enclosing active tab.
3. `/rename-tab` does not appear in inputs that cannot execute Rook static slash commands.
...
- Manual validation with a Rook Agent tab: execute `/rename-tab Agent Work` and confirm the tab label changes while the conversation title/history does not.
