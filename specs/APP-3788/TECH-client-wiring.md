...
**Move `HostId`** from `crates/remote_server/src/host_id.rs` to `crates/rook_core/src/host_id.rs` (same pattern as `SessionId` in `rook_core/src/session_id.rs`). Re-export from `remote_server` for backward compatibility.
