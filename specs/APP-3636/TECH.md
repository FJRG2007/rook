...
- Verify `@` opens the context menu with files/folders and code symbols (no Rook-specific categories) in CLI agent rich input.
...
- Verify no regressions in normal Rook agent input (`@` context still works as before).
