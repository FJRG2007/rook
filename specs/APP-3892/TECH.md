...
## Problem
The code review panel gates all comment and add-as-context button visibility on `AISettings::is_any_ai_enabled()`. This hides the buttons entirely when AI is disabled, even though CLI agent terminals (Claude Code, Gemini) can accept review comments without Rook AI. The implementation needs to decouple button visibility from AI state, restrict non-CLI terminal availability when AI is off, and surface the correct reason when the send button is disabled.
...
- `ReviewDestination` enum in `right_panel.rs`: `None`, `Rook`, `Cli(CLIAgent)`.
- `recompute_terminal_availability` currently computes `ReviewDestination` purely from terminal state — returns `Rook` for any idle, non-executing terminal regardless of AI setting.
