...
The server is adding `POST /harness-support/notify-user` and `POST /harness-support/finish-task` endpoints (see `rook-server` spec). Third-party harnesses invoke server APIs through the `oz` CLI, not directly. We need:

1. Two new `oz harness-support` subcommands that call these endpoints.
2. API client methods in the Rook client to make the HTTP calls.
3. Claude Code plugin skills so the harness can invoke the commands at the right time.

## Relevant Code

- `crates/rook_cli/src/harness_support.rs` — CLI arg definitions (`HarnessSupportCommand` enum)
...
- `../claude-code-rook-internal/plugins/oz-harness-support/` — existing plugin (skills, hooks)

## Current State

`oz harness-support` has two subcommands: `ping` and `report-artifact`. Each follows the same pattern:
1. **CLI layer** (`rook_cli`): clap `Args`/`Subcommand` structs define the command shape.
...
### 1. CLI layer (`crates/rook_cli/src/harness_support.rs`)
...
### 5. Claude Code plugin (`../claude-code-rook-internal/plugins/oz-harness-support/`)
