...
Ctrl-c never reached the TUI view layer: the headless input driver intercepted it before any dispatch (`is_ctrl_c` in `crates/rookui_core/src/runtime/mod.rs`) and called `terminate_app(ForceTerminate)`. Everything needed for contextual handling already existed in the view layer:

- `TuiTerminalSessionView` (`crates/rook_tui/src/terminal_session_view.rs`) owns the transcript, the input view, and `ModelHandle<BlocklistAIController>`, and its view id is the `terminal_surface_id` keying conversation state.
...
### 1. Route ctrl-c through normal dispatch (`crates/rookui_core/src/runtime/mod.rs`)
...
### 2. `TuiEventHandler::new` takes `Box<dyn TuiElement>` (`crates/rookui_core/src/elements/tui/event_handler.rs`)
...
### 3. `ExitConfirmation` state machine (`crates/rook_tui/src/exit_confirmation.rs`)
...
### 5. Root-level exit fallback (`crates/rook_tui/src/root_view.rs`)
...
- `TuiInputView::is_empty(ctx)` / `clear(ctx)` (`crates/rook_tui/src/input/view.rs`); `submit` now reuses `clear`.
...
- `crates/rook_tui/src/exit_confirmation_tests.rs`: arming, in-window exit, expiry, re-arm superseding a stale timer, disarm.
- `crates/rook_tui/src/input/view_tests.rs::clear_empties_buffer_and_resets_scroll`: `clear` empties the buffer, resets scroll, and restores the cursor to the origin.
- `crates/rookui_core/src/runtime/mod_tests.rs`: `keymap_binding_dispatches_typed_action_to_tui_view` (the keymap pass dispatches a bound action to a TUI view) and `unhandled_keymap_binding_falls_through_to_element_pass` (a matched-but-unhandled binding does not swallow the key); manual validation steps live in the product spec.
