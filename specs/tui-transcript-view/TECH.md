...
## Context
This PR builds the first production-shaped conversation transcript view for Rook's TUI. It proves the transcript container and canonical ordering path with two intentionally simple block renderers:
...
The current TUI prompt path is owned by `crates/rook_tui/src/terminal_session_view.rs`. `TuiTerminalSessionView` creates the production AI context/input/action/controller models for its terminal surface, owns a `TuiConversationSelection`, and sends submitted prompts through `BlocklistAIController::send_user_query_in_conversation`. The earlier one-shot stdout prompt-streaming path and separate `TuiConversationModel` are removed in this PR.

RookUI already has a TUI-specific element/view/presenter stack. [`TuiElement`](https://github.com/FJRG2007/rook/blob/e36e8ddf823d6a25a5225251a7db60698f5da74d/crates/rookui_core/src/elements/tui/mod.rs#L96-L140) defines the normal layout, rendering, presentation, event, and cursor lifecycle, while [`TuiPresenter`](https://github.com/FJRG2007/rook/blob/e36e8ddf823d6a25a5225251a7db60698f5da74d/crates/rookui_core/src/presenter/tui.rs#L81-L208) retains laid-out trees and records child-view embeddings. The transcript must return normal visible `TuiElement` trees so this lifecycle remains intact; it must not use a context-free raw-buffer row renderer.

`TerminalModel::BlockList` is the canonical ordered presentation model for a terminal surface. Its heterogeneous [`BlockHeightItem`](https://github.com/FJRG2007/rook/blob/e36e8ddf823d6a25a5225251a7db60698f5da74d/app/src/terminal/model/blocks.rs#L121-L196) sum tree orders terminal blocks and rich content and tracks accumulated height, count, and block count in [`BlockList`](https://github.com/FJRG2007/rook/blob/e36e8ddf823d6a25a5225251a7db60698f5da74d/app/src/terminal/model/blocks.rs#L225-L270). Terminal output updates model-authoritative block heights, while view-measured rich content uses dirty marking and height writeback ([`mark_rich_content_dirty`](https://github.com/FJRG2007/rook/blob/e36e8ddf823d6a25a5225251a7db60698f5da74d/app/src/terminal/model/blocks.rs#L1173-L1180), [`update_rich_content_heights`](https://github.com/FJRG2007/rook/blob/e36e8ddf823d6a25a5225251a7db60698f5da74d/app/src/terminal/model/blocks.rs#L2349-L2351)). The GUI follows the same canonical-order model by inserting one rich-content AI block per exchange in [`TerminalView::handle_ai_history_model_event`](https://github.com/FJRG2007/rook/blob/e36e8ddf823d6a25a5225251a7db60698f5da74d/app/src/terminal/view.rs#L6030-L6220).
...
Change the no-prompt TUI frontend callback in `crates/rook_tui/src/lib.rs`: after app-side authentication, bare `rook-tui` starts a real TUI session instead of printing the authenticated user ID and exiting.

Add a root TUI view that owns login branching and the optional terminal-session child handle. The root window and TUI driver start immediately so the login placeholder can render, but `TuiTerminalSessionView` and the local terminal manager are created only after `TuiLoginPhase::LoggedIn`. `RootTuiView::ensure_terminal_session` creates the terminal child through the root view's own `ViewContext`, preserving normal TUI view hierarchy and parentage, while `crates/rook_tui/src/session.rs` owns the driver and retains the terminal manager handle after login.
...
- selected terminal model test helpers are exposed behind `test-util` for `rook_tui` tests.

### Appearance and color handling
TUI transcript colors are derived from the same `Appearance::theme()` source of truth used by GUI code. The TUI does not own feature-local fixed color constants or a separate `crates/rook_tui/src/theme.rs` color role module.

The Figma node for `Terminal/Dark/True` defines the relevant dark tokens as `background: #050505`, `foreground: #ffffff`, `cyan: #d0d1fe`, `cyan_overlay_1: #d0d1fe1a`, `white: #f1f1f1`, `green: #b4fa72`, `red: #ff8272`, and `ANSI Bright/ANSI Bright black: #8e8e8e`. Rook's regular bundled dark theme uses that `#050505` background, so the TUI does not need a near-duplicate TUI-specific theme.

TUI render code reads the active `RookTheme` and chooses base theme tokens at each call site rather than adding transcript-specific theme methods:
...
The shared conversion lives in `crates/rookui_core/src/elements/tui/color.rs` as `impl From<rookui_core::elements::Fill> for ratatui::style::Color`. This keeps Ratatui-specific knowledge in the TUI element layer. `rook_tui` call sites first convert theme-layer `rook_core::ui::theme::Fill` values into `rookui_core::elements::Fill`, then into TUI `Color`.
...
Add a `TuiBlockListViewportSource` adapter under `crates/rook_tui/src/` over the canonical `TerminalModel::BlockList` sum tree.
...
`BlockList::take_dirty_rich_content_items` is public so the TUI viewport source can consume pending rich-content height invalidations. The `rook_tui` crate accesses that helper and other app-owned model types only through the narrow `rook::tui_export` boundary.

### Transcript view and exchange lifecycle
Add a TUI transcript view under `crates/rook_tui/src/` that owns the generalized viewport state and the terminal-history integration. The terminal session view embeds it as the flex child above the bottom input in this PR. It subscribes to terminal-surface-scoped `BlocklistAIHistoryEvent`s and mirrors the existing GUI model-level lifecycle:
...
```mermaid
flowchart TD
  Init["bare rook-tui"] --> Root["RootTuiView<br/>login shell"]
...
Current focused `rook_tui` crate unit tests cover:
...
Current `rookui::App::test` and `TuiPresenter` coverage verifies:
...
### Manual validation
- Run bare `cargo run -p rook_tui`; verify it enters the alternate screen and displays a bordered input docked at the bottom, with the transcript above it.
...
Run:
- `./script/format`
- `cargo check -p rook_tui --all-targets`
- `cargo nextest run -p rook_tui`
- `cargo nextest run -p rookui_core --features tui`
- `cargo clippy -p rook_tui --all-targets -- -D warnings`
- `cargo clippy -p rookui_core --features tui --all-targets -- -D warnings`
- `cargo fmt -- --check`
## Parallelization
