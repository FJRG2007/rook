...
- **Compile gating**: `cargo check -p rook` and `cargo check -p rook --features queued_prompts_v2` both pass; `cargo fmt` and `cargo clippy --workspace --all-targets --all-features --tests -- -D warnings` per the ROOK.md PR workflow.
