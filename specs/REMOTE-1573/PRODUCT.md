...
4. The setting is persisted locally and synced to cloud via the existing `define_settings_group!` / Rook Drive settings infrastructure, following the pattern of other AI settings (e.g. `orchestration_enabled`). TOML path: `agents.rook_agent.other.cloud_handoff_enabled`.
