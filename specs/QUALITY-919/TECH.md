...
Execution: run the affected tests via `cargo nextest run -p rook` (per AGENTS.md). The implementer must be green on these plus `./script/format` and `cargo clippy` (the `./script/presubmit` versions) before requesting review. No `crates/integration` test is added — disproportionate for this client-internal change; the manual dogfood repro covers end-to-end.
...
## Environment
- Repo/worktree: `~/src/event-registrations/rook`, branch `matthew/event-registrations`. Client-only — no `rook-server` or `rook-proto-apis` changes. The server ancestor stream, `include_self`, `get_ambient_agent_task.children`, and `orchestration_viewer_streamer` already shipped and are enabled in prod.
- New flag touches `crates/rook_features/src/lib.rs`, `app/Cargo.toml`, and `app/src/features.rs`.
