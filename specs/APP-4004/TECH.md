...
CLI agents (Codex, Claude CLI, etc.) running in Rook blocks produce trailing blank lines. Output grid height is based on `max_cursor_point` (furthest row cursor visited), but agents frequently emit newlines/clear screen, leaving empty rows below content. Wastes space, especially in shared sessions.
...
`FeatureFlag::TrimTrailingBlankLines` added to `crates/rook_features/src/lib.rs`, included in `DOGFOOD_FLAGS`.
...
- `crates/rook_features/src/lib.rs` — flag enum + DOGFOOD_FLAGS
