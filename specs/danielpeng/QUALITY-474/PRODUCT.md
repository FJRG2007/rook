...
When a user edits `settings.toml` while Rook is closed (or changes a setting via the UI while offline), cloud sync currently overwrites their local changes on next startup. This feature detects when the local settings file has diverged from the last-known cloud-synced state and preserves the user's local changes instead of silently replacing them.

## Problem

Rook's cloud settings sync uses a "cloud wins" strategy on startup: for every synced setting that exists in the cloud, the cloud value overwrites the local value. This is correct when the user hasn't made local changes, but breaks down in two scenarios:

1. **Offline file edit**: The user edits `settings.toml` while Rook is closed. On next startup, cloud sync overwrites their edit with the (now stale) cloud value.
2. **Offline UI change**: The user changes a setting via the Rook UI while their internet is offline. The change is written to the file but never uploaded to cloud. On next startup (now online), cloud sync overwrites the local change.
...
3. User opens Rook.
4. Rook detects the file has changed since the last cloud sync.
5. Rook treats the local file as authoritative: local values are uploaded to cloud, cloud values do not overwrite local.
6. The user's edit is preserved and synced to other devices.

### Offline UI change

1. User has Rook open but is offline.
2. User changes a setting via the UI. The change is saved to the file.
3. User closes Rook (still offline — the change was never uploaded).
4. User opens Rook (now online).
5. Rook detects the file has changed since the last cloud sync.
...
2. User opens Rook.
3. Rook cannot parse the file. Cloud sync is allowed to restore settings in memory so the app functions.
...
2. User opens Rook.
3. Rook detects the file is missing/empty and treats it as "no local state".
...
No stored hash exists. Rook treats this as "no local changes" and cloud wins, which is the same as today's behavior.

## Success Criteria

1. When a user edits `settings.toml` while Rook is closed and reopens Rook, their edits are preserved and synced to cloud.
2. When a user changes a setting via the UI while offline, closes Rook, and reopens while online, their change is preserved and synced to cloud.
...
- **Manual testing**: Edit `settings.toml` while Rook is closed, reopen, verify the edit persists and syncs to another device.
