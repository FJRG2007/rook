...
**CLI agent icons:**
- `vertical_tabs.rs:1918` — `cli_agent_rook_icon()` maps `CLIAgent` to branded `RookIcon`
...
- **Presubmit:** `cargo clippy` and `cargo fmt` must pass. Run `cargo nextest run -p rook` for the workspace tests.
