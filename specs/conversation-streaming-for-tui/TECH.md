...
## Context
This change adds a one-shot TUI path that sends a prompt through Rook's production AI controller and streams plain-text output to stdout. The reusable coordination model is designed to support a future interactive TUI without introducing an alternate request or response-stream implementation.
...
GUI and TUI composition roots explicitly construct their own `ConversationSelection` implementations and erase them behind `ConversationSelectionHandle = ModelHandle<Box<dyn ConversationSelection>>` (`app/src/terminal/view.rs`, `crates/rook_tui/src/prompt_stream.rs`). The contract centralizes:
...
RookUI `EntityId` is the routing key for a terminal surface. History fields, methods, and events use `terminal_surface_id` terminology consistently:
...
`TuiConversationModel` is the reusable TUI presentation coordinator (`crates/rook_tui/src/conversation_model.rs`). It contains no transcript widgets and coordinates:
...
`send_prompt(...)` targets the current selection or creates a conversation when none is selected. `restore_conversation_by_server_token_and_send_prompt(...)` resolves a supplied server conversation token to the canonical local conversation ID, restores and selects that conversation, then delegates to `send_prompt(...)` (`crates/rook_tui/src/conversation_model.rs`).
### One-shot prompt streaming
`PromptStreamSurface` adapts `TuiConversationModel` events to stdout and application termination (`crates/rook_tui/src/prompt_stream.rs`). It is named for its actual behavior rather than as a test fixture.
...
`PtySpawner` can safely use the standard terminal-server subprocess from a `rook-tui` executable. `rook_tui::run()` asks `rook::run_tui_worker_if_requested()` to dispatch Rook worker invocations before parsing frontend arguments, then passes the TUI initializer into `rook::run_tui(...)` for app bootstrap and authentication (`crates/rook_tui/src/lib.rs`, `app/src/lib.rs`, `app/src/tui.rs`). This lets TUI launches register the same early `PtySpawner` singleton as other Rook launches without recursively starting more TUI frontends, and preserves dispatch for other current-executable workers.
...
The `rook_tui` package mirrors GUI channel binaries. Each channel-specific binary only configures `ChannelState` and calls `rook_tui::run()`. The `rook_tui` library owns frontend argument parsing, TUI conversation coordination, selection, and prompt-stream presentation; the `rook` app crate owns worker dispatch, shared app bootstrap, authentication, AI models, and terminal-manager internals. The TUI frontend accepts:
- `--prompt <text>`
- `--conversation-id <server-conversation-token>`
Bare `cargo run -p rook_tui` uses the OSS/production channel; `./script/run-tui -- --prompt ...` selects the internal local channel when its channel config is available.
## End-to-end flow
```mermaid
flowchart TD
...
- Rook worker invocations dispatch before TUI frontend argument parsing
Manual validation:
- `cargo run -p rook_tui -- --prompt "Reply with exactly: hello from tui"` emits streamed text, the server conversation token as `conversation_id=...`, and `status=Success`
...
Run:
- `./script/format`
- `cargo check -p rook -p rook_tui`
- `cargo check -p rook --tests`
- `cargo check -p rook --features integration_tests --tests`
- `cargo clippy -p rook -p rook_tui --all-targets -- -D warnings`
...
- **One-shot presentation policy leaks into reusable models.** Keep stdout, termination, and unsupported-action behavior in `crates/rook_tui/src/prompt_stream.rs`.
