...
- Full `./script/format` and `cargo clippy` (per ROOK.md) before PR; run the focused queue test suites (`queued_query`, `queued_prompts`, input/view tests). Do not run the app to test.
