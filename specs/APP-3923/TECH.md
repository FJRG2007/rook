...
- Server side: `rook-server/router/handlers/generate_code_review_content.go` (already deployed)
## Current state
Before this branch:
...
- `get_diff_for_commit_message(repo_path, include_unstaged) -> Result<String>`
  ...
  - When `include_unstaged`, iterates `git ls-files --others --exclude-standard -z` (NUL-separated to survive paths with spaces/non-ASCII), skips binaries via `rook_util::file_type::is_buffer_binary(&bytes[..BINARY_CHECK_BYTES])`, and appends synthetic unified-diff hunks for each new file (capped at `MAX_UNTRACKED_FILE_BYTES`) so the LLM sees new-file-only commits.
...
**Privacy / opt-out.** `GitOperationsInCodeReview` gates the UI, but does not address AI-specific privacy concerns (sending diffs to an LLM). See the TODO at the top of `app/src/ai/generate_code_review_content/mod.rs` — follow-up work needs to add an `AISettings` toggle (mirroring `is_shared_block_title_generation_enabled`) and a customer-type guard that excludes Enterprise unless on Rook plan or dogfood, matching the pattern in `terminal/share_block_modal.rs::should_send_title_gen_request`.
...
- Add customer-type guard for Enterprise users (allow Rook plan + dogfood, deny otherwise).
