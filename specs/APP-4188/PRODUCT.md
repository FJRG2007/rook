...
## Summary
The code-review panel's git operations buttons (Commit, Push, Publish, Create PR, Commit and push, Commit and create PR) must succeed regardless of how Rook was launched. When opened from Finder/Dock, Rook inherits launchd's minimal `PATH`, which excludes Homebrew / MacPorts / Nix / asdf / etc. Pushes to LFS-enabled repos fail (LFS `pre-push` hook can't find `git-lfs`), and PR creation fails when `gh` isn't on `/usr/bin`.
## Behavior
...
5. First git op per Rook session may pay a one-time shell-capture cost (<1s typically; bounded by `.zshrc` load). Absorbed by the existing loading state; no new UI. Subsequent ops reuse the cache.
...
## Validation
- Launch Rook from Finder with Homebrew-only `git-lfs`. On a LFS repo, Commit and push → succeeds.
...
- Launch Rook from a terminal → all git ops behave as today.
