...
On Windows and Linux, the "Right Alt as meta" and "Left Alt as meta" settings under Keys in Rook should each independently control only their own physical Alt key. Today, right Alt is never recognized as right Alt, so enabling "Right Alt as meta" is a no-op and enabling "Left Alt as meta" incorrectly treats both Alt keys as meta.

## Problem

Rook users on Windows and Linux who rely on the extra-meta-keys settings to make a single Alt key behave as meta (for example, shell users who want right Alt to emit ESC-prefixed keys while left Alt still works as a regular modifier for keybindings like Ctrl+Alt+R "Resume conversation") cannot do so today. The setting either has no effect (right-alt-as-meta) or applies to both keys at once (left-alt-as-meta), breaking keybindings that need a plain Alt modifier.
...
8. Settings changes take effect on the next keystroke. The user does not have to relaunch Rook.

9. Alt state does not get "stuck":
   - If the user holds Alt, switches windows via Alt+Tab, releases Alt while Rook is not focused, and then refocuses Rook, Rook must not continue to believe that Alt is held. The next character key pressed in Rook reports `alt: false`.
   - If either Alt key release is lost for any other reason (dropped event, OS-level remap), Rook recovers whenever the OS next reports that no Alt is held.
