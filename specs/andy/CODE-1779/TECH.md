...
1. **Drop onto the terminal grid** (e.g. during a long-running command). Handled by `TerminalView::drag_and_drop_files` in `app/src/terminal/view.rs (23007-23085)`. This path already applies `rook_util::path::convert_windows_path_to_wsl` when `session.is_wsl()` before shell-escaping and writing to the PTY. For MSYS2 long-running it deliberately keeps Windows-native paths and skips shell escaping (so native Windows binaries receive the form they expect). **Not changing this path.**

2. **Drop onto the input editor** (typical case when there's no long-running command). The editor element's `drag_and_drop_file` in `app/src/editor/view/element.rs (663-681)` dispatches `EditorAction::DragAndDropFiles`, which routes to `EditorView::drag_and_drop_files` in `app/src/editor/view/mod.rs (7893-7914)`. That function emits `Event::DroppedImageFiles` for image paths (terminal input handles attachment) and, for everything else, calls `rookui::clipboard_utils::escaped_paths_str` followed by `self.user_insert`. **No path conversion happens — this is the bug for both WSL and MSYS2.**
...
Existing helpers:
- `rook_util::path::convert_windows_path_to_wsl` in `crates/rook_util/src/path.rs (673-691)`, with tests at `crates/rook_util/src/path_test.rs (629-649)`.
...
### 1. Add `convert_windows_path_to_msys2` to `rook_util::path`

In `crates/rook_util/src/path.rs`, alongside `convert_windows_path_to_wsl`, add:
...
Add unit tests in `crates/rook_util/src/path_test.rs` matching PRODUCT.md invariant (2):
...
- In `set_active_block_metadata` (line ~13034), alongside the existing `editor.set_shell_family(...)` call, select a transformer based on the session and install or clear it:
  - `session.is_wsl()` → `Rc::new(|p: &str| rook_util::path::convert_windows_path_to_wsl(p))`.
  - `session.is_msys2()` → `Rc::new(|p: &str| rook_util::path::convert_windows_path_to_msys2(p))`.
  - Otherwise → `None`.
...
- **Unit tests for `convert_windows_path_to_msys2` (invariant 2, MSYS2 cases).** In `crates/rook_util/src/path_test.rs`, mirror the existing `test_convert_windows_path_to_wsl` test with MSYS2-equivalent expectations (`/c/...`, `/d/...`, etc.).

- **Existing coverage for the WSL conversion itself (invariant 2, WSL cases).** `test_convert_windows_path_to_wsl` in `crates/rook_util/src/path_test.rs (629-649)` already verifies drive-letter lowercasing, spaces, UNC, and empty-suffix behavior. No new tests needed there.
...
- **Unit test — transformer wiring (invariants 1, 2, 5, 9).** In `app/src/editor/view/mod_test.rs`, add a test that:
  - Creates an `EditorView` with `shell_family: Some(Posix)` and runs two scenarios:
    1. `drag_drop_path_transformer: Some(Rc::new(|p| rook_util::path::convert_windows_path_to_wsl(p)))` — WSL behavior.
    2. `drag_drop_path_transformer: Some(Rc::new(|p| rook_util::path::convert_windows_path_to_msys2(p)))` — MSYS2 behavior.
