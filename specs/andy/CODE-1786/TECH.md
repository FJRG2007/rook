# Run Rook at startup (Windows) — Tech Spec
Linear: [CODE-1786](https://linear.app/warpdotdev/issue/CODE-1786/windows-run-rook-at-startup)
See `PRODUCT.md` for user-visible behavior.
## Context
The macOS "Start Rook at login" feature is already plumbed end-to-end. This ticket extends that plumbing to Windows by adding a new registration backend and broadening the existing setting/UI's platform gate; no new user-visible surface is being designed.
Relevant code today:
...
- `app/src/lib.rs:2279-2362` — `maybe_register_app_as_login_item`. macOS-only. Guards on `ROOK_INTEGRATION`, skips when bundle identifier is missing or equals `dev.rook.Rook-Local`, runs `SMAppService register/unregisterAndReturnError:` off the UI thread, then writes the result back to `app_added_as_login_item`.
- `app/src/settings_view/features_page.rs` — toggle UI:
  ...
  - widget: `LoginItemWidget` (~l4488-4533), rendered only when `add_app_as_login_item.is_supported_on_current_platform()` returns true (~l2481-2486). Label is hard-coded to `"Start Rook at login (requires macOS 13+)"`.
...
- `crates/rook_core/src/channel/state.rs:119-125, 40` — `ChannelState::app_id()` returns the current channel's `AppId` (e.g. `dev.openrook.OpenRook`, `dev.rook.Rook`, `dev.rook.RookPreview`, `dev.rook.RookDev`). We can reuse `application_name()` for the channel-specific registry value name on Windows.
- `app/Cargo.toml:356-377` — Windows-only dependency block. `winreg`, `windows-registry`, and `windows` are already pulled in and available for a new module. `winreg` is already used for reading registry values (`crates/rookui/src/windowing/winit/windows/registry.rs`), so we should follow that pattern for consistency.
## Proposed changes
...
pub fn maybe_register_app_as_login_item(ctx: &mut AppContext) {
    if std::env::var("ROOK_INTEGRATION").is_ok() {
...
- Registry work itself:
  ...
  fn value_name() -> String {
      // e.g. "Rook", "RookPreview", "RookDev", "OpenRook"
  ...
  fn register(exe: &Path) -> std::io::Result<()> {
      ...
      // Quote the path so spaces (e.g. "C:\Program Files\Rook\rook.exe") survive parsing.
...
- The per-channel value name from `application_name()` keeps Dev/Preview/Stable isolated (Behavior 7): `Rook`, `RookPreview`, `RookDev`, `OpenRook`.
...
- `LoginItemWidget::render` (~l4509): change the hard-coded label to a platform-dependent string, e.g.
  ```rust path=null start=null
  #[cfg(target_os = "macos")]
  let label = "Start Rook at login (requires macOS 13+)";
  #[cfg(target_os = "windows")]
  let label = "Start Rook at login";
  ```
...
- **Invariant 7 (per-channel isolation).** With different `ChannelState` app IDs, `value_name()` returns distinct strings (`Rook`, `RookPreview`, `RookDev`). Registering under one does not touch another.
...
Because the real `Software\Microsoft\Windows\CurrentVersion\Run` hive is shared user state that should not be mutated by tests, the registry helpers should accept an injectable subkey path (e.g. `register_with_subkey(subkey, …)`) and the tests should drive them against `Software\Rook\TestRun\<uuid>` under HKCU, cleaning up on drop. Follow the `crates/rookui/src/windowing/winit/windows/registry.rs` style for the read-side helper for consistency.
...
- **Invariant 9 (integration test guard).** With `ROOK_INTEGRATION` set, calling `maybe_register_app_as_login_item` does no registry I/O. Can be asserted by calling with a mock subkey provider that panics if invoked.
Manual validation (Windows):
1. **Invariants 1, 2, 11.** Install a release-bundle Windows build, verify **Features → General** shows *Start Rook at login*, toggle default is on, and `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Rook` exists with the installed exe path. Confirm the entry is visible in **Settings → Apps → Startup** and **Task Manager → Startup apps** with the display name "Rook".
...
3. **Invariant 5.** With toggle on, delete the registry value (or disable from Task Manager). Relaunch Rook; verify the value is **not** recreated and `app_added_as_login_item` stays true. Toggle off then on in-app; verify the value *is* recreated.
4. **Invariant 6.** Sign out and back into Windows. Rook launches without stealing focus; the global hotkey surfaces it as expected.
5. **Invariant 7.** Install both Stable and Preview channels; enable the setting in each; confirm both registry values (`Rook`, `RookPreview`) coexist and unregistering one doesn't affect the other.
...
7. **Invariant 10.** Move/rename the install directory, relaunch Rook, toggle off+on; confirm the registry value points at the new path.
...
- **Dev-build regressions.** Gate actual registry I/O on `ChannelState::is_release_bundle()` to avoid developer machines auto-launching random `target/debug/rook.exe` artifacts.
## Follow-ups
- Linux autostart (`~/.config/autostart/rook.desktop`) is a natural next step with the same setting and the same UX, but is out of scope for this ticket.
