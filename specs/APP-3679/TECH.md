...
The horizontal tab menu's last item is `"Create new tab config..."` which calls `create_and_open_new_tab_config` — writes a TOML template to `~/.rook/tab_configs/` and opens it in the user's editor. The vertical tab menu has the same action under the label `"New worktree"`.
...
- Tab config filesystem watcher in `user_config/native.rs` automatically reloads `~/.rook/tab_configs/` on file changes.
...
3. Write to `~/.rook/tab_configs/worktree_{sanitized_branch}.toml` using `find_unused_worktree_config_path`.
...
7. New tab opens with the worktree commands. File watcher detects the new `.toml` and adds it to `RookConfig.tab_configs`.
8. Modal closes.
...
- **Build check**: `cargo check -p rook` passes with no errors or warnings.
...
- **Filesystem**: Verify `.toml` files appear in `~/.rook/tab_configs/` with correct content.
