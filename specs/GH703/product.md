...
- Closing the underlying unauthenticated `rook://` dispatch primitives (tracked in #655, #666). This spec assumes the dispatch primitive may be reachable by an attacker and hardens the Linear handler defensively.
...
1. **Deeplink dispatch opens a new tab in agent view.** When `rook://linear/work?prompt=<PROMPT>` is dispatched, Rook opens a new tab, enters agent view for a new conversation (as today), and places the decoded `<PROMPT>` string into the agent input buffer for that conversation.
...
4. **"Enter again to send" affordance is shown.** When the Linear deeplink lands a prompt in the input buffer, Rook displays the existing ephemeral "press enter again to send to agent" confirmation message, signaling clearly that the prompt is pending and requires a deliberate keypress to send. The confirmation window matches the existing `ENTER_OR_EXIT_CONFIRMATION_WINDOW` timing.
...
11. **Background-window dispatch.** When Rook is backgrounded (or not the frontmost app) at the time the URL is dispatched, the new tab and its draft prompt must remain pending until the user foregrounds Rook. Nothing is submitted to the LLM during the time the user is not looking at the app. When the user does foreground Rook, they see the prompt in the input buffer and the "enter again to send" affordance exactly as in invariant 1–4; the pending state does not expire or auto-send itself on focus change.
...
## Success Criteria
1. Opening `rook://linear/work?prompt=<anything>` while the focused terminal is in fullscreen agent view results in the prompt appearing in the input buffer with an "enter again to send" confirmation — the prompt is not sent to the LLM.
...
3. Opening the same URL while Rook is backgrounded does not submit the prompt; on foregrounding, the user sees the draft and affordance and must act explicitly.
