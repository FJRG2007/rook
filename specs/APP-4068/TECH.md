...
2. **Multiplexing**: multiple Rook tabs SSH-ing to the same host must share a single
...
- `crates/rook_cli/src/lib.rs` — `WorkerCommand::RemoteServerProxy` / `RemoteServerDaemon`
...
connects to `~/.rook[-channel]/remote-server/server.sock`.
...
task on RookUI's background executor with a dedicated `async_channel` sender.
...
`WorkerCommand::RemoteServerProxy` in `rook_cli/src/lib.rs` dispatches to
`unix::run_proxy()`.
...
has access to RookUI's `ModelContext`. It binds an `async_io::Async<UnixListener>`,
then for each accepted connection spawns a task on the RookUI background executor
...
are dispatched through `ModelSpawner` onto the single-threaded RookUI main loop, so they
cannot interleave.
