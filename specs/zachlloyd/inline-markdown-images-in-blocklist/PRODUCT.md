...
Successfully rendered file-backed images and Mermaid diagrams should both reuse Rook’s existing fullscreen lightbox treatment for click-to-expand viewing.
...
Rook already supports Markdown image blocks elsewhere in the app and already has Mermaid-to-SVG rendering for notebook Markdown. The AI block list currently lacks the equivalent presentation and interaction model. That creates three problems:

- visual content in AI responses is harder to understand because users see raw Markdown instead of the image or diagram
- rendered output is inconsistent with other Markdown-capable surfaces in Rook
...
- Ensure the feature works across Rook’s supported platforms, including WASM.
- Use the same supported image format set that Rook’s existing Markdown/image rendering stack already supports today: JPEG/JPG, PNG, GIF, WebP, and SVG.
...
- Reuse Rook’s existing fullscreen lightbox treatment for both rendered file-backed images and rendered Mermaid diagrams.
...
- Supporting additional image formats beyond the set Rook already renders today.
...
2. Mermaid diagrams authored in Markdown that Rook renders into an image representation at runtime.
...
### Supported formats
For file-backed images, the AI block list should support the same image formats Rook’s existing shared image renderer already supports today:
- JPEG / JPG
- PNG
- GIF
- WebP
...
If the referenced file exists but is not one of those supported formats, Rook should not attempt a degraded or best-effort render. It should show the original Markdown text exactly as it does today.
...
1. Inline image runs
When multiple parser-recognized Markdown image references appear on the same line, separated only by whitespace, Rook should render them as a horizontal image row:
...
- if that path points to a binary file such as PNG, JPG, GIF, or WebP, activating the path should use the platform-default opener instead of Rook's editor or the user's configured `$EDITOR`
...
For file-backed images that render successfully, Rook should also show the referenced file link/path in the layout appropriate for that treatment:
...
- binary file links should open with the operating system's default file opener; on macOS this means using `open` rather than routing through Rook's editor or `$EDITOR`
...
- clicking a rendered image opens it in Rook’s existing fullscreen lightbox treatment
- the lightbox should reuse the same fullscreen overlay treatment already used elsewhere in Rook rather than introducing a new one-off viewer
...
then Rook should show the original Markdown image syntax inline, matching current behavior, instead of rendering an error-specific image container.

For Mermaid diagrams, if rendering fails, Rook should show the original Mermaid Markdown source instead of a rendered diagram.
...
- Activating the source path for a rendered binary asset opens it with the operating system's default app instead of Rook's editor or the user's configured `$EDITOR`.
...
- If a file cannot be found or the file format is unsupported, Rook shows the original Markdown text rather than a broken rendered state.
- If Mermaid rendering fails, Rook shows the original Mermaid Markdown source.
...
- Manual validation that clicking the source path for a rendered PNG or other binary image opens it with the platform-default app rather than Rook's editor.
