...
## Summary
Make rendered Markdown tables use the same blockless visual treatment across every Rook surface that renders Markdown tables.

The block list styling introduced in PR #23908 should become the visual baseline for Markdown tables everywhere they render in Rook, including Markdown notebooks and the Markdown editor. This is a consistency and presentation change, not a redesign of Markdown table semantics or editing behavior.

## Problem
Rook currently renders the same Markdown table with noticeably different visual treatments depending on where the user sees it.
...
This makes the same Markdown content feel like different components in different parts of the product. Users moving between notebooks, the Markdown editor, AI documents, comment editors, file notebooks, and AI output should not have to mentally re-parse the same table because Rook styled it differently in each surface.

## Goals
- Make rendered Markdown tables in every current Rook Markdown-rendering surface match the visual treatment now used in the AI block list.
...
- Ensure the same Markdown table feels like the same component across all Rook Markdown-rendering surfaces.
- Ensure future Rook surfaces that adopt Markdown table rendering inherit this same style by default rather than introducing another table treatment.
...
- Broadly restyling every table-like UI in Rook outside Markdown-rendered tables.
...
The Figma node shows the blockless table treatment that now exists in the AI block list and should be reused for Markdown tables across Rook.
...
- any other current or future Rook surface that renders Markdown tables through the shared Markdown rendering stack or through a dedicated Markdown table renderer

If a Rook surface renders a valid Markdown table as a table, it should adopt the updated styling described here.
...
The same valid Markdown table should look materially the same in the AI block list and in every other Rook surface that renders Markdown tables.
...
### Styling invariants
Rendered Markdown tables in all Rook Markdown-rendering surfaces should adopt the same structural styling introduced for block list tables in PR #23908.
...
## Success criteria
- A Markdown table rendered in any current Rook Markdown-rendering surface uses the same blockless styling pattern as the AI block list table from PR #23908.
- Rook no longer shows two competing visual treatments for rendered Markdown tables across its current Markdown surfaces.
...
- The same Markdown table shown in the AI block list and in other Rook Markdown-rendering surfaces looks materially identical aside from surface-specific typography or layout constraints.
...
## Open questions
None currently. New Rook surfaces that render Markdown tables should inherit this style by default.
