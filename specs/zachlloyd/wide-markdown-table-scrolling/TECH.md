...
- `crates/rookui_core/src/elements/shared_scrollbar.rs` — shared `ScrollbarAppearance` / `ScrollbarGeometry` and scrollbar math
- `crates/rookui_core/src/elements/clipped_scrollable.rs` — selection anchor (`anchor_and_adjust_selection_for_scroll`)
- `crates/rookui_core/src/elements/new_scrollable/mod.rs` — dispatches selection APIs through the anchor helper
- `crates/rookui_core/src/elements/formatted_text_element.rs` — horizontal bounds check for smart selection
- `app/src/notebooks/editor/mod.rs` — notebook table appearance (scrollbar colors, etc.)

## Proposed changes

### Shared scrollbar primitives in `rookui_core`
`crates/rookui_core/src/elements/shared_scrollbar.rs` is the single source of truth for:
...
`crates/rookui_core/src/elements/clipped_scrollable.rs`:
...
- PRODUCT invariants 13–14 (selection anchor stability): `rookui_core::elements::new_scrollable::scrollable_test` regressions for viewport-coordinate selection APIs, re-anchoring existing selections across horizontal scroll, and clearing the anchor on new mouse-down interactions.
...
- `cargo check -p rook_editor` and `cargo check -p rookui_core`
...
If the editor table path reintroduces bespoke thumb sizing or drag math, it will drift from the rest of RookUI. Mitigation: keep geometry and pointer/scroll conversion in `shared_scrollbar.rs` and call the shared helpers from both the editor table renderer and the `new_scrollable` utilities.
