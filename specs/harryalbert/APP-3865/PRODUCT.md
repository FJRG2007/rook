...
2. **Tools panel** — toggles the left panel containing project explorer, global search, rook drive, conversation history (tool icon)
...
- **Rook Drive item kebab button:** When the tools panel is on the right, the kebab `⋮` button renders as a left-aligned overlay (flush with the left edge) instead of appending to the right end of the row. The opened menu also opens toward the center.
- **Rook Drive hover previews and dialogs:** Hover preview sidecars and share/naming dialogs flip to open toward the center.
...
18. Rook Drive item hover previews open toward the center of the screen (left when panel is on the right).
...
21. Rook Drive kebab button appears as a left-edge overlay when the panel is on the right side.
