...
Related: https://github.com/FJRG2007/rook/issues/2618 (set rook theme in launch configuration)

## Summary

The Rook theme is a single global value today (`appearance.themes.theme` in
...
6. Adding, editing, or removing entries in `directory_overrides` while
   Rook is running re-evaluates every open tab. Tabs whose effective theme
...
7a. **`directory_overrides` is stored locally and never synced to Rook's
...
7b. **Diagnostic output never contains raw `directory_overrides` keys.**
    Local logs are routinely shared with Rook support and copied into
...
- **Unknown theme name** — anywhere it appears, the entry is skipped at
  apply time, a one-line warning is written to the Rook log identifying
