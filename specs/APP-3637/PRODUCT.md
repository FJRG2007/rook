...
## Problem
When users compose prompts for CLI agents through Rook's rich input (Ctrl-G or the Compose button), they cannot browse or invoke skills. The normal Rook agent input supports skills, but the CLI agent input does not. Users must manually type `/skill-name` without any discovery or autocomplete.
...
- Hide Rook-specific slash commands that don't apply to CLI agents.

## Non-goals
- Surfacing non-natively-supported skills (e.g., bundled Rook skills like `oz-platform`). Only skills the active CLI agent can interpret should appear.
...
### Trigger
User types `/` at the start of the input in the CLI agent rich input, same trigger as in normal Rook input.
...
Skills that don't match the active CLI agent's supported providers are hidden from the menu entirely. Non-natively supported skills (including bundled Rook skills like `oz-platform`) are not shown — supporting them is a potential follow-up.
...
- Rook-specific slash commands do not appear in the CLI agent menu (except `/skills`).
