...
2. Rook writes the default template to `~/.rook/tab_configs/my_tab_config.toml` and opens it in the user's configured editor.
...
- Oz generates a valid `.toml` file and writes it to `~/.rook/tab_configs/`.
...
- Rook opens a new tab with the specified pane layout, running any configured commands.
...
- `worktree_name_autogenerated` (optional, bool, default false): when `true`, Rook auto-generates the worktree branch name instead of prompting the user. The app detects worktree configs by scanning `commands` for `git worktree`.
...
8. The `tab-configs` bundled skill appears in `/skills` in Rook.
