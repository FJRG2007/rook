...
Tab configs are TOML files in `~/.rook/tab_configs/`. They previously used a recursive `TabConfigLayout` struct with nested `panes: Vec<TabConfigLayout>` for child splits, which became impractical at depth 3+.
...
1. User creates/edits a `.toml` file in `~/.rook/tab_configs/`.
2. Filesystem watcher in `RookConfig` detects the change and calls `load_tab_configs`.
...
- Manual: build and run Rook, create tab configs, verify pane layouts open correctly.
