...
We're releasing a new version of the Rook notification plugin for Claude Code (v2.0.0). Existing users on v1.1.0 won't automatically receive the update because Claude Code's plugin update system is unreliable. We need Rook to detect outdated plugin versions and prompt users to update.
...
- Label: "Update Rook plugin"
- Tooltip: "A new version of the Rook plugin is available"
...
The plugin reports its own version via a `plugin_version` field in the `SessionStart` event when it connects. Rook compares this against a minimum required version. This works identically for local and remote sessions — no filesystem check needed for update detection.
...
On click, Rook runs `marketplace add` (to refresh the local clone) + `plugin update` via the CLI. Same UX pattern as auto-install: persistent toast while running, success/failure toast on completion. A post-update sanity check verifies the on-disk version actually changed.

On success: "Rook plugin updated. Please run /reload-plugins to activate."
