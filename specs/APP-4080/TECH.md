...
- **Global AI disabled blocks a third-party setting**: make the UI toggle usable even when Rook AI is disabled, since the setting also affects plugin-backed third-party CLI sessions.
...
- `cargo test -p rook workspace::view::vertical_tabs_tests`
- `cargo test -p rook terminal::cli_agent_sessions::mod_tests`
- `cargo test -p rook settings::ai_tests`
