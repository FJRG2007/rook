...
Add a user setting named `Use latest user prompt as conversation title in tab names` that controls which conversation text Rook shows in vertical tabs for agent conversations.
...
The desired outcome is that Rook's default tab-name behavior uses stable, title-like conversation labels, while users who prefer prompt-based labels can explicitly opt into using the latest prompt.
...
Rook exposes a setting in the AI settings area, near settings that affect agent input or agent conversation behavior.
...
- Oz agent conversations, including local agent conversations and cloud agent conversations shown in Rook.
- Plugin-backed third-party CLI agent sessions, such as supported Claude Code, Codex, or similar sessions where Rook receives structured session metadata from the agent plugin.
...
- Non-terminal panes such as code panes, notebooks, workflows, settings, files, or Rook Drive objects.
...
- If the preferred text type is missing, Rook falls back to the other text type when available.
- If both title-like metadata and prompt metadata are missing, Rook uses the existing terminal fallback chain.
...
If Rook normally syncs comparable AI display preferences across devices, this setting should follow that same sync behavior. If comparable AI display preferences are local-only, this setting should follow that local-only behavior instead.
...
- **Persistence**: Toggle the setting, restart Rook, and verify the setting value and vertical-tabs behavior are preserved.
