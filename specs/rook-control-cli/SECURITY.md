# rookctrl security architecture
`rookctrl` is a local-control CLI for an already-running Rook app instance. The design is external-only: all callers are same-user processes. There is no inside-Rook/outside-Rook distinction, no verified-terminal invocation context, and no authenticated-user identity layer.
...
5. **App-side enforcement:** The running Rook app verifies the exact granted action and resolves targets deterministically. Close actions flow through normal Rook close behavior so existing app warnings remain authoritative.
...
## Security goals
- Allow same-user processes to control a running Rook instance through a typed, allowlisted interface when Scripting is enabled.
...
- Prevent other OS users from controlling a Rook instance they do not own.
- Support multiple running Rook processes without a shared global port or credential.
...
- Route close actions through normal Rook close behavior so existing app warnings for unsaved files, running processes, and shared sessions remain authoritative.
...
The broker authenticates the connecting process's OS user through kernel peer credentials. It does not prove that the caller is the official `rookctrl` binary, Rook-signed code, or a human-approved invocation. When Scripting is enabled, any process running as the same OS user can:
- Connect to the broker socket and request credentials for any of the 84 actions.
- Invoke `rookctrl` as a confused deputy.
...
- Normal Rook close behavior preserves existing warnings for close actions.
- App-side revalidation catches stale or misused credentials.
A hostile same-user process that can automate the Rook UI, read local state, or invoke `rookctrl` is not made safe by this architecture. The value is preventing easy ambient paths (web-origin, other-user, unauthenticated localhost) and giving honest callers narrow, auditable grants.
## Threat model
### In scope
- Other local OS users attempting to control a Rook instance owned by the current user.
...
- Stale discovery records from exited Rook processes.
- Multiple running Rook instances where ambiguous selection could target the wrong process.
...
- **macOS:** Keychain via the platform Security framework generic-password API. This is same-user secret storage that keeps the value out of `settings.toml` and cloud sync; it is **not** currently constrained to Rook-signed code through a Keychain access-control list.
...
## Discovery registry
Each enabled Rook process writes a discovery record in a secure per-user directory.
...
- Issuing Rook instance.
- The one granted `ActionKind`.
- Issued-at time and expiry time.
- Unique credential ID for revocation.
### Issuance
The broker is a Unix-domain socket inside the owner-only discovery directory. Before reading any request, the broker calls the platform peer-credential API and verifies the connecting process's UID equals Rook's effective UID.
Issuance flow:
...
This prevents accidental overreach and gives Rook a structured point to deny sensitive actions. It does not make untrusted same-user software safe.
### Close behavior
The three destructive actions (`window.close`, `tab.close`, `pane.close`) execute after the same exact-action credential validation as every other action. They flow through normal Rook close behavior so existing app warnings remain authoritative.
...
- Normal Rook close behavior preserves existing warnings for close actions.
- No `input.run` action exists, so `rookctrl` cannot be used to execute terminal commands.
...
These mitigations route operations through intentional flows. They do not guarantee that arbitrary same-user software cannot cause Rook-visible actions.
...
- Authoritative Scripting value: Keychain generic-password item (same-user secret storage; not currently ACL-constrained to Rook-signed code).
...
- Broker: Unix-domain socket with peer credential checks.
### Windows
Fails closed. `rookctrl` is not available on Windows until:
...
Until these are implemented, the `rookctrl` wrapper on Windows returns a structured error and does not fall back to unauthenticated control.
...
- Close actions flow through normal Rook close behavior.
