# Context
`PRODUCT.md` defines a local Rook control CLI (`rookctrl`) with an allowlisted catalog of exactly 84 actions, deterministic addressing across multiple running Rook app processes, and a simple enabled/disabled Scripting setting. `SECURITY.md` is the normative security architecture. If this technical plan and `SECURITY.md` disagree, update the plan before implementing.
The design is external-only: all callers are same-user processes. There is no inside-Rook/outside-Rook distinction, no verified-terminal invocation context, and no authenticated-user identity layer. Security relies on owner-only filesystem discovery, same-user Unix credential broker with kernel peer credentials, short-lived instance-bound exact-action credentials, loopback HTTP transport, and app-side enforcement.
...
- `crates/rook_cli/src/lib.rs` defines existing CLI/parser conventions and channel-specific command naming.
...
- Close actions (`window.close`, `tab.close`, `pane.close`) flow through normal Rook close behavior so existing app warnings remain authoritative.
...
Create a shared protocol crate used by both the app server and the `rookctrl` client. It defines:
...
Design:
- Each Rook process creates a random opaque `instance_id` at startup.
...
Default discovery directory: `~/.rook/local-control/`. `$XDG_RUNTIME_DIR/rook/local-control` is preferred when available. On Unix, the directory is restricted to `0700` and records/sockets to `0600`.
### 3. Credential broker
...
3. Broker calls the platform peer-credential API and verifies the connecting process's UID equals Rook's effective UID. The broker authenticates the OS user, not the calling application.
...
The HTTP handler runs on a Tokio runtime thread. It cannot directly access RookUI state because all UI state is single-threaded on the main app event loop. The bridge transfers work to the main thread.
#### Thread model
...
- **Main app thread:** Owns all RookUI entities (`App`, `AppContext`, views, models). All UI state reads and mutations happen here.
...
### 7. Close behavior
The 3 close actions (`window.close`, `tab.close`, `pane.close`) flow through normal Rook close behavior after exact-action credential validation and deterministic target resolution. Existing warnings for unsaved files, running processes, shared sessions, and similar app state remain authoritative and may cancel the close.
...
- `OutputFormat` enum (`Pretty`, `Json`, `Ndjson`, `Text`) shared from `rook_cli`.
New subcommand types live in `rook_cli::local_control` and follow existing `#[derive(Parser)]` patterns.
### 9. CLI packaging
The shipped product is a bundled `rookctrl` wrapper script that calls the channel-specific Rook binary with a hidden `--rookctrl` flag:
- **macOS:** `Resources/bin/rookctrl` wrapper in the app bundle, same pattern as the Oz wrapper.
- **Linux:** Small `rookctrl` wrapper or symlink in the app package.
- **Windows:** Fails closed until authenticated broker transport is implemented.
Startup: `app/src/lib.rs` recognizes `--rookctrl` before app launch and routes into `rook_cli::local_control`. The control-mode path initializes only command parsing, discovery, credential material, HTTP transport, and output formatting. It does not initialize GUI state, rendering, or terminal session models.
### 10. Feature flag
Gate behind `FeatureFlag::RookControlCli` with Cargo feature `rook_control_cli`.
When disabled:
...
- The `rookctrl` wrapper returns a structured `no_instance` or feature-disabled error.
When enabled:
...
- `resources/bundled/skills/rookctrl/SKILL.md` teaches the built-in agent and users how to discover and invoke the allowlisted CLI surface.
- The skill manager maps `rookctrl` to `FeatureFlag::RookControlCli` through `BundledSkillActivation`. Both skill listing and direct bundled-skill reads enforce the activation state.
...
- `FeatureFlag::RookControlCli` and Cargo feature.
...
- `rookctrl` wrapper entrypoint with `--rookctrl` control-mode dispatch.
...
- Window/tab mutations (including close through normal Rook close behavior).
- Pane mutations (including close through normal Rook close behavior).
- Session actions.
...
sequenceDiagram
    ...
    participant UI as Rook app state
    ...
    BROKER->>BROKER: Verify peer UID == Rook UID
...
```
## Testing
- **Catalog invariant:** Every `ActionKind` with `Implemented` status has a parseable `rookctrl` CLI route, generated help/completion coverage, and an app-side bridge handler.
...
- **Channel isolation:** Discovery tests prove that a CLI scan excludes records published by other Rook channels.
...
- **Bundled skill gate:** Tests verify the `rookctrl` bundled skill is discoverable and readable only while `FeatureFlag::RookControlCli` is enabled, without affecting unrelated bundled skills.
## Risks and mitigations
- **Same-user residual risk:** The broker authenticates the OS user, not the calling application. Any process running as the same user can request credentials. Mitigated by: protected enablement, short expiry, exact-action grants, app-side revalidation, normal Rook close warnings for close actions.
...
- **Heavyweight CLI startup:** Mitigated by: `--rookctrl` routes before GUI launch, control-mode path initializes only what's needed.
