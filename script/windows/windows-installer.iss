...
#endif
// Comments here have to use `//`: the `#include` above ends inside a [Code]
// section, so until [Setup] below this file is read by the Pascal compiler,
// where `;` terminates a statement rather than starting a comment.
//
// The name the user sees: Start menu, Add/Remove Programs, shortcuts and the
// Explorer context menu. MyAppName stays identity - the AppUserModelID, the
// per-user directories under APPDATA and LOCALAPPDATA, the URL scheme and the
// registry keys all have to keep matching what the Rust side derives from the
// channel (paths.rs, channel/state.rs), so renaming it would orphan the data
// of an install already out there. Only the OSS channel passes a display name
// of its own, because in this fork it is not one variant among several
// channels - it is the product.
...
#define ChannelPascalCase \
  ...
  "Unknown")))))
#define AppMutexName "Local\Rook" + ChannelPascalCase + "_SingleInstance"


[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId=rook-terminal-{#ReleaseChannel}
...
; Force close previous Rook if it hasn't shut down yet.
; In the update flow we already warn the user if they have something running and make them confirm
; before running this installer. Therefore, we are good to force close Rook without fear of losing
; unsaved work.
...
; For manual installs: if Rook is running, show a dialog prompting the user to close it
...
[Registry]
Root: HKCU; Subkey: "SOFTWARE\Rook.dev\{#MyAppName}"; Flags: uninsdeletekey
Root: HKCU; Subkey: "SOFTWARE\Rook.dev\{#MyAppName}"; ValueType: string; ValueName: "InstallationPath"; ValueData: "{app}\{#MyAppExeName}"; Flags: uninsdeletevalue
...
; cleanup "Open Rook Here" registry entries
...
; Add "Open Rook in new tab" to directory context menu
...
; Add "Open Rook in new tab" to directory background context menu
...
; Add "Open Rook in new window" to directory context menu
...
; Add "Open Rook in new window" to directory background context menu
...
[Icons]
Name: "{autoprograms}\{#MyAppDisplayName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.rook.{#MyAppName}"
Name: "{autodesktop}\{#MyAppDisplayName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.rook.{#MyAppName}"; Tasks: desktopicon
...
{ Returns true when the installer was launched by Rook's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Rook process. Instead we
  wait for Rook to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Rook first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Rook was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Rook to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Rook to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Rook mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. rook.exe, dev.exe) is unique
          ...
          rely on Rook's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
          cascade-kill any child OpenConsole.exe processes when rook.exe dies. }
      ...
      begin
        Log('Rook has exited; proceeding with file installation.');
        { The minidump crash-reporter is a child process (same exe name) that
          may outlive the main Rook process. It holds the executable file open,
  ...
  { After a successful install, write a helper script for running the Rook CLI. }
  { We use this to add a "rook-" prefix (e.g. "rook-preview.cmd" vs. "preview.exe") }
  if CurStep = ssPostInstall then begin
    { Add Rook to PATH if requested }
