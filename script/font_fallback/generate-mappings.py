...
Assumptions:
- The fallback fonts in the prod `rook-static-assets` bucket are identical to the
  ones stored in the staging `rook-server-staging-static-assets` bucket.
...
# Rook renders these glyphs next to mac modifier glyphs (e.g. ⌘ U+2318, ⌥ U+2325)
...
def download_fallback_fonts():
    ...
    command = f"gcloud storage cp 'gs://rook-static-assets/fallback-fonts/**/*Regular*.ttf' '{FONT_DOWNLOAD_DIR}'"
