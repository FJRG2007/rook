#!/bin/bash
# Builds a structured JSON notification payload for rook://cli-agent.
#
...
# Negotiate the protocol version with Rook.
# Uses min(plugin_current, rook_declared), falling back to 1 if Rook doesn't advertise a version.
negotiate_protocol_version() {
    local rook_version="${ROOK_CLI_AGENT_PROTOCOL_VERSION:-1}"
    if [ "$rook_version" -lt "$PLUGIN_CURRENT_PROTOCOL_VERSION" ] 2>/dev/null; then
        echo "$rook_version"
    else
