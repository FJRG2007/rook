...
rook-server owns the format. It publishes the schema for each version it
supports and validates a tree with the same parser the apply path uses. This
skill carries no copy of the format: a copy ships inside a Rook release, goes
...
```bash
curl -s https://app.rook.dev/api/v1/factory-files/schemas
curl -s https://app.rook.dev/api/v1/factory-files/schemas/<schemaVersion>
```
...
<url>`, or `ROOK_SERVER_ROOT`, to point at a local, staging, or self-hosted
server. No credential is required; `ROOK_API_KEY` is forwarded when the
...
authoring works offline. Both have been tried and removed. A copy inside a Rook
