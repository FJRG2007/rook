...
description: Internal Oz child-agent orchestration playbook for Codex child runs. Use only when the Oz harness system prompt tells you to coordinate with a lead run through the Oz CLI in `OZ_CLI`, using `OZ_RUN_ID` and `OZ_PARENT_RUN_ID`.
---
# Oz Child Agent Orchestration
Use this skill when you are a spawned team agent running inside Codex.
...
- Keep your output concise and operational (for example exact file paths, commands to run, and concrete next steps).
...
Do not assume Codex will surface parent messages automatically.
...
- before deciding you still need to wait for lead agent input,
- before saying you are done, waiting, standing by, or otherwise ending your turn.

### Waiting for replies
If you are waiting on the lead agent, periodically poll for unread messages yourself instead of assuming the harness will wake you automatically. Use a modest cadence rather than a tight loop — for example wait a short interval between checks, then run `"$OZ_CLI" run message list "$OZ_RUN_ID" --unread --limit 25` again. Once a reply arrives, read it and resume work.
Never say you are standing by until a post-send or pre-idle inbox check has shown no unread messages that require action.
