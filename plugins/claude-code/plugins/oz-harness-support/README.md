...
Rook integration for [Claude Code](https://docs.anthropic.com/en/docs/claude-code) running inside Rook's **Oz cloud agent** environments.
...
- **factory-files** — author and validate file-based Rook software factory definitions
...
`f6f4ceac8`. Rook bundles that skill for its own clients; Claude Code loads
filesystem skills from this plugin instead, so it is copied here rather than
resolved from a bundle. Change it in `FJRG2007/rook` and re-mirror; edits
...
- Rook's Oz cloud agent environment (provides the `oz` CLI and `OZ_*` environment variables)
