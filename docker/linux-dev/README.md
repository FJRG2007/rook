...
The Dockerfile in this directory defines a container that has all of the necessary tools installed to quickly get engineers up and running with building Rook on Linux.
...
```
CONTAINER_NAME="rook-client-linux-dev"
...
# container. This can be the `rook` repository or some parent
...
You should be able to SSH into the container and build and run rook without any additional setup (dev account password is "password"):
...
It's possible you'll run into some odd errors while trying to compile Rook; if so, just keep rerunning the cargo command and it should work eventually.
