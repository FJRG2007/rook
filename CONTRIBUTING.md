...
- Issues marked `rook:reserved-internal` are being handled by the Rook team and are not open for contributor PRs.
...
- **Review is largely automated.** When you open a PR, Oz is auto-assigned and produces an initial review. Once Oz approves, it automatically requests a follow-up review from a Rook team subject-matter expert — you do not need to assign human reviewers yourself.

### Readiness labels

The Rook team applies one of the following labels when an issue is ready for contribution:
...
- **`needs-mocks`** — Design mocks are required before implementation can begin. Wait for the Rook team to land them.
- **`rook:reserved-internal`** — The Rook team is reserving this work for internal implementation or alignment. Do not open a spec or code PR for issues with this label; Oz will reject contributor PRs linked to them with an explanatory comment.
...
Steps owned by you (the contributor) are shown in yellow; steps owned by the Rook team or Oz are shown in blue.

```mermaid
flowchart TD
    A[File an issue] --> B{Rook team triages}
    ...
    classDef rookTeam fill:#dbeafe,stroke:#1d4ed8,color:#1e3a8a;
    class A,C,E contributor;
    class B,D,F rookTeam;
```

## Filing a Good Issue

Search [existing issues](https://github.com/FJRG2007/rook/issues) before filing to avoid duplicates. Use the issue templates when filing.

If you're already running Rook, the fastest way to file is the `/feedback` command — it opens a public GitHub issue with relevant context (logs, environment details) automatically attached.
...
- Rook version and OS (see `Settings → About`).
...
- `ROOK_COMMON_SKILLS_INSTALL_TARGET=project ./script/bootstrap` and `ROOK_COMMON_SKILLS_INSTALL_TARGET=global ./script/bootstrap` select the same targets non-interactively.
...
You **do not need to manually request reviewers**. Oz is auto-assigned to PRs that target a ready issue and produces an initial review. After Oz approves, it automatically requests a follow-up review from the appropriate Rook team subject-matter expert.

After you push changes that address Oz's feedback, comment `/rook-agent-review` on the PR to request a re-review — you can do this up to **three times** per PR. If something looks stuck or you need more reviews than that, mention **@oss-maintainers** on the PR to escalate to the team.
...
If a maintainer requests changes to your PR, you will need to request `/rook-agent-review` again and pass it before a re-review can be requested. Oz will request the re-review for you automatically once you pass its reviews.
...
You can use **any coding agent** to implement a contribution — for example, Rook's built-in agent, Claude Code, Codex, Gemini CLI, or others — or no agent at all. This repository ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`AGENTS.md`](AGENTS.md)) that any harness supporting these formats can pick up.
...
- See [AGENTS.md](AGENTS.md) for the full style guide, including RookUI patterns and terminal model locking rules.
