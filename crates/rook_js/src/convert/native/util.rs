...
use super::{FromRookJs, JsFunctionRegistry};
    ...
    T: FromRookJs<'js>,
{
    ...
    if value.is_array() {
        Vec::<T>::from_rook_js(ctx, value, js_function_registry)
    } else {
        Ok(vec![T::from_rook_js(ctx, value, js_function_registry)?])
    }
}
    ...
    T: FromRookJs<'js>,
{
    ...
    T::from_rook_js(ctx, value, js_function_registry)
}
