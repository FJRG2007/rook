...
/// types.
///
/// `I` is the type of the function's input, which must implement `IntoRookJs` and be
/// deserializable.
/// `O` is the type of the function's return value, which must implement `FromRookJs` and be
/// serializable.
