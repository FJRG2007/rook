...
use crate::{FromRookJs, IntoRookJs, JsFunctionId, SerializedJsValue, TypedJsFunctionRef};

impl<I, O> TypedJsFunctionRef<I, O>
where
    I: for<'a> IntoRookJs<'a> + DeserializeOwned + Clone + 'static,
    O: for<'a> FromRookJs<'a> + Serialize + Clone + 'static,
{
...
impl JsFunctionRegistry {
    ...
    ///
    /// `I` and `O` must have corresponding `IntoRookJs`/`FromRookJs` implementations so they
...
pub trait CallableJsFunction {
    ...
    /// `IntoRookJs`-implemmenting Rust type, and returns the serialized bytes representation of
    /// the function's output (which is of a Rust type that implemenets `FromRookJs`).
    fn call(
        &self,
    ...
    O: for<'a> FromRookJs<'a> + Serialize + Clone + 'static,
{
    fn call(
        ...
        let input_value = input.into_rook_js(ctx)?;
        let func = self.js_function.clone().restore(ctx)?;
        let output = O::from_rook_js(ctx, func.call((input_value,))?, function_registry)?;
...
///
/// `I` is the type of the function's input, which must implement `IntoRookJs`.
/// `O` is the type of the function's return value, which must implement `FromRookJs`.
#[derive(Clone)]
...
where
    I: for<'a> IntoRookJs<'a> + DeserializeOwned + 'static,
    O: for<'a> FromRookJs<'a> + Serialize + 'static,
{
