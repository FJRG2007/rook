...
fn test_highlight_urls_unicode() {
    let test_runs = vec![StyledBufferRun {
        run: "This (not https://example.com) is a 🔥 link about a 🇨🇦 🏡:\u{a0}https://rook.dev"
            .to_string(),
...
fn test_links_not_auto_highlighted() {
    ...
    let runs = &[
        StyledBufferRun {
            run: "first link is https://rook.dev ".to_string(),
            text_styles: Default::default(),
            block_style: BufferBlockStyle::PlainText,
        },
        StyledBufferRun {
            run: "http://example.com".to_string(),
            text_styles: TextStylesWithMetadata::default().link("https://rook.dev".to_string()),
            block_style: BufferBlockStyle::PlainText,
        },
        StyledBufferRun {
...
fn unique_markdown_image_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "rook_editor_markdown_image_{}_{nonce}.png",
        std::process::id()
    ))
}
...
fn test_table_inline_style_runs_preserve_markdown_cell_styles() {
    App::test((), |app| async move {
        ...
        app.read(|ctx| {
            ...
            let table = crate::content::text::table_from_internal_format_with_inline_markdown(
                "Header\tValue\nText\t**Bold** *Italic* [Link](https://rook.dev) `code`\n",
                Vec::new(),
            );
