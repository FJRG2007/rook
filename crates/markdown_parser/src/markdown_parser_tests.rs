...
fn test_parse_link_in_tag() {
    ...
    let source = "a [https://google.com](https://rook.dev) link";
    assert_eq!(
        test_parse_markdown(source),
        vec![FormattedTextLine::Line(vec![
            FormattedTextFragment::plain_text("a "),
            FormattedTextFragment::hyperlink("https://google.com", "https://rook.dev"),
            FormattedTextFragment::plain_text(" link")
        ])]
    );
}
...
fn test_autolink_with_escaped_dash() {
    assert_eq!(
        test_parse_markdown("https://rook\\-dev.com"),
        vec![FormattedTextLine::Line(vec![
            FormattedTextFragment::hyperlink("https://rook-dev.com", "https://rook-dev.com"),
        ])]
    );
}
...
fn test_parse_escape_in_style() {
    let source = "Some *styled \\* escaped* [te\\]xt\\^](https://rook.dev)";
    assert_eq!(
        test_parse_markdown(source),
        vec![FormattedTextLine::Line(vec![
            ...
            FormattedTextFragment::plain_text(" "),
            FormattedTextFragment::hyperlink("te]xt^", "https://rook.dev")
        ])]
    );
}
...
fn test_parse_inline_link() {
    assert_eq!(
        parse_all("[basic](https://rook.dev)", parse_inline),
...
fn test_parse_embedded() {
    assert_eq!(
        test_parse_markdown("```rook-embedded-object\nid: workflow-123\n```"),
    ...
    assert_eq!(
        ...
        vec![FormattedTextLine::CodeBlock(CodeBlockText {
            lang: "rook-embedded-object".to_string(),
...
fn test_parse_table_with_links() {
    ...
    if let FormattedTextLine::Table(table) = &result[0] {
        ...
        assert_eq!(link_cell[0].text, "Rook");
        assert!(matches!(
            &link_cell[0].styles.hyperlink,
            Some(Hyperlink::Url(url)) if url == "https://rook.dev"
        ));
    } else {
        panic!("Expected table");
    }
