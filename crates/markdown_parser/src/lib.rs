...
/// This purposefully shadows the `Action` trait from `rookui`.
///
/// Since `rookui` depends on this crate, we can't depend on the `rookui_core::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `rookui_core::Action` implementing this `Action`.
...
impl FormattedTable {
    /// Parse from the internal tab-separated format used in `rook-markdown-table` code blocks.
    ...
    /// Serialize to the internal tab-separated format used in `rook-markdown-table` code blocks.
