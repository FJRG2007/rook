...
use rook::integration_testing::block::{
...
use rook::integration_testing::command_palette::{
    TestStepsExt, close_command_palette, open_command_palette, open_command_palette_and_run_action,
};
use rook::integration_testing::context_chips::assert_working_dir_is_present;
use rook::integration_testing::find::{Find, FindWithinBlockState};
use rook::integration_testing::input::{
    input_contains_string, input_is_empty, open_input_context_menu,
};
use rook::integration_testing::navigation_palette::{
...
use rook::integration_testing::view_getters::{
...
use rook::integration_testing::workspace::assert_tab_count;
use rook::integration_testing::{self, view_of_type};
use rook::pane_group::AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH;
use rook::settings::{
    CompletionsOpenWhileTyping, CtrlTabBehavior, INPUT_MODE, MonospaceFontSize, TabBehavior,
};
use rook::settings_view::keybindings::KeybindingsView;
use rook::settings_view::{FeaturesPageAction, SettingsAction, SettingsSection, SettingsView};
use rook::terminal::alt_screen_reporting::MouseReportingEnabled;
use rook::terminal::available_shells::AvailableShells;
use rook::terminal::block_list_viewport::{InputMode, ScrollLines, ScrollPosition};
use rook::terminal::find::TerminalFindModel;
use rook::terminal::input::{Input, InputSuggestionsMode};
use rook::terminal::keys_settings::KeysSettings;
use rook::terminal::local_tty::TerminalManager as LocalTtyTerminalManager;
use rook::terminal::model::ansi::{Handler, InitShellValue};
use rook::terminal::model::blocks::{BlockHeightItem, BlockHeightSummary, TotalIndex};
use rook::terminal::model::grid::Dimensions;
use rook::terminal::model::grid::grid_handler::TermMode;
use rook::terminal::model::terminal_model::BlockIndex;
use rook::terminal::session_settings::{HonorPS1, SessionSettings, StartupShellOverride};
use rook::terminal::view::{
    ALIAS_EXPANSION_BANNER_SEEN_KEY, BlockVisibilityMode, TerminalAction, TerminalViewState,
};
use rook::terminal::{TerminalView, shell};
use rook::util::bindings::CustomAction;
use rook::workflows::categories::CategoriesView;
use rook::workspace::{
    NEW_SESSION_MENU_BUTTON_POSITION_ID, NEW_TAB_BUTTON_POSITION_ID, Workspace, WorkspaceAction,
};
use rook::{AgentModeEntrypoint, cmd_or_ctrl_shift};
use rookui_core::event::KeyState;
use rookui_core::integration::{AssertionOutcome, StepData, TestStep};
use rookui_core::keymap::{Keystroke, PerPlatformKeystroke, Trigger};
use rookui_core::platform::keyboard::KeyCode;
use rookui_core::platform::{OperatingSystem, TerminationMode};
...
/// Adds a workflow file, containing two workflows, to the mocked out rook
/// config directory and verifies that the workflows appear in the workflow menu.
pub fn test_add_workflows_to_rook_config() -> Builder {
    new_builder()
        .with_setup(move |utils| {
            utils.set_env("ROOK_CONFIG_WATCHER_DELAY_MS", Some((10).to_string()));
            ...
            TestStep::new("Should have no local workflows").add_named_assertion(
                ...
                |app, window_id| {
                    ...
                    workflows.read(app, |workflows, _| {
                        ...
                        // we don't have concurrency with a RookConfig watcher thread
                        assert_eq!(
...
pub fn test_launch_rook_with_theme_in_rook_config() -> Builder {
    new_builder()
        .with_setup(move |utils| {
            utils.set_env("ROOK_CONFIG_WATCHER_DELAY_MS", Some((10).to_string()));
...
/// Adds a theme to the mocked out rook config directory and verifies that
/// the theme appears in the theme picker.
pub fn test_add_theme_to_rook_config() -> Builder {
    new_builder()
        .with_setup(move |utils| {
            utils.set_env("ROOK_CONFIG_WATCHER_DELAY_MS", Some((10).to_string()));
...
pub fn test_suggestions_menu_positioning() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Open Rook Drive")
...
precmd_functions+=(_p9k_precmd)
        ...
        // If the user then switches back to the Rook prompt, we should close the banner.
        .with_step(
...
/// Default auto title. We test that Rook's auto title is used and verify that
...
/// Validate that disabling Rook's auto title feature will not mess with oh-my-zsh settings.
pub fn test_rook_auto_title_disabled() -> Builder {
    new_builder()
        .set_should_run_test(|| {
            // Only run this one on bash and zsh
            let (starter, _) = current_shell_starter_and_version();
...
"#,
            ...
            write_rc_files_for_test(
                &dir,
                r#"
ROOK_DISABLE_AUTO_TITLE="true"
"#,
                [ShellRcType::Zsh],
            );
        })
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        // If Rook title is disabled, we don't set the DISABLE_AUTO_TITLE env variable
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "echo $DISABLE_AUTO_TITLE".to_string(),
            ExpectedExitStatus::Success,
...
/// Checks that the tab title set by the user takes precedence over the Rook's default title and
/// doesn't require any additional setting from the user's POV. This is bash-specific test.
pub fn test_rook_honors_user_title_bash() -> Builder {
    new_builder()
        .set_should_run_test(|| {
...
pub fn test_rook_honors_user_title_zsh() -> Builder {
    new_builder()
        .set_should_run_test(|| {
            // Only run this one on bash
            let (starter, _) = current_shell_starter_and_version();
...
/// directory (for example `wt switch` from worktrunk) keep Rook's per-block CWD
    ...
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Assert OSC 7 updated the previous block's pwd")
                ...
                // `refresh_rook_prompt` in the `BlockWorkingDirectoryUpdated`
...
pub fn test_git_prompt() -> Builder {
    ...
    // here because that would put us in the rook repo. We need to
...
pub fn test_copy_block_command_and_output_honor_ps1_disabled() -> Builder {
    let command = "echo ROOK_COPY_E2E_OUTPUT";
    new_builder()
        .use_tmp_filesystem_for_test_root_directory()
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(execute_command_for_single_terminal_in_tab(
...
pub fn test_copy_block_command_and_output_honor_ps1_enabled() -> Builder {
    let prompt_text = "this is my custom prompt";
    let command = "echo ROOK_PS1_COPY_E2E_OUTPUT";
    new_builder()
        // TODO(CORE-2732): Flakey on linux
        .set_should_run_test(skip_if_powershell_core_2303)
        .with_user_defaults(HashMap::from([(
...
pub fn test_rook_prompt_unsets_zsh_rprompt() -> Builder {
    new_builder()
        .set_should_run_test(|| {
            let (starter, _) = current_shell_starter_and_version();
...
// the workspace view, but we DO force rook drive open to show the dialog, so we can look for that
    ...
    new_builder()
        ...
        .with_steps(
            open_command_palette_and_run_action("Create a New Team Folder")
                .add_assertion(assert_rook_drive_is_closed()),
        )
        .with_steps(
        ...
        .with_steps(
            open_command_palette_and_run_action("Create a New Team Folder")
                .add_assertion(assert_rook_drive_is_open()),
        )
        .with_steps(
            open_command_palette_and_run_action("Create a New Personal Folder")
                .add_assertion(assert_rook_drive_is_open()),
...
pub fn test_context_chips_prompt_at_bootstrap() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Check Rook prompt")
