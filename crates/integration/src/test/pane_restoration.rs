...
use rook::integration_testing::terminal::util::ExpectedExitStatus;
...
use rook::integration_testing::workspace::{assert_tab_count, trigger_undo_close};
    ...
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Check pane count before undo close")
                .add_assertion(move |app, window_id| {
                    let workspace_view = rook::integration_testing::view_getters::workspace_view(app, window_id);
                    ...
                    rookui_core::async_assert_eq!(initial_pane_count, 1, "Should have exactly one pane after grace period expires - closed pane should be cleaned up")
                }),
        )
        .with_step(trigger_undo_close()
            ...
            .add_assertion(move |app, window_id| {
                ...
                workspace_view.read(app, |workspace, ctx| {
                    ...
                    rookui_core::async_assert_eq!(pane_count, 1, "Should still have only one pane after attempted restore - no pane should be restored")
                })
            }),
        )
        .with_step(
...
pub fn test_closed_panes_cleared_on_rearrangement() -> Builder {
    ...
    new_builder()
        ...
        .with_step(
            ...
            rook::integration_testing::pane_group::close_pane_by_index(
                0, // tab index
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    workspace_view.read(app, |workspace, ctx| {
                        ...
                        if visible_pane_count != 2 {
                            rookui_core::integration::AssertionOutcome::failure(format!("Should have 2 visible panes after closing one (got {visible_pane_count} visible, {total_pane_count} total)"))
                        } else {
                            rookui_core::integration::AssertionOutcome::Success
                        }
                    })
                }),
        )
        .with_step(
            ...
            rook::integration_testing::pane_group::move_pane_by_indices(
                0,
                0,
                1,
                rook::pane_group::tree::Direction::Right,
            ),
        )
        .with_step(
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    workspace_view.read(app, |workspace, ctx| {
                        ...
                        if visible_pane_count < 1 {
                            rookui_core::integration::AssertionOutcome::failure(format!("Should have at least 1 visible pane after undo close attempt, got {visible_pane_count}"))
                        } else {
                            rookui_core::integration::AssertionOutcome::Success
                        }
                    })
        ...
        .with_step(
            trigger_undo_close().add_assertion(move |app, window_id| {
                ...
                workspace_view.read(app, |workspace, ctx| {
                    ...
                    if visible_pane_count < 1 {
                        rookui_core::integration::AssertionOutcome::failure(format!("Should have at least 1 visible pane after second undo close attempt, got {visible_pane_count}"))
                    } else {
                        rookui_core::integration::AssertionOutcome::Success
                    }
                })
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    if visible_pane_count == 1 {
                        ...
                        match (original_result, third_result) {
                            (rookui_core::integration::AssertionOutcome::Success, rookui_core::integration::AssertionOutcome::Success) => {
                                rookui_core::integration::AssertionOutcome::Success
                            }
                            _ => rookui_core::integration::AssertionOutcome::failure("Expected to find both original_pane and third_pane content".to_string())
                        }
                    } else {
                        rookui_core::integration::AssertionOutcome::failure(format!("Unexpected pane count: {visible_pane_count}"))
                    }
                }),
        )
}
    ...
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Verify restored pane has correct content in new tab")
                ...
                .add_assertion(move |app, window_id| {
                    let workspace_view =
                        rook::integration_testing::view_getters::workspace_view(app, window_id);
