...
pub fn test_remote_server_navigate_to_repo() -> Builder {
    ...
    builder
        ...
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "mkdir -p /tmp/rook-test-repo && cd /tmp/rook-test-repo && git init -b main && git config user.email test@test.com && git config user.name TestUser && touch file && git add file && git commit -m init".into(),
            ExpectedExitStatus::Success,
            (),
        ))
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "cd /tmp/rook-test-repo".into(),
            ExpectedExitStatus::Success,
            (),
        ))
        ...
        .with_step(
            ...
            .add_named_assertion_with_data_from_prior_step(
                ...
                assert_remote_server_has_navigated(0, "/tmp/rook-test-repo"),
            ),
        )
...
pub fn test_remote_server_completions() -> Builder {
    ...
    builder
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "touch /tmp/rook-rs-completion-target".into(),
            ExpectedExitStatus::Success,
            (),
        ))
        // Trigger the actual tab-completion request path for a remote file.
        .with_step(
            run_completer(0, "cat /tmp/rook-rs-completion-t").set_timeout(Duration::from_secs(15)),
        )
...
pub fn test_remote_server_lazy_load_directory() -> Builder {
    ...
    builder
        ...
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "rm -rf /tmp/rook-lazy-repo && mkdir -p /tmp/rook-lazy-repo/subdir && cd /tmp/rook-lazy-repo && git init -b main && git config user.email test@test.com && git config user.name TestUser && touch file subdir/nested && git add . && git commit -m init".into(),
            ExpectedExitStatus::Success,
            (),
        ))
        ...
        .with_step(
            ...
            .add_named_assertion_with_data_from_prior_step(
                ...
                assert_remote_server_has_navigated(0, "/tmp/rook-lazy-repo"),
            ),
        )
        ...
        .with_step(
            TestStep::new("Load subdirectory via LoadRepoMetadataDirectory")
                .with_action(load_repo_metadata_directory_via_remote_server(
                    0,
                    "/tmp/rook-lazy-repo".to_string(),
                    "/tmp/rook-lazy-repo/subdir".to_string(),
                ))
        ...
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "cat /tmp/rook-lazy-repo/subdir/nested && echo 'file exists'".into(),
            ExpectedExitStatus::Success,
            "file exists",
        ))
}
...
pub fn test_remote_server_file_operations() -> Builder {
    ...
    builder
        ...
        .with_step(
            TestStep::new("Write file via RemoteServerClient proto API")
                .with_action(write_file_via_remote_server(
                    0,
                    "/tmp/rook-rs-test-file.txt".to_string(),
        ...
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "cat /tmp/rook-rs-test-file.txt".into(),
            ExpectedExitStatus::Success,
            "hello from proto",
        ))
        // Step 3: Clean up
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "rm -f /tmp/rook-rs-test-file.txt".into(),
            ExpectedExitStatus::Success,
            (),
        ))
        .with_step(
