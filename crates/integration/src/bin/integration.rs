...
use rook_cli::WorkerCommand;
use rook_core::AppId;
use rook_core::channel::{Channel, ChannelConfig, ChannelState, OzConfig, RookServerConfig};

/// The Rook integration test runner.
#[derive(Debug, Default, Parser, Clone)]
#[command(name = "rook-integration-test")]
...
pub fn main() -> Result<()> {
    ChannelState::set(ChannelState::new(
        ...
        ChannelConfig {
            ...
            logfile_name: "rook_integration.log".into(),
            server_config: RookServerConfig {
    ...
    if let Some(command) = &args.command {
        match command {
            ...
            WorkerCommand::TerminalServer(args) => {
                ...
                rook::terminal::local_tty::run_terminal_server(args);
                return Ok(());
            }
    ...
    rook::run_integration_test(driver)
}
...
fn register_tests() -> HashMap<&'static str, BoxedBuilderFn> {
    ...
    register_test!(test_rook_auto_title_disabled);
    register_test!(test_rook_honors_user_title_bash);
    register_test!(test_rook_honors_user_title_zsh);
    ...
    register_test!(test_add_theme_to_rook_config);
    register_test!(test_palette_opens_when_theme_chooser_is_open);
    #[cfg(target_os = "macos")]
    register_test!(test_preview_config_dir_migration);
    register_test!(test_launch_rook_with_theme_in_rook_config);
    register_test!(test_add_launch_config_to_rook_config);
    register_test!(test_add_workflows_to_rook_config);
    ...
    register_test!(test_rook_prompt_unsets_zsh_rprompt);
    ...
    register_test!(test_ask_rook_ai_keybinding_for_selected_block);
    ...
    register_test!(test_open_in_rook_banner);
    ...
    // File tree tests
    register_test!(test_file_tree_opens_files_in_rook);
