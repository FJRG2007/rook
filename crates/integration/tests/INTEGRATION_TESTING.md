# Integration tests in Rook
This is a short guide into writing integration tests in Rook.
...
I find `with_keystrokes` and `with_input_string` most helpful methods, so far. You can check the implementation (and expand it!) in [rookui_core/src/integration/step.rs](../../rookui_core/src/integration/step.rs).
...
```sh
for i in {0..100}; do
    ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 RUST_BACKTRACE=full ROOK_SHELL_PATH=/bin/bash cargo run -p integration -- test_simple_example
...
* You can copy over a rook.sqlite file from ~/Library/Application\ Support/{rook, dev.rook.Rook-(Dev|Preview|Stable)} directly
* You may want to sanitize some info that is specific to you (i.e. cwd https://staging.rook.dev/block/FNBafyVtxvjmdNIx6HxUM5)


### How to run integration tests?
To run a specific integration test you can use:
```
  ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS="1" cargo run --bin integration -- test_simple_example
```

The `ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS="1"` will force the new terminal window to open, which helps a lot when iterating on your integration test implementation!

### Known issues / limitations
* To determine (from the `TestStep`) which shell is used for the test, you can try checking `ROOK_SHELL_PATH` environment variable (that works within the CI on github) or check the passwd for the user (for local runs).
* Similarly you can run the test with a specific shell by setting the `ROOK_SHELL_PATH` and then running the test. Note that if you're running with fish, you also need to pass in `--features fish_shell` until that feature flag is removed. For example: `ROOK_SHELL_PATH=/usr/local/bin/fish`, then `cargo run --bin integration --features fish_shell -- test_simple_example`
