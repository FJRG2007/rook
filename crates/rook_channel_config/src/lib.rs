//! Loads a per-channel [`ChannelConfig`] for Rook's channel binaries.
//!
//! For non-bundled builds the internal `rook-channel-config` generator is
...
//! binaries and the `rook_tui` binaries so the loading logic lives in one place.
//!
...
use rook_core::channel::ChannelConfig;

/// The name of the config generator binary, expected to be on PATH.
const CONFIG_BIN_NAME: &str = "rook-channel-config";
...
/// `OUT_DIR`); otherwise the `rook-channel-config` generator is invoked at
/// runtime.
#[macro_export]
