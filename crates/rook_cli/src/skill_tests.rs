...
fn test_parse_repo_qualified() {
    let spec: SkillSpec = "rook-internal:code-review".parse().unwrap();
    assert_eq!(spec.org, None);
...
fn test_parse_full_path_with_repo() {
    let spec: SkillSpec = "rook-server:.agents/skills/test/SKILL.md".parse().unwrap();
    assert_eq!(spec.org, None);
    assert_eq!(spec.repo, Some("rook-server".to_string()));
...
fn test_display_repo_qualified() {
    let spec = SkillSpec::with_repo("rook-internal".to_string(), "code-review".to_string());
    assert_eq!(spec.to_string(), "rook-internal:code-review");
}
...
fn test_parse_empty_path_fails() {
    let result: Result<SkillSpec, _> = "rook-internal:".parse();
    assert!(result.is_err());
}
