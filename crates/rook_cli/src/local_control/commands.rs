//! Implementations for user-facing `rookctrl` command groups.
...
fn render_human_readable(action: ActionKind, data: &serde_json::Value) -> String {
    match action {
        ActionKind::AppPing => format!(
            "Rook instance {} is reachable (protocol version {})",
...
/// JSON payload for `rookctrl instance list`.
#[derive(Serialize)]
    ...
    match output_format {
        ...
        OutputFormat::Pretty | OutputFormat::Text => {
            if output.instances.is_empty() {
                println!("No running Rook instances with local control were found.");
                return Ok(());
            }
