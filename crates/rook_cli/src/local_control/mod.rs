//! Command-line interface for controlling a running local Rook app.
mod commands;
mod completions;
mod output;
mod selectors;
...
/// Hidden flag used by the channel-specific Rook app binary to enter `rookctrl` mode.
pub const CONTROL_MODE_FLAG: &str = "--rookctrl";

/// Parsed top-level arguments for `rookctrl`.
#[derive(Debug, Parser)]
#[command(
    name = "rookctrl",
    display_name = "rookctrl",
    about = "Control a running local Rook app instance"
)]
...
impl ControlArgs {
    pub fn from_env() -> Self {
        let bin_name = crate::binary_name().unwrap_or_else(|| "rookctrl".to_owned());
        Self::try_parse_from_args(std::env::args_os(), bin_name).unwrap_or_else(|err| err.exit())
    }

    /// Parse Rook Control arguments only when the wrapper-injected mode flag is present.
    ///
    /// Startup calls this before the normal Rook/Oz parser. Arguments through
    /// `--rookctrl` are removed, and the remaining arguments are parsed as if
    /// the standalone command name were `rookctrl`.
    ...
    where
        ...
        T: Into<OsString>,
    {
        let mut stripped_args = vec![OsString::from("rookctrl")];
        ...
        found_control_mode.then(|| Self::try_parse_from_args(stripped_args, "rookctrl"))
    }
...
/// Top-level `rookctrl` command groups.
#[derive(Debug, Clone, Subcommand)]
pub enum ControlCommand {
    /// Inspect local Rook app instances.
    #[command(subcommand)]
    Instance(InstanceCommand),
    /// Inspect a selected local Rook app.
    #[command(subcommand)]
    App(AppCommand),
    ...
    /// Inspect local Rook windows.
    #[command(subcommand)]
    Window(WindowCommand),

    /// Control local Rook tabs.
    #[command(subcommand)]
    Tab(TabCommand),
    /// Inspect local Rook panes.
    #[command(subcommand)]
    Pane(PaneCommand),

    /// Inspect local Rook sessions.
    #[command(subcommand)]
    ...
    /// Open or toggle local Rook surfaces.
    #[command(subcommand)]
    ...
    ///     source <(path/to/rookctrl completions bash)
    ///
    /// For zsh, add the following to ~/.zshrc:
    ///     source <(path/to/rookctrl completions zsh)
    ///
    /// For fish, add the following to ~/.config/fish/config.fish:
    ///     path/to/rookctrl completions fish | source
    ///
    /// For Powershell, add the following to $PROFILE:
    ///     path\to\rookctrl completions powershell | Out-String | Invoke-Expression
    ///
...
/// Commands that inspect locally discoverable Rook instances.
#[derive(Debug, Clone, Subcommand)]
pub enum InstanceCommand {
    /// List locally discoverable Rook instances.
    List,
...
/// Commands that inspect the selected Rook app instance.
#[derive(Debug, Clone, Subcommand)]
pub enum AppCommand {
    /// Check that the selected local Rook app responds.
    Ping(TargetArgs),

    /// Print protocol and build identity metadata for the selected local Rook app.
    Version(TargetArgs),

    /// Print the active window/tab/pane/session chain.
    Active(TargetArgs),

    /// Focus the selected local Rook app.
    Focus(TargetArgs),
}
...
pub enum WindowCommand {
    /// List windows in the selected local Rook app.
    List(TargetArgs),

    /// Inspect one window in the selected local Rook app.
    Inspect(TargetArgs),
...
/// Commands that control tabs in the selected Rook app instance.
#[derive(Debug, Clone, Subcommand)]
pub enum TabCommand {
    /// List tabs in the selected local Rook app.
    List(TargetArgs),

    /// Inspect one tab in the selected local Rook app.
    Inspect(TargetArgs),
...
/// Commands that inspect local Rook panes.
#[derive(Debug, Clone, Subcommand)]
pub enum PaneCommand {
    /// List panes in the selected local Rook app.
    List(TargetArgs),

    /// Inspect one pane in the selected local Rook app.
    Inspect(TargetArgs),
...
/// Commands that inspect local Rook sessions.
#[derive(Debug, Clone, Subcommand)]
pub enum SessionCommand {
    /// List sessions in the selected local Rook app.
    List(TargetArgs),

    /// Inspect one session in the selected local Rook app.
    Inspect(TargetArgs),
...
pub enum SurfaceCommand {
    ...
    /// Open or toggle Rook Drive.
    #[command(subcommand)]
    RookDrive(SurfaceOpenToggleCommand),
...
/// Commands that inspect Rook themes.
#[derive(Debug, Clone, Subcommand)]
pub enum ThemeCommand {
    ...
    /// Set whether Rook follows the system theme.
...
/// Exact selectors for a target within the selected Rook instance.
#[derive(Debug, Clone, Args, Default)]
pub struct TargetArgs {
    /// Target a specific local Rook instance id from `rookctrl instance list`.
    #[arg(long = "instance", conflicts_with = "pid")]
    pub instance: Option<String>,

    /// Target a specific local Rook process id.
