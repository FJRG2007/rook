...
fn test_parse_user_default() {
    let result = parse_share_request("ben@rook.dev").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@rook.dev"),
...
fn test_parse_user_view() {
    let result = parse_share_request("ben@rook.dev:view").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@rook.dev"),
        _ => panic!("Expected User subject"),
    }
...
fn test_parse_user_edit() {
    let result = parse_share_request("ben@rook.dev:edit").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@rook.dev"),
        _ => panic!("Expected User subject"),
    }
