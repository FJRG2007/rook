...
pub enum Harness {
    /// Use Rook's built-in MAA infrastructure (default).
    #[default]
    Oz,
...
impl ValueEnum for Harness {
    ...
    fn to_possible_value(&self) -> Option<PossibleValue> {
        let mut pv = match self {
            Harness::Oz => {
                PossibleValue::new("oz").help("Use Rook's built-in MAA infrastructure (default)")
            }
...
pub struct RunAgentArgs {
    ...
    ///
    /// Skills are searched in `.agents/skills/`, `.rook/skills/`, `.claude/skills/`, and `.codex/skills/` directories.
    ...
    /// Display agent progress in the Rook interface.
...
pub struct RunCloudArgs {
    ...
    /// Open the agent's session in Rook once it's available.
    #[arg(long = "open")]
    pub open: bool,
    ...
    /// Where this job should be hosted. Setting "rook" runs it on Rook's infrastructure. Any other
    ...
    /// Execution harness for the agent run.
    ///
    /// "oz" (the default) runs on Rook's built-in agent infrastructure. Other
