...
pub mod share;
pub mod task;
// Each of these variables is injected under both its `OZ_` and its `ROOK_` name, carrying the
// identical value. Read sites still use the `OZ_` names.
pub const OZ_RUN_ID_ENV: &str = "OZ_RUN_ID";
pub const ROOK_RUN_ID_ENV: &str = "ROOK_RUN_ID";
pub const OZ_PARENT_RUN_ID_ENV: &str = "OZ_PARENT_RUN_ID";
pub const ROOK_PARENT_RUN_ID_ENV: &str = "ROOK_PARENT_RUN_ID";
pub const OZ_CLI_ENV: &str = "OZ_CLI";
pub const ROOK_CLI_ENV: &str = "ROOK_CLI";
pub const OZ_HARNESS_ENV: &str = "OZ_HARNESS";
pub const ROOK_HARNESS_ENV: &str = "ROOK_HARNESS";
pub const SERVER_ROOT_URL_OVERRIDE_ENV: &str = "ROOK_SERVER_ROOT_URL";
pub const WS_SERVER_URL_OVERRIDE_ENV: &str = "ROOK_WS_SERVER_URL";
pub const SESSION_SHARING_SERVER_URL_OVERRIDE_ENV: &str = "ROOK_SESSION_SHARING_SERVER_URL";

/// Options related to the parent process that spawned this Rook instance.
#[derive(Debug, Default, Clone, clap::Args)]
pub struct ParentOpts {
    /// The ID of the Rook process that spawned this one.
    ///
    /// Used by codepaths that attempt to detect when the parent Rook process
    /// has terminated. Guaranteed to be [`None`] when this is the initial
    /// Rook process, but may also be [`None`] for Rook child processes if the
...
/// Returns whether an argument requests one of Rook's hidden worker modes.
...
/// Rook identity without exposing credentials.
...
pub struct GlobalOptions {
    ...
    #[arg(
        ...
        env = "ROOK_OUTPUT_FORMAT"
    )]
    pub output_format: OutputFormat,
}

/// Normal argument parser for the shared Rook executable across all channels.
///
/// Oz commands are subcommands of this parser, so invoking an `oz` symlink does
/// not require a mode flag. Rook Control uses its separate [`local_control::ControlArgs`]
...
pub struct Args {
    ...
    #[arg(
        ...
        global = true,
        hide = true,
        env = "ROOK_SESSION_SHARING_SERVER_URL"
    )]
...
/// Flags for the Rook application. Additional binaries, like test runners, may use this type
/// along with their own flags, or convert their flags into an `AppArgs` value.
#[derive(Debug, Default, clap::Args, Clone)]
pub struct AppArgs {
    /// True if this instance of Rook was launched at the end of the auto-update process.
    ...
    pub fn from_env() -> Self {
        cfg_if::cfg_if! {
            ...
            if #[cfg(target_family = "wasm")] {
                ...
                // `rook environment` should not return help text)
                ...
                if !FeatureFlag::RookManagedSecrets.is_enabled() {
                    let args: Vec<String> = env::args().collect();
...
* Read the documentation at https://docs.rook.dev/reference/cli
"#
        ));
    ...
    /// Args for the main Rook application, if not running a subcommand.
    pub fn app_args(&self) -> &AppArgs {
        &self.args
    }

    /// Extract the main Rook application args.
...
/// Rook may spawn several worker processes - mostly servers that support the main application.
///
/// These subcommands run those worker processes, which are bundled into the Rook binary.
...
/// CLI-related subcommands. The command-line interface to Rook isn't a full SDK (e.g. with language bindings),
/// but it allows scripting some Rook functionality.
#[derive(Debug, Clone, Subcommand)]
pub enum CliCommand {
    /// Interact with Oz.
    #[command(subcommand)]
...
/// A subcommand of the main Rook application. This includes all [`WorkerCommand`]s as well as app-specific debugging tools.
#[derive(Debug, Clone, Subcommand)]
pub enum Command {
    #[clap(flatten)]
    Worker(WorkerCommand),

    /// Commands that make up the Rook CLI.
    #[clap(flatten)]
    ...
    ///     source <(path/to/rook completions bash)
    ///
    /// For zsh, add the following to ~/.zshrc:
    ///     source <(path/to/rook completions zsh)
    ///
    /// For fish, add the following to ~/.config/fish/config.fish:
    ///     path/to/rook completions fish | source
    ///
    /// For Powershell, add the following to $PROFILE:
    ///     path\to\rook | Out-String | Invoke-Expression
    ///
    /// If no shell is provided, this defaults to the shell that Rook was run from.
    ...
    DumpDebugInfo,
    /// Print the JSON schema for the current Rook channel's settings and exit.
    #[cfg(not(target_family = "wasm"))]
    DumpSettingsSchema {
...
/// Returns a flag that sets the current process as the parent of a Rook subcommand to spawn.
