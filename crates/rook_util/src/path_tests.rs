...
fn test_user_friendly_path_with_home() {
    ...
    assert_eq!(
        user_friendly_path("/Users/blue/rook", Some(home)),
        "~/rook".to_string(),
    );
    assert_eq!(
        user_friendly_path("/Users/admin/rook", Some(home)),
        "/Users/admin/rook".to_string(),
    );
}
...
fn test_resolve_command() {
    ...
    // This path exists in the Rook repo, so it should resolve. The `../../`
    // is because Rust unit tests run from the root of the crate (`rook_util` in
    // this case).
    assert_eq!(
