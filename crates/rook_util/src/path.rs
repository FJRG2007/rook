//! This module contains utilities for dealing with file/directory paths throughout Rook.
use std::borrow::Cow;
...
/// \return any override shell launch path, reading from the ROOK_SHELL_PATH variable.
pub fn rook_shell_path() -> Option<String> {
    // TODO(peter): we ought to tolerate non-Unicode paths here.
    env::var("ROOK_SHELL_PATH").ok()
}
...
/// ```
/// use std::path::Path;
/// use rook_util::path::common_path;
///
...
/// # Examples
/// ```
/// use rook_util::path::file_uri_drive_path_to_windows;
///
...
/// use rook_util::path::group_roots_by_common_ancestor;
///
