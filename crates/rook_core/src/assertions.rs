...
macro_rules! safe_assert {
    ($cond:expr $(,)?) => {{
        ...
        match &$cond {
            (cond) => {
                if !(*cond) {
                    $crate::__rook_errors::report_error!($crate::__anyhow::anyhow!("Assertion `{}` failed", stringify!($cond)));
                }
            }
        }
    }};
    ($cond:expr, $($arg:tt)+) => {{
        ...
        match &$cond {
            (cond) => {
                if !(*cond) {
                    $crate::__rook_errors::report_error!($crate::__anyhow::anyhow!("Assertion `{}` failed: {}", stringify!($cond), format_args!($($arg)+)));
                }
            }
        }
    }};
...
macro_rules! safe_assert_eq {
        ...
        match (&$left, &$right) {
            (left_val, right_val) => {
                if !(*left_val == *right_val) {
                    $crate::__rook_errors::report_error!($crate::__anyhow::anyhow!("Assertion `{} == {}` failed: expected {left_val}, found {right_val}.", stringify!($left), stringify!($right)));
                }
            }
        }
    }};
    ($left:expr, $right:expr, $($arg:tt)+) => {{
        ...
        match (&$left, &$right) {
            (left_val, right_val) => {
                if !(*left_val == *right_val) {
                    $crate::__rook_errors::report_error!($crate::__anyhow::anyhow!("Assertion `{} == {}` failed: expected {left_val}, found {right_val}. {}", stringify!($left), stringify!($right), format_args!($($arg)+)));
                }
            }
        }
    }};
