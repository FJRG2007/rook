...
/// Should not be used directly outside of reusable components. Use color methods on `RookTheme` instead.
pub mod internal_colors {
    use rookui_core::color::ColorU;

    use super::{Fill, RookTheme};
    use crate::ui::color::blend::Blend;
    use crate::ui::color::coloru_with_opacity;

    /// Calculates the font color based on contrast needs for text legibility.
    /// The font color is a mixture of the `rook_theme`'s background and foreground
    /// colors, and the supplied `background` color.
    fn font_color(rook_theme: &RookTheme, background: impl Into<ColorU>) -> ColorU {
        rook_theme.font_color(background).into_solid()
    }
    ...
    pub fn accent(rook_theme: &RookTheme) -> Fill {
        rook_theme.accent()
    }

    /// Hover state for UI elements like buttons to which we want to call attention.
    /// Allows gradients so shouldn't be used for small elements.
    pub fn accent_hover(rook_theme: &RookTheme) -> Fill {
        rook_theme
            .accent()
            .blend(&rook_theme.foreground().with_opacity(40))
    }
    ...
    /// Allows gradients so shouldn't be used for small elements.
    #[allow(dead_code)]
    pub fn accent_pressed(rook_theme: &RookTheme) -> Fill {
        rook_theme
            .accent()
            .blend(&rook_theme.background().with_opacity(30))
    }

    /// The color of most text throughout the UI.
    pub fn text_main(rook_theme: &RookTheme, background: impl Into<ColorU>) -> ColorU {
        coloru_with_opacity(font_color(rook_theme, background), 90)
    }

    /// The color of subheaders and similar lower priority text.
    pub fn text_sub(rook_theme: &RookTheme, background: impl Into<ColorU>) -> ColorU {
        coloru_with_opacity(font_color(rook_theme, background), 60)
    }

    /// The color of text elements that are disabled or the lowest priority.
    pub fn text_disabled(rook_theme: &RookTheme, background: impl Into<ColorU>) -> ColorU {
        coloru_with_opacity(font_color(rook_theme, background), 40)
    }
    ...
    pub fn semantic_text_disabled(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&fg_overlay_5(rook_theme))
            .into()
    }

    pub fn neutral_1(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&rook_theme.foreground().with_opacity(5))
            .into_solid()
    }

    pub fn neutral_2(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&rook_theme.foreground().with_opacity(10))
            .into_solid()
    }

    pub fn neutral_3(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&rook_theme.foreground().with_opacity(15))
            .into_solid()
    }

    pub fn neutral_4(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&rook_theme.foreground().with_opacity(20))
            .into_solid()
    }

    pub fn neutral_5(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&rook_theme.foreground().with_opacity(40))
            .into_solid()
    ...
    pub fn neutral_6(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&rook_theme.foreground().with_opacity(60))
            .into_solid()
    }

    pub fn neutral_7(rook_theme: &RookTheme) -> ColorU {
        rook_theme
            .background()
            .blend(&rook_theme.foreground().with_opacity(90))
            .into_solid()
    }

    pub fn fg_overlay_1(rook_theme: &RookTheme) -> Fill {
        rook_theme.foreground().with_opacity(5)
    }

    pub fn fg_overlay_2(rook_theme: &RookTheme) -> Fill {
        rook_theme.foreground().with_opacity(10)
    }

    pub fn fg_overlay_3(rook_theme: &RookTheme) -> Fill {
        rook_theme.foreground().with_opacity(15)
    }

    pub fn fg_overlay_4(rook_theme: &RookTheme) -> Fill {
        rook_theme.foreground().with_opacity(20)
    }

    pub fn fg_overlay_5(rook_theme: &RookTheme) -> Fill {
        rook_theme.foreground().with_opacity(40)
    }

    pub fn fg_overlay_6(rook_theme: &RookTheme) -> Fill {
        rook_theme.foreground().with_opacity(60)
    }

    pub fn fg_overlay_7(rook_theme: &RookTheme) -> Fill {
        rook_theme.foreground().with_opacity(90)
    }

    pub fn accent_bg_strong(rook_theme: &RookTheme) -> Fill {
        Fill::Solid(rook_theme.background().into_solid())
            .blend(&rook_theme.accent().with_opacity(60))
    }

    pub fn accent_bg(rook_theme: &RookTheme) -> Fill {
        Fill::Solid(rook_theme.background().into_solid())
            .blend(&rook_theme.accent().with_opacity(40))
    }

    pub fn accent_fg_strong(rook_theme: &RookTheme) -> Fill {
        rook_theme
            .foreground()
            .blend(&rook_theme.accent().with_opacity(60))
    }

    pub fn accent_fg(rook_theme: &RookTheme) -> Fill {
        rook_theme
            .foreground()
            .blend(&rook_theme.accent().with_opacity(40))
    }

    pub fn accent_overlay_1(rook_theme: &RookTheme) -> Fill {
        rook_theme.accent().with_opacity(10)
    }

    pub fn accent_overlay_2(rook_theme: &RookTheme) -> Fill {
        rook_theme.accent().with_opacity(25)
    }

    pub fn accent_overlay_3(rook_theme: &RookTheme) -> Fill {
        rook_theme.accent().with_opacity(40)
    }

    pub fn accent_overlay_4(rook_theme: &RookTheme) -> Fill {
        rook_theme.accent().with_opacity(60)
    }
}
