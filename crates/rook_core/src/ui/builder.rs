...
use rookui_core::fonts::{FamilyId, Properties, Weight};
use rookui_core::geometry::vector::{Vector2F, vec2f};
use rookui_core::keymap::Keystroke;
use rookui_core::platform::Cursor;
use rookui_core::ui_components::button::{Button, ButtonVariant};
use rookui_core::ui_components::checkbox::Checkbox;
use rookui_core::ui_components::components::{Coords, UiComponent, UiComponentStyles};
use rookui_core::ui_components::keyboard_shortcut::KeyboardShortcut;
use rookui_core::ui_components::link::{Link, LinkStyles, OnClickFn};
use rookui_core::ui_components::list::{List, ListStyle};
use rookui_core::ui_components::progress_bar::ProgressBar;
use rookui_core::ui_components::radio_buttons::{
    RadioButtonItem, RadioButtonLayout, RadioButtonStateHandle, RadioButtons,
};
use rookui_core::ui_components::slider::{Slider, SliderStateHandle};
use rookui_core::ui_components::switch::{Switch, SwitchStateHandle, TRACK_COLOR};
use rookui_core::ui_components::text::{Paragraph, Span, WrappableText};
use rookui_core::ui_components::text_input::TextInput;
use rookui_core::ui_components::toggle_menu::{
    ToggleMenu, ToggleMenuCallback, ToggleMenuItem, ToggleMenuStateHandle,
};
use rookui_core::ui_components::tool_tip::{Tooltip, TooltipWithSublabel};
use rookui_core::{Element, View, ViewHandle};
...
use super::theme::{Fill, RookTheme};
...
impl UiBuilder {
    ...
    fn default_text_input_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            ...
            border_width: Some(1.),
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            background: Some(self.rook_theme.background().into()),
            border_color: Some(self.rook_theme.foreground().with_opacity(20).into()),
            font_color: None,
            ..Default::default()
        }
    }
    ...
    fn default_progress_bar_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            background: Some(self.rook_theme.background().into()),
            foreground: Some(self.rook_theme.accent().into()),
            width: Some(70.),
            height: Some(2.),
            ..Default::default()
        }
    ...
    pub fn default_tool_tip_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            background: Some(self.rook_theme().tooltip_background().into()),
            font_color: Some(self.rook_theme.background().into_solid()),
            padding: Some(Coords {
                top: 4.,
                bottom: 4.,
                left: 8.,
            ...
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            border_width: Some(1.),
            border_color: Some(self.rook_theme.outline().into()),
            font_family_id: Some(self.ui_font_family),
            font_size: Some(10.),
            ..Default::default()
        }
    ...
    fn default_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        let details = self.rook_theme.details();
        let (background, border) = match variant {
            ButtonVariant::Accent => (
                self.rook_theme.accent().blend(
                    &self
                        .rook_theme
                        .foreground()
                        .with_opacity(*details.accent_button_opacity()),
                ),
                self.rook_theme.foreground().with_opacity(0),
            ),
            ButtonVariant::Basic => (
                self.rook_theme.background().blend(
                    &self
                        .rook_theme
                        .foreground()
            ...
            ButtonVariant::Secondary => {
                return self.base_styles(
                    None,
                    self.rook_theme.outline(),
                    self.rook_theme
                        .main_text_color(self.rook_theme.background()),
                );
            }
            ButtonVariant::Warn => (Fill::warn(), self.rook_theme.foreground().with_opacity(0)),
            ButtonVariant::Error => (Fill::error(), self.rook_theme.foreground().with_opacity(0)),
            ButtonVariant::Outlined => {
                return self.base_styles(
                    None,
                    self.rook_theme.foreground().with_opacity(20),
                    self.rook_theme.sub_text_color(self.rook_theme.background()),
                );
            }
            ButtonVariant::Text => {
            ...
            ButtonVariant::Link => {
                return self.text_button_styles(self.rook_theme.accent());
            }
        };
        let font_color = match variant {
            ButtonVariant::Outlined => self.rook_theme.sub_text_color(background),
            ButtonVariant::Text => unreachable!(),
            _ => self.rook_theme.main_text_color(background),
        };
    ...
    fn disabled_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        let background: Fill = self.rook_theme().surface_3();
        let font_color = self.rook_theme().disabled_text_color(background);
        match variant {
    ...
    fn hovered_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        ...
        let (background, border) = match variant {
            ButtonVariant::Accent => (
                self.rook_theme
                    .accent()
                    .blend(&self.rook_theme.foreground().with_opacity(
            ...
            ButtonVariant::Secondary => (self.rook_theme.surface_3(), self.rook_theme.outline()),
            ButtonVariant::Warn => (
    ...
    fn clicked_button_styles(&self, variant: ButtonVariant) -> UiComponentStyles {
        ...
        self.base_styles(
            ...
            self.rook_theme.main_text_color(background),
        )
    }
    ...
    pub fn default_keyboard_shortcut_styles(&self) -> UiComponentStyles {
        UiComponentStyles {
            ...
            font_color: Some(
                self.rook_theme
                    .hint_text_color(self.rook_theme.background())
                    .into_solid(),
            ),
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            background: Some(internal_colors::fg_overlay_1(self.rook_theme()).into()),
    ...
    pub fn link_styles(&self) -> LinkStyles {
        let font_color: ColorU = self
            .rook_theme
            .accent()
            .on_background(self.rook_theme.surface_2(), MinimumAllowedContrast::Text)
            .into();
        ...
        let hover_link_style = UiComponentStyles {
            border_color: Some(self.rook_theme.accent().into()),
            ..base_link_style
        };
        LinkStyles {
    ...
    pub fn tooltip_link_styles(&self) -> LinkStyles {
        let tooltip_background = self.rook_theme().tooltip_background();
        let font_color: ColorU = self
            .rook_theme
            .accent()
            .on_background(
    ...
    pub fn switch(&self, mouse_state: SwitchStateHandle) -> Switch {
        ...
        Switch::new(
            ...
            switch_margin_styles.merge(self.base_styles(
                ...
                Fill::white(),
                self.rook_theme.main_text_color(self.rook_theme.surface_2()),
            )),
            None,
            Some(switch_margin_styles.merge(self.base_styles(
                Some(self.rook_theme.accent()),
                Fill::white(),
                self.rook_theme.main_text_color(self.rook_theme.surface_2()),
            ))),
            Some(switch_margin_styles.merge(self.base_styles(
                Some(self.rook_theme.disabled_ui_text_color()),
                Fill::white(),
                self.rook_theme.main_text_color(self.rook_theme.surface_2()),
            ))),
        )
    ...
    pub fn checkbox(&self, mouse_state: MouseStateHandle, size: Option<f32>) -> Checkbox {
        ...
        Checkbox::new(
            ...
            self.base_styles(
                None,
                self.rook_theme.outline(),
                self.rook_theme.nonactive_ui_text_color(),
            )
            ...
            Some(
                self.base_styles(
                    Some(self.rook_theme.accent()),
                    self.rook_theme.outline(),
                    self.rook_theme.font_color(self.rook_theme.accent()),
                )
    ...
    pub fn radio_buttons<'a>(
        ...
        RadioButtons::new(
            ...
            self.base_styles(
                None,
                self.rook_theme.nonactive_ui_text_color(),
                self.rook_theme.active_ui_text_color(),
            )
            ...
            self.base_styles(
                None,
                self.rook_theme.disabled_ui_text_color(),
                self.rook_theme.disabled_ui_text_color(),
            )
            .set_font_size(font_size),
            layout,
    ...
    pub fn toggle_menu(
        ...
        ToggleMenu::new(
            ...
            self.toggle_menu_styles(
                default_background.unwrap_or(self.rook_theme.surface_1()),
                self.rook_theme.active_ui_text_color(),
            )
            .set_font_size(font_size),
            self.toggle_menu_styles(
                selected_background.unwrap_or(self.rook_theme.surface_3()),
                self.rook_theme.active_ui_text_color(),
            ),
            self.toggle_menu_styles(
                hover_background.unwrap_or(self.rook_theme.surface_2()),
                self.rook_theme.active_ui_text_color(),
            ),
            on_toggle_change,
        )
    ...
    pub fn span(&self, text: impl Into<Cow<'static, str>>) -> Span {
        Span::new(
            text,
            self.base_styles(
                Some(self.rook_theme.surface_2()),
                self.rook_theme.outline(),
                self.rook_theme.main_text_color(self.rook_theme.surface_2()),
            ),
        )
    ...
    pub fn animated_button(
        ...
        let color = options.color.unwrap_or_else(|| {
            self.rook_theme
                .foreground()
                .on_background(self.rook_theme.surface_2(), MinimumAllowedContrast::NonText)
        });
        ...
        let hovered_styles = base_styles.merge(UiComponentStyles {
            background: Some(internal_colors::fg_overlay_2(&self.rook_theme).into()),
            ..Default::default()
        });
        ...
        let disabled_button_styles = base_styles.merge(UiComponentStyles {
            font_color: Some(
                self.rook_theme()
                    .disabled_text_color(internal_colors::neutral_3(&self.rook_theme).into())
                    .into(),
            ),
            ..Default::default()
        });
    ...
    pub fn copy_button(&self, size: f32, mouse_state: MouseStateHandle) -> Button {
        let color = self.rook_theme.main_text_color(self.rook_theme.surface_2());
    ...
    fn icon_with_tooltip(
        ...
        let icon = Container::new(
            ConstrainedBox::new(
                Icon::new(icon_path, self.rook_theme.active_ui_text_color()).finish(),
            )
    ...
    pub fn keyboard_shortcut_button(
        ...
        let default_styles = UiComponentStyles {
            border_radius: Some(CornerRadius::with_all(Radius::Pixels(4.))),
            border_color: Some(self.rook_theme.surface_3().into()),
            border_width: Some(1.),
            padding: Some(Coords {
                top: 8.,
                bottom: 8.,
        ...
        let hovered_styles = UiComponentStyles {
            background: Some(self.rook_theme().surface_3().into()),
            border_color: Some(self.rook_theme().accent().into()),
            ..default_styles
        };

        let text = Text::new_inline(text, self.ui_font_family(), self.ui_font_size() + 2.)
            .with_color(text_main(&self.rook_theme, self.rook_theme.background()))
        ...
        let keystroke_styles = self
            ...
            .set_font_color(text_main(&self.rook_theme, self.rook_theme.background()));
    ...
    pub fn rook_theme(&self) -> &RookTheme {
        &self.rook_theme
    }
