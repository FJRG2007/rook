...
/// in the Rook ecosystem. It enables events to be defined in any crate while maintaining
...
macro_rules! send_telemetry_from_ctx {
    ($event:expr, $ctx:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            let auth_state =
                <$crate::telemetry::TelemetryContextModel as $crate::rookui_core::SingletonEntity>::handle($ctx)
                    .as_ref($ctx);
            let user_id = auth_state.user_id($ctx);
            let anonymous_id = auth_state.anonymous_id($ctx);
            $crate::rookui_core::record_telemetry_from_ctx!(
                user_id,
                anonymous_id,
                event.name().into(),
                event.payload(),
...
macro_rules! send_telemetry_from_app_ctx {
    ($event:expr, $app_ctx:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            let auth_state =
                <$crate::telemetry::TelemetryContextModel as $crate::rookui_core::SingletonEntity>::handle(
                    $app_ctx,
                )
                .as_ref($app_ctx);
            let user_id = auth_state.user_id($app_ctx.as_ref());
            let anonymous_id = auth_state.anonymous_id($app_ctx.as_ref());
            $crate::rookui_core::record_telemetry_on_executor!(
                user_id,
                anonymous_id,
                event.name().into(),
                event.payload(),
