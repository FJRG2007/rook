...
/// The name of the directory in which to put non-global Rook-specific files.
///
/// This should be used, for example, as the base directory under which
/// repository workflows would be stored (in "./.rook/workflows").
pub const ROOK_CONFIG_DIR: &str = ".rook";

/// The name of the folder that stores Rook execution logs and network logs.
/// This is currently only used on Windows to maintain backwards compatibility.
pub const ROOK_LOGS_DIR: &str = "logs";

fn base_rook_config_dir_name() -> String {
    match ChannelState::channel() {
        // Preview shares the same directory as Stable for backward
        // compatibility — existing users already have config in `.rook`.
        Channel::Stable | Channel::Preview => ROOK_CONFIG_DIR.to_owned(),
        Channel::Oss => format!("{ROOK_CONFIG_DIR}-oss"),
...
/// Returns the home-relative Rook config directory name for the current channel and data profile.
///
/// This preserves the historical `.rook*` directory shape while still isolating dev, local,
/// integration, oss, and optional development profiles.
pub fn rook_home_config_dir_name() -> String {
    let base_dir_name = base_rook_config_dir_name();
...
/// Returns the home-relative Rook config directory for the current channel and data profile.
///
/// Unlike [`data_dir`] and [`config_local_dir`] on non-macOS platforms, this intentionally keeps
/// Rook-authored, user-facing config under a `.rook*` directory in the home directory instead of
/// using the platform XDG/AppData project directories.
pub fn rook_home_config_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|home_dir| home_dir.join(rook_home_config_dir_name()))
}

pub fn rook_home_skills_dir() -> Option<PathBuf> {
    rook_home_config_dir().map(|rook_config_dir| rook_config_dir.join("skills"))
}

pub fn rook_home_mcp_config_file_path() -> Option<PathBuf> {
    rook_home_config_dir().map(|rook_config_dir| rook_config_dir.join(".mcp.json"))
}

/// Returns the macOS config directory name for the current channel and data
/// profile.
///
/// Stable uses `.rook`, while other channels include a channel suffix
/// (e.g., `.rook-dev`, `.rook-local`).
///
...
/// TUI is the exception: it uses `RookTui`, while the corresponding GUI uses
/// `RookOss`.
#[cfg(any(not(target_os = "macos"), test))]
fn gui_app_id_for_channel(channel: Channel, current_app_id: AppId) -> AppId {
    match channel {
        Channel::Oss => AppId::new("dev", "rook", "RookOss"),
        Channel::Stable
        | Channel::Preview
        | Channel::Dev
        | Channel::Integration
...
/// GUI `.rook*` directory, so this fails closed instead of selecting a source
/// profile implicitly.
...
/// Returns the macOS config directory name for the TUI front-end (`rook-tui`)
/// for the current channel.
///
/// This mirrors [`macos_config_dir_name`] but under a `.rook_cli*` directory so
/// the TUI keeps its settings separate from the GUI's `.rook*` directory. Like
...
fn macos_tui_config_dir_name() -> String {
    macos_config_dir_name().replacen(ROOK_CONFIG_DIR, ".rook_cli", 1)
}

/// Returns the path to the directory where non-portable configuration files for
/// the TUI front-end (`rook-tui`) should be stored.
///
...
/// `.rook_cli*` directory (mirroring the GUI's `.rook*`); on other platforms —
...
///
/// This is intentionally distinct from [`rook_home_mcp_config_file_path`] so
...
/// the user had open and cached state of known Rook Drive objects.
...
pub fn secure_state_dir() -> Option<PathBuf> {
    ...
    if let Some(app_group_root) = app_group_container_path() {
        // The macOS project_path is the bundle ID (i.e. `dev.rook.Rook-Stable`).
...
/// data for the TUI front-end (`rook-tui`) should be stored.
///
...
fn project_dirs_for_app_id(
    ...
    cfg_if::cfg_if! {
        if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            ...
            // directories like "rook-terminal" and "rook-terminal-dev", to
            // match our Linux package name.
            let base_app_name = match app_id.application_name() {
                "Rook" => "Rook-Terminal".to_owned(),
                "RookOss" => "Rook-Oss".to_owned(),
                other if other.starts_with("Rook") => other.replace("Rook", "Rook-Terminal-"),
...
pub fn app_group_container_path() -> Option<PathBuf> {
    ...
    static CONTAINER_PATH: LazyLock<Option<PathBuf>> = LazyLock::new(|| {
        ...
        let group_id = format!("{}.dev.rook", crate::macos::APPLE_TEAM_ID);
...
/// Returns the path to resources included in the Rook distribution.
///
/// Unlike [`rookui_core::AssetProvider`] assets, which are generally embedded in the binary, these are
/// stored on the filesystem alongside the rest of Rook.
///
/// ## macOS
/// For the `.app` bundle, the resources directory is `$APP_DIR/Contents/Resources`
/// (e.g. `/Applications/Rook.app/Contents/Resources`). For the standalone CLI build
...
/// containing the Rook executable (e.g. `C:\Program Files\RookDev\resources`).
