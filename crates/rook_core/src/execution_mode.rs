...
/// Execution mode that Rook is running under.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExecutionMode {
    /// Rook is running as a normal desktop app.
    App,
    /// Rook is running as the headless terminal UI.
    Tui,
    /// Rook is running as a CLI.
    Sdk,
    /// Rook is running as the remote server daemon.
    RemoteServerDaemon,
}
...
impl ExecutionMode {
    ...
    pub fn client_id(&self) -> &'static str {
        match self {
            ExecutionMode::App => "rook-app",
            ExecutionMode::Tui => "rook-tui",
            ExecutionMode::Sdk => "rook-cli",
            ExecutionMode::RemoteServerDaemon => "rook-remote-server-daemon",
        }
    }
    ...
    /// or a CLI invocation) before Rook starts, so inheriting it is safe and is the only PATH
...
/// Model tracking the mode that Rook is running in.
///
/// This gates functionality that's disabled when Rook is running in SDK mode.
#[derive(Clone, Debug)]
...
impl AppExecutionMode {
    ...
    /// Whether Rook is running as the headless terminal UI.
    ...
    /// If true, Rook is running in a sandbox like a Docker container or VM, rather than directly
    /// on a user machine.
