...
use crate::channel::config::{
    ChannelConfig, IapConfig, McpOAuthProviderConfig, OzConfig, RookServerConfig,
...
impl ChannelState {
    pub fn init() -> Self {
        ...
        let app_id = AppId::new("dev", "rook", "RookOss");
        Self {
            channel,
    ...
    pub fn uses_staging_server() -> bool {
        ...
        url.host_str() == Some("staging.rook.dev")
    }
    ...
    ///
    /// This is a debugging tool for isolating development instances of Rook, and is not
    /// supported in release builds.
    pub fn data_profile() -> Option<String> {
        if cfg!(debug_assertions) {
            std::env::var("ROOK_DATA_PROFILE").ok()
        } else {
            None
        }
    }
    ...
    /// it optionally includes a suffix derived from the `ROOK_DATA_PROFILE`
    ...
    /// telemetry events. Builds like OpenRook intentionally ship with
    ...
    /// ship crash reports. Builds like OpenRook intentionally ship with
    ...
    /// served by rook-server-rtc (e.g. the agent event SSE stream).
    ///
    ...
    /// keeps override paths (e.g. `ROOK_WS_SERVER_URL=...`) working without a
    ...
    pub fn url_scheme() -> &'static str {
        match Self::channel() {
            ...
            Channel::Integration => "rookintegration",
            Channel::Local => "rooklocal",
            Channel::Oss => "rookoss",
        }
    }
}
