...
impl Channel {
    ...
    /// `--session-sharing-server-url` flags (and their `ROOK_*` env-var equivalents).
    ///
    ...
    /// Returns the Rook Control CLI command name corresponding to this channel.
    pub fn rookctrl_command_name(&self) -> &'static str {
        match self {
            Channel::Stable => "rookctrl",
            Channel::Dev => "rookctrl-dev",
            Channel::Preview => "rookctrl-preview",
            Channel::Local => "rookctrl-local",
            Channel::Integration => "rookctrl-integration",
            Channel::Oss => "rookctrl-oss",
        }
    }
}
