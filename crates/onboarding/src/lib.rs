...
pub const AI_FEATURES: &[&str] = &[
    "Use frontier and open-weight models with Rook Agent",
...
/// User-facing names of the Rook Drive features enabled when the terminal
/// intention is selected with Rook Drive turned on. Shared by the login slide's
/// skip-login confirmation dialog so the list stays in sync with any future
/// surfaces that need it.
pub const ROOK_DRIVE_FEATURES: &[&str] = &["Rook Drive", "Session Sharing"];
...
pub fn init(app: &mut rookui_core::AppContext) {
