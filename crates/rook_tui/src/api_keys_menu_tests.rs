...
use rook::settings::AISettings;
use rook::tui_export::{UserWorkspaces, register_tui_session_view_test_singletons};
...
#[test]
fn filtering_keeps_rook_credit_fallback_pinned() {
    App::test((), |mut app| async move {
        let (input, _, menu) = add_menu(&mut app);
        ...
        app.read(|ctx| {
            ...
            assert_eq!(
                snapshot
                    ...
                    .map(|row| row.title.as_str())
                    .collect::<Vec<_>>(),
                vec!["Google API key", "Rook credit fallback"]
            );
        });
    });
}
    ...
    App::test((), |mut app| async move {
        ...
        menu.update(&mut app, |menu, ctx| {
            let fallback_index = menu
                ...
                .rows
                .iter()
                .position(|row| row.title == "Rook credit fallback")
                .unwrap();
            assert!(menu.select_at_snapshot_index(fallback_index, ctx));
            menu.accept_selected(ctx);
        });
        app.read(|ctx| {
            assert!(*AISettings::as_ref(ctx).can_use_rook_credits_for_fallback);
            assert_eq!(
                menu.as_ref(ctx).footer(ctx),
                Some(TuiApiKeysFooter::RookCreditFallback)
            );
            assert!(menu.as_ref(ctx).is_open(ctx));
        });
    });
