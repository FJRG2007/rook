...
use rookui_core::{AppContext, Entity, EntityId, ModelContext, ModelHandle, ViewHandle};
...
pub(crate) struct MaterializedLocalOzChildSession {
    ...
    pub(crate) task_id: rook::tui_export::AmbientAgentTaskId,
...
impl TuiOrchestrationModel {
        ...
        match request.execution_mode.clone() {
            ...
            StartAgentExecutionMode::Local {
                ...
                self.fail_child_request(
                    &request,
                    format!(
                        "Local {harness_type} child agents aren't supported in Rook Agent CLI yet."
                    ),
                    ctx,
                );
            }
    ...
    fn finish_remote_child_launch(
        ...
        result: Result<rook::tui_export::SpawnAgentResponse, CloudAgentStartupIssue>,
        ctx: &mut ModelContext<Self>,
    ) {
        match result {
            Ok(response) => {
