...
//! (`rook_core::ui::builder`). It owns the theme→style recipes so TUI views
...
use rook_core::ui::color::Opacity;
use rook_core::ui::color::blend::Blend;
use rook_core::ui::theme::color::internal_colors;
use rook_core::ui::theme::{ColorScheme, Fill as ThemeFill, RookTheme};
...
use rookui_core::elements::{Fill as CoreFill, MouseStateHandle};
...
impl TuiUiBuilder {
    ...
    pub(crate) fn from_app(app: &AppContext) -> Self {
        Self {
            rook_theme: Appearance::as_ref(app).theme().clone(),
        }
    }

    fn design_palette(&self) -> TuiDesignPalette {
        match self.rook_theme.inferred_color_scheme() {
    ...
    pub(crate) fn primary_text_style(&self) -> TuiStyle {
        TuiStyle::default()
            .fg(self.foreground_text_color(self.rook_theme.details().main_text_opacity))
    }
    ...
    fn foreground_text_color(&self, opacity: Opacity) -> Color {
        cell_color(
            self.base_background()
                .blend(&self.rook_theme.foreground().with_opacity(opacity)),
        )
    }
    ...
    pub(crate) fn muted_text_style(&self) -> TuiStyle {
        TuiStyle::default()
            .fg(self.foreground_text_color(self.rook_theme.details().sub_text_opacity))
    }
    ...
    pub(crate) fn error_text_style(&self) -> TuiStyle {
        TuiStyle::default().fg(cell_color(ThemeFill::from(
            self.rook_theme.terminal_colors().normal.red,
        )))
    }
    ...
    pub(crate) fn success_glyph_style(&self) -> TuiStyle {
        TuiStyle::default().fg(cell_color(ThemeFill::from(
            self.rook_theme.terminal_colors().normal.green,
        )))
    }
    ...
    pub(crate) fn attention_glyph_style(&self) -> TuiStyle {
        TuiStyle::default().fg(cell_color(ThemeFill::from(
            self.rook_theme.terminal_colors().normal.yellow,
        )))
    }
    ...
    pub(crate) fn input_text_style(&self) -> TuiStyle {
        TuiStyle::default()
            .fg(cell_color(self.rook_theme.foreground()))
    ...
    pub(crate) fn accent_color(&self) -> Color {
        cell_color(ThemeFill::from(
            self.rook_theme.terminal_colors().normal.cyan,
        ))
    }

    /// Theme-blue link text, matching linked filenames in tool-call headers.
    pub(crate) fn link_text_style(&self) -> TuiStyle {
        TuiStyle::default().fg(cell_color(ThemeFill::Solid(self.rook_theme.ansi_fg_blue())))
    }

    pub(crate) fn cloud_run_mark_styles(&self) -> CloudRunMarkStyles {
        let blue = ThemeFill::from(self.rook_theme.terminal_colors().normal.blue);
        let foreground = self.rook_theme.foreground();
        let blend = |opacity| {
        ...
        CloudRunMarkStyles {
            ...
            ansi_bright: TuiStyle::default().fg(cell_color(ThemeFill::from(
                self.rook_theme.terminal_colors().bright.blue,
            ))),
        }
    }
    ...
    pub(crate) fn slash_command_selection_text_style(&self) -> TuiStyle {
        let background_fill = ThemeFill::from(self.rook_theme.terminal_colors().normal.cyan);
        let foreground = self.rook_theme.font_color(background_fill.into_solid());
        TuiStyle::default()
    ...
    pub(crate) fn slash_command_selection_state_suffix_style(&self) -> TuiStyle {
        let background = self.rook_theme.background().into_solid();
        let green = ThemeFill::from(self.rook_theme.terminal_colors().normal.green).into_solid();
        TuiStyle::default()
    ...
    pub(crate) fn input_background(&self) -> Color {
        let accent = ThemeFill::from(self.rook_theme.terminal_colors().normal.cyan);
        cell_color(
            self.base_background()
    ...
    pub(crate) fn shell_command_background(&self) -> Color {
        let accent = ThemeFill::from(self.rook_theme.terminal_colors().bright.green);
    ...
    pub(crate) fn shell_command_accent_style(&self) -> TuiStyle {
        TuiStyle::default().fg(cell_color(ThemeFill::from(
            self.rook_theme.terminal_colors().bright.green,
        )))
    }
    ...
    pub(crate) fn plan_background(&self) -> Color {
        let blue = ThemeFill::Solid(self.rook_theme.ansi_fg_blue());
    ...
    fn base_background(&self) -> ThemeFill {
        match probed_colors().bg {
            ...
            None => self.rook_theme.background(),
        }
    }

    fn cyan_overlay_2(&self) -> ThemeFill {
        let cyan = ThemeFill::from(self.rook_theme.terminal_colors().normal.cyan);
    ...
    pub(crate) fn credential_entry_accent_style(&self) -> TuiStyle {
        TuiStyle::default().fg(cell_color(ThemeFill::from(
            self.rook_theme.terminal_colors().normal.magenta,
        )))
    }
    ...
    pub(crate) fn voice_input_border_style(&self, elapsed: Duration) -> TuiStyle {
        ...
        let lilac_600 = ThemeFill::from(self.rook_theme.terminal_colors().normal.magenta);
    ...
    /// The rooking indicator's base fill: Lilac-200 in dark themes and
    /// Lilac-600 in light themes.
    fn rooking_base_fill(&self) -> ThemeFill {
        ThemeFill::Solid(self.design_palette().brand_primary)
    }

    /// The rooking indicator's base color as a solid color, for per-glyph
    /// shimmer lerping.
    pub(crate) fn rooking_base_color(&self) -> ColorU {
        self.rooking_base_fill().into_solid()
    }

    /// The peak color the "Rooking" shimmer band lerps toward: a theme text
    /// color selected for contrast against the resolved terminal background.
    pub(crate) fn rooking_shimmer_color(&self) -> ColorU {
        self.rook_theme
            .font_color(self.base_background())
            .into_solid()
    }

    /// Style for the rooking indicator's spinner glyph.
    pub(crate) fn rooking_spinner_style(&self) -> TuiStyle {
        TuiStyle::default().fg(cell_color(self.rooking_base_fill()))
    }
    ...
    pub(crate) fn orchestration_surface_background(&self) -> Color {
        let magenta = ThemeFill::from(self.rook_theme.terminal_colors().normal.magenta);
    ...
    pub(crate) fn orchestration_tab_bar_styles(&self) -> TuiTabBarStyles {
        ...
        let selected_fill = ThemeFill::from(self.rook_theme.terminal_colors().normal.magenta);
        let selected_background = cell_color(selected_fill);
        let selected_foreground =
            cell_color(self.rook_theme.font_color(selected_fill.into_solid()));
        TuiTabBarStyles {
    ...
    pub(crate) fn selection_style(&self) -> TuiStyle {
        TuiStyle::default()
            ...
            .bg(cell_color(self.rook_theme.foreground()))
