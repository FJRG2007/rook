...
use rook::tui_export::{dark_theme, light_theme};
use rookui::SingletonEntity;
use rookui_core::elements::MouseStateHandle;
use rookui_core::elements::animation::AnimationClock;
...
use rookui_core::{App, EntityId, EntityIdMap};
...
fn waiting_login_renders_copy_feedback() {
    App::test((), |app| async move {
        ...
        app.read(|app_ctx| {
            let browser_url =
                "https://app.rook.dev/device?user_code=ABCD-EFGH&source=rook-agent-cli";
            for (message, tone) in [
...
fn compact_footer_path_elides_middle_components() {
    assert_eq!(compact_footer_path("~/Documents/GitHub/rook"), "~/…/rook");
...
fn signed_out_welcome_matches_designed_copy_and_layout() {
    App::test((), |app| async move {
        ...
        app.read(|app_ctx| {
            ...
            let title_row = lines
                .iter()
                .position(|line| line.contains("Welcome to Rook"))
                .expect("welcome title renders");
            assert!(title_row > 0 && title_row < 12);
            assert_eq!(
                lines[title_row]
                    .find("Welcome to Rook")
            ...
            assert!(
                lines
                    .iter()
                    .all(|line| !line.contains("https://app.rook.dev/login")),
    ...
    App::test((), |mut app| async move {
        ...
        for (theme, brand_primary, brand_accent) in [
            ...
            app.read(|app_ctx| {
                ...
                assert_eq!(cell_color("Welcome to Rook", 0), brand_primary);
...
fn waiting_login_generated_url_handles_click() {
    App::test((), |app| async move {
        ...
        app.read(|app_ctx| {
            ...
            let row = lines
                .iter()
                .position(|line| line.contains("https://app.rook.dev/device?user_code="))
                .expect("generated URL renders") as u16;
            let col = lines[usize::from(row)]
                .find("https://app.rook.dev/device?user_code=")
...
fn signed_out_welcome_handles_login_link_click() {
    App::test((), |app| async move {
        ...
        app.read(|app_ctx| {
            ...
            let row = lines
                .iter()
                .position(|line| line.contains("Log in with Rook"))
                .expect("login action renders") as u16;
            let col = lines[usize::from(row)]
                .find("Log in with Rook")
    ...
    App::test((), |app| async move {
        ...
        app.read(|app_ctx| {
            ...
            for expected in [
                ...
                "https://app.rook.dev/device?user_code=ABCD-EF",
                "GH&source=rook-agent-cli",
                "Copy URL (c)",
        ...
        app.read(|app_ctx| {
            ...
            for url in [Some(browser_url), None] {
                ...
                if url.is_some() {
                    ...
                    assert!(
                        lines.iter().any(|line| {
                            line.contains("https://app.rook.dev/device?user_code=ABCD-EF")
                        }),
                    ...
                    assert!(
                        lines
                            .iter()
                            .any(|line| line.contains("GH&source=rook-agent-cli")),
