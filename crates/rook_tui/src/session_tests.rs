...
use ai::LLMProvider;
use clap::Parser;
use rook::tui_export::register_tui_session_view_test_singletons;
use rookui::platform::WindowStyle;
use rookui::{AddWindowOptions, SingletonEntity};
use rookui_core::App;
use rookui_core::runtime::TuiDriverStartupError;
...
fn parses_provider_api_key_setup_flag() {
    let args = TuiArgs::try_parse_from(["rook", "--set-provider-api-key", "anthropic"])
...
fn parses_provider_api_key_clear_flag() {
    let args = TuiArgs::try_parse_from(["rook", "--clear-provider-api-key", "google"])
...
fn rejects_unknown_provider_api_key_setup_value() {
    let error = TuiArgs::try_parse_from(["rook", "--set-provider-api-key", "other"])
...
fn provider_api_key_help_lists_supported_providers() {
    let error = TuiArgs::try_parse_from(["rook", "--help"])
...
fn accepts_startup_without_resume() {
    let args = TuiArgs::try_parse_from(["rook"]).expect("empty arguments should parse");
...
fn version_flag_prints_cli_version() {
    let error = TuiArgs::try_parse_from(["rook", "--version"])
