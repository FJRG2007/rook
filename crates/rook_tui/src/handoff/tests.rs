...
use rookui_core::elements::tui::{Modifier, TuiBuffer, TuiBufferExt, TuiRect};
...
fn privacy_invalidation_restores_the_draft_and_removes_handoff_from_commands() {
    ...
    App::test((), |mut app| async move {
        ...
        rook::settings::PrivacySettings::handle(&app).update(&mut app, |privacy_settings, ctx| {
            privacy_settings.is_cloud_conversation_storage_enabled = false;
            ctx.emit(
                rook::settings::PrivacySettingsChangedEvent::UpdateIsCloudConversationStorageEnabled {
                    old_value: true,
                    new_value: false,
                },
            );
