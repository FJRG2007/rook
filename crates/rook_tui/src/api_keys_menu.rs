...
use rook::settings::{AISettings, AISettingsChangedEvent};
use rook::tui_export::{TeamContextResolver, UserWorkspaces};
use rook_core::features::FeatureFlag;
use rook_core::settings::ToggleableSetting as _;
use rook_editor::model::CoreEditorModel;
use rookui::SingletonEntity as _;
use rookui_core::elements::tui::{TuiElement, TuiText};
...
const FALLBACK_DESCRIPTION: &str = "in the event of an error, requests may be routed to use Rook \
credits. Rook will prioritize using your API keys over Rook credits.";
#[derive(Debug, Clone, PartialEq, Eq)]
enum TuiApiKeysRowKind {
    Provider(LLMProvider),
    CustomEndpoint(CustomEndpointId),
    CustomEndpointConfigurationError,
    RookCreditFallbackSetting,
}
#[derive(Debug, Clone)]
...
impl TuiApiKeysMenuModel {
    pub(crate) fn new(
        ...
        ctx.subscribe_to_model(&AISettings::handle(ctx), |model, _, event, ctx| {
            if model.is_open(ctx)
                && matches!(
                    event,
                    AISettingsChangedEvent::CanUseRookCreditsForFallback { .. }
                )
            {
                model.refresh_rows(ctx);
            }
        ...
        match &self.state {
            ...
            TuiApiKeysMenuState::Browsing { list, .. } => {
                Some(match list.selected_row().map(|row| row.kind.clone()) {
                    Some(TuiApiKeysRowKind::RookCreditFallbackSetting) => {
                        TuiApiKeysFooter::RookCreditFallback
                    }
        ...
        match &self.state {
            TuiApiKeysMenuState::Browsing { list, .. } => {
                match list.selected_row().map(|row| row.kind.clone()) {
                    ...
                    Some(TuiApiKeysRowKind::RookCreditFallbackSetting) | None => false,
                }
            }
            TuiApiKeysMenuState::Closed
            | TuiApiKeysMenuState::EditingProvider { .. }
    ...
    pub(crate) fn clear_selected(&mut self, ctx: &mut ModelContext<Self>) {
            ...
            TuiApiKeysMenuState::Browsing { list, .. } => {
                match list.selected_row().map(|row| row.kind.clone()) {
                    ...
                    Some(
                        ...
                        | TuiApiKeysRowKind::RookCreditFallbackSetting,
                    )
                    | None => return,
                }
            }
        ...
        let result = match kind {
            ...
            TuiApiKeysRowKind::CustomEndpointConfigurationError
            | TuiApiKeysRowKind::RookCreditFallbackSetting => return,
        };
        match result {
        ...
        match &self.state {
            ...
            TuiApiKeysMenuState::Browsing { list, .. } => {
                ...
                match kind {
                    ...
                    TuiApiKeysRowKind::CustomEndpointConfigurationError => {}
                    TuiApiKeysRowKind::RookCreditFallbackSetting => self.toggle_fallback(ctx),
                }
            }
            TuiApiKeysMenuState::EditingProvider { provider, .. } => {
    ...
    fn snapshot_row(
        ...
        let (description, state_suffix, is_selectable) = match &row.kind {
            ...
            TuiApiKeysRowKind::RookCreditFallbackSetting => (
                ...
                Some(format!(
                    "({})",
                    if *AISettings::as_ref(ctx).can_use_rook_credits_for_fallback {
                        "on"
                    } else {
                        "off"
                    }
        ...
        let style = if row.kind == TuiApiKeysRowKind::RookCreditFallbackSetting {
    ...
    fn toggle_fallback(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        match result {
            ...
            Err(_) => self.set_browsing_error(
                "Could not save the Rook credit fallback setting.".to_owned(),
                ctx,
            ),
        }
    }
    ...
    fn refresh_rows(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        let rows = all_rows(ctx)
            .into_iter()
            .filter(|row| {
                row.kind == TuiApiKeysRowKind::RookCreditFallbackSetting
...
fn all_rows(ctx: &AppContext) -> Vec<TuiApiKeysRow> {
    ...
    rows.push(TuiApiKeysRow {
        kind: TuiApiKeysRowKind::RookCreditFallbackSetting,
        title: "Rook credit fallback".to_owned(),
        is_selectable: true,
    });
    rows
}
...
pub(crate) fn render_api_keys_footer(
    ...
    let spans = match footer {
        ...
        TuiApiKeysFooter::RookCreditFallback => vec![
            ("enter".to_owned(), key),
            (" to toggle rook credit fallback | ".to_owned(), muted),
            ("esc".to_owned(), key),
            (" to close menu".to_owned(), muted),
        ],
