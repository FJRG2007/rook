...
use rookui::{AddWindowOptions, EntityIdMap};
...
fn render_lines(app: &App, editor: &rookui_core::ViewHandle<TuiEditorView>) -> Vec<String> {
...
fn render_lines_at_width(
    app: &App,
    editor: &rookui_core::ViewHandle<TuiEditorView>,
    width: u16,
) -> Vec<String> {
    app.read(|ctx| {
...
fn scroll_offset(app: &App, editor: &rookui_core::ViewHandle<TuiEditorView>) -> u32 {
...
fn cmd_bindings_dispatch_expected_editor_commands() {
    ...
    impl rookui_core::Entity for CommandRecorder {
        type Event = ();
    }

    impl rookui_core::TuiView for CommandRecorder {
        fn ui_name() -> &'static str {
            TuiEditorView::ui_name()
        }

        fn render(&self, _app: &rookui_core::AppContext) -> Box<dyn TuiElement> {
            Box::new(TuiText::new(""))
        }
    }

    impl rookui_core::TypedActionView for CommandRecorder {
        type Action = TuiEditorViewAction;

        fn handle_action(
            &mut self,
            action: &Self::Action,
            _ctx: &mut rookui_core::ViewContext<Self>,
        ) {
