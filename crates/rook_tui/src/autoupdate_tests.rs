...
const BINARY_NAME: &str = "rook-tui-dev";
const HELPER_MODE_ENV: &str = "ROOK_TUI_AUTOUPDATE_HELPER_MODE";
const HELPER_ROOT_ENV: &str = "ROOK_TUI_AUTOUPDATE_HELPER_ROOT";
const HELPER_VERSION_ENV: &str = "ROOK_TUI_AUTOUPDATE_HELPER_VERSION";
const HELPER_READY_ENV: &str = "ROOK_TUI_AUTOUPDATE_HELPER_READY";
const HELPER_RELEASE_ENV: &str = "ROOK_TUI_AUTOUPDATE_HELPER_RELEASE";

fn temp_root(name: &str) -> tempfile::TempDir {
    tempfile::Builder::new()
        .prefix(&format!("rook-tui-autoupdate-{name}-"))
        .tempdir()
        .unwrap()
}
...
fn detects_managed_install_layout() {
    let exe = Path::new("/home/user/.rook/tui/versions/v0.2026.01.01.00.00.dev_00/rook-tui-dev");
    let layout = InstallLayout::from_canonical_exe_path(exe).unwrap();

    assert_eq!(layout.root, Path::new("/home/user/.rook/tui"));
    assert_eq!(
        layout.versions_dir,
        Path::new("/home/user/.rook/tui/versions")
    );
    assert_eq!(
        layout.current_pointer,
        Path::new("/home/user/.rook/tui/current")
    );
    assert_eq!(
        layout.running_version_dir,
        Path::new("/home/user/.rook/tui/versions/v0.2026.01.01.00.00.dev_00")
    );
...
fn detects_homebrew_cask_installations_across_supported_prefixes() {
    for exe in [
        "/opt/homebrew/Caskroom/rook-agent-cli/v1/rook-tui-stable",
        "/usr/local/Caskroom/rook-agent-cli/v1/rook-tui-stable",
        "/home/linuxbrew/.linuxbrew/Caskroom/rook-agent-cli/v1/rook-tui-stable",
    ] {
        assert_eq!(
...
fn rejects_unrelated_homebrew_casks_and_binary_names() {
    for exe in [
        "/opt/homebrew/Caskroom/other/v1/rook-tui-stable",
        "/opt/homebrew/Caskroom/rook-agent-cli/v1/rook-preview",
        "/opt/homebrew/bin/rook-tui-stable",
    ] {
        assert_eq!(
            InstallMethod::from_canonical_exe_path(Path::new(exe)),
...
fn installer_dir_argument_strips_verbatim_prefix() {
    assert_eq!(
        installer_dir_argument(Path::new(r"\\?\C:\Users\dev\AppData\Local\Rook\tui-dev"))
            .to_string_lossy(),
        r"/DIR=C:\Users\dev\AppData\Local\Rook\tui-dev"
    );
}
...
fn rejects_unmanaged_exe_paths() {
    assert_eq!(
        InstallLayout::from_canonical_exe_path(Path::new("/home/user/.rook/tui/rook-tui-dev")),
        None
    );
    assert_eq!(
        InstallLayout::from_canonical_exe_path(Path::new("/repo/target/debug/rook-tui-dev")),
        None
    );
    assert!(
