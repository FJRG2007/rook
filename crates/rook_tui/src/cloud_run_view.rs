...
use rookui_core::elements::CrossAxisAlignment;
...
use rookui_core::keymap::{self, EditableBinding};
...
impl TuiCloudRunView {
    ...
    fn bar_focused_kill_target(
        &self,
        ctx: &AppContext,
    ) -> Option<(rook::tui_export::AIConversationId, usize)> {
        let snapshot = self.compute_orchestration_tab_snapshot(ctx)?;
    ...
    fn kill_child_agent(
        &mut self,
        conversation_id: rook::tui_export::AIConversationId,
        ctx: &mut ViewContext<Self>,
    ) {
