...
use rook_core::cli_agent_protocol::{
    CLI_AGENT_NOTIFICATION_SENTINEL, CLIAgentNotification, ROOK_CLI_AGENT_PROTOCOL_VERSION_ENV,
    ROOK_CLIENT_VERSION_ENV,
};
use rook_terminal::model::escape_sequences::{C0, C1, tmux_passthrough};
use rookui::{AppContext, Entity, EntityId, ModelContext, ModelHandle, SingletonEntity as _};

use crate::tool_call_labels::tool_call_label;

const ROOK_TUI_AGENT_NAME: &str = "rook-tui";
const MAX_NOTIFICATION_DESCRIPTION_CHARS: usize = 320;

/// Returns whether the host terminal advertises structured Rook notifications.
///
/// Requiring both values matches the Claude plugin's structured-notification
/// capability check and prevents standalone TUI runs from emitting Rook OSC.
pub(crate) fn host_supports_cli_agent_notifications() -> bool {
    let protocol_version = env::var_os(ROOK_CLI_AGENT_PROTOCOL_VERSION_ENV);
    let client_version = env::var_os(ROOK_CLIENT_VERSION_ENV);
    has_rook_host_capability(protocol_version.as_deref(), client_version.as_deref())
}

fn has_rook_host_capability(
...
impl CliAgentOscEventPublisher {
    ...
    pub(crate) fn publish_session_start(&self, ctx: &AppContext) {
        ...
        notification.summary = Some("Rook Agent session started.".to_owned());
    ...
    pub(crate) fn publish_prompt_submit(&self, prompt: String, ctx: &AppContext) {
        ...
        notification.summary = Some("Rook Agent is working on your request.".to_owned());
        self.publish(notification);
    }
    ...
    fn handle_history_event(&self, event: &BlocklistAIHistoryEvent, ctx: &AppContext) {
        ...
        notification.summary = Some(
            match new_status {
                ConversationStatus::Success => "Rook Agent completed your request.",
                ConversationStatus::Error => "Rook Agent encountered an error.",
                ConversationStatus::Cancelled => "Rook Agent was cancelled.",
                ConversationStatus::InProgress => "Rook Agent is working on your request.",
                ConversationStatus::Blocked { .. } => "Rook Agent is waiting for your input.",
                ConversationStatus::TransientError => "Rook Agent is reconnecting.",
                ConversationStatus::WaitingForEvents => "Rook Agent is waiting for events.",
            }
            .to_owned(),
        );
    ...
    fn notification(&self, event: &str, ctx: &AppContext) -> CLIAgentNotification {
        ...
        CLIAgentNotification {
            ...
            cwd,
            ..CLIAgentNotification::new(ROOK_TUI_AGENT_NAME, event)
        }
    }
