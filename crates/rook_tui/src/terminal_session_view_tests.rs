...
use rook::terminal::model::ansi::{Handler, InputBufferValue, Mode};
use rook::tui_export::{
...
use rookui_core::keymap::{Context, DescriptionContext, Keystroke, Trigger};
...
struct FocusTestFixture {
    window_id: rookui_core::WindowId,
    ...
    App::test((), |mut app| async move {
        app.update(|ctx| {
            ...
            let row = render_status_footer_row(
                FooterSegments {
                    ordered: vec![
                        ...
                        FooterSegment::WorkingDirectory("/tmp/rook".to_owned()),
            ...
            assert_eq!(
                ...
                vec![
                    "██░░ 57% context remaining | ⊢ feature/statusline | ▶▶ | /tmp/rook | July 20, 2026"
                        .to_owned()
                ],
            );
    ...
    App::test((), |mut app| async move {
        app.update(|ctx| {
            ...
            assert_eq!(
                ...
                vec![
                    "▶▶ | model | /tmp/rook ⊢ main | ⊢ main • ↑1 ↓2 | ☰ 6 • +31 -12 | 40 credits | PR #123 | ██░░ 57% context remaining | July 20, 2026 • 1:08pm | ❒ 1/10 | ◉ Voice"
                        .to_owned()
                ],
            );
        });
    ...
    App::test((), |mut app| async move {
        ...
        view.read(&app, |view, _| {
            assert!(
                !Arc::ptr_eq(
                    ...
                    &view.rooking_auto_approve_mouse,
                ),
                "footer and rooking controls must not share retained mouse state",
            );
        });
        ...
        let (mut element, scene, buffer) = view.read(&app, |view, ctx| {
            ...
            let controls = TuiFlex::column()
                .child(view.render_rooking_indicator("Rooking...", Duration::ZERO, ctx))
        ...
        let (rooking_col, rooking_row) = footer_label_position(&buffer, "▶▶ Auto approve off");
        ...
        view.read(&app, |view, _| {
            ...
            assert!(!view.rooking_auto_approve_mouse.lock().unwrap().is_clicked());
        });
        assert!(
            ...
            "rooking control must not cancel the footer's armed click",
        );
        view.read(&app, |view, _| {
            assert!(!view.footer_auto_approve_mouse.lock().unwrap().is_clicked());
            assert!(!view.rooking_auto_approve_mouse.lock().unwrap().is_clicked());
        ...
        assert!(dispatch_session_event(
            ...
            &mut element,
            scene.clone(),
            &left_mouse_down(rooking_col, rooking_row),
        ));
        view.read(&app, |view, _| {
            assert!(!view.footer_auto_approve_mouse.lock().unwrap().is_clicked());
            assert!(view.rooking_auto_approve_mouse.lock().unwrap().is_clicked());
        ...
        assert!(
            dispatch_session_event(
                ...
                &mut element,
                scene,
                &left_mouse_up(rooking_col, rooking_row),
            ),
            "footer control must not cancel the rooking control's armed click",
        );
    });
}
...
fn log_bundle_success_message_includes_the_absolute_path() {
    let path = std::path::Path::new("/tmp/rook-20260718-132640.zip");
    assert_eq!(
        log_bundle_success_message(path),
        "Log bundle saved to /tmp/rook-20260718-132640.zip"
    );
}
...
fn settings_reload_failure_renders_as_an_error_footer_hint() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            RookConfig::handle(ctx).update(ctx, |_, ctx| {
                ctx.emit(RookConfigUpdateEvent::SettingsErrors(
...
fn fork_slash_command_replaces_the_surface_and_renders_original_resume_guidance() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            rook::tui_export::GlobalResourceHandlesProvider::handle(ctx).update(
                ctx,
                |provider, _| {
...
fn nld_slash_command_toggles_and_reports_its_effects() {
    App::test((), |mut app| async move {
        let _agent_mode = rook_core::features::FeatureFlag::AgentMode.override_enabled(true);
        let fixture = focus_test_fixture(&mut app);
        let (view, _) = add_focus_test_session(&mut app, &fixture, true);
        flush_events();
...
fn status_slash_command_opens_dedicated_status_menu_via_shared_structure() {
    App::test((), |mut app| async move {
        ...
        assert!(
            rendered.contains("test_user@rook.dev"),
            "Email value:\n{rendered}"
        );
...
fn zero_state_position_stays_stable_across_shell_bootstrap() {
    App::test((), |mut app| async move {
        ...
        let title_row = |lines: &[String]| {
            lines
                .iter()
                .position(|line| line.contains("Rook Agent CLI"))
...
fn zero_state_running_command_hint_shows_attachment() {
    App::test((), |mut app| async move {
        ...
        let lines = render_session(&mut app, &view, 80, 40);
        assert!(
            lines.iter().any(|line| line.contains("Rook Agent CLI")),
        ...
        view.update(&mut app, |view, ctx| {
            {
                ...
                // Advance past RookInput, then leave an unfinished startup-script
...
fn zero_state_renders_with_only_zero_height_bootstrap_blocks() {
    App::test((), |mut app| async move {
        ...
        let title_row = lines
            .iter()
            .position(|line| line.contains("Rook Agent CLI"))
            .expect("zero state should render the Rook Agent CLI title");
        assert!(
            title_row < 28,
...
fn first_zero_state_stays_hidden_while_markers_load_and_reconciles_without_replacing_session() {
    App::test((), |mut app| async move {
        ...
        assert!(lines.iter().any(|line| line.contains("Rook Agent CLI")));
        assert!(
            lines
                .iter()
                .all(|line| !line.contains("What’s different about Rook"))
        );
...
fn background_session_does_not_receive_first_run_onboarding() {
    App::test((), |mut app| async move {
        ...
        assert!(lines.iter().any(|line| line.contains("Welcome to Rook")));
        assert!(
            lines
                .iter()
                .any(|line| line.contains("What’s different about Rook"))
        );
    });
}
...
fn zero_state_transitions_through_bootstrap_lifecycle() {
    App::test((), |mut app| async move {
        ...
        view.update(&mut app, |view, _| {
            ...
            terminal_model.block_list_mut().reinit_shell();
            terminal_model.update_blockheight_items(TRANSCRIPT_BLOCK_SPACING.block_padding, 0.0);
            // Advance past RookInput to ScriptExecution.
            terminal_model.simulate_block("bootstrap", "");
...
fn footer_renders_agent_sections_left_aligned() {
    App::test((), |mut app| async move {
        app.update(|ctx| {
            ...
            let row = render_status_footer_row(
                FooterSegments {
                    ordered: vec![
                        ...
                        FooterSegment::WorkingDirectory("/home/user/rook".to_owned()),
                        FooterSegment::GitBranch("main".to_owned()),
                        FooterSegment::CreditUsage(usage),
                        FooterSegment::GitDiff {
                            files_changed: 2,
            ...
            assert_eq!(
                lines,
                vec!["TestModel | /home/user/rook ⊢ main | 2.5 credits | ☰ 2 • +3 -1"],
    ...
    App::test((), |mut app| async move {
        app.update(|ctx| {
            ...
            assert_eq!(
                lines,
                vec![format!(
                    "{SHELL_MODE_HINT} /home/user/rook ⊢ main | ☰ 2 • +3 -1"
                )],
...
fn vim_mode_slash_command_is_registered_in_command_registry() {
    use rook::tui_export::SlashCommandSurfaces;
...
fn resume_shell_commands_use_shared_tui_launcher() {
    ...
    assert_eq!(
        ...
        "rook-preview --resume conversation-token"
    );
}
...
fn set_teams_and_register_window(
    app: &mut App,
    names: &[&str],
    window_id: rookui::WindowId,
) -> Vec<rook::tui_export::ServerId> {
    let team_uids: Vec<rook::tui_export::ServerId> = (1..=names.len() as i64)
        .map(rook::tui_export::ServerId::from)
        .collect();
    let teams = team_uids
        .iter()
        .copied()
