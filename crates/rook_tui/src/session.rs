//! The headless `rook-tui` front-end's session bootstrap.
//!
//! [`run`] boots the real headless Rook app via [`rook::run_tui`]. Once shared
...
use rook::settings::{TuiThemeSettings, TuiZeroStateSettings, TuiZeroStateSettingsChangedEvent};
#[cfg(feature = "voice_input")]
use rook::settings::{TuiVoiceSettings, TuiVoiceSettingsChangedEvent};
use rook::tui_export::{AIConversationAutoexecuteMode, Appearance, ServerConversationToken};
use rook::{TuiLoginEvent, TuiLoginModel, TuiLoginPhase};
...
use rookui_core::platform::{TerminationMode, WindowStyle};
use rookui_core::runtime::{TuiDriverStartupError, TuiFocusPolicy, spawn_tui_driver};
use rookui_core::{AddWindowOptions, AppContext, ModelHandle, ViewHandle};
...
#[derive(Debug, Parser)]
#[command(name = "rook", version = CLI_VERSION)]
struct TuiArgs {
    #[command(subcommand)]
    command: Option<TuiCommand>,

    /// Resume an Oz/Rook conversation by server token.
    #[arg(long)]
    ...
    #[arg(long, env = "ROOK_API_KEY")]
    api_key: Option<String>,

    /// Securely store a model-provider API key for Rook Agent CLI.
    #[arg(
        long,
    ...
    /// Remove a securely stored model-provider API key from Rook Agent CLI.
    #[arg(
        long,
        value_name = LLMProvider::API_KEY_PROVIDER_VALUE_NAME,
        value_parser = LLMProvider::from_api_key_slug,
...
/// `rook_cli::Command::DumpSettingsSchema`); this local copy lets
...
/// Boots the headless Rook app and mounts the transcript-capable TUI session.
pub fn run() -> Result<()> {
    ...
    // If this process was re-exec'd as a Rook worker (e.g. the terminal
    // server), dispatch that instead of starting another TUI — otherwise the
    // worker re-exec would recursively launch TUIs.
    if let Some(result) = rook::run_tui_worker_if_requested() {
        return result;
    }
    ...
    if let Some(TuiCommand::DumpSettingsSchema { output_path }) = args.command {
        rook::features::init_feature_flags();
        return rook::settings::dump_settings_schema(output_path.as_deref());
    }
    ...
    if let Some(command) = provider_api_key_command {
        return rook::run_tui_cli_command(Box::new(move |ctx| {
            ...
            let result = ApiKeyManager::handle(ctx)
                ...
                .and_then(|()| rook::tui_export::notify_tui_api_keys_changed());
            match result {
                Ok(()) => {
...
fn init(
    ...
    rook_core::send_telemetry_from_app_ctx!(TuiStartupTelemetryEvent::from_environment(), ctx);
