...
use rookui_core::elements::tui::{Color, TuiBufferExt, TuiElement, TuiRect, TuiText};
use rookui_core::presenter::tui::TuiPresenter;

use super::{SPINNER_TIMELINE, render_rooking_indicator_row};
...
fn renders_the_indicator_row_and_requests_a_repaint() {
    App::test((), |mut app| async move {
        ...
        app.read(|app_ctx| {
            let element = render_rooking_indicator_row(
                "Rooking",
                Duration::ZERO,
...
#[test]
fn shimmer_only_applies_to_the_rooking_label_and_groups_its_ellipsis() {
    App::test((), |mut app| async move {
        app.update(|ctx| {
            ctx.add_singleton_model(|_| Appearance::mock());
        });
        app.read(|app_ctx| {
            ...
            let base = TuiUiBuilder::from_app(app_ctx).rooking_base_color();
