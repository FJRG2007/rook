//! Background auto-updater for the headless `rook-tui` front-end.
//!
//! Follows the "native installer" model used by peer CLIs (e.g. Claude Code):
//! the installer (`rook-server/download/tui_install.sh`) lays installs out as
//!
//! ```text
//! <root>/                      # ~/.rook/tui by default
//!   versions/<version>/        # binary + resources/ per installed version
//!   current                    # active version symlink (Unix) or text pointer (Windows)
//! <bin-dir>/rook[-<channel>]   # stable symlink (Unix) or launcher (Windows)
//! ```
//!
...
//! or the `ROOK_TUI_DISABLE_AUTOUPDATE` environment variable; re-running the
...
use rook::settings::TuiAutoupdateSettings;
use rook_core::channel::{Channel, ChannelState};
use rook_core::{safe_warn, send_telemetry_from_ctx};
...
const DISABLE_ENV_VAR: &str = "ROOK_TUI_DISABLE_AUTOUPDATE";
/// The stable cask token shared by the Rook-managed tap and the planned
/// official Homebrew package.
const HOMEBREW_CASK_TOKEN: &str = "rook-agent-cli";

/// User-facing status for Homebrew installations with a newer cask.
pub(crate) const HOMEBREW_UPDATE_STATUS: &str =
    "update available — run brew upgrade --cask rook-agent-cli";
...
struct InstallLayout {
    /// Root of the versioned install (e.g. `~/.rook/tui`).
    root: PathBuf,
    /// `<root>/versions`.
    ...
    /// The channel-suffixed binary name (e.g. `rook-tui-dev`).
    binary_name: String,
}
...
#[derive(Clone, Debug, PartialEq, Eq)]
enum InstallMethod {
    /// Installed by Rook's native versioned installer.
    Native(InstallLayout),
    /// Installed from the `rook-agent-cli` Homebrew cask.
    Homebrew,
...
fn is_homebrew_cask_exe_path(exe: &Path) -> bool {
    if exe.file_name() != Some(OsStr::new("rook-tui-stable")) {
        return false;
    }
    let components = exe
        .components()
...
impl VersionLease {
    ...
    pub(crate) fn acquire_for_current_process() -> Result<Option<Self>> {
        InstallLayout::detect()
            .map(|layout| {
                Self::acquire(&layout)
                    .context(
                        "failed to protect this managed Rook Agent CLI version; retry the \
                         command, or reinstall Rook Agent CLI if the problem persists",
                    )
                    .map(Some)
            })
            .unwrap_or(Ok(None))
    ...
    fn acquire(layout: &InstallLayout) -> Result<Self> {
        ...
        if !is_complete_version_dir(layout, &layout.running_version_dir) {
            bail!(
                "the managed Rook Agent CLI version at {:?} was retired while this process was \
                 starting; retry the command, or reinstall Rook Agent CLI if the problem persists",
    ...
    fn determine(ctx: &AppContext) -> Self {
        if std::env::var_os(DISABLE_ENV_VAR).is_some() {
            return Self::Disabled {
                reason: "opted out via the ROOK_TUI_DISABLE_AUTOUPDATE environment variable",
            };
        }
        ...
        VersionDirState::Invalid => {
            bail!(
                "refusing to replace incomplete or invalid installed TUI version at \
                 {version_dir:?}; remove that directory or reinstall Rook Agent CLI, then retry"
            );
        }
...
async fn install_update(layout: InstallLayout, latest_version: String) -> Result<UpdateOutcome> {
    ...
    let output = command::r#async::Command::new(&installer.path)
        ...
        .output()
        .await
        .context("failed to launch the Rook Agent CLI installer")?;
    if !output.status.success() {
        bail!(
            "Rook Agent CLI installer exited with {}: {}",
            output.status,
    ...
    blocking::unblock(move || {
        ...
        if version_dir_state(&layout, &version_dir)? != VersionDirState::Complete {
            bail!("Rook Agent CLI installer did not create a complete version at {version_dir:?}");
        }
        if !current_points_at(&layout, &latest_version) {
            bail!("Rook Agent CLI installer did not activate expected version {latest_version:?}");
        }
        prune_old_versions(&layout, &latest_version);
        Ok(UpdateOutcome::Installed {
            version: latest_version,
...
/// Fetches the latest version for this channel: from the Rook server's
    ...
    let versions = match from_server {
        ...
        Err(error) => {
            ...
            if releases_base_url.is_empty() {
                return Err(error.context("failed to fetch channel versions from the Rook server"));
            }
            log::warn!(
                "Failed to fetch channel versions from the Rook server ({error:#}); \
...
/// The Rook Agent CLI artifact endpoint for a release channel.
...
async fn download_windows_installer(
    ...
    let (path, mut file) =
        installer.context("failed to allocate a unique Rook Agent CLI installer path")?;
    ...
    let response = client
        ...
        .send()
        .await
        .context("failed to download the Rook Agent CLI installer")?
        .error_for_status()
        .context("failed to download the Rook Agent CLI installer")?;
...
async fn download_update(
    ...
    let response = client
        ...
        .error_for_status()
        .context("failed to download the TUI update")?;
    let archive_path = staging_dir.join("rook-tui.tar.gz");
