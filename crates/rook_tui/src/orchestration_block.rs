...
use rook::tui_export::{
    ...
    HarnessAvailabilityModel, LLMPreferences, LLMPreferencesEvent, ORCHESTRATION_ROOK_WORKER_HOST,
...
use rookui_core::elements::tui::TuiElement;
use rookui_core::keymap::macros::*;
use rookui_core::keymap::{self, FixedBinding};
use rookui_core::{
...
impl TuiOrchestrationBlock {
    ...
    pub(crate) fn new(
        ...
        // the active page.
        ctx.subscribe_to_model(
            &rook::tui_export::ConnectedSelfHostedWorkersModel::handle(ctx),
            |me, _, event, ctx| {
                let rook::tui_export::ConnectedSelfHostedWorkersEvent::Changed = event;
                me.refresh_active_page(ctx);
                ctx.notify();
            },
        );
