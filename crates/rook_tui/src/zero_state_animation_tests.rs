...
use rookui::{EntityIdMap, SingletonEntity as _};
...
use super::{
    ...
    MIN_ANIMATION_ROWS, MOMENTUM_SETTLE_DURATION, REPAINT_INTERVAL, RookLogoStyles,
...
#[test]
fn logo_mask_preserves_the_offset_rook_faces() {
    assert!(rook_logo_contains(0.25, -0.65));
    assert!(rook_logo_contains(-0.55, 0.45));
    assert!(!rook_logo_contains(-0.85, -0.85));
    assert!(!rook_logo_contains(0.0, 0.9));
}
...
fn representative_ascii_fixtures_have_distinct_dimensions() {
    ...
    assert_eq!(AsciiArtMask::parse(ROOK_W_ART).unwrap().size(), (9, 5));
}
...
fn relative_ascii_paths_resolve_from_the_tui_config_directory() {
    let config_dir = Path::new("/tmp/rook-tui-config");
    assert_eq!(
...
fn startup_loader_falls_back_for_missing_or_invalid_art_only() {
    ...
    for path in ["missing.txt", "invalid.txt"] {
        ...
        assert!(matches!(config.shape.as_ref(), ZeroStateShape::BuiltInRook));
...
fn custom_animation_element_paints_and_requests_another_frame() {
    App::test((), |mut app| async move {
        let config = Arc::new(custom_config(ROOK_W_ART, 4.0, 0.18));
        let (_, view) = app.update(|ctx| {
