...
use rookui::{EntityIdMap, SingletonEntity};
...
fn homebrew_update_status_shows_the_upgrade_command() {
    assert_eq!(
        ...
        Some("update available — run brew upgrade --cask rook-agent-cli")
    );
}
...
fn render_to_buffer(
    ...
    app_ctx: &rookui_core::AppContext,
    w: u16,
    h: u16,
) -> TuiBuffer {
...
fn login_line_shows_signed_in_account_email() {
    App::test((), |mut app| async move {
        ...
        assert!(
            lines
                .iter()
                .any(|line| line.contains("Signed in as test_user@rook.dev")),
