//! Shared fixtures for `rook_tui` unit tests.
#![cfg_attr(not(test), allow(dead_code))]
use std::any::Any;
use std::sync::Arc;

use parking_lot::FairMutex;
use rook::settings::SettingsFileError;
use rook::tui_export::{
...
use rookui::{AddSingletonModel, App, EntityId, ModelHandle};
use rookui_core::elements::tui::{TuiElement, TuiText};
use rookui_core::{AppContext, Entity, TuiView, TypedActionView, ViewHandle, WindowId};
...
pub(crate) fn add_test_action_model_and_events(
    ...
    if !app.read(|ctx| ctx.has_singleton_model::<rook_core::telemetry::TelemetryContextModel>()) {
