//! The in-progress `⋮ Rooking... (Ns)` indicator row rendered between the
//! transcript and the input box while the selected conversation is in
//! progress — the TUI counterpart of the GUI's rooking indicator — and its
...
use rook::tui_export::format_credits;
use rookui_core::AppContext;
use rookui_core::elements::animation::{AnimationClock, Keyframe, KeyframeTimeline};
use rookui_core::elements::shimmer_math::ShimmerConfig;
...
/// for `elapsed`.
pub(crate) fn render_rooking_indicator_row(
    ...
    let spinner = render_spinner(clock, builder.rooking_spinner_style());

    let label = TuiShimmeringText::new(
        label,
        builder.rooking_base_color(),
        builder.rooking_shimmer_color(),
...
#[cfg(test)]
#[path = "rooking_indicator_tests.rs"]
mod tests;
