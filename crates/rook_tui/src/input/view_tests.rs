...
use rook::tui_export::VoiceInput;
use rook::tui_export::{
...
use rookui_core::event::{KeyEventDetails, ModifiersState};
...
fn agent_mode_render_has_prompt_gutter() {
    App::test((), |mut app| async move {
        app.update(|ctx| {
            ...
            assert_eq!(prefix.bg, rookui_core::elements::tui::Color::Reset);
            assert_eq!(prefix.modifier, prefix_style.add_modifier);
            assert!(
                !prefix
                    .modifier
                    .contains(rookui_core::elements::tui::Modifier::BOLD)
            );
...
fn enable_vim_mode(app: &mut rookui::App) {
