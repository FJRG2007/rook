...
use rookui::{AddWindowOptions, App, SingletonEntity as _, ViewHandle};
use rookui_core::elements::tui::{TuiBufferExt, TuiRect};
use rookui_core::keymap::Keystroke;
use rookui_core::presenter::tui::TuiPresenter;
use rookui_core::{TuiView as _, TypedActionView as _, WindowInvalidation};
...
fn environment_selector_is_searchable() {
    App::test((), |mut app| async move {
        let (block, _) = test_block(&mut app, &request("oz", remote("env-1", "rook")));
...
fn cloud_managed_credential_harness_inserts_the_api_key_page() {
    let state = TuiOrchestrationBlock::config_state_from_request(
        &request("claude", remote("env-1", "rook")),
        None,
    );
    assert_eq!(
        TuiOrchestrationBlock::page_sequence(&state),
...
fn edit_state_carries_the_request_auth_secret() {
    let mut with_secret = request("claude", remote("env-1", "rook"));
...
fn build_request_carries_card_fields_and_edited_run_wide_state() {
    let original = request("oz", remote("env-1", "rook"));
...
impl OrchestrationBlockController for TestController {
    ...
    fn snapshot_for_page(
        ...
        let (rows, selected_id) = match page {
            ...
            ConfigPage::Harness => (vec![row("oz", "Rook")], "oz"),
            ConfigPage::ApiKey => (vec![row("", "Skip")], ""),
            ConfigPage::Host => (vec![row("rook", "Rook")], "rook"),
...
fn test_block(
    ...
    app.update(rook_core::telemetry::testing::MockTelemetryContextProvider::register);
