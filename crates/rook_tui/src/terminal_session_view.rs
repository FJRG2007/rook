...
#[cfg(feature = "voice_input")]
use rook::tui_export::slash_commands;
use rook::tui_export::{
    ...
    ResolvedTeamScope, RookConfig, RookConfigUpdateEvent, ServerConversationToken, ServerId,
    ...
    StartAgentExecutorEvent, StartAgentRequest, StaticCommand, TeamContextForOperation,
    ...
    UserWorkspacesEvent, WAKEUP_THROTTLE_PERIOD, block_context_from_terminal_model,
...
use rookui_core::r#async::{SpawnedFutureHandle, Timer};
...
#[cfg(feature = "voice_input")]
use rookui_core::elements::tui::{TuiAnimated, TuiDispatchEventResult, TuiEventHandler};
use rookui_core::keymap::macros::*;
use rookui_core::keymap::{self, EditableBinding, FixedBinding};
use rookui_core::platform::TerminationMode;
#[cfg(feature = "voice_input")]
...
use crate::rooking_indicator::{render_response_summary, render_rooking_indicator_row};
...
const ZERO_STATE_ASCII_INITIAL_LOAD_FAILED_HINT: &str =
    "Could not load custom ASCII art. Using the built-in Rook logo.";
...
fn tui_cli_shell_command(channel: Channel, arguments: &str) -> String {
    let launcher = match channel {
        ...
        Channel::Stable => "rook",
        Channel::Dev => "rook-dev",
        Channel::Preview => "rook-preview",
        Channel::Oss => "rook-oss",
        Channel::Integration => "rook-integration",
    };
...
pub(crate) struct TuiTerminalSessionView {
    ...
    rooking_auto_approve_mouse: MouseStateHandle,
...
impl TuiTerminalSessionView {
    ...
    pub(crate) fn new(
        ...
        ctx.subscribe_to_model(&RookConfig::handle(ctx), |view, _, event, ctx| {
            if let RookConfigUpdateEvent::SettingsErrors(error) = event {
        ...
        // The rooking indicator between the transcript and the input box
        ...
        ctx.subscribe_to_model(&model_events, |view, _, event, ctx| match event {
            ...
            ModelEvent::BackgroundBlockStarted => {
                rook::send_telemetry_from_ctx!(TelemetryEvent::BackgroundBlockStarted, ctx);
                ctx.notify();
            }
            _ => {}
        });
        ...
        let mut view = Self {
            ...
            rooking_auto_approve_mouse: MouseStateHandle::default(),
    ...
    pub(crate) fn start_orchestrated_child(
        &mut self,
        task_id: rook::tui_export::AmbientAgentTaskId,
        prompt: String,
        conversation_id: AIConversationId,
        ctx: &mut ViewContext<Self>,
    ) {
        ...
        let conversation = match result {
            ...
            Some(CloudConversationData::CLIAgent(_)) => {
                self.fail_conversation_restore(
                    request_id,
                    "Rook Agent CLI only supports Oz/Rook conversations.".to_owned(),
                    ctx,
                );
                return;
            }
    ...
    /// Re-renders on history events that can change the rooking indicator:
    ...
    fn render_rooking_indicator(
        ...
        let auto_approve = TuiHoverable::new(
            self.rooking_auto_approve_mouse.clone(),
            TuiText::new(format!(
                "▶▶ Auto approve {}",
        ...
        .finish();
        render_rooking_indicator_row(label, elapsed, auto_approve, ctx)
    }
    ...
    /// metadata flows (rookified sessions); until then fall back to the
    ...
    fn handle_accepted_conversation(
        ...
        rook::send_telemetry_from_ctx!(TuiConversationMenuTelemetryEvent::ItemSelected, ctx);
                ...
                ctx.spawn(
                    async move {
                        tokio::task::spawn_blocking(|| {
                            let path = rook_logging::create_log_bundle_zip()?;
...
impl TuiTerminalSessionView {
    ...
    fn render_session_content(&self, ctx: &AppContext) -> (Box<dyn TuiElement>, bool) {
        ...
        // footer, and the rooking/summary row are omitted; the blocker
        ...
        // While the selected conversation is in progress (the GUI rooking
        // indicator's core condition), the animated rooking indicator sits
        ...
        if let Some(conversation) = selected_conversation {
            if conversation.status().is_in_progress() {
                let rooking_elapsed = conversation
                    .latest_exchange()
                    .and_then(|exchange| exchange.time_since_start());
                if let Some(elapsed) = rooking_elapsed {
                    ...
                    content = content.child(
                        TuiContainer::new(self.render_rooking_indicator(label, elapsed, ctx))
                            .with_padding_top(1)
                            .finish(),
                    );
                }
