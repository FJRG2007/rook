...
use rook::settings::{TuiZeroStateObject, TuiZeroStateSettings, TuiZeroStateSettingsChangedEvent};
use rook_core::safe_warn;
use rook_core::settings::Setting;
use rookui::SingletonEntity;
use rookui_core::{AppContext, Entity};
...
impl ZeroStateShape {
    pub(super) fn contains(&self, x: f64, y: f64) -> bool {
        match self {
            Self::BuiltInRook => rook_logo_contains(x, y),
    ...
    pub(super) fn cell_aspect_ratio(&self) -> f64 {
        match self {
            Self::BuiltInRook => BUILT_IN_LOGO_CELL_ASPECT_RATIO,
...
impl Default for ZeroStateAnimationConfig {
    fn default() -> Self {
        Self {
            ...
            shape: Arc::new(ZeroStateShape::BuiltInRook),
            rotation_period: Duration::from_secs_f64(
                rook::settings::DEFAULT_TUI_ZERO_STATE_ROTATION_PERIOD_SECONDS,
            ),
            extrusion_depth: rook::settings::DEFAULT_TUI_ZERO_STATE_EXTRUSION_DEPTH,
            load_failure: None,
        }
    }
}
...
impl ZeroStateAnimationConfig {
    ...
    pub(crate) fn register(ctx: &mut AppContext) {
        let config_dir = rook_core::paths::tui_config_local_dir();
    ...
    pub(crate) fn load(
        ...
        let (active_object, shape, load_failure) = match load_object_shape(object, config_dir) {
            ...
            Err((path, error)) => {
                safe_warn!(
                    safe: (
                        "Could not load custom Rook Agent CLI zero-state ASCII art; using the built-in Rook logo"
                    ),
                    full: (
                        "Could not load custom Rook Agent CLI zero-state ASCII art; using the built-in Rook logo: path={} error={error}",
                        path.display()
                    )
                );
                (
                    TuiZeroStateObject::BuiltIn,
                    ZeroStateShape::BuiltInRook,
    ...
    pub(super) fn reload_object(
        ...
        match load_object_shape(object, config_dir) {
            ...
            Err((path, error)) => {
                safe_warn!(
                    safe: (
                        "Could not reload custom Rook Agent CLI zero-state ASCII art; keeping the current object"
                    ),
                    full: (
                        "Could not reload custom Rook Agent CLI zero-state ASCII art; keeping the current object: path={} error={error}",
                        path.display()
                    )
                );
...
fn load_object_shape(
    ...
    match object {
        TuiZeroStateObject::BuiltIn => Ok(ZeroStateShape::BuiltInRook),
