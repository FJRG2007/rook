...
use rookui::{AddWindowOptions, ModelHandle, ReadModel, SingletonEntity as _, UpdateModel};
use rookui_core::elements::tui::{TuiBufferExt, TuiRect, text_width};
use rookui_core::presenter::tui::TuiPresenter;
use rookui_core::{App, TuiView as _, TypedActionView as _, WindowId};
...
fn cloud_view(
    surface_id: rookui::EntityId,
    ctx: &rookui::AppContext,
) -> rookui::ViewHandle<TuiCloudRunView> {
...
fn dispatch_and_recv(
    ...
    let parent_conversation_id = app.read(|ctx| {
        rook::tui_export::BlocklistAIHistoryModel::as_ref(ctx)
...
fn local_harness_children_fail_cleanly() {
    App::test((), |mut app| async move {
        ...
        assert_error_containing(outcome, "aren't supported in Rook Agent CLI yet");
