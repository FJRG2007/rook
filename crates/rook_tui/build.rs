//! Build script for the `rook_tui` channel binaries.
//!
//! When the `release_bundle` feature is enabled and the internal
//! `rook-channel-config` generator is on PATH, this embeds each channel's config
//! JSON into `OUT_DIR` so the per-channel bins can `include_str!` it via
//! `rook_channel_config::load_config!`. Mirrors the equivalent logic in
...
/// If the `release_bundle` feature is enabled and `rook-channel-config` is
...
fn generate_channel_config_if_needed(target_os: &str) {
    ...
    // for external contributors building Rook OSS.
