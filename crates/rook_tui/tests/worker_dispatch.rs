...
fn dispatches_worker_invocation_instead_of_tui_frontend() {
    let empty_file =
        std::env::temp_dir().join(format!("rook-tui-worker-dispatch-{}", std::process::id()));
    std::fs::write(&empty_file, []).expect("failed to create empty worker-dispatch fixture");

    let output = Command::new(env!("CARGO_BIN_EXE_rook-tui-oss"))
        .arg("ripgrep-search")
        .arg("__rook_tui_worker_dispatch_probe__")
        .arg(&empty_file)
        .output()
        .expect("failed to invoke rook-tui worker");
    ...
    assert!(
        !String::from_utf8_lossy(&output.stdout).contains("Welcome to Rook TUI"),
