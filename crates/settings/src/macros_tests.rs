...
pub fn init_and_register_preferences(ctx: &mut AppContext) {
    ctx.add_singleton_model(move |_| {
        crate::PublicPreferences::new(Box::<
            rookui_extras::user_preferences::in_memory::InMemoryPreferences,
        >::default())
    });
    ctx.add_singleton_model(move |_| {
...
impl EventListener {
    fn new(ctx: &mut rookui_core::ModelContext<Self>) -> Self {
...
impl rookui_core::Entity for EventListener {
    type Event = ();
}
...
fn test_set_value_raises_changed_event_no_save() {
    rookui_core::App::test((), |mut app| async move {
...
fn test_public_setting_writes_to_public_prefs_when_flag_enabled() {
    let _guard = rook_features::FeatureFlag::SettingsFile.override_enabled(true);
    rookui_core::App::test((), |mut app| async move {
        app.update(init_and_register_preferences);
        app.add_singleton_model(|_| SettingsManager::default());
        TestSettings::register(&mut app);
...
fn test_public_setting_uses_private_prefs_when_flag_disabled() {
    let _guard = rook_features::FeatureFlag::SettingsFile.override_enabled(false);
    rookui_core::App::test((), |mut app| async move {
        app.update(init_and_register_preferences);
        app.add_singleton_model(|_| SettingsManager::default());
        TestSettings::register(&mut app);
