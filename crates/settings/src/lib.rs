...
// Re-export rookui_core for use by macros.
pub use rookui_core;
...
impl PublicPreferences {
    ...
    pub fn reload_from_disk(&self) -> Result<(), rookui_extras::user_preferences::Error> {
        self.0.reload_from_disk()
    }
}

impl rookui_core::Entity for PublicPreferences {
    type Event = ();
}

impl rookui_core::SingletonEntity for PublicPreferences {}
...
impl rookui_core::Entity for PrivatePreferences {
    type Event = ();
}

impl rookui_core::SingletonEntity for PrivatePreferences {}
...
pub enum SettingsMode {
    ...
    Gui,
    /// The headless terminal-UI front-end (the `rook_tui` crate).
    Tui,
}

impl SettingsMode {
    /// Whether settings for this mode are synced to the cloud (Rook Drive). The
...
impl SettingSurfaces {
    ...
    /// The headless terminal-UI front-end (the `rook_tui` crate) only.
...
pub trait Setting {
    ...
    /// Returns whether and how this setting is synced to the cloud via Rook Drive.
    ...
    fn set_value_from_cloud_sync(
        &mut self,
        new_value: Self::Value,
        ctx: &mut rookui_core::ModelContext<Self::Group>,
    ) -> anyhow::Result<()>;
