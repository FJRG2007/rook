...
fn element(initial_elapsed: Duration) -> TuiShimmeringText {
    element_with_text("Rooking", initial_elapsed)
}
...
fn paints_base_color_before_the_band_reaches_the_text() {
    ...
    for (index, char) in "Rooking".chars().enumerate() {
...
fn appends_and_paints_grouped_suffix_with_one_color_across_the_sweep() {
    ...
    for elapsed in [
        ...
        let (buffer, _) = render(element_with_text("Rooking", elapsed).with_grouped_suffix("..."));
        assert_eq!(buffer.to_lines()[0], "Rooking...");
    ...
    let (buffer, _) =
        render(element_with_text("Rooking", config.period / 2).with_grouped_suffix("..."));
...
fn leaves_trailing_ellipsis_ungrouped_by_default() {
    let config = ShimmerConfig::default();
    let (buffer, _) = render(element_with_text("Rooking...", config.period / 2));
