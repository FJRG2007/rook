...
//!
//! ## How does a11y work in Rook?
//! Because Rook uses its own rust UI framework (rookui), we don’t benefit from the built-in
...
//!
//! RookUI framework right now provides 3 ways of announcing what’s happening in the app:
...
pub struct AccessibilityContent {
    ...
    /// Frame support is a work-in-progress in Rook and right now this field is omitted and not set.
    pub frame: Option<RectF>,
    /// The role a given element has. Note that we use our own, RookUI-defined roles (vs those that
...
impl AccessibilityContent {
    ...
    pub fn new_without_help<T>(value: T, role: RookA11yRole) -> Self
    where
        T: Into<String>,
    {
    ...
    pub fn new<V, H>(value: V, help: H, role: RookA11yRole) -> Self
    where
        V: Into<String>,
        H: Into<String>,
    {
    ...
    fn new_internal<V, H>(value: V, help: Option<H>, role: RookA11yRole) -> Self
    where
        V: Into<String>,
        H: Into<String>,
    {
...
impl std::fmt::Display for RookA11yRole {
...
impl ActionAccessibilityContent {
    pub fn from_debug() -> Self {
        Self::CustomFn(|action| {
            AccessibilityContent::new_without_help(format!("{action:?}."), RookA11yRole::UserAction)
        })
    }
}
