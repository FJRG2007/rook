    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::MissingHeaderIdentifier => {
                write!(f, "Image did not contain the 'rook-img:' prefix.")
            }
        }
    }
}
...
impl CustomImageHeader {
    ...
    fn try_from_bytes(data: &[u8]) -> Result<(CustomImageHeader, &[u8]), CustomHeaderParsingError> {
        if !data.starts_with(b"rook-img:") {
...
impl Asset for ImageType {
    fn try_from_bytes(data: &[u8]) -> anyhow::Result<ImageType> {
        ...
        if data.starts_with(b"rook-img:") {
            let (custom_rook_header, data) = match CustomImageHeader::try_from_bytes(data) {
                Ok((custom_rook_header, data)) => (custom_rook_header, data),
                Err(err) => return Err(anyhow!(err.to_string())),
            };

            let data = data.into();
            let Some(img) = (match custom_rook_header.image_format {
                CustomImageFormat::Rgb => {
                    let dynamic_image = ImageBuffer::from_raw(
                        custom_rook_header.width,
                        custom_rook_header.height,
                        data,
                    )
                ...
                return Err(anyhow!(
                    "Could not convert custom rook image into approprate dynamic image."
                ));
            };
