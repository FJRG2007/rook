...
/// itself, a process that produces a tree of rendering primitives (in [`rookui`](crate),
...
pub trait TypedActionView {
    ...
    /// If your Action enum has tons of actions, and only some of them are meaningful, you can use helper implementations from the `rookui_core::accessibility` (like `ActionAccessibilityContent::Default()` or `ActionAccessibility_content::from_debug()`).
