...
pub struct AppContext {
    ...
    /// that `rookui_core` does not depend on `reqwest`.
    #[allow(clippy::type_complexity)]
...
impl AppContext {
    ...
    /// Used to load fallback fonts without pulling `reqwest` into `rookui_core`.
    ...
    pub fn open_view_tree_debug_window(&mut self, target_window_id: WindowId) {
        ...
        let options = AddWindowOptions {
            ...
            window_instance: Some("dev.rook.rookui-debug".to_owned()),
