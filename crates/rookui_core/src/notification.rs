    ...
    pub fn notifications_error_banner_title(&self) -> &str {
        match self {
            ...
            | NotificationSendError::PermissionsNotYetGranted => {
                "Rook tried to send you a notification for the last block but does not have permission."
            }
            NotificationSendError::Other { .. } => {
                "Rook tried to send you a notification for the last block, but something went wrong."
            }
        }
    }
}
