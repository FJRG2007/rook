...
pub const VIDEO_ENABLED_ENV_VAR: &str = "ROOK_INTEGRATION_TEST_VIDEO";

/// Environment variable that sets the output directory for video recordings
/// and screenshots. Defaults to `$TMPDIR/rook_integration_video_captures` when
/// unset.
pub const VIDEO_DIR_ENV_VAR: &str = "ROOK_INTEGRATION_TEST_VIDEO_DIR";
...
pub fn output_dir() -> PathBuf {
    let dir = std::env::var(VIDEO_DIR_ENV_VAR)
        .map(PathBuf::from)
        .unwrap_or_else(|_| std::env::temp_dir().join("rook_integration_video_captures"));
