...
pub struct LogConfig {
    ...
    /// Optional in-session size threshold for `rook.log`. When `Some(n)` and the active
    /// file accumulates more than `n` bytes during a single execution, it is rotated to
    /// `rook.log.in_session.0` and a fresh active file is opened. Older `.in_session.N`
