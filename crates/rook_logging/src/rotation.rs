//! In-session size-based rotation for `rook.log`.
//!
//! The existing startup rotation (`rotate_log_files`) handles the
//! per-execution boundary: each launch's `rook.log` becomes `rook.log.old.N`
...
//! threshold. Rotated copies land at `rook.log.in_session.N` (distinct from
