...
const TUI_LOG_SUBDIRECTORY: &str = "rook-cli";
...
/// 1) Renaming the log file from the main process (which just panicked) to `rook.log.old.temp`.
/// 2) Moving the crash recovery process log (which is located at `rook.log.recovery`) to the usual
///    path rook logs are located (log_directory/rook.log).
///    The temp log file (`rook.log.old.temp`) will ultimately be rotated to `rook.log.old.0` the next
...
pub async fn rotate_files(channel_file_name: &str, max_rotation: usize) -> Result<()> {
    ...
    // Rename `rook.log.old.temp` (the temporary file) to `rook.log.old.0`.
...
/// location at `log_directory/rook.log`.
...
/// Collects paths to the current rook instance's log file and any older
...
fn collect_log_paths_in(log_directory: &Path, logfile_name: &str) -> Result<Vec<PathBuf>> {
    ...
    if files.is_empty() {
        return Err(anyhow::anyhow!(
            "No rook logs were found for {logfile_name}"
        ));
    }
...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if rook_errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "rookui_core::core::redraw_frame" => sentry_log::LogFilter::Ignore,
        ...
        t if t.starts_with("rook::crash_reporting::") => sentry_log::LogFilter::Ignore,
...
fn init_internal(
    ...
    /// Returns an empty file named `rook.log` to log the current execution, and
    ...
    fn setup_log_files_for_current_execution(
        ...
        let main_log_path = if is_from_crash_recovery_process {
            ...
            // not rename the old main log file to `rook.log.temp` like we do below because this
            ...
            // Rename the old main log file to `rook.log.temp`.
    ...
    let integration_test = env::var("ROOK_INTEGRATION").is_ok();
    ...
    if use_logfile {
        ...
        // applies to the main process's `rook.log`.
...
fn init_log_directory() -> Result<std::path::PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            Ok(rook_core::paths::state_dir())
        } else if #[cfg(windows)] {
            Ok(rook_core::paths::state_dir().join(rook_core::paths::ROOK_LOGS_DIR))
        } else {
