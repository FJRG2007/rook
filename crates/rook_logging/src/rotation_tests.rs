...
fn writes_below_threshold_do_not_rotate() {
    ...
    let mut w = RotatingFileWriter::open(tmp.path(), "rook.log", 1024, 3).unwrap();
    w.write_all(b"hello world\n").unwrap();
    w.flush().unwrap();
    assert_eq!(read(&tmp.path().join("rook.log")), "hello world\n");
...
fn crossing_threshold_rotates_to_in_session_zero() {
    let tmp = tempfile::tempdir().unwrap();
    let mut w = RotatingFileWriter::open(tmp.path(), "rook.log", 16, 3).unwrap();
    w.write_all(b"first batch ").unwrap(); // 12 bytes
    w.write_all(b"more content").unwrap(); // crosses 16 → rotate before write
    w.flush().unwrap();
    assert_eq!(
        read(&tmp.path().join("rook.log.in_session.0")),
        "first batch "
    );
    assert_eq!(read(&tmp.path().join("rook.log")), "more content");
}

#[test]
fn repeated_rotations_shift_slots_up() {
    let tmp = tempfile::tempdir().unwrap();
    let mut w = RotatingFileWriter::open(tmp.path(), "rook.log", 8, 3).unwrap();
    ...
    w.flush().unwrap();
    assert_eq!(read(&tmp.path().join("rook.log")), "ddddddddd\n");
    assert_eq!(
        read(&tmp.path().join("rook.log.in_session.0")),
        "ccccccccc\n"
    );
    assert_eq!(
        read(&tmp.path().join("rook.log.in_session.1")),
        "bbbbbbbbb\n"
    );
    assert_eq!(
        read(&tmp.path().join("rook.log.in_session.2")),
        "aaaaaaaaa\n"
    );
}
...
fn overflow_drops_the_oldest_slot() {
    let tmp = tempfile::tempdir().unwrap();
    let mut w = RotatingFileWriter::open(tmp.path(), "rook.log", 8, 2).unwrap();
    w.write_all(b"aaaaaaaaa\n").unwrap();
...
fn oversized_first_write_does_not_promote_empty_file_to_in_session_zero() {
    ...
    assert_eq!(read(&tmp.path().join("rook.log")), "next\n");
}

#[test]
fn zero_max_rotation_truncates_in_place_without_sidecar() {
    let tmp = tempfile::tempdir().unwrap();
    let mut w = RotatingFileWriter::open(tmp.path(), "rook.log", 8, 0).unwrap();
    ...
    assert_eq!(read(&tmp.path().join("rook.log")), "second batch\n");
}

#[test]
fn zero_max_bytes_disables_rotation_entirely() {
    let tmp = tempfile::tempdir().unwrap();
    let mut w = RotatingFileWriter::open(tmp.path(), "rook.log", 0, 3).unwrap();
    for _ in 0..100 {
        w.write_all(b"line\n").unwrap();
    }
    w.flush().unwrap();
    assert!(!tmp.path().join("rook.log.in_session.0").exists());
    assert_eq!(read(&tmp.path().join("rook.log")).len(), 500);
}
