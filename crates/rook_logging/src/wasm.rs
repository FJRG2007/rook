...
pub fn init(_config: LogConfig) -> Result<()> {
    ...
    init_logger(Config::new(log::Level::Info).module_prefix("rook"));
    ...
    fn log(&self, record: &Record<'_>) {
        if self.enabled(record.metadata()) {
            ...
            match record.level() {
                ...
                Level::Error => {
                    if record.target() == rook_errors::LOG_TARGET {
                        let error = format!(
                            "ERROR: {}\n\n{}:{}",
                            record.args(),
                            record.file().unwrap_or_else(|| record.target()),
                        ...
                        rook_web_event_bus::emit_event(
                            rook_web_event_bus::RookEvent::ErrorLogged { error },
                        );
                    }
