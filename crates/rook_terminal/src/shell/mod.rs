...
impl Shell {
    ...
    ///
    /// These sequences are bound to Rook shell functions during session bootstrap that print the
...
impl From<ShellType> for rook_util::path::ShellFamily {
...
impl ShellType {
    ...
    /// on the target (e.g. via SSH or Auto-Rookify). A plain `PathBuf` would pick the
    ...
    pub fn native_completions_generator_command(self, hex_encoded_line: &str) -> String {
        match self {
            ShellType::Zsh | ShellType::Bash => {
                format!("rook_run_generator_command_native_completions {hex_encoded_line}")
            }
            ShellType::Fish => {
                format!(" rook_run_generator_command_native_completions {hex_encoded_line}")
            }
            ShellType::PowerShell => {
                format!("Rook-Run-GeneratorCommand-NativeCompletion {hex_encoded_line}")
            }
        }
    }
    ...
    /// If true, Rook will bootstrap the shell if it's the login shell on the remote host.
                        ...
                        output_string
                            ...
                            .flat_map(|line| {
                                ...
                                for ext in PATHEXT {
                                    ...
                                    // Rook about this command as-is and also sans-suffix, e.g.
                                    // "git" and "git.exe".
    ...
    pub fn shell_command_to_get_all_functions(&self) -> Option<&'static str> {
        match self {
            ShellType::PowerShell => Some(
                ...
                -not $_.Name.StartsWith('Rook') } | Select-Object -ExpandProperty Name; \
...
impl ShellLaunchData {
    ...
    fn to_native_path_encoding(&self, path_str: &str) -> Option<PathBuf> {
        match self {
            ...
            // The container is Unix; Rook runs on the host, so paths are
