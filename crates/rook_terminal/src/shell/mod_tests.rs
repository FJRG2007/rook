...
/// Regression test for https://github.com/FJRG2007/rook/issues/10474.
///
/// `rc_file_paths` is rendered into a shell command that runs on the *target*
/// (e.g. an SSH remote, or a subshell during Auto-Rookify). The path separator
/// must therefore depend on the target OS, not the host that built Rook.
...
fn test_rc_file_paths_use_target_os_separator() {
    ...
    // On Windows the only Auto-Rookify-supported shell is PowerShell; Unix
