...
fn test_secrets_serialization() {
    let mut blockgrid = mock_blockgrid("foo zach@rook.dev bar");
...
fn test_find_url_with_delimiter() {
    ...
    let blockgrid = mock_blockgrid("https://google.com/search?q=rook");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 0 }),
...
fn test_find_url_omits_trailing_periods() {
    ...
    let blockgrid = mock_blockgrid("Visit https://github.com/FJRG2007/rook/issues.");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 10 }),
    ...
    let blockgrid = mock_blockgrid("Visit https://github.com/rook.dev/Rook/issues.");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 10 }),
...
fn test_hyperlink_at_point_spans_contiguous_cells() {
    let _flag = rook_core::features::FeatureFlag::OscHyperlinks.override_enabled(true);
...
fn test_hyperlink_at_point_short_circuits_when_flag_disabled() {
    let _flag = rook_core::features::FeatureFlag::OscHyperlinks.override_enabled(false);
    let mut grid = GridHandler::new_for_test(5, 20);
