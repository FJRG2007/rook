...
pub(super) struct State {
    ...
    /// correctly. Notably, Zsh does NOT support this well in bracketed paste mode (which we use for all Rook
...
impl ansi::Handler for GridHandler {
    ...
    fn input(&mut self, c: char) {
        ...
        // `not(feature = "test-util")` covers other crates (e.g. `rook_tui`) that
        // build real terminal models against `rook`'s test helpers.
    ...
    fn identify_terminal<W: std::io::Write>(&mut self, writer: &mut W, intermediate: Option<char>) {
        match intermediate {
            ...
            Some('>') => {
                ...
                // Rook, rather than passing the higher `Pv` value, we decided to use one of the
    ...
    fn report_xtversion<W: std::io::Write>(&mut self, writer: &mut W) {
        ...
        let _ = writer.write_all(format!("\x1bP>|Rook({version})\x1b\\").as_bytes());
    }
    ...
    fn set_color(&mut self, _: usize, _: rookui_core::color::ColorU) {
        report_error!(
