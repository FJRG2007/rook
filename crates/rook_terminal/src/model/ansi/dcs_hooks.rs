...
/// Indicates that the following JSON-encoded message is hex-encoded for Rook's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Rook's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Rook's lifecycle
...
/// Enum representing all possible JSON payloads for Rook's DCS's.
///
...
pub(super) enum DProtoHook {
    ...
    SourcedRcFileForRook {
        value: SourcedRcFileForRookValue,
    },
    FinishUpdate {
...
impl<'de> Deserialize<'de> for DProtoHook {
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(match raw.hook.as_str() {
            ...
            "SourcedRcFileForRook" => DProtoHook::SourcedRcFileForRook {
                value: parse_hook_value::<_, D::Error>(raw.value)?,
            },
...
impl DProtoHook {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            DProtoHook::SourcedRcFileForRook { .. } => "SourcedRcFileForRook",
    ...
    pub fn session_id(&self) -> Option<SessionId> {
        match self {
            ...
            DProtoHook::SourcedRcFileForRook { .. } => None,
        }
    }
    ...
    pub fn requires_registered_session(&self) -> bool {
        match self {
            ...
            DProtoHook::SourcedRcFileForRook { .. } => false,
        }
    }
    ...
    pub fn default_from_name(hook: &str) -> Option<Self> {
        match hook {
            ...
            "SourcedRcFileForRook" => Some(DProtoHook::SourcedRcFileForRook {
                value: Default::default(),
            }),
...
impl PromptMetadata {
    ...
    ///
    /// This relies on the assumption that the rook_precmd shell function (responsible for writing
...
/// Received from the pty after the shell has finished executing Rook's
/// bootstrap script.
///
...
/// Received from the pty when Rook's SSH wrapper is executed, prior to
...
pub struct SSHValue {
    ...
    /// Rook must not tear down such a master on session exit. Defaults to
...
/// created; if the session is for a subshell, this triggers Rook's bootstrap process.
/// Otherwise, it's ignored.
///
/// NOTE: snippets installed by older Rook versions may also include a `tmux` field; serde
/// ignores unknown fields, so it is simply dropped now that the tmux SSH flow is removed.
#[derive(Debug, Default, Deserialize, Serialize, PartialEq, Eq)]
pub struct SourcedRcFileForRookValue {
    pub shell: String,
    pub uname: Option<String>,
}
...
/// The binding is triggered when Rook writes the `ESC-i` escape sequence to the pty.
/// Rook usually does this after a block completes, to collect any typeahead
...
/// Received from the pty when rook_finish_update is called at the end of an
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Rook client drop
