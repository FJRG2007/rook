...
impl Handler for MockHandler {
    ...
    fn sourced_rc_file(&mut self, data: SourcedRcFileForRookValue) {
        self.d_proto_hooks
            .push(DProtoHook::SourcedRcFileForRook { value: data })
    }
...
fn parse_sourced_rc_file_hook() {
    let rc_file_hook = r#"{"hook": "SourcedRcFileForRook", "value": { "shell": "zsh" }}"#;
    let bytes = [
    ...
    assert_eq!(handler.d_proto_hooks.len(), 1);
    match handler.d_proto_hooks.first().unwrap() {
        DProtoHook::SourcedRcFileForRook { value } => assert_eq!(
            *value,
...
fn parse_sourced_rc_file_hook_with_uname() {
    let rc_file_hook =
        r#"{"hook": "SourcedRcFileForRook", "value": { "shell": "zsh", "uname": "Darwin" }}"#;
    let bytes = [
        UNENCODED_JSON_DCS_START,
        &Vec::from(rc_file_hook.as_bytes()),
        DCS_END,
