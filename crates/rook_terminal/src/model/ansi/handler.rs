...
pub trait Handler {
    ...
    /// Callback for the Rook CommandFinished hook.
    ...
    /// Callback for a Rook precmd hook with completion metadata.
    fn precmd_with_completion_metadata(&mut self, _data: PrecmdValue) {}

    /// Callback for a prompt-only Rook precmd hook.
    ...
    /// Callback for the Rook preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Rook bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Rook pre-interactive SSH session hook - called once
    ...
    /// Callback for the Rook ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Rook exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Rook client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Rook).
    ...
    /// created. If the session is for a subshell, this should triggers Rook's bootstrap process.
    /// Otherwise, it's ignored.
    fn sourced_rc_file(&mut self, _data: SourcedRcFileForRookValue) {}
    ...
    /// These allow external programs to trigger notifications in Rook.
