...
//! executing one of Rook's DCS hooks, etc. [`Handler`] should be implemented by
...
/// Marks an OSC as one that is sent by Rook logic registered in the shell.
///
/// 9277 spells out "ROOK" on a dialpad :).
const ROOK_IN_BAND_GENERATOR_OSC_MARKER: &[u8] = b"9277";
const ROOK_IN_BAND_GENERATOR_START_BYTE: &[u8] = b"A";
const ROOK_IN_BAND_GENERATOR_END_BYTE: &[u8] = b"B";

/// Marks an OSC that is used for messages containing shell hooks.
const ROOK_OSC_MARKER: &[u8] = b"9278";
/// Marks an OSC that is used for resetting ConPTY's grid. This is useful for performing a series
/// of checks ensuring that Rook's grids and ConPTY's grid are in sync.
const ROOK_RESET_GRID_OSC_MARKER: &[u8] = b"9279";
...
const ROOK_COMPLETIONS_OSC_MARKER: &[u8] = b"9280";
const ROOK_COMPLETIONS_START_BYTE: &[u8] = b"A";
const ROOK_COMPLETIONS_END_BYTE: &[u8] = b"B";
const ROOK_COMPLETIONS_MATCH_RESULT_BYTE: &[u8] = b"C";
const ROOK_COMPLETIONS_REPLACEMENT_SPAN_BYTE: &[u8] = b"S";
const ROOK_COMPLETIONS_MATCH_UPDATE_METADATA: &[u8] = b"D?";

const ROOK_KV_START_BYTE: &[u8] = b"A";
const ROOK_KV_ENTRY_BYTE: &[u8] = b"B";
const ROOK_KV_END_BYTE: &[u8] = b"C";
...
fn parse_osc_7_cwd(payload: &[u8]) -> Option<String> {
    ...
    #[cfg(windows)]
    let decoded = rook_util::path::file_uri_drive_path_to_windows(&decoded).into_owned();
...
/// between each update.
///
/// There are two mechanisms to prevent Rook from falling too behind:
...
impl<'a, H: Handler + 'a, W: io::Write> Performer<'a, H, W> {
    ...
    fn handle_decoded_hook(&mut self, hook: Result<DProtoHook, serde_json::Error>) {
        ...
        match hook {
            ...
            Ok(DProtoHook::SourcedRcFileForRook { .. }) => {
                // The SourcedRCFileForRook hook should only be emitted by the
                // shell without hex encoding. The RC file snippet given to
                // users is not hex-encoded for the sake of transparency and
                // debugability.
                report_error!("Received hex-encoded SourcedRcFileForRook escape sequence.");
            }
    ...
    fn handle_unencoded_hook(&mut self, hook: Result<DProtoHook, serde_json::Error>) {
        // Currently, only the `SourcedRcFileForRook`, `InitShell`, and `InitSubshell` DCS's may
        ...
        match hook {
            ...
            Ok(DProtoHook::SourcedRcFileForRook { value }) => {
                self.handler.sourced_rc_file(value);
            }
            Ok(_) => {
                report_error!("Received non hex-encoded hook that is not SourcedRcFileForRook");
            }
            Err(err) => {
                log::warn!("Received malformed SourcedRcFileForRook hook {err:#}");
            }
        }
    }
    ...
    W: io::Write + 'a,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Received a Rook OSC used for in-band generators.
            ROOK_IN_BAND_GENERATOR_OSC_MARKER => match params.get(1) {
                Some(&ROOK_IN_BAND_GENERATOR_START_BYTE) => {
                    log::info!("Received a Rook OSC marker for starting in-band command output.");
                    self.handler.start_in_band_command_output();
                }
                Some(&ROOK_IN_BAND_GENERATOR_END_BYTE) => {
                    self.handler.end_in_band_command_output(true);
                }
                _ => {
                    log::warn!("Received a Rook OSC marker missing required param.");
                }
            },

            // Received a Rook OSC used for shell hooks.
            ROOK_OSC_MARKER => {
                ...
                match json_marker_char {
                    HEX_ENCODED_JSON_MARKER => {
                        ...
                        let Some(data_str) = params
                            ...
                            log::warn!("Rook OSC marker did not contain payload");
                            return;
                        };
                        safe_debug!(
                            safe: ("Received Rook OSC string for shell hook"),
                            full: ("Received Rook OSC string for shell hook with JSON payload: {:?}", data_str)
                        );
            ...
            ROOK_RESET_GRID_OSC_MARKER => {
                log::debug!("Received Rook OSC string for reset grid");
                self.handler.on_reset_grid();
            }

            // Received a Rook OSC used for completions.
            ROOK_COMPLETIONS_OSC_MARKER => match params.get(1) {
                Some(&ROOK_COMPLETIONS_START_BYTE) => {
                    self.handler.start_completions_output();
                }
                Some(&ROOK_COMPLETIONS_END_BYTE) => {
                    self.handler.end_completions_output();
                }
                Some(&ROOK_COMPLETIONS_MATCH_RESULT_BYTE) => {
                    let Some(match_text) = decode_hex_completions_payload(params.get(2)) else {
                        log::warn!(
                            "Rook completions match result OSC marker was missing or had an \
                ...
                Some(&ROOK_COMPLETIONS_REPLACEMENT_SPAN_BYTE) => {
                    let Some(data_str) = params
                        .get(2)
                        .map(|osc_data| String::from_utf8_lossy(osc_data))
                    else {
                        log::warn!(
                            "Rook completions replacement span OSC marker did not contain payload"
                        );
                        return;
                    };
                    let Some((start_str, length_str)) = data_str.split_once(',') else {
                        safe_warn!(
                            safe: (
                                "Rook completions replacement span OSC marker payload was not a \
                ...
                Some(bytes) if bytes.starts_with(ROOK_COMPLETIONS_MATCH_UPDATE_METADATA) => {
                    ...
                    match &parameter[2..] {
                        ...
                        _ => {
                            safe_warn!(
                                safe: (
                                    "Invalid Rook OSC marker parameter for completions match \
                                     metadata"
                                ),
                                full: (
                                    "Invalid Rook OSC marker parameter for completions match \
                ...
                _ => {
                    log::warn!("Received a Rook OSC completions marker missing required param.");
                }
            },
