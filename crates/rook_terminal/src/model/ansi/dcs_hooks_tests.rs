...
fn every_hook_tag_dispatches_to_the_matching_variant() {
    let cases = [
        ...
        (
            "SSH",
            serde_json::json!({"socket_path": "/tmp/rook.sock", "remote_shell": "bash"}),
        ),
        (
            "InitShell",
...
fn sourced_rc_file_hook_parses_frozen_snippet_format() {
    ...
    // parsing forever.
    let json = r#"{"hook": "SourcedRcFileForRook", "value": {"shell": "zsh", "uname": "Darwin"}}"#;
    let DProtoHook::SourcedRcFileForRook { value } =
        serde_json::from_str::<DProtoHook>(json).unwrap()
    else {
        panic!("expected a SourcedRcFileForRook hook");
    };
...
fn sourced_rc_file_hook_ignores_legacy_tmux_field() {
    let json = r#"{"hook": "SourcedRcFileForRook", "value": {"shell": "zsh", "uname": "Darwin", "tmux": false}}"#;
...
fn ssh_hook_round_trips_through_serialization() {
    let hook = DProtoHook::SSH {
        value: SSHValue {
            socket_path: "/tmp/rook.sock".into(),
    ...
    assert_eq!(value.socket_path, PathBuf::from("/tmp/rook.sock"));
