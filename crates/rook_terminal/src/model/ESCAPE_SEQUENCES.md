...
In Rook we operate on ANSI escape codes, so they always have a form of:
...
## Use cases already covered in Rook

### Mouse tracking
Programs such as `vim` or `tmux` allow users to use the mouse within the app. There's couple modes of operations for mouse tracking (more [here](https://www.xfree86.org/current/ctlseqs.html#Mouse%20Tracking)), but the one we care about in Rook is `SGR`. 
