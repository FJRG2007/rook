...
fn host_bash_command_sets_history_size_sentinels() {
    ...
    assert_eq!(
        env_value(&command, "ROOK_INITIAL_HISTFILESIZE"),
        Some(Some(BASH_HISTORY_SIZE_SENTINEL.to_owned()))
    );
    assert_eq!(
        env_value(&command, "ROOK_INITIAL_HISTSIZE"),
        Some(Some(BASH_HISTORY_SIZE_SENTINEL.to_owned()))
    );
}
...
fn host_non_bash_command_does_not_set_history_size_sentinels() {
    ...
    assert_eq!(env_value(&command, "ROOK_INITIAL_HISTFILESIZE"), None);
    assert_eq!(env_value(&command, "ROOK_INITIAL_HISTSIZE"), None);
}
