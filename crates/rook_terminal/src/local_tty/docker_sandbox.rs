...
//!
//! This module owns everything specific to running a Rook shell inside a
...
/// Prefix for generated container names: `rook-sandbox-<id>`.
const DOCKER_SANDBOX_NAME_PREFIX: &str = "rook-sandbox";

/// Root directory on the host under which Docker-sandbox scratch files
/// (bash init scripts, empty workspace mount points) live.
///
/// Lives under the Rook per-user cache directory rather than `/tmp` so:
...
fn docker_sandbox_host_root() -> PathBuf {
    rook_core::paths::cache_dir().join("docker-sandbox")
}

/// Resolves the absolute path to the `sbx` CLI binary using the Rook
/// process's `PATH`.
///
/// Rook's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Rook is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Rook panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Rook writes this sandbox's bash init script.
