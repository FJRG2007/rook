...
use rook_core::session_id::SessionId;
use rook_errors::report_error;
#[cfg(windows)]
use rook_util::path::windows::{powershell_5_path, powershell_7_path, wsl_path};
use rook_util::path::{
    canonicalize_git_bash_path, is_msys2_path, resolve_executable, rook_shell_path,
};
...
/// * On macOS, this includes `$APP_PATH/Contents/Resources/bin`, in which we put a wrapper around the Rook CLI.
...
pub fn extra_path_entries() -> impl Iterator<Item = PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            if let Some(resources_path) = rook_core::paths::bundled_resources_dir() {
...
/// executable binary for one of Rook's supported shell types (bash, fish, zsh).
...
impl ShellStarter {
    ...
    /// used to spawn a shell process for a new top-level Rook session. If a WSL Distribution is
    ...
    pub fn init(preferred_shell: impl AvailableShell) -> Option<ShellStarterSourceOrWslName> {
        ...
        if let Some(rook_shell_env_var) = rook_shell_path() {
            let (rook_shell_path, shell_type) = supported_shell_path_and_type(&rook_shell_env_var)
                .unwrap_or_else(|| {
                    panic!("Cannot spawn shell; $ROOK_SHELL_PATH is invalid: {rook_shell_env_var}")
                });
            let session_id = generate_session_id();
            let args = arguments_for_session_spawning_command(
                rook_shell_path.as_path().to_string_lossy().as_ref(),
                shell_type,
                session_id,
            );
            return Some(
...
pub struct DirectShellStarter {
    ...
    /// Arguments to be passed to the shell binary at [`shell_path`] when spawning a new Rook
    /// session.
    args: Vec<OsString>,
...
pub enum ShellStarterSource {
    ...
    /// The user chose the path to the shell by setting the `ROOK_SHELL_PATH` environment variable.
    Environment(DirectShellStarter),
    /// The default shell for the user (as indicated by the user's passwd entry on UNIX).
    /// On Windows, this an ordered list of shells hardcoded _by Rook_.
...
impl WslShellStarter {
    ...
    pub fn home_directory(&self) -> Option<PathBuf> {
        ...
        rook_util::path::convert_wsl_to_windows_host_path(
...
fn arguments_for_session_spawning_command(
    ...
    match shell_type {
        ShellType::Zsh => {
            ...
            // begins with a space. We use this to hide Rook bootstrap commands from the history.
            vec![
                "-c".to_owned().into(),
        ...
        ShellType::Bash => {
             ...
             * 5. Send an InitShell message to Rook through escape sequences.
             * The rook_send_message function is inlined here.
        ...
        ShellType::Fish => {
            ...
            vec![
                ...
                format!(
                    ...
                    // Fish's implementation of this breaks Rook by emitting `OSC 133 A` but not
                    // `OSC 133 B` afterwards, which we have assumed. This is a temporary workaround.
                    // See this issue: https://github.com/FJRG2007/rook/issues/7588
