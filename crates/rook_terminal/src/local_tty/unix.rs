...
/// static/musl Rook binary cannot do in-process.
...
fn build_host_shell_command(
    ...
    builder.env("TERM_PROGRAM", "RookTerminal");
    ...
    if let Some(version) = ChannelState::app_version() {
        ...
        // We also insert this rook-specific version so that
        // plugins can do rook-specific version checks without worrying
    ...
    if enable_ssh_wrapper {
        builder.env("ROOK_USE_SSH_WRAPPER", "1");
    } else {
        builder.env("ROOK_USE_SSH_WRAPPER", "0");
    }
    ...
    builder.env("ROOK_IS_LOCAL_SHELL_SESSION", "1");

    // Only advertise the protocol version when the HOA notifications feature is enabled.
    // Without it, Rook can't render structured CLI agent notifications,
    ...
    if shell_debug_mode {
        builder.env("ROOK_SHELL_DEBUG_MODE", "1");
    }
    if honor_ps1 {
        builder.env("ROOK_HONOR_PS1", "1");
    } else {
        builder.env("ROOK_HONOR_PS1", "0");
    }
    ...
    builder.env(
        "ROOK_PROMPT_NODE_VERSION_ENABLED",
    ...
    let path_append = extra_path_entries()
        ...
        .join(":");
    builder.env("ROOK_PATH_APPEND", path_append);
    ...
    if matches!(shell_starter.shell_type(), ShellType::Bash) {
        ...
        builder.env("ROOK_INITIAL_HISTFILESIZE", BASH_HISTORY_SIZE_SENTINEL);
        builder.env("ROOK_INITIAL_HISTSIZE", BASH_HISTORY_SIZE_SENTINEL);
    }
    ...
    if let Some(start_dir) = start_dir {
        builder.env("ROOK_INITIAL_WORKING_DIR", start_dir);
    }
...
fn spawn_command_in_pty(
    ...
    let is_isolated = rook_isolation_platform::detect().is_some();
    ...
    unsafe {
        ...
        command.pre_exec(move || {
            ...
            if is_isolated {
                ...
                // in case of OOM. If the Rook process is killed while hosting an ambient
...
fn build_docker_sandbox_command(
    ...
    // split. Many of these (e.g. `ROOK_USE_SSH_WRAPPER`,
    // `SSH_SOCKET_DIR`, `HISTFILESIZE`, `ROOK_IS_LOCAL_SHELL_SESSION`)
    ...
    builder.env("ROOK_HONOR_PS1", if honor_ps1 { "1" } else { "0" });
    builder.env(
        "ROOK_PROMPT_NODE_VERSION_ENABLED",
        if node_version_chip_enabled { "1" } else { "0" },
    );
    ...
    builder.env("ROOK_INITIAL_HISTFILESIZE", BASH_HISTORY_SIZE_SENTINEL);
    builder.env("ROOK_INITIAL_HISTSIZE", BASH_HISTORY_SIZE_SENTINEL);
    // Intentionally do NOT set `ROOK_INITIAL_WORKING_DIR` for sandboxes:
...
/// Rook panes/sandboxes don't race on or share the same host directories.
///
/// The actual sandbox creation + attachment happens via
/// `sbx run --name rook-sandbox-<id> shell WORKSPACE ... -- -c "cd /home/agent && exec bash --rcfile ..."`
...
/// under the Rook cache dir, and (2) the stopped `rook-sandbox-<id>`
...
///   - `sbx rm --force rook-sandbox-<id>` to drop the container,
...
fn prepare_docker_sandbox(starter: &DockerSandboxShellStarter) -> Result<()> {
    ...
    // the per-user Rook cache dir rather than `/tmp`) prevents the init
    // script from being read or symlink-attacked by anyone other than the
    // Rook user. The file itself is left at the default mode so the
