...
pub fn record_pty_throughput<M>(
    mut pty_reads_rx: Receiver<Arc<Vec<u8>>>,
    model: Arc<FairMutex<M>>,
    should_record_bytes: impl Fn(&M) -> bool + Send + Sync + 'static,
    record_max_throughput: impl Fn(usize) + Send + Sync + 'static,
    ...
    M: Send + 'static,
{
    ...
    let should_record_bytes = Arc::new(should_record_bytes);
    let record_max_throughput = Arc::new(record_max_throughput);
        ...
        .spawn(async move {
                ...
                .is_some()
            {
                ...
                *num_bytes_read_in_last_second = 0;
                ...
                {
                    if *max_throughput > 0 {
                        record_max_throughput(*max_throughput);
                    }
                    *max_throughput = 0;
