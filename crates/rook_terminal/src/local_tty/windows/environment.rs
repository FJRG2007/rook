...
const HONOR_PS1_NAME: &str = "ROOK_HONOR_PS1";
const PROMPT_NODE_VERSION_ENABLED_NAME: &str = "ROOK_PROMPT_NODE_VERSION_ENABLED";
const INITIAL_WORKING_DIR_NAME: &str = "ROOK_INITIAL_WORKING_DIR";
const USE_SSH_WRAPPER_NAME: &str = "ROOK_USE_SSH_WRAPPER";
const SSH_REUSE_CONTROL_MASTER_NAME: &str = "ROOK_SSH_REUSE_CONTROL_MASTER";
const SHELL_DEBUG_MODE_NAME: &str = "ROOK_SHELL_DEBUG_MODE";
const TERM_PROGRAM_NAME: &str = "TERM_PROGRAM";
const IS_LOCAL_SESSION_NAME: &str = "ROOK_IS_LOCAL_SHELL_SESSION";
const SSH_SOCKET_DIR: &str = "SSH_SOCKET_DIR";
const PATH_APPEND_NAME: &str = "ROOK_PATH_APPEND";
...
pub(super) fn get_shell_environment_variables(options: &PtyOptions) -> Vec<u16> {
    ...
    env.insert(
        map_key(ROOK_CLIENT_VERSION_ENV.into()),
        EnvEntry {
            preferred_key: ROOK_CLIENT_VERSION_ENV.into(),
    ...
    // Set ROOK_PATH_APPEND with additional PATH entries to append
...
fn wsl_env_allowlist(include_initial_working_dir: bool) -> OsString {
    let mut entries = vec![
        ...
        format!("{ROOK_CLIENT_VERSION_ENV}/u"),
    ...
    if FeatureFlag::HOANotifications.is_enabled() {
        entries.push(format!("{ROOK_CLI_AGENT_PROTOCOL_VERSION_ENV}/u"));
    }
