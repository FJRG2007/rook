...
impl AuthState {
    ...
    /// 2. ROOK_USER_SECRET environment variable
    ...
    pub fn initialize(ctx: &AppContext) -> Self {
        ...
        // Try ROOK_USER_SECRET environment variable.
        if let Some(persisted) = option_env!("ROOK_USER_SECRET")
    ...
    /// Returns whether the user considered onboarded to Rook.
    ...
    /// Anonymous users are real Rook users, but have no providers linked in Firebase.
    ...
    /// originated from viewing Rook on web.
...
// Adapter for the [`rook_managed_secrets`] crate, which needs to access the current user.
impl rook_managed_secrets::ActorProvider for AuthState {
