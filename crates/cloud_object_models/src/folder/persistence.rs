...
pub fn upsert_folders(
    ...
    conn.transaction::<(), Error, _>(|conn| {
        for cloud_folder in cloud_folders {
            ...
            let folder_is_rook_pack = cloud_folder.model().is_rook_pack;
            upsert_cloud_object(
                conn,
                ObjectType::Folder,
                cloud_folder.id,
                ...
                Box::new(move |conn| {
                    let new_folder = NewFolder {
                        ...
                        is_rook_pack: folder_is_rook_pack,
                    };
                ...
                Box::new(move |conn, folder_id| {
                    diesel::update(folders.filter(schema::folders::dsl::id.eq(folder_id)))
                        .set((
                            ...
                            is_rook_pack.eq(folder_clone.model().is_rook_pack),
                        ))
                        .execute(conn)?;
                    Ok(())
                }),
        ...
        .filter_map(|folder| {
            ...
            Some(CloudFolder::new(
                ...
                CloudFolderModel {
                    ...
                    is_rook_pack: folder.is_rook_pack,
                },
