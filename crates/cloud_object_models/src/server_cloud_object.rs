...
where
    ...
    S: Serializer<T>,
{
    type GqlType = rook_graphql::generic_string_object::GenericStringObject;
...
impl TryFromGql for ServerFolder {
    type GqlType = rook_graphql::folder::Folder;

    fn try_from_gql(value: Self::GqlType) -> Result<Self> {
        let uid = ServerId::from_string_lossy(value.metadata.uid.inner());
        Ok(Self::new(
            SyncId::ServerId(uid),
            CloudFolderModel::new(&value.name, value.is_rook_pack),
            value.metadata.try_into()?,
            value.permissions.try_into()?,
        ))
    }
...
impl TryFromGql for ServerNotebook {
    type GqlType = rook_graphql::notebook::Notebook;
...
impl TryFromGql for ServerWorkflow {
    type GqlType = rook_graphql::workflow::Workflow;
...
impl TryFrom<rook_graphql::object::CloudObject> for ServerCloudObject {
    type Error = anyhow::Error;

    fn try_from(value: rook_graphql::object::CloudObject) -> Result<Self, Self::Error> {
        match value {
            rook_graphql::object::CloudObject::AIConversation(_) => Err(anyhow::anyhow!(
                "AIConversation is not a supported object type for this operation"
            )),
            rook_graphql::object::CloudObject::Folder(folder) => Ok(ServerCloudObject::Folder(
                ServerFolder::try_from_gql(folder)?,
            )),
            rook_graphql::object::CloudObject::GenericStringObject(gso) => {
                server_gso_to_cloud_object(gso)
            }
            rook_graphql::object::CloudObject::Notebook(notebook) => Ok(
                ServerCloudObject::Notebook(ServerNotebook::try_from_gql(notebook)?),
            ),
            rook_graphql::object::CloudObject::Workflow(workflow) => Ok(
                ServerCloudObject::Workflow(Box::new(ServerWorkflow::try_from_gql(workflow)?)),
            ),
            rook_graphql::object::CloudObject::Unknown => {
...
fn server_gso_to_cloud_object(
    ...
    match gso.format {
        ...
        rook_graphql::generic_string_object::GenericStringObjectFormat::JsonPreference => Ok(
        ...
        rook_graphql::generic_string_object::GenericStringObjectFormat::JsonWorkflowEnum => Ok(
        ...
        rook_graphql::generic_string_object::GenericStringObjectFormat::JsonAIFact => Ok(
        ...
        rook_graphql::generic_string_object::GenericStringObjectFormat::JsonMCPServer => Ok(
        ...
        rook_graphql::generic_string_object::GenericStringObjectFormat::Unknown => Err(anyhow::anyhow!(
