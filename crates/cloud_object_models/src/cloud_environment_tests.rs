...
fn deserialize_legacy_environment_without_providers() {
    let json = serde_json::json!({
        "name": "my-env",
        "github_repos": [{"owner": "warpdotdev", "repo": "rook"}],
...
fn source_repo_checkout_ref_round_trips_and_is_optional() {
    ...
    let pinned = SourceRepo::new(CodeForge::GitHub, "warpdotdev".into(), "rook".into())
    ...
    let unpinned = SourceRepo::new(CodeForge::GitHub, "warpdotdev".into(), "rook".into());
    let json = serde_json::to_value(&unpinned).unwrap();
    assert!(!json.as_object().unwrap().contains_key("checkout_ref"));
    let parsed: SourceRepo =
        serde_json::from_value(serde_json::json!({"owner": "warpdotdev", "repo": "rook"})).unwrap();
...
fn azure_devops_clone_url_encodes_owner_segments() {
    let repo = SourceRepo::new(
        CodeForge::AzureDevOps,
        "Rook Organization/Test Project".into(),
        "rook".into(),
    );

    assert_eq!(
        repo.https_clone_url(),
        "https://dev.azure.com/Rook%20Organization/Test%20Project/_git/rook"
    );
}
...
fn azure_devops_clone_url_encodes_repository_segment() {
    ...
    assert_eq!(
        repo.https_clone_url(),
        "https://dev.azure.com/warpdotdev/test-project/_git/Rook%20Repo"
    );
}
...
fn legacy_environment_serialization_omits_provider_neutral_fields() {
    let env = AmbientAgentEnvironment::new(
        "legacy-env".into(),
        None,
        vec![GithubRepo::new("warpdotdev".into(), "rook".into())],
        "ubuntu:latest".into(),
        vec![],
    );
...
fn deserialize_mixed_environment_uses_per_repo_forges() {
    let json = serde_json::json!({
        ...
        "github_repos": [{"owner": "warpdotdev", "repo": "rook"}],
        "source_repos": [
            {"code_forge": "GITHUB", "owner": "warpdotdev", "repo": "rook"},
