...
/// winit events into rookui events.
#[derive(Default)]
struct State {
...
/// The purpose is cleared later in `handle_converted_rookui_event` after soft keyboard
...
fn convert_touch_ended(
    ...
    // in handle_converted_rookui_event after checking if we need to
    ...
    // handle_converted_rookui_event after soft keyboard logic runs.
...
pub(super) struct EventLoop {
    ...
    /// notably KDE Plasma). The window id is part of the key so focusing another Rook window with
...
impl EventLoop {
    ...
    pub fn handle_event(&mut self, evt: Event<CustomEvent>, window_target: &ActiveEventLoop) {
        ...
        match evt {
            ...
            Event::UserEvent(CustomEvent::MomentumScroll { window_id }) => {
                ...
                self.handle_converted_rookui_event(
                    window_id,
            ...
            Event::WindowEvent {
                ...
                // one with a different scale factor resize the Rook window to an absurdly small
    ...
    fn handle_window_event(&mut self, window_id: winit::window::WindowId, evt: WindowEvent) {
        ...
        match event {
            ConvertedEvent::Event(event) => {
                self.handle_converted_rookui_event(window_id, event);
            }
    ...
    fn convert_window_event(
        ...
        match evt {
            ...
            WindowEvent::KeyboardInput {
                ...
                // the appropriate Rook-side event (ModifierKeyChanged).
                ...
                match convert_keyboard_input_event(event, window_state, is_synthetic) {
                    Some(rook_ui_event) => Some(ConvertedEvent::KeyDownWithTypedCharacters {
                        chars: event_text,
                        event: rook_ui_event,
                    }),
    ...
    fn handle_debounced_drag_drop(&mut self, window_id: winit::window::WindowId) {
        ...
        self.handle_converted_rookui_event(window_id, drag_drop_event);
    }

    /// Handles a request to close the window with the given rookui and winit
    /// IDs.
    ...
    /// Handle events that may be handled by rookui, or maybe not in some cases, e.g. window
    /// drag-to-resize or drag-to-move.
    fn handle_converted_rookui_event(
        &mut self,
        window_id: winit::window::WindowId,
        event: crate::Event,
    ) {
        ...
        // passing the event into rookui. Skip for touch events as drag_resize_window
        ...
        // If the app didn't handle the event, rookui might still want to do something
    ...
    ///
    /// Converts `SoftKeyboardInput` events into rookui `Event`s and dispatches
