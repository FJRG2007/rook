...
use rookui_core::{ApplicationBundleInfo, WindowId, platform};
...
impl platform::Delegate for AppDelegate {
    ...
    fn open_character_palette(&self) {
        ...
        dispatch::Queue::main().exec_async(move || {
            ...
            // first responder's cursor location. In our case, that will be the Rook
    ...
    fn show_native_platform_modal(&self, id: ModalId, modal: AlertDialog) {
        ...
        // SAFETY: `showModal:modalId:` is a custom rook NSApplication selector.
        unsafe {
            let app = &*app::get_rook_app().cast::<NSApplication>();
        ...
        dispatch::Queue::main().exec_async(move || {
            ...
            match termination_mode {
                ...
                TerminationMode::ForceTerminate | TerminationMode::ContentTransferred => {
                    // `setForceTermination` is a custom rook NSApplication selector.
...
pub unsafe extern "C-unwind" fn rook_on_request_notification_permissions_completed(
...
/// This function is marked unsafe because it retrieves the pointer to the callback
/// function that we sent down to the Objective-C code.
pub unsafe extern "C-unwind" fn rook_on_notification_send_error(
