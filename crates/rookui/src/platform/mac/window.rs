...
use rookui_core::accessibility::AccessibilityContent;
use rookui_core::actions::StandardAction;
use rookui_core::r#async::{Timer, executor};
use rookui_core::event::ModifiersState;
use rookui_core::platform::{
...
use rookui_core::rendering::GPUPowerPreference;
use rookui_core::windowing::WindowCallbacks;
use rookui_core::{DisplayId, DisplayIdx, Event, OptionalPlatformWindow, Scene, WindowId};
...
impl platform::WindowManager for WindowManager {
    ...
    fn app_is_active(&self) -> bool {
        // SAFETY: `get_rook_app()` returns the running NSApplication subclass instance.
        let app = unsafe { &*app::get_rook_app().cast::<NSApplication>() };
        app.isActive()
    }
    ...
    fn ordered_window_ids(&self) -> Vec<WindowId> {
        ...
        for i in 0..count {
            ...
            // SAFETY: `is_rook_window` is an FFI call into the RookWindow class, and
            // rook windows always carry the window-state ivar.
            unsafe {
                if is_rook_window(&window).as_bool() {
...
unsafe extern "C" {
    ...
    fn is_rook_window(window: &NSWindow) -> Bool;
...
impl Window {
    pub fn open(
        ...
        autoreleasepool(move |_| {
            ...
            let test_mode = cfg!(feature = "integration_tests")
                && std::env::var("ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS").is_err();
            ...
            // SAFETY: `native_window` is either null or a valid `RookWindow`.
            let Some(native_window_ref) = (unsafe { native_window.as_ref() }) else {
                return Err(anyhow!("RookWindow returned nil from initializer"));
            };
            ...
            if options.fullscreen_state == FullscreenState::Fullscreen {
                ...
                // SAFETY: `enqueueFullscreenTransition` is a custom RookWindow selector.
                let _: () = unsafe { msg_send![native_window_ref, enqueueFullscreenTransition] };
            }

            let native_view = native_window_ref
                .contentView()
                .expect("RookWindow always has a content view");
            ...
            unsafe {
                ...
                let native_window_delegate = native_window_ref
                    .delegate()
                    .expect("RookWindow always has a delegate");
            ...
            rook_view_did_change_backing_properties(as_objc_object(&native_view), true);
    ...
    pub fn active_window_id() -> Option<WindowId> {
        ...
        // SAFETY: `is_rook_window` and the window-state ivar accessor are FFI calls
        // into the RookWindow class.
        unsafe {
            if is_rook_window(&native_window).as_bool() {
    ...
    pub fn frontmost_window_id() -> Option<WindowId> {
        // SAFETY: `get_frontmost_window` returns null or a valid `RookWindow`, and
    ...
    pub fn key_window_is_modal_panel() -> bool {
        ...
        // `isModalPanel` is a custom RookWindow selector returning a BOOL.
    ...
    pub fn close_ime_on_active_window() {
        ...
        // SAFETY: `is_rook_window` is an FFI call into the RookWindow class.
        if unsafe { is_rook_window(&native_window) }.as_bool() {
            Self::send_close_ime_msg(&native_window);
        }
    }

    fn send_close_ime_msg(native_window: &NSWindow) {
        // SAFETY: rook windows carry the window-state ivar, and the content view is a
        // RookHostView exposing the custom `closeIMEAsync` selector.
        unsafe {
    ...
    pub fn set_all_windows_background_blur_radius(blur_radius_pixels: u8) {
        ...
        for i in 0..windows.count() {
            ...
            // SAFETY: `is_rook_window` / `set_window_background_blur_radius` are FFI
            // calls into the RookWindow class.
            unsafe {
                if is_rook_window(&window).as_bool() {
    ...
    /// Returns a reference to a `RookWindow` identified by `window_id`, if any.
    ///
    /// # Safety
    ...
    pub fn close_window_async(window_id: WindowId, termination_mode: TerminationMode) {
        ...
        // is a custom RookWindow selector taking a BOOL.
        unsafe {
            if let Some(window) = Self::find_window_with_id(window_id) {
...
impl platform::Window for Window {
    ...
    fn toggle_fullscreen(&self) {
        // `enqueueFullscreenTransition` is a custom RookWindow selector.
        // SAFETY: messaging a valid window.
        let _: () = unsafe { msg_send![self.0.window(), enqueueFullscreenTransition] };
    }

    fn toggle_maximized(&self) {
        // `zoomAsync:` is a custom RookWindow selector taking a nil sender.
        // SAFETY: messaging a valid window.
...
impl WindowState {
    ...
    /// ivar and clears it in `rook_dealloc_window`.
    ...
    pub fn metal_layer(&self) -> Retained<CAMetalLayer> {
        let view = self
            .window()
            .contentView()
            .expect("RookHostView content view");
        let layer = view
            .layer()
            .expect("RookHostView always has a backing layer");
        layer
...
impl platform::WindowContext for WindowState {
    ...
    fn render_scene(&self, scene: Rc<Scene>) {
        *self.next_scene.borrow_mut() = Some(scene);
        // `setNeedsDisplayAsync` is a custom RookWindow selector.
        // SAFETY: messaging a valid window.
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_view_did_change_backing_properties(this: &Object, async_callback: bool) {
    // SAFETY: `this` is a RookHostView carrying the window-state ivar; its backing
    ...
    if !size.is_zero() {
        ...
        layer
            ...
            .expect("RookHostView backing layer is a CAMetalLayer")
...
#[unsafe(no_mangle)]
pub extern "C-unwind" fn rook_get_accessibility_contents(object: &mut Object) -> id {
...
#[unsafe(no_mangle)]
pub extern "C-unwind" fn rook_ime_position(object: &mut Object, content_rect: NSRect) -> NSRect {
    let state = unsafe { get_window_state(object) };
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_view_set_frame_size(this: &Object, size: NSSize, async_callback: bool) {
    // SAFETY: `this` is a RookHostView carrying the window-state ivar; its backing
    // layer is always a CAMetalLayer.
    let (window, layer) = unsafe {
        let window = get_window_state(this);
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_update_layer(this: &Object) {
    ...
    unsafe {
        ...
        // SAFETY: rook_update_layer should only be invoked for windows
        // created via Window::open(), which always sets a non-None device.
        let device = window
            .device
            .as_ref()
            .expect("rook_update_layer should not be called for a window that has no real display");
        // SAFETY: rook_update_layer is only invoked by the event loop,
        // which should never attempt to draw a window while it is already
        // being drawn.
        let mut renderer_manager = window
            .renderer_manager
            .as_ref()
            .expect("rook_update_layer should never be called twice in parallel")
            .borrow_mut();
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_handle_view_event(
    this: &Object,
    native_event: id,
...
/// Returns whether this event was handled.
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_handle_first_mouse_event(this: &Object, native_event: id) -> bool {
    let window = unsafe { get_window_state(this) };
    let event =
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_handle_insert_text(this: &Object, characters: id) {
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_handle_drag_and_drop(this: &Object, paths: id, point: NSPoint) {
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_handle_file_drag(this: &Object, point: NSPoint) {
    let window = unsafe { get_window_state(this) };
    let location = vec2f(point.x as f32, window.logical_size().y() - point.y as f32);
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_handle_file_drag_exit(this: &Object) {
    let window = unsafe { get_window_state(this) };
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_update_ime_state(this: &mut Object, ime_active: bool) {
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_marked_text_updated(
    this: &mut Object,
    marked_text: id,
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_marked_text_cleared(this: &mut Object) {
    let state = unsafe { get_window_state(&*this) };
    app::callback_dispatcher()
        .for_window(&Window(state.clone()))
...
#[unsafe(no_mangle)]
pub extern "C-unwind" fn rook_dispatch_standard_action(this: id, tag: NSInteger) {
...
#[unsafe(no_mangle)]
pub extern "C-unwind" fn rook_app_window_moved(this: id, rect: NSRect) {
    let state = unsafe { get_window_state(&*this) };
    let point = Vector2F::new(rect.origin.x as f32, rect.origin.y as f32);
    let size = Vector2F::new(rect.size.width as f32, rect.size.height as f32);
...
#[unsafe(no_mangle)]
pub extern "C-unwind" fn rook_dealloc_window(native_window: &mut Object) {
    log::info!("dealloc native window {native_window:p}");
    let state;
    // SAFETY: `native_window` is a RookWindow being deallocated; its content view
    ...
    unsafe {
        ...
        let native_window_delegate = window.delegate().expect("RookWindow always has a delegate");
        let _ = remove_state_ivar_from_object(
