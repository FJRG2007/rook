...
// NSWindow.delegate is a weak reference, so the RookWindowDelegate we create in
// `create_rook_nswindow` / `create_rook_nspanel` would otherwise be leaked with a +1
...
static const void *kRookWindowDelegateAssocKey = &kRookWindowDelegateAssocKey;

NSWindowStyleMask rookWindowMask = NSWindowStyleMaskClosable | NSWindowStyleMaskMiniaturizable |
...
void rook_dealloc_window(id self);
void rook_dispatch_standard_action(id self, NSInteger tag);
void rook_app_window_moved(id self, NSRect rect);
void rook_open_panel_file_selected(id urls, void *callback);
void rook_save_panel_file_selected(id url, void *callback);
...
@interface RookWindow : NSWindow <RookWindowProtocol>
@end

@interface RookWindowDelegate : NSObject <NSWindowDelegate>
@end

@implementation RookWindowDelegate {
    void *windowState;
...
- (void)windowDidMove:(NSNotification *)notification {
    if (windowState) {
        ...
        rook_app_window_moved(self, window.frame);
    }
}

- (void)windowWillStartLiveResize:(NSNotification *)notification {
    RookWindow *rook_window = notification.object;
    RookHostView *rook_view = rook_window.contentView;
    ...
    [rook_view setPresentsWithTransaction:YES];
}
...
- (void)windowDidEndLiveResize:(NSNotification *)notification {
    ...
    [rook_view setAsyncCallback:YES];
    [rook_view setPresentsWithTransaction:NO];
}
...
- (BOOL)windowShouldClose:(NSWindow *)window {
    ...
    BOOL okToClose = rook_app_should_close_window(application, window);
...
- (void)windowWillClose:(NSNotification *)note {
    if (windowState) {
        rook_app_window_will_close([NSApplication sharedApplication], self);
    }
}
...
- (void)windowWillEnterFullScreen:(NSNotification *)notification {
    NSWindow<RookWindowProtocol> *window = notification.object;
...
void init_rook_nswindow(NSWindow<RookWindowProtocol> *window, bool testMode, bool hideTitleBar) {
...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
    ...
    if ([event type] == NSEventTypeKeyDown) {
        ...
        // See #9709.
        if ([(RookHostView *)self.contentView hasMarkedText]) {
        ...
        BOOL keyBindingsDisabled = rook_app_are_key_bindings_disabled_for_window(application, self);
        // If Rook has assigned a binding for this keystroke.
        BOOL keystrokeIsAssigned = rook_app_has_binding_for_keystroke(application, event);

        BOOL triggersCustomAction = rook_app_has_custom_action_for_keystroke(application, event);
...
- (void)closeWindowAsync:(BOOL)forceTermination {
    dispatch_async(dispatch_get_main_queue(), ^{
      RookWindowDelegate *delegate = self.delegate;
      if (forceTermination) {
...
+ (RookWindow *)createWithContentRect:(NSRect)contentRect
    ...
    NSWindowStyleMask mask = rookWindowMask;

    if (hideTitleBar) {
        mask |= NSWindowStyleMaskFullSizeContentView;
    }

    RookWindow *window_result = [[RookWindow alloc] initWithContentRect:contentRect
                                                              styleMask:mask
                                                                backing:NSBackingStoreBuffered
                                                                  defer:NO];
    init_rook_nswindow(window_result, testMode, hideTitleBar);
...
@interface RookPanel : NSPanel <RookWindowProtocol>
@end

@implementation RookPanel {
    // The windowState is managed on the Rust side.
    void *windowState;
    // Height constraint for the titlebar view (also indicates if constraints are configured)
    NSLayoutConstraint *_titleBarHeightConstraint;
...
- (void)performClose:(id)sender {
    rook_dispatch_standard_action(self, [sender tag]);
}
...
// Note this returns a retained object ("create" rule).
+ (RookPanel *)createWithContentRect:(NSRect)contentRect
                         metalDevice:(id)metalDevice
                      hidingTitleBar:(BOOL)hideTitleBar
          backgroundBlurRadiusPixels:(uint8)backgoundBlurRadiusPixels
                        withTestMode:(BOOL)testMode {
    NSWindowStyleMask mask = rookWindowMask | NSWindowStyleMaskNonactivatingPanel;

    if (hideTitleBar) {
        mask |= NSWindowStyleMaskFullSizeContentView;
    }

    RookPanel *window_result = [[RookPanel alloc] initWithContentRect:contentRect
                                                            styleMask:mask
                                                              backing:NSBackingStoreBuffered
                                                                defer:NO];
    init_rook_nswindow(window_result, testMode, hideTitleBar);
...
// Attaches a RookWindowDelegate to |window| and ties its lifetime to the window.
//
...
static void attach_rook_window_delegate(NSWindow *window) {
    RookWindowDelegate *delegate = [[RookWindowDelegate alloc] init];
    [window setDelegate:delegate];
    objc_setAssociatedObject(window, kRookWindowDelegateAssocKey, delegate,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [delegate release];
}

// \return a new, retained RookPanel with the given content rect.
id create_rook_nspanel(NSRect contentRect, id metalDevice, BOOL hideTitleBar,
    ...
    RookPanel *window = [RookPanel createWithContentRect:contentRect
                                             metalDevice:metalDevice
                                          hidingTitleBar:hideTitleBar
                              backgroundBlurRadiusPixels:backgroundBlurRadiusPixels
                                            withTestMode:testMode];

    RookHostView *hostView = [[[RookHostView alloc] initWithFrame:contentRect
                                                      metalDevice:metalDevice
                                               enableTitlebarDrag:NO
                                                         testMode:testMode] autorelease];

    attach_rook_window_delegate(window);
...
// \return a new, retained RookWindow with the given content rect.
id create_rook_nswindow(NSRect contentRect, id metalDevice, BOOL hideTitleBar,
                        uint8 backgroundBlurRadiusPixels, BOOL testMode) {
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    ...
    RookWindow *window = [RookWindow createWithContentRect:contentRect
                                               metalDevice:metalDevice
                                            hidingTitleBar:hideTitleBar
                                backgroundBlurRadiusPixels:backgroundBlurRadiusPixels
                                              withTestMode:testMode];
...
BOOL is_rook_window(id window) {
    return [window isKindOfClass:[RookWindow class]] || [window isKindOfClass:[RookPanel class]];
}
...
// @param rookRole - the role of the given element (we're using our own, internally defined roles,
//                    check rookui::accessibility)
...
void set_accessibility_contents(id window, NSString *value, NSString *help, NSString *rookRole,
...
void open_file_picker(void *callback, NSArray<NSString *> *fileTypes, BOOL allowFiles,
    ...
    [openPanel beginWithCompletionHandler:^(NSInteger result) {
      // rook_open_panel_file_selected must be called unconditionally to avoid a memory leak
      if (result == NSModalResponseOK) {
          dispatch_async(dispatch_get_main_queue(), ^{
            rook_open_panel_file_selected([openPanel URLs], callback);
          });
      } else {
          dispatch_async(dispatch_get_main_queue(), ^{
            rook_open_panel_file_selected([NSArray array], callback);
          });
      }
    }];
}
...
void open_save_file_picker(void *callback, NSString *defaultFilename, NSString *defaultDirectory) {
    ...
    [savePanel beginWithCompletionHandler:^(NSInteger result) {
      // rook_save_panel_file_selected must be called unconditionally to avoid a memory leak
      if (result == NSModalResponseOK) {
          dispatch_async(dispatch_get_main_queue(), ^{
            rook_save_panel_file_selected([savePanel URL], callback);
          });
      } else {
          dispatch_async(dispatch_get_main_queue(), ^{
            rook_save_panel_file_selected(nil, callback);
          });
      }
    }];
}
...
void show_window_and_focus_app(RookWindow<RookWindowProtocol> *window, bool bringToFront) {
    previouslyActiveAppPID = [PreviousStateHelper storePreviousState];
...
void hide_window(RookWindow<RookWindowProtocol> *window) {
    NSRunningApplication *runningApp = [[NSWorkspace sharedWorkspace] frontmostApplication];
...
// dance.
void set_window_alpha(RookWindow<RookWindowProtocol> *window, double alpha) {
    [window setAlphaValue:alpha];
}

void set_window_title(id window, NSString *title) {
    if ([window isKindOfClass:[RookPanel class]] && [window isVisible]) {
...
void set_titlebar_height(id window, CGFloat height) {
    if ([window conformsToProtocol:@protocol(RookWindowProtocol)]) {
        [(id<RookWindowProtocol>)window configureTitlebarHeight:height];
    }
}

void position_and_order_front(RookWindow<RookWindowProtocol> *window) {
...
void position_at_given_location(RookWindow<RookWindowProtocol> *window, NSPoint origin) {
...
void order_front_without_focus(RookWindow<RookWindowProtocol> *window, NSPoint origin) {
