...
NSMutableDictionary<NSNumber *, RookHotKey *> *_hotKeys;
...
OSStatus HotkeyPressedHandler(EventHandlerCallRef _inCaller __unused, EventRef inEvent,
    ...
    RookHotKey *hotkey = _hotKeys[@(hotKeyID.id)];
    if (hotkey) {
        rook_app_send_global_keybinding((NSApplication *)inUserData, hotkey->_modifierKeys,
                                        hotkey->_keyCode);
        return noErr;
    }
...
void *registerGlobalHotkey(NSUInteger key, NSUInteger modifiers) {
    ...
    [_hotKeys setObject:[[[RookHotKey alloc] initWithEventHotKey:hotKeyRef
                                                         keyCode:key
...
@interface RookMenuItemDelegate : NSObject <NSMenuDelegate> {
    // Rust expects an ivar with this name.
    void *rustWrapper;
}
@end

@implementation RookDelegate {
    // Rust expects an ivar with this name.
    void *rustWrapper;
...
- (id)init {
    ...
    // rookui to still be run via `cargo run` since the app is not bundled in this case. Note this
...
- (void)applicationWillFinishLaunching:(NSNotification *)note {
    ...
    if (rustWrapper) rook_app_will_finish_launching(note.object);
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == NSAppThemeChangeContext) {
        if (rustWrapper) rook_app_os_appearance_changed(self);
    } else {
...
- (void)applicationDidBecomeActive:(NSNotification *)note {
    if (rustWrapper) rook_app_did_become_active(note.object);
}
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication *)application {
    ...
    if (!forceTermination) {
        ...
        okToTerminate = rook_app_should_terminate_app(application, systemInitiated);
    }
    ...
    if (okToTerminate) {
        if (systemInitiated) {
            ...
            // below, which returns `NSTerminateCancel`) makes macOS treat Rook
...
- (void)applicationDidResignActive:(NSNotification *)note {
    if (rustWrapper) rook_app_did_resign_active(note.object);
}

- (void)applicationWillTerminate:(NSNotification *)note {
    if (rustWrapper) rook_app_will_terminate(note.object);
}

- (void)application:(NSApplication *)sender openFiles:(NSArray<NSString *> *)filenames {
    if (rustWrapper) rook_app_open_files(sender, filenames);
}

- (void)application:(NSApplication *)application openURLs:(NSArray<NSURL *> *)urls {
    if (rustWrapper) rook_app_open_urls(application, urls);
}
...
- (void)keyWindowChanged:(NSNotification *)note {
    ...
    //  2. When a new window is created, rook will activate it; if we recursively call back into
    //     rook then we will cause the app to be mutably borrowed while already borrowed.
    if (!hasPendingActiveWindowChange) {
        hasPendingActiveWindowChange = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
          self->hasPendingActiveWindowChange = NO;
          if (self->rustWrapper) rook_app_active_window_changed(self);
        });
    }
}
...
- (void)windowMoved:(NSNotification *)note {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      if (self->rustWrapper) rook_app_window_did_move(self);
    });
}

- (void)windowResized:(NSNotification *)note {
    dispatch_async(dispatch_get_main_queue(), ^{
      if (self->rustWrapper) rook_app_window_did_resize(self);
    });
}

- (void)screenChanged:(NSNotification *)note {
    dispatch_async(dispatch_get_main_queue(), ^{
      if (self->rustWrapper) rook_app_screen_did_change(self);
    });
}
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
    // Trigger rook_menu_item_needs_update for every item with our class set as its represented
    // object.
    Class rookHandlerClass = [RookCustomMenuItemHandler class];
    for (NSMenuItem *item in menu.itemArray) {
        id obj = item.representedObject;
        if ([obj isKindOfClass:rookHandlerClass]) {
    ...
    internetReachable.reachableBlock = ^(Reachability *reach __unused) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        if (self->isReachable == nil || [self->isReachable intValue] == 0) {
            ...
            if (self->rustWrapper) rook_app_internet_reachability_changed(self, YES);
        }
      });
    };
    ...
    internetReachable.unreachableBlock = ^(Reachability *reach __unused) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        if (self->isReachable == nil || [self->isReachable intValue] > 0) {
            ...
            if (self->rustWrapper) rook_app_internet_reachability_changed(self, NO);
        }
      });
    };
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        if (self->isReachable == nil) {
            ...
            if (self->rustWrapper)
                rook_app_internet_reachability_changed(self, internetIsReachable);
        }
      });
    });
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
    ...
    // Handle what happens when the user clicks the notification. Rook doesn't support any actions
    ...
    if ([response.actionIdentifier isEqualToString:UNNotificationDefaultActionIdentifier]) {
        ...
        if (rustWrapper) {
            rook_app_notification_clicked(self, response.notification.date.timeIntervalSince1970,
                                          data);
        }
    }
}
...
@implementation RookApplication {
    // Rust expects an ivar with this name.
    void *rustWrapper;
}

- (void)setForceTermination {
    RookDelegate *delegate = (RookDelegate *)self.delegate;
...
- (void)showModal:(NSAlert *)alert modalId:(NSUInteger)modalId {
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      rook_app_process_modal_response(self, modalId, response - NSAlertFirstButtonReturn,
                                      disable_modal);
    });
}
...
RookApplication *get_rook_app() {
    // Set up the delegate (once).
    // The delegate is deliberately leaked.
    RookApplication *app = [RookApplication sharedApplication];
    static dispatch_once_t once;
    static id sharedDelegate;
    dispatch_once(&once, ^{
      sharedDelegate = [[RookDelegate alloc] init];
      [app setDelegate:sharedDelegate];

      // Hack to work around the fact that rook is frequently tested as a
...
NSMenu *make_delegated_menu(NSString *title) {
    ...
    result.delegate = (RookDelegate *)[[RookApplication sharedApplication] delegate];
    return result;
}
...
// The pointer will be provided back to Rook in the callbacks (see menus.h).
// The result is autoreleased.
NSMenuItem *make_rook_custom_menu_item(void *context) {
    RookCustomMenuItemHandler *handler =
        [[[RookCustomMenuItemHandler alloc] initWithContext:context] autorelease];
