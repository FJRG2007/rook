...
void rook_view_did_change_backing_properties(RookHostView *, BOOL);
void rook_view_set_frame_size(RookHostView *, NSSize, BOOL);
void rook_update_layer(RookHostView *);
BOOL rook_handle_view_event(RookHostView *, NSEvent *, BOOL);
BOOL rook_handle_first_mouse_event(RookHostView *, NSEvent *);
void rook_handle_insert_text(RookHostView *, id);
void rook_update_ime_state(RookHostView *, BOOL);
void rook_handle_drag_and_drop(RookHostView *, NSArray *, NSPoint);
void rook_handle_file_drag(RookHostView *, NSPoint);
void rook_handle_file_drag_exit(RookHostView *);
NSRect rook_ime_position(RookHostView *, NSRect *);
id rook_get_accessibility_contents(RookHostView *);
void rook_marked_text_updated(RookHostView *, NSString *, NSRange);
void rook_marked_text_cleared(RookHostView *);

@implementation NSPasteboard (Rook)
...
- (void)viewDidChangeBackingProperties {
    if (self.readyForRook) rook_view_did_change_backing_properties(self, asyncCallback);
...
- (void)setFrameSize:(NSSize)size {
    ...
    if (size.height >= self.window.minSize.height && size.width >= self.window.minSize.width) {
        ...
        if (self.readyForRook && changed) {
            rook_view_set_frame_size(self, size, asyncCallback);
        }
    }
}

- (void)displayLayer:(CALayer *)layer {
    if (!testMode && self.readyForRook) {
        rook_update_layer(self);
    }
}
...
- (BOOL)keyDownImpl:(NSEvent *)event {
    ...
    BOOL handled = NO;
    if (self.readyForRook) {
        handled = rook_handle_view_event(self, event, wasComposing || [self hasMarkedText]);
    }
    ...
    if ([textToInsert length] > 0 && !handled) {
        rook_handle_insert_text(self, (NSString *)textToInsert);
...
- (BOOL)acceptsFirstMouse:(NSEvent *)event {
    ...
    // and we explicity fire the event here so that Rook can handle it.
    if (self.readyForRook) rook_handle_first_mouse_event(self, event);
...
- (void)mouseDown:(NSEvent *)event {
    if (self.readyForRook) {
        BOOL eventHandled = rook_handle_view_event(self, event, NO);
        if (self->titlebarDragEnabled && !eventHandled && [self mouseInTitleBar:event]) {
            // If Rook doesn't do anything with the event, indicated by returning `false`, and
...
- (void)mouseUp:(NSEvent *)event {
    ...
    BOOL rook_handled = NO;
    if (self.readyForRook) {
        rook_handled = rook_handle_view_event(self, event, NO);
    }
    if (!rook_handled) {
...
- (void)otherMouseDown:(NSEvent *)event {
    if (self.readyForRook) rook_handle_view_event(self, event, NO);
}
...
- (RookHostView *)initWithFrame:(NSRect)frame
...
- (NSDragOperation)draggingUpdated:(id<NSDraggingInfo>)sender {
    ...
    if (self.readyForRook) {
        ...
        if ([types containsObject:NSPasteboardTypeFileURL]) {
            rook_handle_file_drag(self, localPoint);
            return YES;
        }
    }
    return NSDragOperationNone;
...
- (void)draggingExited:(id<NSDraggingInfo>)sender {
    if (self.readyForRook) {
        rook_handle_file_drag_exit(self);
    }
}
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
    ...
    if (self.readyForRook && (dragOperation & NSDragOperationCopy)) {
        NSArray *types = [pasteboard types];
        if ([types containsObject:NSPasteboardTypeFileURL]) {
            rook_handle_drag_and_drop(self, [pasteboard getFilePaths], localPoint);
            return YES;
        }
    }
    return NO;
...
- (id)accessibilityValue {
    return rook_get_accessibility_contents(self);
}
...
- (NSRect)firstRectForCharacterRange:(NSRange)range
    ...
    if (self.readyForRook) {
        ...
        NSRect rect = rook_ime_position(self, &contentRect);
        return rect;
    } else {
        return NSZeroRect;
    }
...
- (void)insertText:(id)string replacementRange:(NSRange)replacementRange {
    if (self.readyForRook) {
        ...
        if (interpretingKeyEvents) {
            ...
            rook_handle_insert_text(self, (NSString *)characters);
        }
...
- (void)setMarkedText:(id)string
    ...
    if (self.readyForRook) {
        rook_marked_text_updated(self, markedText.string, selectedRange);
        if ([markedText length] > 0) {
            rook_update_ime_state(self, YES);
        } else {
            rook_update_ime_state(self, NO);
        }
    }
}
