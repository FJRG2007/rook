...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_menu_item_needs_update(item: id, ctx: *mut c_void) {
...
#[unsafe(no_mangle)]
extern "C-unwind" fn rook_menu_item_triggered(_item: id, ctx: *mut c_void) {
    let func = &MenuItemData::read_context(ctx).triggered;
    callback_dispatcher().menu_item_triggered(func);
}

#[unsafe(no_mangle)]
extern "C-unwind" fn rook_menu_item_deallocated(ctx: *mut c_void) {
...
unsafe extern "C" {
    ...
    fn make_rook_custom_menu_item(ctx: *mut c_void) -> id;
...
fn resolve_standard_action(action: StandardAction) -> StandardMenuItemProperties {
    ...
    match action {
        ...
        StandardAction::Hide => make(ns_string!("Hide Rook"), sel!(hide:), cmd, ns_string!("h")),
...
unsafe fn apply_changes(changes: MenuItemPropertyChanges, item: id) {
    // Wrap in a local autorelease pool: AppKit invokes `rook_menu_item_needs_update`
    ...
    unsafe {
        match menu_item {
            MenuItem::Custom(custom_menu_item) => {
                ...
                let nsmenu_item = make_rook_custom_menu_item(MenuItemData::into_context(data));
