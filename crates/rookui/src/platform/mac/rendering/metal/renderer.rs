...
use rookui_core::fonts::{self, RasterizedGlyph, SubpixelAlignment, canvas};
use rookui_core::platform::CapturedFrame;
use rookui_core::rendering::texture_cache::TextureCache;
use rookui_core::rendering::{self};
use rookui_core::scene::{CornerRadius, GlyphFade, GlyphKey, Icon, Image, Layer, Scene};
...
impl super::super::Renderer for Renderer {
    fn render(&mut self, scene: &Scene, window: &WindowState, font_cache: &fonts::Cache) {
        // SAFETY: `render` is called via `rook_update_layer`, which is only be invoked for
