...
use rookui::elements::{Align, ConstrainedBox, ParentElement, Rect, Stack};
...
impl RootView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let font_family = rookui::fonts::Cache::handle(ctx).update(ctx, |cache, _| {
            cache
        ...
        Self {
            text: "Rook shimmer: 👩‍💻with ligatures — fi fl 🇺🇸".to_string(),
            font_family,
            font_size: 28.0,
            start,
            end,
