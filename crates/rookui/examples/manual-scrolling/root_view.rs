...
impl Element for ScrollableElement {
    ...
    fn after_layout(&mut self, _: &mut rookui::AfterLayoutContext, _: &AppContext) {}

    fn paint(&mut self, origin: Vector2F, ctx: &mut rookui::PaintContext, _app: &AppContext) {
    ...
    fn dispatch_event(
        &mut self,
        _: &rookui::event::DispatchedEvent,
        _: &mut rookui::EventContext,
        _: &AppContext,
    ) -> bool {
        false
    }
...
impl NewScrollableElement for ScrollableElement {
    ...
    fn scroll(&mut self, delta: rookui::units::Pixels, axis: Axis, ctx: &mut rookui::EventContext) {
        match axis {
            Axis::Horizontal => (),
            Axis::Vertical => {
