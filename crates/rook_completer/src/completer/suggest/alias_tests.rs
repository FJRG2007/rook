...
pub fn test_expand_command_aliases() {
    ...
    let result = rookui_core::r#async::block_on(expand_command_aliases(
    ...
    let result =
        rookui_core::r#async::block_on(expand_command_aliases("aliasForTest ", false, &ctx));
    ...
    #[cfg(not(feature = "v2"))]
    {
        ...
        let result =
            rookui_core::r#async::block_on(expand_command_aliases("test twelve ", false, &ctx));
...
pub fn test_expand_command_aliases_should_not_expand_if_no_space_after_alias() {
    ...
    let result = rookui_core::r#async::block_on(expand_command_aliases("test twelve", false, &ctx));
    ...
    let result =
        rookui_core::r#async::block_on(expand_command_aliases("aliasForTest twelve", false, &ctx));
    assert_eq!(result.expanded_command_line, "test twelve");
    assert_eq!(result.tokens_from_command, vec!["test", "twelve"]);
    // "twelve" isn't a valid subcommand, so we should use the "test" signature.
    #[cfg(not(feature = "v2"))]
