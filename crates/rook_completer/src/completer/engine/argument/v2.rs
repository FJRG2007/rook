...
async fn generate_suggestions_for_argument_value(
    ...
    match argument_value {
        ArgumentValue::Suggestion(suggestion) => {
            let rook_suggestion: Suggestion = suggestion.clone().into();
            match matcher.get_match_type(parsed_token.as_str(), rook_suggestion.display.as_str()) {
                Some(match_type) => vec![MatchedSuggestion::new(rook_suggestion, match_type)],
                None => vec![],
            }
        }
