...
impl CommandRegistry {
    ...
    fn new_with_embedded_signatures() -> Self {
        ...
        Self::register_rook_signatures(&registry);

        registry
    }

    /// Register signatures for Rook CLI commands.
    ///
    /// Ideally this would be done outside of the `rook_completer` crate, but it's not currently
    /// possible to configure the shared [`Self::global_instance`].
    fn register_rook_signatures(registry: &Self) {
        ...
        for channel in channels {
            ...
            let mut clap_cmd = rook_cli::Args::clap_command();
            let signature =
