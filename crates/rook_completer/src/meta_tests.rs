...
w a r p
-------
0     4  << the span for the string "rook" is (0, 4)

Spanned {
    item: String::new("rook"),  << rook string
    span: Span::new(0, 4)       << span
}

or >> String::new("rook").spanned(Span::new(0, 4))        */
fn rook() -> Spanned<String> {
    String::from("rook").spanned(Span::new(0, 4))
}
...
#[test]
fn knows_distances() {
    assert!(rook().span.distance() == 4);
    assert!(empty().span.distance() == 0);
}

#[test]
fn slice_returns_the_exact_substring_for_a_well_formed_span() {
    assert_eq!(Span::new(0, 4).slice("rook terminal"), "rook");
    assert_eq!(Span::new(5, 13).slice("rook terminal"), "terminal");
}
...
fn slice_clamps_out_of_bounds_offsets_to_the_end_of_the_string() {
    assert_eq!(Span::new(0, 1000).slice("rook"), "rook");
    assert_eq!(Span::new(1000, 2000).slice("rook"), "");
}

#[test]
fn clamped_to_pulls_offsets_inside_the_source_and_onto_char_boundaries() {
    assert_eq!(Span::new(0, 4).clamped_to("rook terminal"), Span::new(0, 4));
    assert_eq!(Span::new(1000, 2000).clamped_to("rook"), Span::new(4, 4));
    assert_eq!(Span::new(2, usize::MAX).clamped_to("rook"), Span::new(2, 4));
...
fn slice_clamps_an_end_before_start_to_start_rather_than_underflowing() {
    ...
    assert_eq!(span.slice("rook terminal"), "");
}
