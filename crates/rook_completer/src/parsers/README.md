...
Let's say we are interested in parsing the input `rook --disable-telemetry`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "rook --disable_telemetry";
let start_offset = 0;
...
let example = Spanned { item: String::from("rook"), span: Span::new(0,4) };
assert_eq!(example.item, "rook".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = String::from("rook").spanned(Span::new(0,4));
assert_eq!(example.item, "rook".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = "rook -p --disable-telemetry";
...
let expected_word1 = String::from("rook").spanned(Span::new(0,4));
...
For more involved inputs (say commands separated by `|` and/or having `;`) the lite parser will effectively create the necessary `LitePipeline`s to express it, let's explore what happens if we lite parse the input `rook config-set --extension-path="/path/to/dir" ; echo $ROOK_VAR"` (*two pipelines here due to the ; character*), like so:

```rust
let input = "rook config-set --extension-path=\"/path/to/dir\" ; echo $ROOK_VAR";
let start_offset = 0;
