...
#[test]
fn test_tokenizer_rook_special_chars() {
    // Test string includes rook-related terms with hyphen, underscore, forward slash, backslash, and colon
    let test_string = "rook-cli/launch_command:run C:\\\\Program_Files\\\\Rook\\\\core-engine.dll check_status:/dev/rook_drive-0";
    let tokens = token_stream_helper(test_string);

    assert_eq!(tokens.len(), 25);
    assert_token(&tokens[0], 0, "rook-cli/launch_command:run", 0, 27);
    assert_token(&tokens[1], 1, "rook", 0, 4);
    ...
    assert_token(
        &tokens[7],
        7,
        "C:\\\\Program_Files\\\\Rook\\\\core-engine",
        28,
        64,
    );
    assert_token(&tokens[15], 15, "dll", 65, 68);
    assert_token(&tokens[16], 16, "check_status:/dev/rook_drive-0", 69, 99);
}
...
fn test_searcher() {
    ...
    let search_strings = ["run rook on web server", "run rook-on-web server"];
    ...
    let result = searcher.search_full_doc("rook on web").unwrap();
    assert_eq!(
        result.len(),
        2,
    ...
    assert_eq!(
        result[0].values.name, "run rook-on-web server",
...
fn test_searcher_scores() {
    ...
    let search_strings = ["run rook on web server", "run rook_on_web:server"];
    ...
    let result = searcher.search_full_doc("rook").unwrap();
    assert_eq!(
        result.len(),
        2,
        "both search strings should match with the custom tokenizer"
...
fn test_searcher_async() {
    ...
    let search_strings = [
        ...
        "Run rook-on-web",
        "Run fresh rook-local and clear rook-dev permissions",
    ...
    assert_eq!(
        result[0].name, "Run fresh rook-local and clear rook-dev permissions",
...
/// insert/delete call made in between -- as Rook Drive's per-object updates do -- ended up placed
...
fn test_searcher_async_rebuild_preserves_operation_order_with_interleaved_updates() {
    ...
    // incremental updates land (as Rook Drive's per-object insert calls do), then a second
