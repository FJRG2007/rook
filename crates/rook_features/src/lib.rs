...
pub enum FeatureFlag {
    ...
    /// If `true`, fetch updated Rook channel versions from the Rook server endpoint instead of
    /// from GCP directly.
    FetchChannelVersionsFromRookServer,
    ...
    /// Used to gate an experiment we're doing on RookDev ONLY
    ...
    /// Whether the user is part of the Rook Alpha Program (AI Trusted Testers).
    ...
    /// Enable multiselect in Notebooks and Rook Text.
    RichTextMultiselect,
    ...
    /// Enables next action prediction within Rook, powered by AI.
    AgentPredict,

    /// Enables receiving shared Rook Drive objects.
    SharedWithMe,
    ...
    /// Playground for reducing Rook UI clutter.
    MinimalistUI,
    ...
    /// Adds aliases for executing Rook Drive workflows.
    WorkflowAliases,
    ...
    /// Enables support for Rook Packs.
    RookPacks,
    ...
    /// Enables Rook Drive objects (like workflows) as context in AI context menu
    DriveObjectsAsContext,

    /// Expands code diff edits to replace the current pane instead of opening in a new tab.
    ExpandEditToPane,
    /// Enables fallback model load output messaging in the rooking indicator.
    FallbackModelLoadOutputMessaging,

    /// Names the model doing the work in the rooking indicator (e.g. "Rooking with
    ...
    /// Enables the /create-environment slash command for setting up Rook Environments
    ...
    /// Rook's notebook viewer instead of showing the raw JSON in the code editor.
    ...
    /// Enables Rook Managed Secrets functionality.
    RookManagedSecrets,
    ...
    /// Enables agent tips displayed below the rooking indicator in Agent Mode.
    AgentTips,
    ...
    /// Enables auto-syncing ambient plans to Rook Drive.
    SyncAmbientPlans,

    /// Enables platform skills support (--skill flag) for agent runs.
    ///
    /// Skills are loaded from `.agents/skills/`, `.rook/skills/`, `.claude/skills/`, and `.codex/skills/`
    ...
    /// Enables the OpenRook launch modal announcing Rook going open-source.
    ...
    /// Enables the launch modal announcing the Rook Agent CLI.
    AgentCliLaunchModal,
    ...
    /// Ctrl-G intercepts the keystroke and opens Rook's input editor instead of $EDITOR.
    CLIAgentRichInput,
    ...
    /// CLIs (e.g. `claude`) to execute prompts instead of Rook's agent harness.
    AgentHarness,

    /// Enables workspace- and block-snapshot handoff between cloud agent runs
    /// and the local Rook client.
    /// When enabled:
    ...
    /// Enables the install/update chip for the OpenCode Rook plugin.
    /// Requires HOANotifications to also be enabled.
    OpenCodeNotifications,

    /// Enables the install/update chip for the Codex Rook notification plugin.
    /// Requires HOANotifications to also be enabled.
    CodexNotifications,

    /// Enables the Codex Rook plugin marketplace integration.
    /// When disabled, Codex uses native OSC9 notifications.
    CodexPlugin,

    /// Enables the install/update chip for the Gemini CLI Rook extension.
    /// Requires HOANotifications to also be enabled.
    GeminiNotifications,
    ...
    /// Enables Rook local control through the standalone rookctrl CLI.
    RookControlCli,
    ...
    /// Replaces the in-block rookification banner with a rookify footer.
    RookifyFooter,
    ...
    /// route eliglible models to GEAP instead of Rook-managed inference.
    GeminiEnterprise,
    ...
    /// `rook_id` values in MCP configs and as bare identifiers in CLI
    /// `--mcp` arguments, resolved server-side at run setup.
    WellKnownMcpIds,

    /// Automatically attaches the Rook-hosted Factory MCP server
    ...
    /// `PricingTransparencyEnabled` flag in rook-server, but is a fully independent
    ...
    /// Uses fzf or atuin for history search instead of Rook's command search.
    ShellWidgetHandoff,
...
/// Features enabled only for the RookLocal developer build.
...
pub const DOGFOOD_FLAGS: &[FeatureFlag] = &[
    ...
    // These are enabled via 100% experiment on prod rook-server,
    ...
    FeatureFlag::RookingModelName,
    FeatureFlag::LrcActivitySignal,
    FeatureFlag::StoredScreenshots,
];

/// Features enabled for feature preview build users (e.g.: Friends of Rook).
/// All PREVIEW_FLAGS are also automatically added to dogfood builds (RookDev).
...
/// Features enabled for all release builds (i.e.: everything but RookLocal).
...
impl FeatureFlag {
    ...
    pub fn flag_description(&self) -> Option<&'static str> {
        ...
        match self {
            ...
            CloudEnvironments => {
                Some("Enables creating and managing Rook Environments via the CLI.")
            }
            CreateEnvironmentSlashCommand => Some(
                "Enables the /create environment slash command for setting up Rook Environments with custom configurations.",
            ),
            ...
            SettingsFile => Some(
                "Enables configuring Rook via a user-editable `settings.toml` file, with hot reload and error reporting for invalid values.",
            ),
