...
/// CLI-level selector for choosing one discovered Rook instance.
    ...
    match selector {
        ...
        InstanceSelector::Id(instance_id) => records
            ...
            .ok_or_else(|| {
                ControlError::new(
                    ...
                    format!("no Rook instance with id {}", instance_id.0),
                )
            }),
        InstanceSelector::Pid(pid) => records
            ...
            .ok_or_else(|| {
                ControlError::new(
                    ErrorCode::NoInstance,
                    format!("no Rook instance with pid {pid}"),
                )
            }),
    }
}
...
fn select_active(records: &[InstanceRecord]) -> Result<InstanceRecord, ControlError> {
    match records {
        [] => Err(ControlError::new(
            ErrorCode::NoInstance,
            "no local Rook control instances were discovered",
        )),
        [record] => Ok(record.clone()),
        _ => Err(ControlError::new(
            ErrorCode::AmbiguousInstance,
            "multiple local Rook control instances were discovered; pass --instance",
        )),
    }
}
