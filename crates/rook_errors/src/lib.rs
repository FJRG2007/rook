//! Explicit error reporting for Rook.
//!
...
//! Sentry without creating a dependency cycle with `rook_core`.
//!
...
// dependencies (reqwest/tokio/websocket), so they are feature-gated and enabled by `rook_core`;
...
/// `log::error!` no longer creates Sentry events (see `sentry_log_filter` in `rook_logging`), so
