...
fn authorize_url_contains_required_params() {
    ...
    assert!(url.contains("referrer=rook"));
...
fn manual_code_exchange_rejects_blank_code() {
    ...
    let result = rookui_core::r#async::block_on(exchange.exchange("   "));
...
fn cancelling_loopback_wait_releases_listener() {
    ...
    for _ in 0..CANCEL_CYCLES {
        ...
        let result = rookui_core::r#async::block_on(run_oauth_flow(
            listener,
            PkceParams::generate(),
            cancellation,
            release_tx,
...
fn release_signal_lets_a_caller_rebind_without_joining_the_result() {
    ...
    let result_thread = std::thread::spawn(move || {
        rookui_core::r#async::block_on(run_oauth_flow(
            listener,
            PkceParams::generate(),
            cancellation,
            release_tx,
    ...
    rookui_core::r#async::block_on(release_rx.recv())
