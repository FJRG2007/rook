...
pub const ROOK_SKILL_DIRS_ENV: &str = "ROOK_SKILL_DIRS";

/// Parse the `ROOK_SKILL_DIRS` environment variable into a list of directory paths.
///
...
pub fn parse_skills_dirs_env() -> Vec<PathBuf> {
    let Ok(val) = std::env::var(ROOK_SKILL_DIRS_ENV) else {
        return Vec::new();
    };
    val.split(',')
        .map(str::trim)
...
/// Resolve entries parsed from [`ROOK_SKILL_DIRS_ENV`] against a base directory.
///
...
/// layout used by `ROOK_SKILL_DIRS`. Entries that are not directories are skipped
/// with a warning.
///
...
pub fn read_skills_for_skills_dirs(dirs: &[PathBuf]) -> Vec<ParsedSkill> {
    dirs.iter()
        .filter(|dir| {
            ...
            log::warn!(
                "ROOK_SKILL_DIRS: skipping '{}' — not a directory or does not exist",
                dir.display()
            );
            false
        })
