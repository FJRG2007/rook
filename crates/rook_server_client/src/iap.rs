...
/// rook-server request. Generous enough to cover a couple of STS/IAM round trips
/// plus a retry.
...
/// Env var carrying a Rook OIDC task-identity JWT (audience = the WIF provider
/// resource name), injected by rook-server so a cold sandboxed runner can
/// bootstrap its first IAP mint without calling the IAP-gated identity-token
/// endpoint.
const STAGING_IAP_BOOTSTRAP_TOKEN_ENV_VAR: &str = "ROOK_STAGING_IAP_BOOTSTRAP_JWT";

pub type PathResolver = Box<dyn Fn(&mut AppContext) -> BoxFuture<'static, Option<String>>>;

/// Mints a Rook-signed OIDC identity token for a given audience. Implemented in
...
impl IapManager {
    ...
    /// Used to gate rook-server requests until IAP access is established.
    ...
    pub fn start_refresh(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // Make `gcloud` findable even when Rook is launched from the macOS GUI
        // (i.e. in environments without something like `~/.zshrc && RookDev` happening to init cli path)
    ...
    /// Federation (Rook OIDC JWT -> STS -> IAM `generateIdToken`). No gcloud.
    fn start_wif_refresh(
        ...
        // i.e. GCP SDK calling `oz federate issue-token --audience rook-cloud-agent-otel`
    ...
    fn apply_refresh_result(&mut self, result: Result<CachedToken>, ctx: &mut ModelContext<Self>) {
        ...
        match result {
            Ok(cached) => {
                ...
                // i.e. `oz federate issue-token --audience rook-cloud-agent-otel`
                // needs to transit IAP to get the token from rook-server
                if self.managed_mint.is_some() {
                    cache::write(&cached.token);
                }
                state.set_loaded(cached);
                self.consecutive_failures = 0;
                log::info!("Rook Staging IAP token refreshed");
                ctx.emit(IapManagerEvent::StateChanged);
                ctx.notify();
                self.schedule_next_refresh(expires_at, ctx);
            }
            Err(err) => {
                ...
                log::warn!("Rook Staging IAP token fetch failed: {message}");
...
async fn resolve_wif_identity_token(
    ...
    if get_expires_at(&injected_jwt).is_ok() {
        ...
        minter
            ...
            .await
            .map_err(|err| anyhow::anyhow!("failed to mint Rook identity token: {err:#}"))
    }
}

/// Mints an IAP-valid ID token for a sandboxed Oz runner via Workload Identity
/// Federation: Rook OIDC JWT -> GCP STS federated token -> IAM `generateIdToken`
...
async fn fetch_iap_token_via_wif(
    ...
    // Leg 1: obtain the Rook-signed OIDC JWT used as the STS subject token.
    let identity_token =
...
fn fetch_iap_token(
    ...
    cmd
        ...
        .args(args);
    // allows rook to resolve `gcloud` cli path
...
mod cache {
    ...
    fn cache_path() -> std::path::PathBuf {
        rook_core::paths::rook_home_config_dir()
            .unwrap_or_else(rook_core::paths::state_dir)
            .join("staging")
            .join("iap_cache.jwt")
    }
