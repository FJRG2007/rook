...
use rook_graphql::client::RequestOptions;
use rook_server_auth::auth_state::AuthState;
use rook_server_auth::credentials::AuthToken;
#[cfg(feature = "agent_mode_evals")]
use rook_server_auth::credentials::Credentials;
...
pub const AMBIENT_WORKLOAD_TOKEN_HEADER: &str = "X-Rook-Ambient-Workload-Token";

/// Header key for the cloud agent task ID attached to ambient-agent requests.
pub const CLOUD_AGENT_ID_HEADER: &str = "X-Rook-Cloud-Agent-ID";
...
pub const TEAM_UID_HEADER: &str = "X-Rook-Team-Uid";

/// IDs in the staging database that were created specifically for evals.
///
/// Keep this list in sync with `script/populate_agent_mode_eval_user.sql` in rook-server.
#[cfg(feature = "agent_mode_evals")]
...
pub struct BaseClient {
    ...
    ambient_workload_token: Arc<Mutex<Option<rook_isolation_platform::WorkloadToken>>>,
...
impl BaseClient {
    pub fn new(
        ...
        if let Some(eval_user_id) = eval_user_id {
            ...
            // inserts in rook-server at eval startup:
    ...
    pub async fn get_or_create_ambient_workload_token(&self) -> Result<Option<String>> {
        ...
        let workload_token = match rook_isolation_platform::issue_workload_token(Some(
            AMBIENT_WORKLOAD_TOKEN_DURATION,
        ))
        .await
        {
            Ok(token) => token,
            Err(rook_isolation_platform::IsolationPlatformError::NoIsolationPlatformDetected) => {
                return Ok(None);
            }
