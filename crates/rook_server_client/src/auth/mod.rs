...
use rook_graphql::mutations::expire_api_key::{
    ExpireApiKey, ExpireApiKeyResult, ExpireApiKeyVariables,
};
use rook_graphql::mutations::generate_api_key::{
    GenerateApiKey, GenerateApiKeyInput, GenerateApiKeyResult, GenerateApiKeyVariables,
};
use rook_graphql::mutations::mint_custom_token::{MintCustomTokenResult, MintCustomTokenVariables};
use rook_graphql::mutations::set_user_is_onboarded::{
    SetUserIsOnboarded, SetUserIsOnboardedResult, SetUserIsOnboardedVariables,
};
use rook_graphql::mutations::update_user_settings::{
    UpdateUserSettings, UpdateUserSettingsInput, UpdateUserSettingsResult,
    UpdateUserSettingsVariables,
};
use rook_graphql::queries::api_keys::{
    ApiKeyProperties, ApiKeyPropertiesResult, ApiKeys, ApiKeysVariables,
};
use rook_graphql::queries::get_user::{GetUser, GetUserVariables, UserOutput as GqlUserOutput};
...
pub const EXPERIMENT_ID_HEADER: &str = "X-Rook-Experiment-Id";
...
pub trait AuthClient: Send + Sync {
    /// Creates an anonymous user who is allowed to use Rook but may lack the ability
    ...
    /// Queries rook-server for a set of the currently logged-in user's fields.
    ...
    async fn create_api_key(
        ...
        expires_at: Option<rook_graphql::scalars::Time>,
...
impl AuthClientImpl {
    ...
    async fn update_settings(
        ...
        let operation = UpdateUserSettings::build(UpdateUserSettingsVariables {
            input,
            request_context: rook_graphql::client::get_request_context(),
        });
    ...
    fn on_settings_updated(
        ...
        match result {
            ...
            UpdateUserSettingsResult::UserFacingError(error) => Err(anyhow!(
                rook_graphql::client::get_user_facing_error_message(error)
            )),
...
impl AuthClient for AuthClientImpl {
    async fn create_anonymous_user(
        ...
        let operation = CreateAnonymousUser::build(CreateAnonymousUserVariables {
            input: rook_graphql::mutations::create_anonymous_user::CreateAnonymousUserInput {
                anonymous_user_type,
                expiration_type: rook_graphql::mutations::create_anonymous_user::AnonymousUserExpirationType::NoExpiration,
                referral_code,
            },
            request_context: rook_graphql::client::get_request_context(),
        });
    ...
    async fn fetch_new_custom_token(&self) -> Result<MintCustomTokenResult> {
        let operation = rook_graphql::mutations::mint_custom_token::MintCustomToken::build(
    ...
    async fn fetch_user_properties<'a>(
        ...
        match response.user {
            rook_graphql::queries::get_user::UserResult::UserOutput(user_output) => Ok(user_output),
            rook_graphql::queries::get_user::UserResult::Unknown => {
    ...
    async fn get_user_settings(&self) -> Result<Option<SyncedUserSettings>> {
        ...
        match response.user {
            rook_graphql::queries::get_user_settings::UserResult::UserOutput(user_output) => {
                Ok(user_output
                    .user
                    .settings
            ...
            rook_graphql::queries::get_user_settings::UserResult::Unknown => {
