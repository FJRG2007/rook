...
use rook_server_auth::credentials::{AuthToken, Credentials, LoginToken};
...
#[test]
fn device_authorization_uses_rook_agent_cli_client() {
    let client = AuthSession::create_oauth_client();

    assert_eq!(client.client_id().as_str(), "rook-agent-cli");
}
