#include "RookDockTilePlugin.h"

@implementation RookDockTilePlugIn {
...
- (instancetype)init {
    ...
    if (self) {
        @try {
            ...
            NSString *timestamp = [formatter stringFromDate:[NSDate date]];
            NSString *logPath = [NSString stringWithFormat:@"/tmp/rook_docktile_%@.log", timestamp];
            NSError *error = nil;
            [[NSFileManager defaultManager] createFileAtPath:logPath contents:nil attributes:nil];
            _logFileHandle = [NSFileHandle fileHandleForWritingAtPath:logPath];
            [self logMessage:@"RookDockTilePlugin initialized"];
...
- (NSString*)convertAppIconNameToFileName:(NSString*)appIconName isDev:(BOOL)isDev isLocal:(BOOL)isLocal isPreview:(BOOL)isPreview {
    ...
    return fileName ?: isLocal ? @"local" : isDev ? @"dev" : isPreview ? @"preview" : @"rook_2";
}
...
// Protocol method that is invoked by the system when the dock for Rook is updated.
...
- (void)setDockTile:(NSDockTile *)dockTile {
    @try {
        ...
        if (dockTile) {
            ...
            NSBundle *pluginBundle = [NSBundle bundleForClass:[RookDockTilePlugIn class]];    
            NSString *path = [[pluginBundle bundlePath] stringByAppendingPathComponent:@"Contents/Info.plist"];
            NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];    
            NSString *bundleId = dict[@"MainAppBundleIdentifier"];
...
- (void)dealloc {
    @try {
        [self logMessage:@"RookDockTilePlugin deallocating"];
