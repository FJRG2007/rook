...
if ($PSEdition -eq 'Desktop' -or $IsWindows) {
    ...
    if ((Get-ExecutionPolicy -Scope MachinePolicy) -eq $EP::Restricted -or (Get-ExecutionPolicy -Scope UserPolicy) -eq $EP::Restricted) {
        Write-Error 'ExecutionPolicy is Restricted. Unable to Rookify this PowerShell session.'
        ...
        $global:_rook_PSProcessExecPolicy = $(Get-ExecutionPolicy -Scope Process)
...
function prompt {
    ...
    $function:global:prompt = $global:_rookOriginalPrompt
    $username = [Environment]::UserName
    $global:_rookSessionId = [uint64]@@ROOK_SESSION_ID@@
    $msg = ConvertTo-Json -Compress -InputObject @{ hook = 'InitShell'; value = @{ session_id = $_rookSessionId; shell = 'pwsh'; user = $username; hostname = [System.Net.Dns]::GetHostName() } }
