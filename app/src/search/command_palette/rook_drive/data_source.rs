...
/// The Rook Drive spaces a window can see: its team, plus the user's personal and
...
/// Datasource that searches the Rook Drive objects visible in a single window.
pub struct DataSource {
    searcher: Box<dyn RookDriveSearcher>,
    scope: WindowScope,
}
...
impl DataSource {
    ...
    pub fn new(window_id: WindowId, ctx: &mut ModelContext<Self>) -> Self {
        if rook_core::features::FeatureFlag::UseTantivySearch.is_enabled() {
    ...
    pub fn new_fuzzy(window_id: WindowId, ctx: &mut ModelContext<Self>) -> Self {
        Self::new_with_searcher(window_id, Box::new(FuzzyRookDriveSearcher::default()), ctx)
    }

    #[cfg(not(target_family = "wasm"))]
    fn new_full_text(window_id: WindowId, ctx: &mut ModelContext<Self>) -> Self {
        let searcher = Box::new(full_text_searcher::FullTextRookDriveSearcher::new(
    ...
    fn rebuild_index(&mut self, ctx: &mut ModelContext<Self>) {
        self.searcher
            ...
            .unwrap_or_else(|err| {
                report_error!(err.context("Error refreshing Rook Drive search index"));
            });
    }
...
impl RookDriveSearcher for FuzzyRookDriveSearcher {
    fn insert_searchable_object(
        &mut self,
        object: &dyn CloudObject,
        object_type: ObjectType,
...
mod full_text_searcher {
    ...
    use rook_search_core::define_search_schema;
    ...
    use crate::search::command_palette::rook_drive::data_source::{RookDriveSearcher, WindowScope};
    use crate::search::command_palette::rook_drive::env_var_collection_search_item::{
        ENV_VAR_NAME_SEPARATOR, EnvVarCollectionSearchItem,
    };
    use crate::search::command_palette::rook_drive::notebook_search_item::NotebookSearchItem;
    use crate::search::command_palette::rook_drive::workflow_search_item::WorkflowSearchItem;
    use crate::search::env_var_collections::fuzzy_match::FuzzyMatchEnvVarCollectionResult;
    use crate::search::notebooks::fuzzy_match::FuzzyMatchNotebookResult;
    ...
    /// Memory budget for the search index of rook drive.
    /// Rook could potentially have a lot of objects, so we increase it from the default of 50MB to 100MB
    const MEMORY_BUDGET: usize = 100_000_000; // TODO: is 100MB really necessary?

    // All Rook Drive objects are boosted due to multiple fields being a part of the same total score,
    ...
    pub(crate) struct FullTextRookDriveSearcher {
    ...
    impl FullTextRookDriveSearcher {
    ...
    impl RookDriveSearcher for FullTextRookDriveSearcher {
        fn insert_searchable_object(
            &mut self,
            object: &dyn CloudObject,
            object_type: ObjectType,
