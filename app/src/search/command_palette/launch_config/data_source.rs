...
impl DataSource {
    ...
    fn new_fuzzy(ctx: &mut ModelContext<Self>) -> Self {
        ctx.subscribe_to_model(&RookConfig::handle(ctx), Self::handle_config_event);
    ...
    fn handle_config_event(
        &mut self,
        _: ModelHandle<RookConfig>,
        event: &RookConfigUpdateEvent,
        ctx: &mut ModelContext<Self>,
    ) {
        if matches!(event, RookConfigUpdateEvent::LaunchConfigs) {
...
impl LaunchConfigSearcher for FuzzyLaunchConfigSearcher {
    ...
    fn refresh_search_index(&mut self, app: &AppContext) {
        self.configs = RookConfig::as_ref(app)
            .launch_configs()
            .iter()
