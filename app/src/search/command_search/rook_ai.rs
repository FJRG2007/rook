...
const OPEN_ROOK_AI_ITEM_BODY_TEXT: &str = "Ask Rook AI for command suggestions";
const TRANSLATE_WITH_ROOK_AI_ITEM_BODY_TEXT: &str = "Translate into shell command using Rook AI";

#[derive(Clone, Debug)]
pub enum RookAISearchItem {
    /// Translates the query within command search.
    Translate,

    /// Opens RookAI with the query.
    Open,
}

impl RookAISearchItem {
    fn item_body_text(&self) -> &'static str {
        match self {
            RookAISearchItem::Translate => TRANSLATE_WITH_ROOK_AI_ITEM_BODY_TEXT,
            RookAISearchItem::Open => OPEN_ROOK_AI_ITEM_BODY_TEXT,
        }
    }
}

impl SearchItem for RookAISearchItem {
    ...
    fn render_icon(
        ...
        // Since the Rook AI logo color is hardcoded, let's find the best
        ...
        let icon = if FeatureFlag::AgentMode.is_enabled() {
            ...
            UIIcon::AiAssistant.to_rookui_icon(color.into()).finish()
        };
    ...
    fn accept_result(&self) -> CommandSearchItemAction {
        match self {
            RookAISearchItem::Translate => CommandSearchItemAction::TranslateUsingRookAI,
            RookAISearchItem::Open => CommandSearchItemAction::OpenRookAI,
        }
    }
    ...
    fn accessibility_label(&self) -> String {
        format!("Rook AI: {}", self.item_body_text())
    }
}

/// The Rook AI data source provides two different types of results:
/// - synchronous: the synchronous result provided by this data source is a
///   single item that opens/translates using Rook AI when selected.
...
impl SyncDataSource for RookAIDataSource {
    type Action = CommandSearchItemAction;
    ...
    fn run_query(
        ...
        if query.filters.is_empty() {
            Ok(vec![RookAISearchItem::Translate.into()])
        } else {
            // Since the query matched, the `#` filter must be applied in this case.
            Ok(vec![RookAISearchItem::Open.into()])
        }
    }
}
...
impl AsyncDataSource for RookAIDataSource {
    type Action = CommandSearchItemAction;
