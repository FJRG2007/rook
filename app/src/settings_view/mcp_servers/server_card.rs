...
use rook_core::ui::icons::{ICON_DIMENSIONS, Icon};
use rook_core::ui::theme::AnsiColorIdentifier;
use rook_core::ui::theme::color::internal_colors;
use rook_errors::report_error;
use rookui::accessibility::ActionAccessibilityContent;
use rookui::elements::{
...
impl ServerCardView {
        ...
        Hoverable::new(toggle_mouse_state, move |_is_hovered| {
            Flex::row()
                ...
                .with_child(
                    Container::new(
                        ConstrainedBox::new(
                            chevron_icon.to_rookui_icon(text_color.into()).finish(),
                        )
    ...
    fn render_title_chip(chip: &TitleChip, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let inner: Box<dyn Element> = if let Some(icon) = chip.leading_icon {
            Flex::row()
                ...
                .with_spacing(2.)
                .with_child(
                    ConstrainedBox::new(icon.to_rookui_icon(chip_color.into()).finish())
    ...
    fn render_actions_row(&self, state: &MouseState, appearance: &Appearance) -> Box<dyn Element> {
        ...
        if self.render_options.show_add_icon {
            let add_icon = rookui::elements::Icon::new(
                Icon::Plus.into(),
