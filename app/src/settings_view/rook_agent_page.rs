//! The "Rook Agent" settings page, shown under the Agents umbrella.
//!
//! Covers Rook's own AI: the global toggle, Active AI suggestions, agent
...
use crate::settings::{
    ...
    AwsBedrockCredentialsEnabled, CanUseRookCreditsForFallback, EnableAiCommandSearchHashTrigger,
...
const CUSTOM_INFERENCE_LEARN_MORE_URL: &str =
    "https://docs.rook.dev/agents/inference/custom-inference-endpoint/";
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "AI",
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleGlobalAI,
                )),
                context,
                flags::IS_ANY_AI_ENABLED,
            )
            .with_group(bindings::BindingGroup::RookAi),
        ],
        app,
    );
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "Active AI",
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleActiveAI,
                )),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleAIInputAutoDetection,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleNLDInTerminal,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleAiCommandSearchHashTrigger,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "Next Command",
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleIntelligentAutosuggestions,
                )),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "prompt suggestions",
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::TogglePromptSuggestions,
                )),
                &(context.clone() & id!(flags::IS_ACTIVE_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "code suggestions",
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleCodeSuggestions,
                )),
                &(context.clone()
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::custom(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleShowAgentTips,
                )),
        ...
        vec![
            ToggleSettingActionPair::custom(
                SettingActionPairDescriptions::new(
                    "Show Rook Agent changelog in new agent conversation view",
                    "Hide Rook Agent changelog in new agent conversation view",
                ),
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleShowOzUpdatesInZeroState,
                )),
                SettingActionPairContexts::new(
                    context.clone()
                        & id!(flags::IS_ANY_AI_ENABLED)
        ...
        let mode_bindings: Vec<FixedBinding> = ThinkingDisplayMode::iter()
            .map(|mode| {
                ...
                FixedBinding::empty(
                    ...
                    builder(SettingsAction::RookAgent(
                        RookAgentPageAction::SetThinkingDisplayMode(mode),
                    )),
                    ai_context.clone() & !id!(context_flag),
                )
                .with_group(bindings::BindingGroup::RookAi.as_str())
            })
            .collect();
        ...
        let mode_bindings: Vec<FixedBinding> = OrchestrationMessageDisplayMode::iter()
            .map(|mode| {
                ...
                FixedBinding::empty(
                    mode.command_palette_description(),
                    builder(SettingsAction::RookAgent(
                        RookAgentPageAction::SetOrchestrationMessageDisplayMode(mode),
                    )),
                    ai_context.clone() & !id!(context_flag),
                )
                .with_group(bindings::BindingGroup::RookAi.as_str())
        ...
        let mode_bindings: Vec<FixedBinding> = PromptSubmissionMode::iter()
            .map(|mode| {
                ...
                FixedBinding::empty(
                    mode.command_palette_description(),
                    builder(SettingsAction::RookAgent(
                        RookAgentPageAction::SetPromptSubmissionMode(mode),
                    )),
                    ai_context.clone() & !id!(context_flag),
                )
                .with_group(bindings::BindingGroup::RookAi.as_str())
        ...
        let lrc_mode_bindings: Vec<FixedBinding> = LongRunningCommandSubmissionMode::iter()
            .map(|mode| {
                ...
                FixedBinding::empty(
                    mode.command_palette_description(),
                    builder(SettingsAction::RookAgent(
                        RookAgentPageAction::SetLongRunningCommandSubmissionMode(mode),
                    )),
                    ai_context.clone()
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleNaturalLanguageAutosuggestions,
                )),
                &(context.clone() & id!(flags::IS_ACTIVE_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleSharedTitleGeneration,
                )),
                &(context.clone() & id!(flags::IS_ACTIVE_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleGitOperationsAutogen,
                )),
                &(context.clone() & id!(flags::IS_ACTIVE_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "voice input",
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleVoiceInput,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::custom(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleUseAgentToolbar,
                )),
                SettingActionPairContexts::new(
                    context.clone()
                        & id!(flags::IS_ANY_AI_ENABLED)
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleIncludeAgentCommandsInHistory,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
            ...
            ToggleSettingActionPair::custom(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleAutoApproveBypassesCommandDenylist,
                )),
                SettingActionPairContexts::new(
                    context.clone()
                        & id!(flags::IS_ANY_AI_ENABLED)
            ...
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleShowConversationHistory,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                ...
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleFileBasedMcp,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                "Rook credit fallback",
                builder(SettingsAction::RookAgent(
                    RookAgentPageAction::ToggleCanUseRookCreditsForFallback,
                )),
                &(context.clone() & id!(flags::IS_ANY_AI_ENABLED)),
                flags::ROOK_CREDIT_FALLBACK_FLAG,
            )
            .with_group(bindings::BindingGroup::RookAi)
            .is_supported_on_current_platform(
...
fn member_byo_keys_allowed_for_view(ctx: &ViewContext<RookAgentPageView>) -> bool {
...
pub struct RookAgentPageView {
    page: PageType<Self>,
    self_handle: WeakViewHandle<Self>,
    voice_input_toggle_key_dropdown: ViewHandle<Dropdown<RookAgentPageAction>>,
    voice_input_language_dropdown: ViewHandle<FilterableDropdown<RookAgentPageAction>>,
    ...
    thinking_display_mode_dropdown: ViewHandle<Dropdown<RookAgentPageAction>>,
    orchestration_message_display_mode_dropdown: ViewHandle<Dropdown<RookAgentPageAction>>,
    default_prompt_submission_mode_dropdown: ViewHandle<Dropdown<RookAgentPageAction>>,
    lrc_submission_mode_dropdown: ViewHandle<Dropdown<RookAgentPageAction>>,
    #[cfg(feature = "local_fs")]
    conversation_layout_dropdown: ViewHandle<Dropdown<RookAgentPageAction>>,
...
impl RookAgentPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
            ...
            dropdown.add_items(
                values
                    ...
                    .map(|val| {
                        DropdownItem::new(
                            val.display_name(),
                            RookAgentPageAction::SetVoiceInputToggleKey(val),
                        )
                    })
                    .collect(),
                ctx,
        ...
        let voice_input_language_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.add_items(
                VOICE_INPUT_LANGUAGES
                    ...
                    .map(|&(code, name)| {
                        DropdownItem::new(
                            name,
                            RookAgentPageAction::SetVoiceInputLanguage(code.to_string()),
                        )
                    })
                    .collect(),
                ctx,
            ...
            dropdown.set_selected_by_action(
                RookAgentPageAction::SetVoiceInputLanguage(current_code),
                ctx,
            );
        ...
        // Set initial selection based on current setting value.
        {
            ...
            thinking_display_mode_dropdown.update(ctx, |dropdown, ctx| {
                dropdown.set_selected_by_action(
                    RookAgentPageAction::SetThinkingDisplayMode(current_mode),
                    ctx,
                );
            });
        }
            ...
            OtherAIWidget::create_orchestration_message_display_mode_dropdown(ctx);
        {
            ...
            orchestration_message_display_mode_dropdown.update(ctx, |dropdown, ctx| {
                dropdown.set_selected_by_action(
                    RookAgentPageAction::SetOrchestrationMessageDisplayMode(current_mode),
                    ctx,
                );
            });
        }
            ...
            OtherAIWidget::create_default_prompt_submission_mode_dropdown(ctx);
        {
            ...
            default_prompt_submission_mode_dropdown.update(ctx, |dropdown, ctx| {
                dropdown.set_selected_by_action(
                    RookAgentPageAction::SetPromptSubmissionMode(current_mode),
                    ctx,
                );
            });
        }
        ...
        let lrc_submission_mode_dropdown = OtherAIWidget::create_lrc_submission_mode_dropdown(ctx);
        {
            ...
            lrc_submission_mode_dropdown.update(ctx, |dropdown, ctx| {
                dropdown.set_selected_by_action(
                    RookAgentPageAction::SetLongRunningCommandSubmissionMode(current_mode),
                    ctx,
                );
            });
        }
            ...
            match event {
                ...
                AISettingsChangedEvent::OrchestrationMessageDisplayMode { .. } => {
                    ...
                    me.orchestration_message_display_mode_dropdown
                        .update(ctx, |dropdown, ctx| {
                            dropdown.set_selected_by_action(
                                RookAgentPageAction::SetOrchestrationMessageDisplayMode(
                                    current_mode,
                                ),
                                ctx,
                            );
                ...
                AISettingsChangedEvent::LongRunningCommandSubmissionMode { .. } => {
                    ...
                    me.lrc_submission_mode_dropdown
                        .update(ctx, |dropdown, ctx| {
                            dropdown.set_selected_by_action(
                                RookAgentPageAction::SetLongRunningCommandSubmissionMode(
                                    current_mode,
                                ),
                                ctx,
                            );
        ...
        let add_router_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("+ Add router", SecondaryTheme)
                ...
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(RookAgentPageAction::OpenAddCustomRouter);
                })
        });
        #[cfg(feature = "local_fs")]
        {
            let is_enabled = rook_core::features::FeatureFlag::CustomModelRouters.is_enabled()
                && is_any_ai_enabled;
        ...
        let custom_inference_add_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("+ Add custom model", SecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(RookAgentPageAction::OpenAddCustomEndpointModal);
                })
        });
        ...
        let conversation_layout_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            let items = vec![
                DropdownItem::new(
                    "New Tab",
                    RookAgentPageAction::SetConversationLayout(OpenConversationPreference::NewTab),
                ),
                DropdownItem::new(
                    "Split Pane",
                    RookAgentPageAction::SetConversationLayout(
        ...
        // Subscribe to RookConfig to refresh router views when files change.
        #[cfg(feature = "local_fs")]
        ctx.subscribe_to_model(
            &crate::user_config::RookConfig::handle(ctx),
            |me, _, event, ctx| {
                use crate::user_config::RookConfigUpdateEvent;
                if matches!(event, RookConfigUpdateEvent::ModelConfigs) {
    ...
    fn handle_set_default_model_modal_event(
        ...
        match event {
            ...
            SetDefaultModelModalBodyEvent::SetDefault(id) => {
                // Mirror `RookAgentPageAction::SetBaseModel`: set the active
    ...
    fn hide_set_default_model_modal(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.emit(RookAgentPageEvent::HideModal);
        ctx.notify();
    }
    ...
    fn show_set_default_model_modal(
        ...
        ctx.emit(RookAgentPageEvent::ShowModal);
        ctx.notify();
    }
    ...
    /// they always consume Rook credits.
    ...
    fn should_offer_default_model_switch(ctx: &ViewContext<Self>) -> bool {
        ...
        // filter anyone who can still run Rook-hosted models. (A strict
    ...
    fn maybe_prompt_set_default_model_for_provider(
        ...
        let description = format!(
            "You added your own {provider_name} API key, but your default model is currently set \
             to {current_default}, which won't work without Rook credits. Would you like to change \
             your default model?"
        );
    ...
    fn maybe_prompt_set_default_model_for_custom_endpoint(
        ...
        let description = format!(
            "You added the \"{}\" custom endpoint, but your default model is currently set to \
             {current_default}, which won't work without Rook credits. Would you like to change \
             your default model?",
            endpoint.name
        );
        self.show_set_default_model_modal(description, choices, ctx);
                    ...
                    ActionButton::new("Edit", SecondaryTheme)
                        ...
                        .on_click(move |ctx| {
                            ctx.dispatch_typed_action(
                                RookAgentPageAction::OpenEditCustomEndpointModal(index),
                            );
                        })
                });
                button.update(ctx, |button, ctx| {
    ...
    fn build_page(ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        PageType::new_categorized(
            categories,
            Some(PageTitle::new("Rook Agent").with_trailing_element(
    ...
    fn handle_detection_denylist_editor_event(
        ...
        match event {
            ...
            EditorEvent::Escape => ctx.emit(RookAgentPageEvent::FocusModal),
            _ => {}
        }
    }
    ...
    fn create_router_views(
        ...
        if !rook_core::features::FeatureFlag::CustomModelRouters.is_enabled() {
            return Vec::new();
        }
        let routers: Vec<crate::ai::custom_model_routers::CustomModelRouter> =
            RookConfig::as_ref(ctx).custom_model_routers().clone();
        routers
            .into_iter()
            .map(|router| {
                ...
                ctx.subscribe_to_view(&view, move |me, _, event, ctx| match event {
                    CustomRouterViewEvent::OpenFile(path) => {
                        ctx.emit(RookAgentPageEvent::OpenCustomRouterFile(path.clone()));
                    }
                    CustomRouterViewEvent::Edit => {
                        let r = router_clone.clone();
                        ctx.emit(RookAgentPageEvent::OpenCustomRouterEditor(Some(r)));
                    }
                    CustomRouterViewEvent::Delete => {
                        if let Some(path) = &router_clone.source_path {
                            ...
                            {
                                if let Err(e) =
                                    crate::user_config::RookConfig::delete_custom_model_router(path)
                                {
...
impl View for RookAgentPageView {
    fn ui_name() -> &'static str {
        "RookAgentPage"
    }

    fn render(&self, app: &rookui::AppContext) -> Box<dyn rookui::Element> {
        self.page.render(self, app)
    }
}

#[allow(clippy::large_enum_variant)]
pub enum RookAgentPageEvent {
    FocusModal,
    #[cfg(feature = "local_fs")]
...
impl Entity for RookAgentPageView {
    type Event = RookAgentPageEvent;
}

#[derive(Debug, Clone, PartialEq)]
pub enum RookAgentPageAction {
    OpenUrl(String),
    ...
    ToggleVoiceInput,
    ToggleCanUseRookCreditsForFallback,
...
impl TypedActionView for RookAgentPageView {
    type Action = RookAgentPageAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            RookAgentPageAction::OpenUrl(url) => {
                ctx.open_url(url.as_str());
            }
            RookAgentPageAction::SetVoiceInputToggleKey(key) => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
            ...
            RookAgentPageAction::SetVoiceInputLanguage(language) => {
            ...
            RookAgentPageAction::ToggleCanUseRookCreditsForFallback => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
                            .can_use_rook_credits_for_fallback
                            .toggle_and_save_value(ctx)
                    );
                });
                ctx.notify();
            }
            RookAgentPageAction::HyperlinkClick(hyperlink) => {
                ctx.notify();
                ctx.open_url(&hyperlink.url);
            }
            RookAgentPageAction::ToggleShowInputHintText => {
            ...
            RookAgentPageAction::SignupAnonymousUser => {
                ctx.emit(RookAgentPageEvent::SignupAnonymousUser);
            }
            RookAgentPageAction::ToggleAwsBedrockAutoLogin => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.aws_bedrock_auto_login.toggle_and_save_value(ctx));
                });
                ctx.notify();
            }
            RookAgentPageAction::ToggleAwsBedrockCredentialsEnabled => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
            ...
            RookAgentPageAction::RefreshAwsBedrockCredentials => {
                #[cfg(not(target_family = "wasm"))]
            ...
            RookAgentPageAction::RefreshGeminiEnterpriseCredentials => {
                #[cfg(not(target_family = "wasm"))]
                ApiKeyManager::handle(ctx).update(ctx, |manager, ctx| {
            ...
            RookAgentPageAction::ToggleGeminiEnterpriseCredentialsEnabled => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
            ...
            RookAgentPageAction::ToggleCloudAgentComputerUse => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
            ...
            #[cfg(feature = "local_fs")]
            RookAgentPageAction::SetConversationLayout(layout) => {
            ...
            #[cfg(feature = "local_fs")]
            RookAgentPageAction::OpenAddCustomRouter => {
                ctx.emit(RookAgentPageEvent::OpenCustomRouterEditor(None));
            }
            RookAgentPageAction::OpenAddCustomEndpointModal => {
                self.show_add_custom_endpoint_modal(ctx);
            }
            RookAgentPageAction::OpenEditCustomEndpointModal(index) => {
                self.show_edit_custom_endpoint_modal(*index, ctx);
            }
            RookAgentPageAction::ToggleCloudHandoff => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
            ...
            RookAgentPageAction::ToggleAmpersandHandoff => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
            ...
            RookAgentPageAction::ToggleAutoHandoffOnSleep => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(
                        settings
            ...
            RookAgentPageAction::ToggleAgentAttribution => {
                // The updated value syncs to rook-server automatically via
            ...
            RookAgentPageAction::ConnectGrokSubscription => {
                #[cfg(not(target_family = "wasm"))]
                self.start_grok_oauth(ctx);
            }
            RookAgentPageAction::CancelGrokSubscriptionConnect => {
                #[cfg(not(target_family = "wasm"))]
                self.cancel_grok_oauth(ctx);
            }
            RookAgentPageAction::DisconnectGrokSubscription => {
...
impl SettingsPageMeta for RookAgentPageView {
    fn section() -> SettingsSection {
        SettingsSection::RookAgent
    }
...
impl From<ViewHandle<RookAgentPageView>> for SettingsPageViewHandle {
    fn from(view_handle: ViewHandle<RookAgentPageView>) -> Self {
        SettingsPageViewHandle::RookAgent(view_handle)
    }
}
                    ...
                    Container::new(
                        ui_builder
                            ...
                            .on_click(move |ctx, _, _| {
                                ctx.dispatch_typed_action(RookAgentPageAction::SignupAnonymousUser);
                            })
                            .finish(),
                    )
        ...
        row.add_child(
            Container::new(
                ui_builder
                    ...
                    .build()
                    .on_click(move |ctx, _, _| {
                        ctx.dispatch_typed_action(RookAgentPageAction::ToggleGlobalAI);
                    })
                    .finish(),
            )
            .with_padding_right(TOGGLE_BUTTON_RIGHT_PADDING)
...
fn is_shared_block_title_generation_toggleable(
    view_handle: &WeakViewHandle<RookAgentPageView>,
    app: &AppContext,
) -> bool {
    FeatureFlag::SharedBlockTitleGeneration.is_enabled()
        && AISettings::as_ref(app)
...
impl SharedBlockTitleGenerationWidget {
    fn new(ctx: &ViewContext<RookAgentPageView>) -> Self {
        Self {
...
impl NaturalLanguageDetectionWidget {
    fn render_natural_language_detection_section(
        ...
        autodetection_toggle: SwitchStateHandle,
        nld_in_terminal_toggle: SwitchStateHandle,
        view: &RookAgentPageView,
        ai_settings: &AISettings,
        appearance: &Appearance,
        app: &rookui::AppContext,
    ) -> Box<dyn rookui::Element> {
            ...
            section.add_children([
                ...
                Container::new(
                    ...
                    .register_default_click_handlers(|url, ctx, _| {
                        ctx.dispatch_typed_action(RookAgentPageAction::HyperlinkClick(url));
                    })
                    .finish(),
                )
...
impl VoiceWidget {
    fn render_voice_section(
        ...
        let voice_input_description_text_fragments = vec![
            FormattedTextFragment::plain_text(
                "Voice input allows you to control Rook by speaking directly to your terminal (powered by ",
            ),
...
impl OtherAIWidget {
    fn create_thinking_display_mode_dropdown(
        ctx: &mut ViewContext<RookAgentPageView>,
    ) -> ViewHandle<Dropdown<RookAgentPageAction>> {
        let items: Vec<DropdownItem<RookAgentPageAction>> = ThinkingDisplayMode::iter()
            .map(|mode| {
                DropdownItem::new(
                    mode.display_name(),
                    RookAgentPageAction::SetThinkingDisplayMode(mode),
    ...
    fn create_default_prompt_submission_mode_dropdown(
        ...
        let items: Vec<DropdownItem<RookAgentPageAction>> = PromptSubmissionMode::iter()
            .map(|mode| {
                DropdownItem::new(
                    mode.display_name(),
                    RookAgentPageAction::SetPromptSubmissionMode(mode),
    ...
    fn create_lrc_submission_mode_dropdown(
        ...
        let items: Vec<DropdownItem<RookAgentPageAction>> =
    ...
    fn create_orchestration_message_display_mode_dropdown(
        ...
        let items: Vec<DropdownItem<RookAgentPageAction>> = OrchestrationMessageDisplayMode::iter()
            .map(|mode| {
                DropdownItem::new(
                    mode.display_name(),
                    RookAgentPageAction::SetOrchestrationMessageDisplayMode(mode),
...
impl SettingsWidget for ShowOzUpdatesInZeroStateWidget {
    ...
    fn render(
        ...
        render_ai_setting_toggle::<ShouldShowOzUpdatesInZeroState>(
            "Show Rook Agent changelog in new conversation view",
            RookAgentPageAction::ToggleShowOzUpdatesInZeroState,
...
impl SettingsWidget for AgentAttributionWidget {
    ...
    fn search_terms(&self) -> &str {
        "agent attribution commit pull request co-author author credit oz rook"
    }
    ...
    fn render(
        ...
        let toggle = if state.is_forced_by_org {
            ...
            ui_builder
                ...
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(RookAgentPageAction::ToggleAgentAttribution);
                })
                .finish()
        };

        let toggle_row = build_toggle_element(
            render_body_item_label::<RookAgentPageAction>(
        ...
        Flex::column()
            ...
            .with_child(render_ai_setting_description(
                "Rook Agent can add attribution to commit messages and pull requests it creates",
                !state.is_disabled,
                app,
            ))
            .finish()
    ...
    fn render(
        ...
        let toggle = if is_forced_by_org {
            ...
            ui_builder
                ...
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(RookAgentPageAction::ToggleCloudAgentComputerUse);
                })
                .finish()
        };
        ...
        Flex::column()
            .with_child(toggle_row)
            .with_child(render_ai_setting_description(
                "Enable computer use in cloud agent conversations started from the Rook app.",
                !is_disabled,
                app,
            ))
            .finish()
    ...
    fn render(
        ...
        let handoff_toggle = if is_force_disabled {
            ...
            ui_builder
                ...
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(RookAgentPageAction::ToggleCloudHandoff);
                })
                .finish()
        };
...
impl SettingsWidget for AutoHandoffOnSleepWidget {
    ...
    fn render(
        ...
        let auto_handoff_on_sleep_toggle = ui_builder
            ...
            .build()
            .on_click(move |ctx, _, _| {
                ctx.dispatch_typed_action(RookAgentPageAction::ToggleAutoHandoffOnSleep);
            })
            .finish();
        ...
        Flex::column()
            ...
            .with_child(render_ai_setting_description(
                "When macOS is about to sleep, automatically moves the most recently focused running local Rook Agent conversation to Cloud Mode so it can keep working.",
                true,
                app,
            ))
            .finish()
...
impl SettingsWidget for AmpersandHandoffWidget {
    ...
    fn render(
        ...
        let ampersand_toggle = ui_builder
            ...
            .build()
            .on_click(move |ctx, _, _| {
                ctx.dispatch_typed_action(RookAgentPageAction::ToggleAmpersandHandoff);
            })
            .finish();
...
struct ApiKeysWidget {
    ...
    can_use_rook_credits_for_fallback: SwitchStateHandle,
...
impl ApiKeysWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        let grok_connect_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Connect", SecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(RookAgentPageAction::ConnectGrokSubscription);
                })
        });
        let grok_cancel_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Cancel", SecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(RookAgentPageAction::CancelGrokSubscriptionConnect);
                })
        });
        ...
        let grok_disconnect_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Disconnect", DangerSecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(RookAgentPageAction::DisconnectGrokSubscription);
                })
        });
        ...
        Self {
            ...
            can_use_rook_credits_for_fallback: Default::default(),
    ...
    fn render_team_key_info_icon(
        ...
        Hoverable::new(mouse_state, move |state| {
            let icon = ConstrainedBox::new(Icon::Info.to_rookui_icon(icon_color).finish())
                .with_width(13.)
                .with_height(13.)
                .finish();
    ...
    fn render_custom_inference_description(
        ...
        if show_provider_keys {
            add_paragraph(vec![FormattedTextFragment::plain_text(
                "Use your own API keys from model providers for Rook Agent. API keys are used to make requests to your chosen model provider. Using auto models or models you do not have available API keys for will consume Rook credits.",
            )]);
        }
        ...
        if show_provider_keys || show_custom_endpoints {
            add_paragraph(vec![FormattedTextFragment::plain_text(
                "API keys added here are stored only on this device, not on Rook's servers.",
            )]);
    ...
    fn render_grok_subscription_row(
        ...
        let label = Flex::row()
            ...
            .with_child(
                ConstrainedBox::new(Icon::XLogo.to_rookui_icon(text_color).finish())
                    .with_width(14.)
                    .with_height(14.)
                    .finish(),
            )
        ...
        let description = Container::new(
            Text::new(
                "Connect your SuperGrok subscription to use Grok models in the Rook Agent through your xAI account.",
                appearance.ui_font_family(),
                CONTENT_FONT_SIZE,
            )
        ...
        if let Some(tokens) = grok_tokens {
            ...
            let check = ConstrainedBox::new(
                Icon::Check
                    .to_rookui_icon(appearance.theme().ansi_fg_green().into())
                    .finish(),
            )
            .with_width(12.)
            .with_height(12.)
    ...
    fn render_rook_credit_fallback_toggle(
        &self,
        view: &RookAgentPageView,
        app: &AppContext,
    ) -> Box<dyn Element> {
        ...
        let toggle = render_ai_setting_toggle::<CanUseRookCreditsForFallback>(
            "Rook credit fallback",
            RookAgentPageAction::ToggleCanUseRookCreditsForFallback,
            *ai_settings.can_use_rook_credits_for_fallback,
            ai_settings.is_any_ai_enabled(app),
            self.can_use_rook_credits_for_fallback.clone(),
            &view.local_only_icon_tooltip_states,
            app,
        );

        let description = render_ai_setting_description(
            "When enabled, agent requests may be routed to one of Rook's provided models in the event of an error. Rook will prioritize using your API keys over your Rook credits.",
            ai_settings.is_any_ai_enabled(app),
            app,
        );
...
impl SettingsWidget for ApiKeysWidget {
    ...
    fn render(
        ...
        // Rook credit fallback applies to member-provided API keys, not custom endpoints.
        if is_byo_enabled && show_provider_keys {
            column.add_child(
                Container::new(self.render_rook_credit_fallback_toggle(view, app))
                    .with_margin_top(16.)
                    .finish(),
            );
        }
        ...
        if !is_byo_enabled && show_provider_keys {
                ...
                UserWorkspaces::as_ref(app).team_for_view_handle(&self.view_handle, app)
            {
                if team.billing_metadata.customer_type == CustomerType::Enterprise {
                    vec![
                        FormattedTextFragment::hyperlink("Contact sales", "mailto:sales@rook.dev"),
                        FormattedTextFragment::plain_text(
            ...
            .register_default_click_handlers_with_action_support(|hyperlink_lens, event, ctx| {
                match hyperlink_lens {
                    ...
                    HyperlinkLens::Action(action_ref) => {
                        if let Some(action) =
                            action_ref.as_any().downcast_ref::<RookAgentPageAction>()
                        {
...
struct AwsBedrockWidget {
    self_handle: WeakViewHandle<RookAgentPageView>,
...
impl AwsBedrockWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        let refresh_credentials_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Refresh", SecondaryTheme)
                ...
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(RookAgentPageAction::RefreshAwsBedrockCredentials);
                })
        });
    ...
    fn render_aws_bedrock_section(
        ...
        let toggle_description = if is_admin_enforced {
            "Rook loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.".to_string()
        } else {
            "Rook loads and sends local AWS CLI credentials for Bedrock-supported models."
                .to_string()
        };
        ...
        fn render_credential_status_card(
            ...
            let icon = Container::new(
                ConstrainedBox::new(icon.to_rookui_icon(title_color).finish())
                    .with_width(16.)
                    .with_height(16.)
                    .finish(),
            )
...
impl GeminiEnterpriseWidget {
    ...
    fn render_gemini_enterprise_section(
        ...
        let toggle_description = if is_admin_enforced {
            "Rook routes eligible requests through your workspace's Gemini Enterprise Google Cloud \
...
/// exposed for the `rook://settings?widget=custom_router` deeplink (see
...
impl SettingsWidget for CustomModelRoutersWidget {
    ...
    fn render(
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            let errors = RookConfig::as_ref(app).custom_model_router_errors();
            ...
            for view_handle in &view.router_views {
                column.add_child(
                    Container::new(rookui::elements::ChildView::new(view_handle).finish())
                        .with_margin_top(8.)
                        .finish(),
                );
            }
