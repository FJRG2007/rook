...
/// Download the Inno Setup install wizard, the same one users run on the first Rook install, and
...
const UPDATE_LOG_FILENAME: &str = "rook_update.log";

fn autoupdate_log_file() -> Result<PathBuf> {
    rook_logging::log_directory().map(|dir| dir.join(UPDATE_LOG_FILENAME))
}
...
pub(super) fn check_and_report_update_errors(ctx: &mut AppContext) {
    ...
    let has_mutex_timeout =
        memchr::memmem::find(&contents_lowercase, b"rook mutex still held after timeout").is_some();
...
pub(super) fn relaunch() -> Result<()> {
    ...
    // The Inno Setup install wizard will run without user input. It will re-launch Rook after
    ...
    Command::new(&installer_path)
        .args([
            ...
            &log_arg,
            "/update=1",
            // Do not forcibly kill Rook via RestartManager. The installer will wait for
            // Rook to exit naturally by polling the single-instance mutex instead.
            "/NOCLOSEAPPLICATIONS",
