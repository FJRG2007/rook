...
) -> rook_graphql::object_actions::ActionType {
    match action {
        ObjectActionType::Execute => rook_graphql::object_actions::ActionType::Executed,
    }
}

/// Converts the graphql action type ("EXECUTED", etc) to ObjectActionType.
fn try_into_object_action_type(
    action_type: rook_graphql::object_actions::ActionType,
) -> Result<ObjectActionType, anyhow::Error> {
    match action_type {
        rook_graphql::object_actions::ActionType::Executed => Ok(ObjectActionType::Execute),
    }
}

/// Converts the graphql action entry (SingleAction, BundledActions) into its ObjectAction corollary.
fn try_into_object_action(
    record: &rook_graphql::object_actions::ActionRecord,
    uid: ObjectUid,
    hashed_sqlite_id: HashedSqliteId,
) -> Result<ObjectAction, anyhow::Error> {
    match record {
        rook_graphql::object_actions::ActionRecord::SingleAction(s) => Ok(ObjectAction {
        ...
        rook_graphql::object_actions::ActionRecord::BundledActions(b) => Ok(ObjectAction {
        ...
        rook_graphql::object_actions::ActionRecord::Unknown => {
...
pub fn object_action_history_from_gql(
    history: rook_graphql::object_actions::ObjectActionHistory,
