...
use rook_server_client::HttpStatusError;
use rook_server_client::auth::{AuthClientImpl, AuthEvent, EXPERIMENT_ID_HEADER};
use rook_server_client::base_client::{
    AmbientHeaderPolicy, AuthenticatedGraphqlConfig, BaseClient, GraphqlRoutingConfig,
    TEAM_UID_HEADER,
};
use rook_server_client::iap::{IapManager, IapState};
...
/// We use a special error code header `X-Rook-Error-Code` to allow the server to send
...
const ROOK_ERROR_CODE_HEADER: &str = "X-Rook-Error-Code";
...
const ROOK_ERROR_CODE_OUT_OF_CREDITS: &str = "OUT_OF_CREDITS";

/// Error code indicating the user has reached their cloud agent concurrency limit.
const ROOK_ERROR_CODE_AT_CAPACITY: &str = "AT_CLOUD_AGENT_CAPACITY";
...
pub enum AIApiError {
    ...
    #[error("Rook is currently overloaded. Please try again later.")]
    ServerOverloaded,
...
impl AIApiError {
    ...
    fn from_response_error(
        ...
        // For HTTP 429 errors, check the X-Rook-Error-Code header to distinguish
    ...
    /// Returns the appropriate error for a 429 response by checking the X-Rook-Error-Code header.
    fn error_for_429(headers: &::http::HeaderMap, body: Option<String>) -> Self {
        if headers
            .get(ROOK_ERROR_CODE_HEADER)
            .and_then(|v| v.to_str().ok())
            == Some(ROOK_ERROR_CODE_OUT_OF_CREDITS)
        {
...
/// An API wrapper struct with methods to requests to rook-server.
///
...
impl ServerApi {
    ...
    pub fn send_graphql_request<'a, QF, O: rook_graphql::client::Operation<QF> + Send + 'a>(
        &'a self,
        operation: O,
    ...
    where
        QF: 'a,
    {
        rook_server_client::graphql_helpers::send_graphql_request(
            &self.base_client,
            operation,
            timeout,
        )
    ...
    fn send_graphql_request_for_team<'a, QF, O: rook_graphql::client::Operation<QF> + Send + 'a>(
        &'a self,
        operation: O,
        ...
        QF: 'a,
    {
        match Self::team_uid_header_value(team_scope) {
            Some(team_uid) => {
                rook_server_client::graphql_helpers::send_team_scoped_graphql_request(
                    &self.base_client,
                    operation,
                    None,
                    team_uid,
            ...
            None => rook_server_client::graphql_helpers::send_graphql_request(
                &self.base_client,
                operation,
                None,
            ),
    ...
    ///
    /// The stream is served by rook-server-rtc (not the main rook-server pool),
    ...
    async fn error_from_response(response: http_client::Response) -> anyhow::Error {
        ...
        let is_at_capacity = response
            ...
            .get(ROOK_ERROR_CODE_HEADER)
            .and_then(|v| v.to_str().ok())
            == Some(ROOK_ERROR_CODE_AT_CAPACITY);
    ...
    /// Fetches updated Rook Channel Versions from Rook Server. If it is the first such request of
    ...
    pub async fn fetch_channel_versions(
        ...
        if include_changelogs {
            log::info!("Fetching channel versions and changelogs from Rook server");
        } else {
            log::info!("Fetching channel versions (without changelogs) from Rook server");
        }
        ...
        log::info!("Received channel versions from Rook server: {versions}");
        Ok(versions)
    }
}
...
impl ServerApiProvider {
    ...
    /// and includes standard Rook request headers.
