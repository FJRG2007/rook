...
use rookui::{Entity, ModelContext, ModelHandle, RequestState, SingletonEntity};
...
pub struct Listener {
    ...
    /// so that the user gets a snappier experience once they start using Rook Drive.
...
impl Listener {
        ...
        match event {
            SystemStatsEvent::CpuWasAwakened => {
                ...
                if self.should_subscribe_to_updates {
                    self.get_rook_drive_updates(ctx);
                }
            }
    ...
    fn get_rook_drive_updates(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // TODO: we should investigate having get_rook_drive_updates (and in turn,
