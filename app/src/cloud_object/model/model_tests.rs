...
fn test_object_editor_timeout() {
    App::test((), |mut app| async move {
        ...
        CloudModel::handle(&app).update(&mut app, |model, _ctx| {
            ...
            notebook.metadata.current_editor_uid = Some("ian@rook.dev".to_string());
        });
...
/// way, the folders containing the updated object move to the top of the Rook Drive index if it's
