...
impl CloudModel {
    ...
    /// Number of cloud objects that have errored in some way and are visible in the Rook Drive index
    pub fn num_visible_errored_objects(&self) -> usize {
        self.objects_by_id
            .values()
            .filter(|object| object.renders_in_rook_drive() && object.metadata().is_errored())
            .count()
    }
    ...
    pub fn current_revision(&self, id: &SyncId) -> Option<&Revision> {
        self.objects_by_id
            .get(&id.uid())
            .and_then(|rook_cloud_object| rook_cloud_object.metadata().revision.as_ref())
    }
