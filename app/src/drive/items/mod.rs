...
use super::cloud_object_styling::rook_drive_icon_color;
use super::index::{DriveIndexAction, DriveIndexSection, rook_drive_section_header_position_id};
...
pub trait RookDriveItem {
    ...
    fn icon(&self, appearance: &Appearance, color: Option<Fill>) -> Option<Box<dyn Element>> {
        ...
        let icon_fill = color.unwrap_or(rook_drive_icon_color(appearance, object_type).into());
        Some(Icon::from(object_type).to_rookui_icon(icon_fill).finish())
    }
...
impl RookDriveItemId {
    pub fn drive_row_position_id(&self) -> String {
        match self {
            ...
            Self::Space(space) => {
                rook_drive_section_header_position_id(&DriveIndexSection::Space(*space))
            }
            Self::Trash => "Trash".to_string(),
        }
    }
}
/// This uniquely identifies an item in Rook Drive index
/// Includes spaces (which CloudObjectTypeAndId does not entail)
#[derive(Debug, Clone, PartialEq, Eq, Copy)]
pub enum RookDriveItemId {
    AIFactCollection,
    MCPServerCollection,
...
impl Clone for Box<dyn RookDriveItem> {
    fn clone(&self) -> Self {
        self.clone_box()
    }
}
