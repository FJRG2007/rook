...
fn web_logout_url_rejects_unsafe_device_auth_continuations() {
    let server_root_url = ChannelState::server_root_url();
    for continue_url in [
        "https://example.com/device?user_code=ABCD-EFGH&source=rook-agent-cli".to_owned(),
        format!(
            "{}/login?user_code=ABCD-EFGH&source=rook-agent-cli",
            server_root_url.trim_end_matches('/')
        ),
        format!(
            "{}/device?source=rook-agent-cli",
            server_root_url.trim_end_matches('/')
        ),
        format!(
...
fn web_logout_url_encodes_device_auth_continuation() {
    let continue_url = format!(
        "{}/device?user_code=ABCD-EFGH&source=rook-agent-cli",
