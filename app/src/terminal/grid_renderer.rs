...
use rookui::elements::{Border, CornerRadius, DEFAULT_UI_LINE_HEIGHT_RATIO, Fill, Radius};
use rookui::fonts::{FamilyId, FontId, Properties, Style, Weight};
use rookui::geometry::rect::RectF;
use rookui::geometry::vector::{Vector2F, vec2f};
use rookui::image_cache::{AnimatedImageBehavior, CacheOption, FitType, Image, ImageCache};
use rookui::platform::LineStyle;
...
fn render_grid_without_ligatures<'a>(
    ...
    if FeatureFlag::ITermImages.is_enabled() {
        ...
        if !foreground_image_ids.is_empty() {
            ctx.scene.start_layer(rookui::ClipBounds::ActiveLayer);
    ...
    for (offset, row_idx) in visible_rows.enumerate() {
        ...
        for col in 0..grid.columns() {
            ...
            // When Rook draws the cursor (SHOW_CURSOR on), we keep the cell
