...
use crate::terminal::view::{RememberForRookification, TerminalAction};
...
pub struct RookifyBannerState {
    ...
    /// This keybinding gets rendered in the Rookification banner, but we can't look it up
    ...
    pub initialize_rookify_keybinding: Option<Keystroke>,
    pub hover_state: MouseStateHandle,
}

impl RookifyBannerState {
    pub fn new(command: String, initialize_rookify_keybinding: Option<Keystroke>) -> Self {
        Self {
            command,
            height: 0.0,
            initialize_rookify_keybinding,
    ...
    fn remember_for_rookification(&self, should_remember: bool) -> RememberForRookification {
        if should_remember {
            RememberForRookification::RememberSubshellCommand(self.command.to_owned())
        } else {
            RememberForRookification::DoNotRememberSubshellCommand
        }
    }
}
...
pub fn render_rookification_banner(
    state: &RookifyBannerState,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let yes_button = render_yes_button(
        state,
        &state.initialize_rookify_keybinding,
        &state.accept_button_mouse_state,
        appearance,
    );

    let remember = state.remember_for_rookification(true);
    let dont_ask_button = Container::new(
        appearance
            ...
            .on_click(move |ctx, _, _| {
                ctx.dispatch_typed_action(TerminalAction::DismissRookifyBanner(
                    remember.to_owned(),
                ));
            })
            .finish(),
    ...
    let do_not_remember = state.remember_for_rookification(false);
...
fn render_yes_button(
    state: &RookifyBannerState,
    initialize_rookification_keybinding: &Option<Keystroke>,
    mouse_state: &MouseStateHandle,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let yes_button = match initialize_rookification_keybinding {
