...
use rookui::prelude::{ChildView, Container};
...
impl TerminalView {
    ...
    fn maybe_render_header_back_button(&self, app: &AppContext) -> Box<dyn Element> {
        if !FeatureFlag::AgentView.is_enabled() || rookui::platform::is_mobile_device() {
        ...
        let pane_indicator = if should_render_ambient_agent_indicator {
            ...
            }) = shared_session.kind().as_viewer()
            {
                ...
                Some(
                    ConstrainedBox::new(
                        icons::Icon::Sharing
                            .to_rookui_icon(shared_session_indicator_color(appearance).into())
                            .finish(),
                    )
