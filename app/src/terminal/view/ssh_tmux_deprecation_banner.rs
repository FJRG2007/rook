...
//! tmux-based SSH rookification flow. It explains that tmux SSH rookification has been turned
//! off in favor of Rook's SSH extension (remote server) and links to the docs.
//!
...
use crate::terminal::rookify::render::SSH_DOCS_URL;
use crate::ui_components::icons::Icon;

const BANNER_TITLE: &str = "Tmux SSH rookification has been deprecated";

const BANNER_BODY: &str = "Rook now connects to remote sessions using the SSH extension, which is \
...
impl View for SshTmuxDeprecationBanner {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Rook icon to match the other rookification blocks.
        let icon = Container::new(
            ConstrainedBox::new(Icon::Rook.to_rookui_icon(fg_color.into()).finish())
                .with_width(16.)
                .with_height(16.)
                .finish(),
        )
        ...
        let close = Hoverable::new(self.close_mouse_state.clone(), move |_| {
            ConstrainedBox::new(Icon::X.to_rookui_icon(close_icon_color.into()).finish())
                .with_width(16.)
                .with_height(16.)
                .finish()
        })
