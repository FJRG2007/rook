...
use super::{OpenablePath, check_openable_in_rook};
...
fn test_single_markdown_file() {
    let result = async_io::block_on(check_openable_in_rook(
...
fn test_any_text_file_type_supported() {
    ...
    // .svg is a non-markdown file, and we allow opening it in the Rook Editor (as a "code" file).
    assert_eq!(
        result,
        Some(OpenablePath {
