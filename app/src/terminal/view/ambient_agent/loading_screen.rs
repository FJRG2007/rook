...
use rookui::prelude::{CornerRadius, Radius};
...
use rookui::{AppContext, Entity, ModelHandle, SingletonEntity, WeakViewHandle};
...
/// Renders the cloud mode loading screen with shimmering rook logo and tips.
pub fn render_cloud_mode_loading_screen<T: Entity>(
    ...
    // Create the shimmering rook loading text element
    let shimmer_element =
        shimmering_rook_loading_text(message, loading_font_size, shimmer_handle.clone(), app);
...
fn render_tier_limits_footer<T: Entity>(
    ...
    let info_icon = ConstrainedBox::new(
        Icon::Info
            .to_rookui_icon(blended_colors::text_sub(theme, theme.surface_1()).into())
            .finish(),
    )
...
pub fn render_cloud_mode_error_screen(
    ...
    let error_background = rook_core::ui::color::coloru_with_opacity(error_color.into(), 10);
...
pub fn render_cloud_mode_github_auth_required_screen(
    ...
    let auth_icon = ConstrainedBox::new(Icon::Info.to_rookui_icon(accent_color).finish())
        .with_width(ERROR_ICON_SIZE)
        .with_height(ERROR_ICON_SIZE)
        .finish();
    ...
    let auth_background: rookui::elements::Fill = theme.surface_2().into();
...
pub fn render_cloud_mode_cancelled_screen(appearance: &Appearance) -> Box<dyn Element> {
    ...
    let cancelled_icon = ConstrainedBox::new(
        Icon::SlashCircle
            .to_rookui_icon(title_color.into())
            .finish(),
    )
    .with_width(ERROR_ICON_SIZE)
    .with_height(ERROR_ICON_SIZE)
    ...
    // Dark background (surface_2) with subtle border
    let cancelled_background: rookui::elements::Fill = theme.surface_2().into();
