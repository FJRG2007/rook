...
impl AuthSecretFtuxView {
    ...
    fn render_compact_harness_picker(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let chevron = ConstrainedBox::new(
            UiIcon::ChevronDown
                .to_rookui_icon(internal_colors::text_sub(theme, theme.surface_1()).into())
                .finish(),
        )
        .with_width(12.)
        .with_height(12.)
            ...
            && let Some(menu) = &self.harness_menu
        {
            stack.add_positioned_overlay_child(
                ...
                OffsetPositioning::offset_from_parent(
                    rookui::geometry::vector::vec2f(0., 4.),
    ...
    fn render_description(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let learn_more_label =
            format!("Learn more about authentication for {harness_name} in Rook.");
    ...
    fn render_button(
        ...
        .with_cursor(if disabled {
            rookui::platform::Cursor::Arrow
        } else {
            rookui::platform::Cursor::PointingHand
        })
        .on_click(move |ctx, _, _| {
            if disabled {
                return;
