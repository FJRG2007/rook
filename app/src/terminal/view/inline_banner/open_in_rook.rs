...
use crate::terminal::view::open_in_rook::OpenablePath;
use crate::terminal::view::{InlineBannerId, TerminalAction};
use crate::util::openable_file_type::OpenableFileType;

#[derive(Clone, Copy, Debug)]
pub enum OpenInRookBannerAction {
    OpenFile,
    LearnMore,
    Close,
}

pub struct OpenInRookBannerState {
...
/// Given an openable file, format a file-specific title for the Open in Rook banner.
fn file_title_text(openable_path: &OpenablePath) -> String {
    match openable_path.file_type {
        OpenableFileType::Markdown => {
            "Did you know that Rook can directly display Markdown files?".to_string()
        }
            ...
            cfg_if::cfg_if! {
                if #[cfg(not(target_family = "wasm"))] {
                    ...
                    match language.as_ref().map(|language| language.display_name()) {
                        Some(display_name) => {
                            format!("Did you know that Rook can directly edit {display_name} files?")
                        }
                        None => "Did you know that Rook can directly edit code?".to_string(),
                    }
                } else {
                    // The `languages` crate is not available on WASM, so use a fallback message.
                    "Did you know that Rook can directly edit code?".to_string()
                }
            }
        }
    }
...
pub fn render_open_in_rook_banner(
    state: &OpenInRookBannerState,
    view_id: EntityId,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let button_text = match state.target.file_type {
        OpenableFileType::Markdown => "View in Rook",
        OpenableFileType::Code | OpenableFileType::Text => "Edit in Rook",
    };
    ...
    let open_button = InlineBannerTextButton {
        ...
        button_state: InlineBannerButtonState {
            on_click_event: TerminalAction::OpenInRookBanner(OpenInRookBannerAction::OpenFile),
        ...
        position_id: Some(format!("open_in_rook_banner_button_{view_id}")),
    ...
    let learn_more_button = InlineBannerTextButton {
        ...
        text_color: appearance.theme().active_ui_text_color().into_solid(),
        button_state: InlineBannerButtonState {
            on_click_event: TerminalAction::OpenInRookBanner(OpenInRookBannerAction::LearnMore),
    ...
    let close_button = InlineBannerCloseButton(InlineBannerButtonState {
        on_click_event: TerminalAction::OpenInRookBanner(OpenInRookBannerAction::Close),
