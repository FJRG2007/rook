...
use rookui::{AppContext, Element, EventContext};
...
/// If appropriate, returns a GridTooltipLink for opening the file in rook.
...
fn open_in_rook_tooltip(
    ...
    use crate::util::tooltips::should_show_open_in_rook_link;

    if !should_show_open_in_rook_link(&path, app) {
        return None;
    }
    ...
    Some(GridTooltipLink {
        ...
        action: TerminalAction::OpenCodeInRook {
            path,
    ...
    pub(super) fn render_grid_tooltip(
        ...
        if let Some(link) = &self.open_grid_link_tool_tip {
            ...
            #[cfg(feature = "local_fs")]
            {
                ...
                {
                    open_in_rook = open_in_rook_tooltip(
                        path.clone(),
                        file_link.get_inner().line_and_column_num,
                        &mut detail,
                        self.mouse_states.open_in_rook_tooltip.clone(),
                        app,
                    );
