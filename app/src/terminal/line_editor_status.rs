...
use rookui::{Entity, ModelContext, ModelHandle};
...
/// This prevents Rook sending an escape sequence to an arbitrary running program.
///
...
pub struct LineEditorStatus {
    ...
    /// not its a Rookified session, so to in order properly signal downstream that the line editor
    /// (for Rookified sessions) is active, we must check if there was a corresponding precmd
...
impl LineEditorStatus {
    ...
    fn mark_line_editor_active_after_delay(
        ...
        let abort_handle = ctx.spawn_abortable(
            async move { rookui::r#async::Timer::after(delay).await },
            |me, _, ctx| {
