...
use rookui::units::{IntoLines, IntoPixels, Lines};
use rookui::{AppContext, EntityId, ViewHandle, record_trace_event};
...
pub struct BlockList {
    ...
    rook_prompt_height_lines: f32,

    /// Executor used for spawning threads in the background.
    background_executor: Arc<Background>,

    show_rook_bootstrap_block: bool,
    ...
    /// to the PTY before rook_precmd is executed after the first command. `rook_precmd` is used
...
impl BlockList {
    ...
    pub fn new(
        ...
        event_proxy: ChannelEventListener,
        background_executor: Arc<Background>,
        show_rook_bootstrap_input: bool,
        show_in_band_command_blocks: bool,
        show_memory_stats: bool,
        honor_ps1: bool,
        is_inverted: bool,
    ...
    /// block.
    /// 3. Create the `BootstrapStage::RookInput` block through
    /// `create_rook_input_block`. From here on, there is always a default
    ...
    fn new_internal(
        ...
        BlockList {
            ...
            rook_prompt_height_lines: sizes.rook_prompt_height_lines,
            background_executor,
            show_rook_bootstrap_block: show_rook_bootstrap_input,
            show_in_band_command_blocks,
            show_memory_stats,
            honor_ps1,
    ...
    fn create_rook_input_block(&mut self) {
        self.create_new_block(
            BlockId::new(),
            BootstrapStage::RookInput,
            Default::default(),
            None,
        );
        self.start_active_block();
        self.update_active_block_height();
        self.bootstrap_stage = BootstrapStage::RookInput;
    }
    ...
    /// `create_rook_input_block`. For non-block items before then, we should
    ...
    pub fn set_show_bootstrap_block(&mut self, show_bootstrap_block: bool) {
        self.show_rook_bootstrap_block = show_bootstrap_block;
        self.update_blocks_and_sumtree(
            None,
            None,
    ...
    fn seek_down_to_next_grid(
        ...
        match grid_type {
            GridType::Output => {
                ...
                // vs the next "command" (in the Rook prompt case), when using the combined grid, rather than
    ...
    fn create_new_block(
        ...
        let mut block = Block::new(
            ...
            bootstrap_stage,
            self.show_rook_bootstrap_block,
    ...
    pub fn bootstrap_block_contents(&self) -> String {
        ...
        for block in self.blocks.iter() {
            match block.bootstrap_stage() {
                BootstrapStage::RookInput | BootstrapStage::ScriptExecution => {
...
impl ansi::Handler for BlockList {
    ...
    fn set_hyperlink(&mut self, hyperlink: Option<rook_terminal::model::ansi::Hyperlink>) {
    ...
    fn clear_screen(&mut self, mode: ClearMode) {
        ...
        match mode {
            ...
            ClearMode::All => {
                ...
                // Rook's custom logic for "clear". It's not immediately straightforward because a
                ...
                // quite right with Rook's custom clear screen logic.
            }
            _ => {}
        }
