...
//! Rook. This struct is abstracted away from Block, for the purposes of enabling same-line prompt,
...
pub struct HeaderGrid {
    ...
    /// The height of the Rook prompt in lines (non-PS1).
...
impl HeaderGrid {
    ...
    pub fn set_honor_ps1(&mut self, honor_ps1: bool) {
        ...
        if !self.honor_ps1 {
            // If we are switching to Rook prompt (from PS1), we need to clear the cached prompt end point
    ...
    fn is_command_finished_and_empty(&self) -> bool {
        if !self.honor_ps1 {
            // If we are using Rook prompt, we expect the combined grid cursor to be at the start, if
    ...
    pub fn prompt_height(&self) -> Lines {
        ...
        if self.is_command_empty() {
            ...
            self.rook_prompt_height_lines.into_lines()
        }
    }
...
impl ansi::Handler for HeaderGrid {
    ...
    fn prompt_only_precmd(&mut self, data: PromptMetadata) {
            ...
            && honor_ps1 != self.honor_ps1
        {
            ...
            // switch the prompt mode via the $ROOK_HONOR_PS1 environment variable.
            ...
            // to be emitted. This may be de-synced from Rook settings in particular niche cases (which are
