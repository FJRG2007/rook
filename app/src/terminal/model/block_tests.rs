...
pub fn test_long_running_block_bottom_padding() {
    rookui::r#async::block_on(async {
        ...
        rookui::r#async::Timer::after(Duration::from_millis(duration)).await;
...
#[test]
pub fn test_image_completion_drops_in_rook_input_stage() {
    let _iterm_images = FeatureFlag::ITermImages.override_enabled(true);
    let mut block = TestBlockBuilder::new()
        .with_bootstrap_stage(BootstrapStage::RookInput)
        .build();
    block.start();
...
pub fn test_set_current_working_directory_updates_pwd_and_emits_cwd_event() {
    ...
    rookui::r#async::block_on(pin!(events_rx).for_each(|event| match event {
...
pub fn test_block_emits_block_completed_event_for_in_band_command() {
    ...
    let block_completed_event =
        rookui::r#async::block_on(pin!(events_rx).find(|event| match event {
...
#[test]
fn test_command_and_output_to_string_excludes_rook_prompt() {
    let block_index = BlockIndex::zero();
...
/// Regression test (CORE-1947): checks Rook prompt case for is_command_empty. Specifically,
...
/// be against (0, 0) in the combined grid, for the Rook prompt case, rather than checking against the
/// prompt grid (which we do in the PS1 active case).
#[test]
fn test_command_is_empty_rook_prompt() {
    let block_index = BlockIndex::zero();
    ...
    // Note that we are indicating Rook prompt, not PS1 here!
    let mut block = create_test_block_with_grids(
        block_index,
        prompt_and_command_grid,
        rprompt_grid,
