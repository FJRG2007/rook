...
pub enum BootstrapStage {
    /// Rook is re-parsing historical blocks for this session. We haven't yet started
    /// bootstrapping.
    RestoreBlocks,
    /// Rook is writing the bootstrap script into the running shell.
    RookInput,
    /// Execution of any shell startup scripts such as .rc or .profile files.
    ScriptExecution,
    /// Model is fully bootstrapped (i.e the `Bootstrap` message was successfully received by Rook).   
    Bootstrapped,
...
impl BootstrapStage {
    pub fn next_stage(&self) -> Self {
        match self {
            BootstrapStage::RestoreBlocks => BootstrapStage::RookInput,
    ...
    /// RookInput is the one block that is hidden by default (unless debug mode is on).
    pub fn is_hidden(&self) -> bool {
        matches!(self, BootstrapStage::RookInput)
    }
