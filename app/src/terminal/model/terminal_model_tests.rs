...
fn ignores_non_inline_iterm_file_payload_without_overwriting_cwd_file() {
    ...
    let attacker_bytes = b"touch /tmp/rook-pwned\n";
...
pub fn test_restored_empty_command_block() {
    ...
    // The mocked terminal model comes with a RookInput block and the active block.
...
fn test_restored_blocks_on_different_host() {
    let restored_blocks = [
        SerializedBlock {
            ...
            stylized_output: str_to_byte_vec("RookTerminal"),
            pwd: Some("/".to_owned()),
            git_head: None,
            git_branch_name: None,
            virtual_env: None,
    ...
    assert_eq!(blocks[0].output_to_string(), "RookTerminal",);
