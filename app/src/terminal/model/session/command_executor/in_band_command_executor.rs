...
use rook_terminal::model::Point;
use rook_util::on_cancel::OnCancelFutureExt;
            ...
            }) = running_command.as_ref()
            {
                ...
                let in_band_command = match shell.shell_type() {
                    ShellType::PowerShell => {
                        format!(
                            "Rook-Run-GeneratorCommand {id} '{escaped_command}' -ErrorAction Ignore"
                        )
                    }
                    ShellType::Fish => {
                        ...
                        // fish.
                        format!(" rook_run_generator_command {id} '{escaped_command}'")
                    }
                    _ => {
                        format!("rook_run_generator_command {id} '{escaped_command}'")
                    }
...
impl CommandExecutor for InBandCommandExecutor {
    ...
    /// `rook_run_generator_command`/`Rook-Run-GeneratorCommand` shell script API that is declared as
    /// part of Rook's bootstrap script.
    ///
    ...
    /// shell side in `rook_preexec`). This merely clears the running and pending command IDs from
...
pub fn is_in_band_command(command: &str) -> bool {
    ...
    trimmed.starts_with("Rook-Run-GeneratorCommand")
        || trimmed.starts_with("rook_run_generator_command")
}
