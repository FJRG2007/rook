...
fn parse_stop_notification() {
    ...
    let notif = parse_event(Some("rook://cli-agent"), body).unwrap();
...
fn returns_none_for_invalid_json() {
    assert!(parse_event(Some("rook://cli-agent"), "not json").is_none());
}
...
fn rejects_unsupported_schema_version() {
    ...
    assert!(parse_event(Some("rook://cli-agent"), body).is_none());
}
...
fn parse_auggie_stop_notification() {
    // Mirrors what the community auggie-rook plugin emits on the Stop hook.
...
fn stale_timer_callback_after_disarming_event_does_not_overwrite_newer_status() {
    // RookUI's `SpawnedFutureHandle::abort` only takes effect the next time
