...
/// Manages the Rook notification plugin for a specific CLI agent.
///
...
pub(crate) trait CliAgentPluginManager: Send + Sync {
    /// The minimum plugin version required by this Rook build.
    ...
    /// Whether the Rook notification plugin is installed.
    ...
    /// Install the Rook notification plugin.
    ...
    /// Update the Rook notification plugin to the latest version.
    /// Default returns an error — only agents with `can_auto_install() == true` should override.
    ...
    fn install_success_message(&self) -> &'static str {
        "Rook plugin installed. Please restart the session to activate."
    }

    /// Toast message shown after a successful auto-update.
    fn update_success_message(&self) -> &'static str {
        "Rook plugin updated. Please restart the session to activate."
    }
...
/// doesn't have Rook notification plugin support.
