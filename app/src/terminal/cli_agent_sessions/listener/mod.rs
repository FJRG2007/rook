use rookui::{EntityId, ModelContext, ModelHandle, SingletonEntity};
...
fn create_handler(agent: &CLIAgent) -> Option<Box<dyn CLIAgentSessionHandler>> {
    match agent {
        ...
        // (https://github.com/augmentmoogi/auggie-rook,
        ...
        // RookTui emits OSC 777 events directly (no external plugin needed).
        CLIAgent::Claude
        | CLIAgent::OpenCode
        | CLIAgent::Gemini
        | CLIAgent::Auggie
        ...
        | CLIAgent::Pi
        | CLIAgent::OhMyPi
        | CLIAgent::RookTui => Some(Box::new(DefaultSessionListener)),
...
impl CLIAgentSessionHandler for CodexSessionHandler {
    /// Before Codex enabled support for hooks, we relied on OSC 9 to trigger notifications in Rook.
...
impl rookui::Entity for CLIAgentSessionListener {
    type Event = ();
}
