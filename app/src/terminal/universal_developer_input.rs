...
use rook_core::ui::color::{Opacity, Rgb, coloru_with_opacity};
...
use rookui::ui_components::components::UiComponentStyles;
        ...
        let (is_ssh_without_remote_server, is_subshell) = active_block_metadata
            ...
            .map(|session| {
                ...
                let has_connected_remote_server = matches!(
                    session_type,
                    SessionType::RookifiedRemote { host_id: Some(_) }
                );
                // The @ menu requires repo metadata which is only available for:
                // - Local sessions
                // - RookifiedRemote sessions with a connected remote server (host_id is Some)
                //
                // Block when:
                // - SSH wrapper session without a remote server upgrade
                // - RookifiedRemote still connecting (host_id is None)
                //
                // Note: is_ssh_wrapper_session() is set at bootstrap time and stays true
                // even after the session transitions to RookifiedRemote with a host_id.
                ...
                let is_ssh_without_remote_server = !has_connected_remote_server
                    && (session.is_ssh_wrapper_session()
                        || matches!(session_type, SessionType::RookifiedRemote { host_id: None }));
...
impl View for UniversalDeveloperInputButtonBar {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn rookui::Element> {
        ...
        let create_divider = || {
            Container::new(
                rookui::elements::ConstrainedBox::new(
        ...
        let build_buttons = |model_selector_element: Box<dyn rookui::Element>| {
