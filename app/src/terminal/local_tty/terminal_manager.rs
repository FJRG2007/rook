...
/// Adapts a TUI-owned surface manager to Rook's type-erased manager contract.
...
impl<S> TerminalManager<S> {
        ...
        BoxManager: FnOnce(Self) -> Box<dyn TerminalManagerTrait> + 'static,
    {
        ...
        // This is purely for measuring throughput on RookDev.
...
impl<S> TerminalManager<S> {
    ...
    /// Sends bindkey to notify shell process to switch to Rook prompt logic for prompt
    /// with the combined prompt/command grid (we unset the PS1, but save the value for potential
    /// future restoration).
    pub fn send_switch_to_rook_prompt_bindkey(&self, app_ctx: &mut AppContext) {
        self.pty_controller.update(app_ctx, |pty_controller, ctx| {
            pty_controller.send_switch_to_rook_prompt_bindkey(ctx);
        });
    }
    ...
    fn create_pty(
        ...
        // shown (the Rook prompt, the agent footer, or the CLI agent footer). When it
        ...
        // `enable_ssh_rookification` is the single source of truth for whether the SSH
        // wrapper is active. The bootstrap scripts check `ROOK_USE_SSH_WRAPPER` (derived
        // from this value) before invoking `rook_ssh_helper`, which spawns the ControlMaster
        // and opens agent-protocol channels.
        let enable_ssh_wrapper = *RookifySettings::as_ref(ctx)
...
pub fn get_shell_starter(
    ...
    shell_starter_or_wsl_name
        .and_then(|starter| {
            rookui::r#async::block_on(async { starter.to_shell_starter_source().await })
        })
        .map(|starter_source| {
