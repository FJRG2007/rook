...
maybe_define_setting!(AddedSubshellCommands, group: RookifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    ...
    private: false,
    toml_path: "rookify.subshells.added_subshell_commands",
    description: "Additional regex patterns for commands that should be recognized as subshells.",
});

maybe_define_setting!(SubshellCommandsDenylist, group: RookifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    surface: settings::SettingSurfaces::GUI,
    private: false,
    toml_path: "rookify.subshells.subshell_commands_denylist",
    description: "Commands that should not trigger the subshell rookification prompt.",
});

maybe_define_setting!(SshHostsDenylist, group: RookifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    surface: settings::SettingSurfaces::GUI,
    private: false,
    toml_path: "rookify.ssh.ssh_hosts_denylist",
    description: "SSH hosts that should not trigger the rookification prompt.",
});

maybe_define_setting!(EnableSshRookification, group: RookifySettings, {
    type: bool,
    default: true,
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    surface: settings::SettingSurfaces::GUI,
    private: false,
    toml_path: "rookify.ssh.enable_ssh_rookification",
    description: "Whether to enable Rook features in SSH sessions.",
});

// NOTE: This setting has been unified into `enable_ssh_rookification` and is no
...
// and forward it to `enable_ssh_rookification`. It can be deleted in a future
...
// https://github.com/FJRG2007/rook/issues/13228. The migration below reads this
// value and forwards an opt-out to `enable_ssh_rookification`. When it was synced,
...
// `enable_ssh_rookification` even after the user turned it back on. Keeping it
...
maybe_define_setting!(EnableSshWrapper, group: RookifySettings, {
    type: bool,
    default: true,
    supported_platforms: SupportedPlatforms::ALL,
    ...
    toml_path: "rookify.ssh.enable_legacy_ssh_wrapper",
    description: "Deprecated: unified into enable_ssh_rookification. Retained only for one-time migration.",
});
...
maybe_define_setting!(UseSshTmuxWrapper, group: RookifySettings, {
    type: bool,
    default: false,
    ...
    surface: settings::SettingSurfaces::GUI,
    private: false,
    toml_path: "rookify.ssh.use_ssh_tmux_wrapper",
    description: "Deprecated: whether to use a tmux-based wrapper for SSH rookification.",
});
...
maybe_define_setting!(SshTmuxDeprecationNoticePending, group: RookifySettings, {
    type: bool,
    default: false,
    supported_platforms: SupportedPlatforms::OR(SupportedPlatforms::MAC.into(), SupportedPlatforms::LINUX.into()),
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    surface: settings::SettingSurfaces::GUI,
    private: false,
    toml_path: "rookify.ssh.ssh_tmux_deprecation_notice_pending",
    description: "Internal: whether to show the one-time tmux SSH deprecation notice.",
});

/// Controls how Rook handles the SSH extension (remote server binary) when connecting
...
pub enum SshExtensionInstallMode {
    ...
    AlwaysInstall,
    /// Never install; fall back to wrapper-only SSH rookification.
    NeverInstall,
}

maybe_define_setting!(SshExtensionInstallModeSetting, group: RookifySettings, {
    ...
    surface: settings::SettingSurfaces::GUI,
    private: false,
    toml_path: "rookify.ssh.ssh_extension_install_mode",
    description: "Controls SSH extension installation behavior.",
});
...
pub struct RookifySettings {
    ...
    /// A list of commands that we shouldn't attempt to rookify. These can be added either b/c the
    /// "don't ask again" button was clicked in the trigger banner, or it was added explicitly on
    /// the Rookify settings page. This represents the raw, serialized value.
    ...
    /// A list of hosts that we shouldn't attempt to rookify. This supports regex.
    /// These can be added either b/c the "don't ask again" button was clicked in the trigger banner,
    /// or it was added explicitly on the Rookify settings page.
    /// While this could live in the `SshSettings` group, the custom processing shared with the other
    /// subshell logic better justifies it living in the `RookifySettings` group.
    ...
    /// This setting controls whether we should ever rookify ssh sessions.
    pub enable_ssh_rookification: EnableSshRookification,

    /// Deprecated: unified into `enable_ssh_rookification`. Retained only so the one-time
...
lazy_static! {
    ...
    static ref SUBSHELL_COMMAND_REGEXES: Vec<Regex> = vec![
        ...
        // https://github.com/FJRG2007/rook/issues/2736
...
impl RookifySettings {
    fn new_from_storage(ctx: &mut ModelContext<Self>) -> Self {
        ...
        Self {
            ...
            ssh_hosts_denylist,
            enable_ssh_rookification: EnableSshRookification::new_from_storage(ctx),
    ...
    pub fn new_with_defaults(_ctx: &mut ModelContext<Self>) -> Self {
        ...
        Self {
            ...
            parsed_ssh_hosts_denylist: Self::parse_ssh_hosts_denylist(&ssh_hosts_denylist),
            ssh_hosts_denylist,
            enable_ssh_rookification: EnableSshRookification::new(None),
    ...
    pub fn register(ctx: &mut AppContext) {
        ...
        ctx.subscribe_to_model(&handle, |settings, event, ctx| {
            settings.update(ctx, |me, _| match event {
                RookifySettingsChangedEvent::AddedSubshellCommands { .. } => {
                    me.parsed_added_subshell_commands =
                        Self::parse_added_subshell_commands(&me.added_subshell_commands)
                }
                RookifySettingsChangedEvent::SubshellCommandsDenylist { .. } => {
                    me.parsed_subshell_command_denylist =
                        Self::parse_subshell_command_denylist(&me.subshell_command_denylist)
                }
                RookifySettingsChangedEvent::SshHostsDenylist { .. } => {
                    me.parsed_ssh_hosts_denylist =
                        Self::parse_ssh_hosts_denylist(&me.ssh_hosts_denylist)
                }
                RookifySettingsChangedEvent::EnableSshRookification { .. } => {}
                RookifySettingsChangedEvent::EnableSshWrapper { .. } => {}
                RookifySettingsChangedEvent::UseSshTmuxWrapper { .. } => {}
                RookifySettingsChangedEvent::SshTmuxDeprecationNoticePending { .. } => {}
        ...
        // setting to `false` (via `rookify.ssh.enable_legacy_ssh_wrapper = false` in their
        // TOML config or the old `EnableSSHWrapper` storage key), honour that intent by
        // disabling `enable_ssh_rookification` — the canonical setting that now controls the
        ...
        // https://github.com/FJRG2007/rook/issues/13228, where syncing the trigger caused
        // the migration to re-fire every launch and repeatedly disable rookification.
        handle.update(ctx, |me, ctx| {
            if me.enable_ssh_wrapper.is_value_explicitly_set() && !*me.enable_ssh_wrapper.value() {
                if let Err(e) = me.enable_ssh_rookification.set_value(false, ctx) {
                    report_error!(e.context(
                        "Failed to migrate enable_ssh_wrapper → enable_ssh_rookification"
                    ));
                }
...
/// define_settings_group!(RookifySettings). Since we didn't use that macro we define it manually
/// here. It's the event emitted by the setter methods when a setting value changes.
pub enum RookifySettingsChangedEvent {
    AddedSubshellCommands {
...
impl Entity for RookifySettings {
    type Event = RookifySettingsChangedEvent;
}

impl SingletonEntity for RookifySettings {}
...
impl RookifySettings {
    ...
    pub fn is_compatible_subshell_command(&self, command: &str, shell_family: ShellFamily) -> bool {
        ...
        if SshRookifyCommand::matches(command).is_some_and(|command| command.is_ssh_like_command())
        {
            return true;
        }
        ...
        // While in-band generators are our best option for rookifying ssh sessions from powershell, hard-code
        // the rookify subshell banner to show up.
    ...
    pub fn add_subshell_command(&mut self, command_to_add: &str, ctx: &mut ModelContext<Self>) {
        ...
        // care of emitting the RookifySettingsChangedEvent::AddedSubshellCommands event to keep
    ...
    /// Check if the user has asked us to remember a command and avoid asking to rookify a subshell.
