...
use rookui::{EntityId, SingletonEntity as _, ViewContext, ViewHandle};

use super::success_block::RookifySuccessBlock;
...
pub enum SshBlockState {
    RookifySuccess {
        handle: ViewHandle<RookifySuccessBlock>,
    },
}
...
impl SshBlockState {
    ...
    pub fn get_block_view_id(&self) -> EntityId {
        match self {
            SshBlockState::RookifySuccess { handle, .. } => handle.id(),
        }
    }

    pub fn on_rookified_session_complete(
        &self,
        ctx: &mut ViewContext<TerminalView>,
    ) -> Option<EntityId> {
        match self {
            SshBlockState::RookifySuccess { handle } => {
                handle.update(ctx, |block, ctx| {
                    block.on_rookified_session_complete(ctx);
                });
            }
        }
        None
...
/// Temporary state used to trigger Rookification.
#[derive(Default)]
struct RookifyTriggerState {
    block_id: Option<BlockId>,

    /// Lets us abort an attempt to auto rookify if the subshell command
    /// hasn't completed.
    auto_rookify_abort_handle: Option<SpawnedFutureHandle>,
    ...
    /// The command which may trigger ssh Rookification
    pending_command: Option<String>,
    /// The Host which may trigger ssh Rookification
    pending_rookify_ssh_host: Option<String>,

    /// Which, if any, SSH block is currently added to the blocklist.
    ssh_block_state: Option<SshBlockState>,

    ssh_rookify_timeout_handle: Option<SpawnedFutureHandle>,
...
#[derive(Default)]
pub struct RookifyState {
    session_id: Option<SessionId>,

    pending_state: Option<RookifyTriggerState>,
...
impl RookifyState {
    ...
    pub fn add_auto_rookify_abort_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.auto_rookify_abort_handle = Some(spawned_future_handle);
    }

    pub fn abort_auto_rookify(&mut self) {
        if let Some(abort_handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.auto_rookify_abort_handle.take())
        {
            abort_handle.abort();
        };
    }

    pub fn add_ssh_rookify_timeout_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.ssh_rookify_timeout_handle = Some(spawned_future_handle);
    }

    pub fn abort_ssh_rookify_timeout(&mut self) {
        self.replace_timeout_id();
        if let Some(handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.ssh_rookify_timeout_handle.take())
        {
            handle.abort();
        };
    }
    ...
    pub fn get_pending_ssh_host(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &RookifyTriggerState| state.pending_rookify_ssh_host.clone())
    }

    pub fn get_pending_ssh_command(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &RookifyTriggerState| state.pending_command.clone())
    }

    pub fn take_pending_ssh_host(&mut self) -> Option<String> {
        self.pending_state
            .as_mut()
            .and_then(|state: &mut RookifyTriggerState| state.pending_rookify_ssh_host.take())
    }

    pub fn clear_pending_ssh_host(&mut self) {
        if let Some(ref mut pending_state) = self.pending_state.as_mut() {
            pending_state.pending_rookify_ssh_host = None;
        }
    }
    ...
    pub fn set_pending_ssh_host(&mut self, command: String, ssh_host: Option<String>) {
        ...
        pending_state.pending_rookify_ssh_host = ssh_host;
    }
    ...
    /// and we have a Rookify Success block.
    fn on_rookified_session_complete(
        &mut self,
        state: RookifyTriggerState,
        ctx: &mut ViewContext<TerminalView>,
    ) -> Option<EntityId> {
    ...
    pub fn on_rookify_start(&mut self, active_session_id: Option<SessionId>) {
        self.session_id = active_session_id;
    }

    /// Called whenever a block is completed, to determine whether a Rookified session
    /// has been completed.
    pub fn get_completed_rookify_session_id(
        &mut self,
        ...
        if let Some(state) = self.pending_state.take() {
            return self.on_rookified_session_complete(state, ctx);
        };
        None
    }
}
