...
fn test_parsed_subshell_commands_updated_via_self_subscription() {
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            assert!(
                RookifySettings::as_ref(ctx)
                    .parsed_added_subshell_commands
                    .is_empty()
            );
        });

        RookifySettings::handle(&app).update(&mut app, |settings, ctx| {
            settings
        ...
        app.read(|ctx| {
            let parsed = &RookifySettings::as_ref(ctx).parsed_added_subshell_commands;
            assert_eq!(
                parsed.len(),
                1,
...
/// `enable_ssh_rookification` on first launch after the migration.
#[test]
fn test_enable_ssh_wrapper_false_migrates_to_enable_ssh_rookification_false() {
    App::test((), |mut app| async move {
        initialize_settings_for_tests(&mut app);
        ...
        app.update(|ctx| {
            RookifySettings::handle(ctx).update(ctx, |me, ctx| {
                if me.enable_ssh_wrapper.is_value_explicitly_set()
                    && !*me.enable_ssh_wrapper.value()
                {
                    me.enable_ssh_rookification
                        .set_value(false, ctx)
                        .expect("migration set enable_ssh_rookification");
        ...
        app.read(|ctx| {
            let settings = RookifySettings::as_ref(ctx);
            assert!(
                !*settings.enable_ssh_rookification.value(),
                "enable_ssh_rookification should be false after migration"
            );
...
/// forward an opt-out to `enable_ssh_rookification`; if they synced, a stale cloud
...
/// See https://github.com/FJRG2007/rook/issues/13228.
#[test]
fn test_deprecated_ssh_wrapper_migration_triggers_are_not_synced() {
    assert_eq!(
        ...
        "enable_legacy_ssh_wrapper must not sync — a stale synced value re-arms the \
         migration and re-disables enable_ssh_rookification (#13228)"
    );
    assert_eq!(
...
/// opt-out once, and a user who then re-enables Rookify SSH keeps it. Because the
...
#[test]
fn test_legacy_wrapper_migration_is_one_time_and_preserves_reenabled_rookification() {
    /// Mirrors the one-time migration body from `RookifySettings::register`.
    ...
    App::test((), |mut app| async move {
        ...
        // The user re-enables Rookify SSH.
        RookifySettings::handle(&app).update(&mut app, |settings, ctx| {
            settings
        ...
        // default; the migration is a no-op and does not re-disable rookification.
        run_migration(&mut app);
        app.read(|ctx| {
            assert!(
                *RookifySettings::as_ref(ctx)
                    .enable_ssh_rookification
                    .value(),
                "re-enabled Rookify SSH persists across launches (#13228)"
            );
        });
    });
}
...
/// spuriously disable `enable_ssh_rookification`.
#[test]
fn test_enable_ssh_wrapper_default_does_not_affect_enable_ssh_rookification() {
    App::test((), |mut app| async move {
        initialize_settings_for_tests(&mut app);
        ...
        app.read(|ctx| {
            ...
            assert!(
                *settings.enable_ssh_rookification.value(),
                "enable_ssh_rookification should remain true when no migration is needed"
            );
        });
    });
}
...
fn test_wsl_subshell_detection_success() {
    ...
    .for_each(|cmd| {
        assert!(
            RookifySettings::is_built_in_subshell_match(cmd),
            "{} failed to match",
            *cmd
        )
    });
