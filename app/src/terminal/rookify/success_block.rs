...
use super::settings::RookifySettings;
use super::{RookificationSource, render, subshell_bootstrap_success_block_bytes};
...
#[derive(Debug, Clone)]
pub enum RookifySuccessBlockEvent {
    OpenRookifySettings,
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub enum RookifySuccessBlockAction {
    ClearAutoRookifySnippet,
    OpenRookifySettings,
    OpenUrl(String),
}

struct AutoRookifySnippet {
    /// On subshell initialization, this will contain the output grid to display,
    /// containing info like how to auto-rookify the subshell.
...
pub struct RookifySuccessBlock {
    source: RookificationSource,
    spawning_command: String,
    learn_more_link_mouse_states: MouseStateHandle,
    auto_rookify_snippet: Option<AutoRookifySnippet>,
}

impl RookifySuccessBlock {
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&RookifySettings::handle(ctx), move |_, _, _, ctx| {
            ctx.notify();
        });
        ...
        let is_auto_rookify_configured = subshell_info
            .as_ref()
            .map(|info| info.was_triggered_by_rc_file_snippet)
            .unwrap_or_default();

        let auto_rookify_snippet = if is_auto_rookify_configured {
            None
        } else {
            subshell_info.and_then(|subshell_info| {
                // If rookification wasn't triggered automatically, show a snippet about
                // how to automatically rookify.
        ...
        let auto_rookify_snippet = auto_rookify_snippet.map(|(output_grid, can_write_to_rc)| {
            AutoRookifySnippet {
                description: (if !output_grid.is_empty() {
                    "Run the following to automatically Rookify in the future:"
                } else {
                    "In remote subshells, Rook runs commands in the background to power completions, syntax highlighting, and other features."
                }).into(),
        ...
        Self {
            ...
            spawning_command,
            auto_rookify_snippet,
        }
    }

    pub fn selected_text(&self) -> Option<String> {
        self.auto_rookify_snippet
            .as_ref()
    ...
    pub fn render_title_ui(&self, theme: &RookTheme, appearance: &Appearance) -> Box<dyn Element> {
        let header_contents = render::build_header_row(
            "Session Rookified",
            Icon::new(UiIcon::Rook.into(), theme.active_ui_detail()),
            theme,
            appearance,
        )
        .with_margin_right(8.)
    ...
    fn render_learn_more_link(&self, appearance: &Appearance) -> Box<dyn Element> {
        let url = match self.source {
            RookificationSource::Ssh => SSH_DOCS_URL,
            RookificationSource::Subshell => SUBSHELL_DOCS_URL,
        };
        ...
        appearance
            ...
            .link(
                ...
                Some(Box::new({
                    move |ctx| {
                        ctx.dispatch_typed_action(RookifySuccessBlockAction::OpenUrl(
                            url.to_owned(),
                        ));
                    }
                })),
    ...
    /// Fired when a block ends and we are not in a Rookified session.
    pub fn on_rookified_session_complete(&mut self, ctx: &mut ViewContext<Self>) {
        self.clear_auto_rookify_snippet(ctx);
    }

    pub fn clear_auto_rookify_snippet(&mut self, ctx: &mut ViewContext<Self>) {
        self.auto_rookify_snippet = None;
        ctx.notify();
    }
    ...
    pub fn render_output_grid(
        ...
        let auto_rookify_snippet = self.auto_rookify_snippet.as_ref()?;

        if auto_rookify_snippet.output_grid.is_empty() {
            return None;
        }

        let shell_language = ProgrammingLanguage::Shell(auto_rookify_snippet.shell_type);
        let runnable_command = render_runnable_code_snippet(
            &auto_rookify_snippet.output_grid,
            if auto_rookify_snippet.can_write_to_rc {
                Some(&shell_language)
            } else {
                None
            },
            Some(Box::new({
                move |code_snippet, ctx| {
                    ...
                    ctx.dispatch_typed_action(RookifySuccessBlockAction::ClearAutoRookifySnippet);
                }
            })),
            Some(Box::new({
                move |code_snippet, ctx| {
            ...
            Some(auto_rookify_snippet.code_snippet_handles.clone()),
            app,
        );

        let semantic_selection = SemanticSelection::as_ref(app);
        let selected_text = auto_rookify_snippet.selected_text.clone();

        // TODO(Simon): Implement full selection and copying functionality for the RookifySuccessBlock.
        ...
        let output_grid = SelectableArea::new(
            auto_rookify_snippet.selection_handle.clone(),
        ...
        let output_grid = Flex::column()
            .with_child(
                Container::new(
                    Text::new(
                        auto_rookify_snippet.description.clone(),
...
impl Entity for RookifySuccessBlock {
    type Event = RookifySuccessBlockEvent;
}

pub const ROOKIFY_SUCCESS_BLOCK_VISIBLE_KEY: &str = "RookifySuccessBlockVisible";

impl View for RookifySuccessBlock {
...
impl TypedActionView for RookifySuccessBlock {
    type Action = RookifySuccessBlockAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            RookifySuccessBlockAction::OpenRookifySettings => {
                ctx.emit(RookifySuccessBlockEvent::OpenRookifySettings);
            }
            RookifySuccessBlockAction::OpenUrl(url) => {
                ctx.open_url(url);
            }
            RookifySuccessBlockAction::ClearAutoRookifySnippet => {
                self.clear_auto_rookify_snippet(ctx);
            }
        }
    }
