...
use rookui::ui_components::components::{UiComponent as _, UiComponentStyles};
use rookui::{AppContext, Element, EventContext, PaintContext, SingletonEntity as _};
...
const ROOK_DRIVE_ENV_VAR_COLLECTION_ICON_COLOR: u32 = 0xC464FFFF;
const ICON_MARGIN: f32 = 4.;
const TERMINAL_ICON: &str = "bundled/svg/terminal.svg";
pub const HORIZONTAL_TEXT_MARGIN: f32 = 20.;
pub const SSH_DOCS_URL: &str = "https://docs.rook.dev/terminal/rookify/ssh";
pub const SUBSHELL_DOCS_URL: &str = "https://docs.rook.dev/terminal/rookify/subshells";
...
/// UI helper to render the ssh command that caused the rookification prompt.
...
pub fn description_row(text: &str, theme: &RookTheme, appearance: &Appearance) -> Box<dyn Element> {
...
/// Renders a "Never Rookify this host" link or nothing.
pub fn render_never_rookify_ssh_link(
    ssh_host: &Option<String>,
    app: &AppContext,
    appearance: &Appearance,
    mouse_state_handle: MouseStateHandle,
    on_never_rookify: fn(&mut EventContext<'_>, ssh_host: String),
) -> Option<Box<dyn Element>> {
    let Some(ssh_host) = ssh_host else {
        return None;
    };

    let settings = RookifySettings::handle(app);
    if settings.as_ref(app).is_ssh_host_denylisted(ssh_host) {
        // Should only happen if user manually attempts to Rookify a denylisted host.
        return None;
    }
    ...
    let link = appearance
        .ui_builder()
        .link(
            "Never Rookify this host".into(),
            None,
            Some(Box::new({
                let ssh_host = ssh_host.clone();
                move |ctx| on_never_rookify(ctx, ssh_host.to_owned())
            })),
            mouse_state_handle,
        )
        .soft_wrap(false)
...
fn get_subshell_flag_info(subshell_source: &SubshellSource, theme: &RookTheme) -> (String, Fill) {
    match subshell_source {
        SubshellSource::EnvVarCollection(environment_name) => (
            environment_name.to_string(),
            Fill::Solid(ColorU::from_u32(ROOK_DRIVE_ENV_VAR_COLLECTION_ICON_COLOR)),
        ),
...
pub fn render_subshell_flag_pole(
    max_height: f32,
    fill: impl Into<rookui::elements::Fill>,
) -> Box<dyn Element> {
