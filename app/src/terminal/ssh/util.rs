...
/// For some [`SshRookifyCommand`]s, we do not support parsing
...
impl SshRookifyCommand {
    pub fn matches(command: &str) -> Option<SshRookifyCommand> {
        let command = if let Some(suffix) = command.strip_prefix("command ") {
            suffix
        } else {
            command
        ...
        if INTERACTIVE_SSH.is_match(command) {
            Some(SshRookifyCommand::Ssh)
        } else if GCLOUD_REGEX.is_match(command) {
            Some(SshRookifyCommand::SshLike(SshLikeCommand::Gcloud))
        } else if ELASTIC_BEANSTALK_REGEX.is_match(command) {
            Some(SshRookifyCommand::SshLike(SshLikeCommand::ElasticBeanstalk))
        } else if DIGITAL_OCEAN_DROPLET_REGEX.is_match(command) {
            Some(SshRookifyCommand::SshLike(
...
pub fn parse_interactive_ssh_command(command: &str) -> Option<InteractiveSshCommand> {
    match SshRookifyCommand::matches(command) {
        Some(SshRookifyCommand::Ssh) => InteractiveSshCommand::parse_ssh_command(command),
        Some(SshRookifyCommand::SshLike(SshLikeCommand::Gcloud)) => {
        ...
        Some(SshRookifyCommand::SshLike(SshLikeCommand::DigitalOceanDroplet)) => {
            Some(InteractiveSshCommand::default())
        }
        None => None,
    }
...
/// Creates an sftp command that copies a given local file into the pwd in the rookified ssh session.
