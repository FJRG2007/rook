...
impl<T: EventLoopSender> RemoteServerController<T> {
    ...
    fn on_ssh_init_shell_requested(&mut self, info: SessionInfo, ctx: &mut ModelContext<Self>) {
        ...
        let rook_owns_control_master = !external_control_master;
    ...
    fn on_binary_check_complete(
        ...
        match result {
            Ok(true) => {
                ...
                let rook_owns_control_master = transport.rook_owns_control_master();
            ...
            Ok(false) => {
                let install_mode = *RookifySettings::as_ref(ctx)
    ...
    fn connect_session_for_current_identity(
        ...
        let transport =
            SshTransport::new(socket_path, auth_context.clone(), rook_owns_control_master);
        RemoteServerManager::handle(ctx).update(ctx, |mgr, ctx| {
            mgr.connect_session(
                session_id,
                transport,
