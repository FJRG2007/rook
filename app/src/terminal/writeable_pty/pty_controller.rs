...
/// restore the PS1 from the saved PS1 value (we had unset the PS1 for Rook prompt).
const SWITCH_TO_PS1_ESCAPE_SEQUENCE: &[u8] = &[escape_sequences::C0::ESC, b'p'];
/// Used to let the shell know we are switching to the Rook prompt via a bindkey \ew. This will
/// unset the PS1 to ensure we don't have a double prompt (PS1 and Rook prompt).
const SWITCH_TO_ROOK_PROMPT_ESCAPE_SEQUENCE: &[u8] = &[escape_sequences::C0::ESC, b'w'];
...
impl<T: EventLoopSender> PtyController<T> {
    pub fn new(
        ...
        ctx.subscribe_to_model(&model_event_dispatcher, |me, _, event, ctx| match event {
            ...
            ModelEvent::HonorPS1OutOfSync => {
                // We force re-sync the PS1 state of Rook settings with the shell's environment variable, $ROOK_HONOR_PS1, via
                ...
                if honor_ps1 {
                    ...
                    me.send_switch_to_rook_prompt_bindkey(ctx);
                }
            }
    ...
    /// future restoration).
    pub fn send_switch_to_rook_prompt_bindkey(&mut self, ctx: &mut ModelContext<Self>) {
        self.pending_writes.push_back(PtyWrite::Bytes {
            bytes: SWITCH_TO_ROOK_PROMPT_ESCAPE_SEQUENCE.into(),
        });
        self.execute_next_queued_write(ctx);
    ...
    fn write_bootstrap_script_to_shell(
        ...
        if bootstrap::should_use_rc_file_bootstrap_method(shell_type, pending_session_info) {
            ...
            for (i, chunk) in chunks.into_iter().enumerate() {
                ctx.spawn(
                    rookui::r#async::Timer::after(std::time::Duration::from_millis(i as u64 * 50)),
