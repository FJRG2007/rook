...
pub enum ReviewDestination {
    ...
    None,
    /// A Rook agent terminal is available (input box visible, not executing).
    Rook,
...
impl ReviewTerminalUnavailableReason {
    fn label(&self) -> &'static str {
        match self {
            ...
            Self::AIDisabled => "AI is disabled for Rook review destinations",
    ...
    pub fn new(ctx: &mut ViewContext<RightPanelView>) -> Self {
        CodeReviewState {
            dropdown: ctx.add_typed_action_view(|ctx| {
                ...
                dropdown.set_top_bar_height(rook_core::ui::icons::ICON_DIMENSIONS, ctx);
...
impl RightPanelView {
    ...
    /// considered available (non-CLI Rook terminals require AI to be on).
            ...
            .map(|tv| {
                tv.read(ctx, |t, ctx| {
                    t.active_cli_agent(ctx)
                        ...
                        .unwrap_or(ReviewDestination::Rook)
                })
            })
