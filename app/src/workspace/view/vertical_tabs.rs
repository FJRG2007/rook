...
use rook_core::ui::theme::{AnsiColorIdentifier, Fill as RookThemeFill, RookTheme};
use rookui::elements::{
...
use rookui::prelude::Align;
use rookui::text_layout::ClipConfig;
    ...
    let mut row = Hoverable::new(mouse_state, move |state| {
        ...
        if show_pin {
            let pin_icon = ConstrainedBox::new(
                RookIcon::PinFilledDiagonal
                    .to_rookui_icon(theme.sub_text_color(theme.background()))
                    .finish(),
            )
...
/// in the detail sidecar — CLI agent sessions with rich status, Rook Agent conversations, or
...
fn render_vertical_tab_hover_indicator(theme: &RookTheme) -> Box<dyn Element> {
    ConstrainedBox::new(
        Container::new(Empty::new().finish())
...
fn render_control_bar(
    ...
    let search_icon = ConstrainedBox::new(RookIcon::Search.to_rookui_icon(sub_text).finish())
        ...
        TypedPane::Terminal(terminal_pane) => {
            ...
            if let Some(icon) = cli_agent_session.and_then(|session| session.agent.icon()) {
                let color = cli_agent_session
                    ...
                    .map(RookThemeFill::Solid)
                    .unwrap_or_else(|| theme.accent());
                return icon.to_rookui_icon(color).finish();
            }

            let icon = if terminal_view.is_ambient_agent_session(app) {
                RookIcon::CloudFilled
            } else if terminal_view
                .selected_conversation_display_title(app)
                .is_some()
            {
                // Local agent conversation: use the Rook agent logo glyph to
            ...
            let color = match icon {
                RookIcon::CloudFilled => theme.main_text_color(theme.background()),
                ...
                RookIcon::Agent => theme.main_text_color(theme.background()),
                RookIcon::Terminal => disabled_text,
                _ => sub_text,
            };
            icon.to_rookui_icon(color).finish()
        }
        TypedPane::Code(_) => icon_from_file_path(&props.title, appearance)
            .unwrap_or_else(|| RookIcon::Code2.to_rookui_icon(sub_text).finish()),
        typed => {
            let fill = typed
                .rook_drive_object_type()
                .map(|object_type| {
                    RookThemeFill::Solid(rook_drive_icon_color(appearance, object_type))
                })
                .unwrap_or(sub_text);
            typed.icon().to_rookui_icon(fill).finish()
        }
    }
}
    ...
    let button = Hoverable::new(
        ...
        move |hover_state| {
            let icon = ConstrainedBox::new(
                RookIcon::Settings
                    .to_rookui_icon(if is_popup_open { main_text } else { sub_text })
                    .finish(),
            )
            .with_width(16.)
            .with_height(16.)
...
fn render_group_action_buttons(
    ...
    let kebab_button = Hoverable::new(kebab_mouse_state, move |button_state| {
        let mut container = Container::new(
            ConstrainedBox::new(RookIcon::DotsVertical.to_rookui_icon(meta_color).finish())
    ...
    let close_button = Hoverable::new(close_mouse_state, move |button_state| {
        let mut container = Container::new(
            ConstrainedBox::new(RookIcon::X.to_rookui_icon(meta_color).finish())
                .with_width(GROUP_ACTION_BUTTON_ICON_SIZE)
                .with_height(GROUP_ACTION_BUTTON_ICON_SIZE)
                .finish(),
        )
...
fn render_tab_group_header_icon_button(
    icon: RookIcon,
    icon_size: f32,
    icon_color: RookThemeFill,
    hover_background: RookThemeFill,
    mouse_state: MouseStateHandle,
...
fn render_grouped_tabs_header(
    ...
    let mut hoverable = Hoverable::new(mouse_states.header.clone(), move |state| {
        let border_fill = if is_header_selected {
            ...
            RookThemeFill::Solid(ColorU::transparent_black())
        };
...
fn resolve_icon_with_status_variant(
    ...
    let drive_color = |object_type: DriveObjectType| -> RookThemeFill {
        RookThemeFill::Solid(rook_drive_icon_color(appearance, object_type))
    };
    ...
    match typed {
        ...
        // Rook Drive object types use their established index colors
...
fn render_title_indicator(theme: &RookTheme) -> Box<dyn Element> {
    ConstrainedBox::new(
        RookIcon::CircleFilled
            .to_rookui_icon(theme.accent())
            .finish(),
    )
...
fn render_synced_inputs_indicator() -> Box<dyn Element> {
    ConstrainedBox::new(
        UiIcon::LinkHorizontal
            .to_rookui_icon(ColorU::from_u32(TAB_INDICATOR_SYNCED_COLOR).into())
            .finish(),
    )
...
impl TypedPane<'_> {
    ...
    fn rook_drive_object_type(&self) -> Option<DriveObjectType> {
        typed_pane_rook_drive_object_type(self)
    }

    fn supports_vertical_tabs_detail_sidecar(&self) -> bool {
        matches!(self, TypedPane::Terminal(_) | TypedPane::Code(_))
            || self.rook_drive_object_type().is_some()
    }
    ...
    fn icon(&self) -> RookIcon {
        match self {
            TypedPane::Terminal(_) => RookIcon::Terminal,
            TypedPane::Code(_) => RookIcon::Code2,
            TypedPane::CodeDiff => RookIcon::Diff,
            TypedPane::File => RookIcon::File,
            TypedPane::Notebook { is_plan: true } => RookIcon::Compass,
            TypedPane::Notebook { is_plan: false } => RookIcon::Notebook,
            TypedPane::Workflow { is_ai_prompt: true } => RookIcon::Prompt,
            TypedPane::Workflow {
                is_ai_prompt: false,
            } => RookIcon::Workflow,
            TypedPane::Settings | TypedPane::EnvironmentManagement => RookIcon::Gear,
            TypedPane::EnvVarCollection => RookIcon::EnvVarCollection,
            TypedPane::AIFact => RookIcon::BookOpen,
            TypedPane::AIDocument => RookIcon::Compass,
            TypedPane::ExecutionProfileEditor => RookIcon::Lightning,
            TypedPane::Other => RookIcon::File,
        }
    }
}
...
fn pane_display_title_and_subtitle(
    ...
    if matches!(typed, TypedPane::Code(_)) && !title.is_empty() {
        ...
        let parent = rook_util::path::user_friendly_path(&parent_raw, home_str).to_string();
        (filename, parent)
    } else {
        (
            if title.is_empty() {
...
fn render_git_branch_text(
    ...
    Flex::row()
        ...
        .with_spacing(2.)
        .with_child(
            ConstrainedBox::new(UiIcon::GitBranch.to_rookui_icon(text_color).finish())
...
pub(super) fn render_summary_pane_kind_icon_circle(
    ...
    let theme = appearance.theme();
    // Route all Rook agent kinds, plus ambient CLI agents, through
    ...
    let (icon_element, background): (Box<dyn Element>, ElementFill) = match kind {
        ...
        SummaryPaneKind::CLIAgent { agent, .. } => {
            ...
            let icon_element = agent
                .icon()
                .map(|icon| {
                    icon.to_rookui_icon(RookThemeFill::Solid(icon_color))
                        .finish()
                })
                .unwrap_or_else(|| {
                    RookIcon::Terminal
        ...
        | SummaryPaneKind::Other => {
            ...
            (
                icon.to_rookui_icon(icon_color).finish(),
                internal_colors::fg_overlay_2(theme).into(),
            )
        }
    };
...
/// Maps Rook agents and ambient CLI agents to the shared icon-with-status renderer.
...
fn summary_pane_kind_icon(
    ...
    match kind {
        SummaryPaneKind::Terminal => (RookIcon::Terminal, main_text),
        ...
        SummaryPaneKind::OzAgent { .. } => (RookIcon::Agent, main_text),
        SummaryPaneKind::CLIAgent { agent, .. } => (
            agent.icon().unwrap_or(RookIcon::Terminal),
            RookThemeFill::Solid(agent.brand_icon_color()),
        ),
        SummaryPaneKind::Code { .. } => (RookIcon::Code2, sub_text),
        SummaryPaneKind::CodeDiff => (RookIcon::Diff, sub_text),
        SummaryPaneKind::File => (RookIcon::File, sub_text),
        ...
        SummaryPaneKind::Settings | SummaryPaneKind::EnvironmentManagement => {
            (RookIcon::Gear, main_text)
        }
        SummaryPaneKind::EnvVarCollection => (
            RookIcon::EnvVarCollection,
            drive_color(DriveObjectType::EnvVarCollection),
        ),
        SummaryPaneKind::AIFact => (RookIcon::BookOpen, drive_color(DriveObjectType::AIFact)),
        SummaryPaneKind::AIDocument => (RookIcon::Compass, sub_text),
        SummaryPaneKind::ExecutionProfileEditor => (RookIcon::Lightning, sub_text),
        SummaryPaneKind::Other => (RookIcon::File, sub_text),
    }
}
...
/// 2. Rook Agent conversation title + status
/// 3. Terminal title
            ...
            .with_child(
                ConstrainedBox::new(
                    UiIcon::AlertTriangle
                        .to_rookui_icon(error_color.into())
                        .finish(),
                )
                .with_width(BADGE_ICON_SIZE)
                .with_height(BADGE_ICON_SIZE)
...
fn render_pull_request_badge_content(label: &str, appearance: &Appearance) -> Box<dyn Element> {
    ...
    Flex::row()
        ...
        .with_spacing(4.)
        .with_child(
            ConstrainedBox::new(UiIcon::Github.to_rookui_icon(main_text_color).finish())
                .with_width(BADGE_ICON_SIZE)
                .with_height(BADGE_ICON_SIZE)
                .finish(),
        )
                ...
                _ => {
                    match pane_group.code_view_from_pane_id(pane_id, app) {
                        Some(code_view) => {
                            ...
                            // matching against configured directories, so e.g. rook-internal/code.rs
                            // inherits the color assigned to rook-internal.
                            code_view
                                .as_ref(app)
                                .local_path(app)
                                .as_deref()
...
pub(super) fn render_settings_popup(
    ...
    let make_divider = |theme: &RookTheme| {
        Container::new(
            ConstrainedBox::new(
                Container::new(Empty::new().finish())
                    .with_background(internal_colors::fg_overlay_2(theme))
...
fn render_compact_subtitle_option(
    ...
    Hoverable::new(mouse_state, move |hover_state| {
        let check_icon: Box<dyn Element> = if is_selected {
            ConstrainedBox::new(RookIcon::Check.to_rookui_icon(main_text).finish())
                .with_width(ICON_SIZE)
                .with_height(ICON_SIZE)
                .finish()
        } else {
    ...
    Hoverable::new(mouse_state, move |hover_state| {
            ...
            (info_mouse_state.clone(), info_tooltip_text.clone())
        {
            ...
            let info_icon = Hoverable::new(info_ms, move |info_hover| {
                let icon = ConstrainedBox::new(UiIcon::Info.to_rookui_icon(info_color).finish())
...
struct DetailSidecarTextColors {
    main: RookThemeFill,
    sub: RookThemeFill,
    disabled: RookThemeFill,
}

fn detail_sidecar_background(theme: &RookTheme) -> ColorU {
    theme
        .background()
...
fn detail_sidecar_border_fill(theme: &RookTheme) -> ThemeFill {
    theme
        .background()
        .blend(&internal_colors::fg_overlay_4(theme))
}

fn detail_sidecar_text_colors(theme: &RookTheme) -> DetailSidecarTextColors {
...
fn render_detail_status_pill(
    ...
    Container::new(
        Flex::row()
            ...
            .with_spacing(4.)
            .with_child(
                ConstrainedBox::new(icon.to_rookui_icon(RookThemeFill::Solid(color)).finish())
                    .with_width(12.)
                    .with_height(12.)
                    .finish(),
            )
            .with_child(
                Text::new_inline(status.to_string(), appearance.ui_font_family(), 10.)
                    .with_color(RookThemeFill::Solid(color).into())
                    .finish(),
            )
            .finish(),
    )
...
fn render_rook_drive_object_detail_section(
    props: &PaneProps<'_>,
    appearance: &Appearance,
    app: &AppContext,
) -> Box<dyn Element> {
...
fn typed_pane_rook_drive_object_type(typed: &TypedPane<'_>) -> Option<DriveObjectType> {
    match typed {
...
fn render_detail_section(
    ...
    match &props.typed {
        ...
        | TypedPane::EnvVarCollection
        | TypedPane::AIFact
        | TypedPane::AIDocument => render_rook_drive_object_detail_section(props, appearance, app),
        TypedPane::CodeDiff
        | TypedPane::File
        | TypedPane::Settings
        | TypedPane::EnvironmentManagement
