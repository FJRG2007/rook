...
const HERO_IMAGE_PATH: &str = "async/png/onboarding/openrook_launch_banner.png";
const REPO_URL: &str = "https://github.com/FJRG2007/rook";
const CONTRIBUTING_URL: &str = "https://github.com/FJRG2007/rook/blob/master/CONTRIBUTING.md";
const OZ_URL: &str = "https://oz.rook.dev";
...
const FEATURE_ITEMS: &[FeatureItem] = &[
    FeatureItem {
        icon: Icon::HeartHand,
        title: "Contribute",
        description: "Rook's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.",
    ...
    FeatureItem {
        ...
        description: "The Rook repo is managed by an agent-first workflow powered by Oz, our cloud agent orchestration platform.",
        inline_link: Some(InlineLink {
            text: "Oz",
            url: OZ_URL,
        }),
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings([FixedBinding::new(
        "escape",
        OpenRookLaunchModalAction::Close,
        id!(OpenRookLaunchModal::ui_name()),
    )]);
}

#[derive(Clone, Debug)]
pub enum OpenRookLaunchModalAction {
    Close,
    VisitRepo,
}

#[derive(Clone, Debug)]
pub enum OpenRookLaunchModalEvent {
    Close,
}
...
impl OpenRookLaunchModal {
    ...
    fn render_title(appearance: &Appearance) -> Box<dyn Element> {
        Text::new("Rook is now open-source", appearance.ui_font_family(), 20.)
    ...
    fn render_description(appearance: &Appearance) -> Box<dyn Element> {
        Text::new(
            "You, our community, can participate in building Rook using an agent-first workflow.",
...
impl Entity for OpenRookLaunchModal {
    type Event = OpenRookLaunchModalEvent;
}

impl View for OpenRookLaunchModal {
    fn ui_name() -> &'static str {
        "OpenRookLaunchModal"
    }

    fn on_focus(&mut self, _focus_ctx: &rookui::FocusContext, ctx: &mut ViewContext<Self>) {
        ctx.focus_self();
    }
...
impl TypedActionView for OpenRookLaunchModal {
    type Action = OpenRookLaunchModalAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            OpenRookLaunchModalAction::Close => {
                ctx.emit(OpenRookLaunchModalEvent::Close);
            }
            OpenRookLaunchModalAction::VisitRepo => {
