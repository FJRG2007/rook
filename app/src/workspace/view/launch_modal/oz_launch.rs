...
impl Slide for OzLaunchSlide {
    ...
    fn title(&self) -> &'static str {
        match self {
            ...
            OzLaunchSlide::LaunchCredits => {
                "1,000 free cloud agent credits when you upgrade to Rook Build"
            }
        }
    }
    ...
    fn content(&self) -> &'static str {
        match self {
            ...
            OzLaunchSlide::AgentManagement => {
                "View all of your agents across local and cloud sessions in the Rook app or at [oz.rook.dev](https://oz.rook.dev). Join live agent sessions, continue tasks locally, and steer agents with one click."
            }
            OzLaunchSlide::LaunchCredits => {
                "Upgrade to Build this month and receive 1,000 extra credits to try using Oz. Credits are only eligible for Oz runs in Rook-hosted cloud environments."
            }
        }
    }
    ...
    fn on_close(&self, ctx: &mut rookui::ViewContext<super::LaunchModal<Self>>) {
        ctx.dispatch_typed_action(&WorkspaceAction::StartAgentOnboardingTutorial(
            OnboardingTutorial::NoProject {
                intention: OnboardingIntention::AgentDrivenDevelopment,
            },
...
pub fn init(app: &mut rookui::AppContext) {
