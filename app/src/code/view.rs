...
use crate::notebooks::file::{MarkdownDisplayMode, renders_in_rook_notebook_viewer};
...
impl CodeView {
    ...
    fn update_markdown_mode_segmented_control(&mut self, ctx: &mut ViewContext<Self>) {
        let renders_in_notebook_viewer = self
            ...
            .map(|loc| renders_in_rook_notebook_viewer(std::path::Path::new(&loc.display_path())))
            .unwrap_or(false);
    ...
    fn overflow_menu_items(&self, ctx: &AppContext) -> Vec<MenuItem<CodeViewAction>> {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            let renders_in_notebook_viewer = local_path
                .as_ref()
                .map(renders_in_rook_notebook_viewer)
                .unwrap_or_else(|| {
                    active_location
                        .map(|loc| {
                            renders_in_rook_notebook_viewer(std::path::Path::new(
                                &loc.display_path(),
                            ))
                        })
                        .unwrap_or(false)
