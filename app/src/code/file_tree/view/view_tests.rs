...
fn std_path(path: &std::path::Path) -> rook_util::standardized_path::StandardizedPath {
    rook_util::standardized_path::StandardizedPath::try_from_local(path).unwrap()
}
...
fn build_repo_state(repo_root: &std::path::Path) -> FileTreeState {
    ...
    let src_dir = Entry::Directory(DirectoryEntry {
        path: rook_util::standardized_path::StandardizedPath::try_from_local(
...
fn flattened_paths(
    view: &FileTreeView,
    root: &std::path::Path,
) -> Vec<rook_util::standardized_path::StandardizedPath> {
...
fn repo_transition_unregisters_lazy_loaded_path() {
    VirtualFS::test("file_tree_repo_transition", |dirs, mut vfs| {
        ...
        let canonical_repo_root =
            rook_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo_root)
                .unwrap();
            ...
            file_tree_view.read(&app, |view, _ctx| {
                assert!(
                    view.registered_lazy_loaded_paths.contains(
                        &rook_util::standardized_path::StandardizedPath::try_from_local(
                            &displayed_root
                        )
                        .unwrap()
                    )
                ...
                let displayed_std =
                    rook_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                assert_eq!(
                    view.root_directories
                        .get(&displayed_std)
            ...
            file_tree_view.read(&app, |view, _ctx| {
                let displayed_std =
                    rook_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                let repo_std =
                    rook_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &rook_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &rook_util::standardized_path::StandardizedPath::try_from_local(&src_dir)
                        .unwrap(),
                    ctx,
                );
            });
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &rook_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &rook_util::standardized_path::StandardizedPath::try_from_local(&nested_dir)
                        .unwrap(),
                    ctx,
                );
            });
...
fn click_on_file_under_absorbed_descendant_keeps_file_selected() {
    ...
    VirtualFS::test(
        ...
        |dirs, mut vfs| {
            vfs.mkdir("code/rook-server")
                .with_files(vec![Stub::FileWithContent(
                    "code/rook-server/main.rs",
                    "fn main() {}\n",
                )]);
            let code = dirs.tests().join("code");
            let rook_server = code.join("rook-server");
            let main_rs = rook_server.join("main.rs");
            ...
            App::test((), |mut app| async move {
                ...
                // Seed with `code` as the only root and expand rook-server so
                ...
                file_tree_view.update(&mut app, |view, ctx| {
                    view.toggle_folder_expansion(&std_path(&code), &std_path(&rook_server), ctx);
                });
                await_directory_loaded(&mut app, &repository_metadata_model, &code, &rook_server)
                    .await;
                ...
                // the file's repo/parent (rook-server) to the active set.
                file_tree_view.update(&mut app, |view, ctx| {
                    view.set_root_directories(vec![rook_server.clone(), code.clone()], ctx);
                });

                file_tree_view.read(&app, |view, _ctx| {
                    // Selection is still on main.rs, not on rook-server.
...
fn pending_focus_target_does_not_re_scroll_after_first_apply() {
    ...
    VirtualFS::test("file_tree_pending_respects_user_scroll", |dirs, mut vfs| {
        vfs.mkdir("tree/rook-server")
            .with_files(vec![Stub::FileWithContent(
                "tree/rook-server/main.rs",
                "fn main() {}\n",
            )]);
        let tree = dirs.tests().join("tree");
        let rook_server = tree.join("rook-server");
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.set_is_active(true, ctx);
                view.set_root_directories(vec![rook_server.clone(), tree.clone()], ctx);
            });
            await_repository_indexed(&mut app, &repository_metadata_model, &tree).await;
            ...
            // should still land on rook-server, but `scrolled` must stay
            // true (no re-scroll).
            ...
            file_tree_view.read(&app, |view, _ctx| {
                ...
                let root_dir = view.root_directories.get(&std_path(&tree)).unwrap();
                let path = root_dir.items.get(selected.index).unwrap().path();
                assert_eq!(path, &std_path(&rook_server));
                let pending = view.pending_focus_target.as_ref().expect("pending");
...
fn focus_follows_absorbed_descendant_once_its_item_is_materialized() {
    VirtualFS::test("file_tree_focus_follow_deferred", |dirs, mut vfs| {
        ...
        App::test((), |mut app| async move {
            ...
            // User cd's into rook-server with ~/tree as the ancestor root.
            // The rook-server entry should be materialized by indexing and
            ...
            file_tree_view.read(&app, |view, _ctx| {
                ...
                // Selection landed on rook-server's directory header.
                let selected = view.selected_item.clone().expect("selection set");
                ...
                assert_eq!(selected_item.path(), &std_path(&rook_server));
                ...
                assert_eq!(pending.path, std_path(&rook_server));
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                ...
                // Sanity: the first item is the root header, not rook-server.
                assert_ne!(
                    root_dir.items.first().unwrap().path(),
                    &std_path(&rook_server)
                );
                view.select_id(&id, ctx);
            });
