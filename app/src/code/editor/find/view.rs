...
use rook_editor::search::{SearchEvent, Searcher};
...
pub use rookui::geometry::vector::vec2f;
...
impl CodeEditorFind {
    ...
    pub fn emit_result_a11y_content(&mut self, ctx: &mut ViewContext<Self>) {
        let content = if let Some(match_index) = self.searcher.as_ref(ctx).selected_match() {
            ...
            AccessibilityContent::new_without_help("No results.", RookA11yRole::UserAction)
        };
    ...
    fn render_next_match_button(
        ...
        Container::new(
            ConstrainedBox::new(match_icon.to_rookui_icon(icon_color).finish())
                .with_height(height)
                .with_width(height)
                .finish(),
        )
    ...
    fn render_search_button(
        ...
        Container::new(
            ConstrainedBox::new(Icon::Search.to_rookui_icon(icon_color).finish())
                .with_height(height)
                .with_width(height)
                .finish(),
        )
