...
//! directory (`~/.rook-preview`) on macOS.
//!
//! Historically, Stable and Preview shared `~/.rook` on macOS. To give
//! Preview its own directory without breaking existing users, this migration
//! symlinks each top-level entry from `~/.rook` into `~/.rook-preview` on
...
use rook_core::paths::{ROOK_CONFIG_DIR, data_dir};
...
/// Migrates Preview's config directory from the shared `.rook` location to
/// `.rook-preview` by creating symlinks from each top-level entry in `.rook`
...
pub(crate) fn migrate_preview_config_dir_if_needed() {
    ...
    let old_dir = home.join(ROOK_CONFIG_DIR);
...
pub(crate) fn migrate_config_dir_via_symlinks(old_dir: &Path, new_dir: &Path) {
    ...
    // migration itself or by ensure_rook_watch_roots_exist on a subsequent
