...
use rookui::text_selection_utils::{
...
impl EditorElement {
    ...
    fn draw_cursors(
        ...
        for cursor in cursors {
                    ...
                    remote_selections_data.get_mut(&cursor.replica_id)
            {
                ...
                ctx.scene.start_layer(rookui::ClipBounds::None);
    ...
    pub fn with_input_editor_icons(
        ...
        let cycle_next_command_hint = if self.should_show_cycle_next_command_hint(is_cycling, ctx) {
            ...
            Some(
                self.render_cycle_next_command_hint(rook_core::ui::theme::Fill::Solid(
    ...
    fn render_cycle_next_command_hint(&self, color: Fill) -> Box<dyn Element + 'static> {
        ...
        Flex::row()
            ...
            .with_children([
                Container::new(
                    ConstrainedBox::new(Icon::ArrowDown.to_rookui_icon(color).finish())
