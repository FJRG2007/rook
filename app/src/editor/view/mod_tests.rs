    ...
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor = EditorView::new_with_base_text(
                "word ~/.rook/themes/foo-bar.yaml thing",
                Default::default(),
                ctx,
            );
            editor.select_word(&DisplayPoint::new(0, 8), ctx);
            assert_eq!(editor.selected_text(ctx), "~/.rook/themes/foo-bar.yaml");
            editor
        });
    });
}
...
fn test_smart_select_with_drag() {
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor = EditorView::new_with_base_text(
                "word ~/.rook/themes/foo-bar.yaml andy@rook.dev",
                Default::default(),
                ctx,
            );
            editor.select_word(&DisplayPoint::new(0, 8), ctx);
...
fn test_autosuggestions() -> Result<()> {
    use rookui::text_layout::LayoutCache;
...
fn test_add_next_occurrence() {
    ...
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor =
                EditorView::new_with_base_text("rookwordrookwordrook", Default::default(), ctx);
            editor
            ...
            editor.undo(ctx);
            assert_eq!(editor.buffer_text(ctx), "rookwordrookwordrook");
            editor.redo(ctx);
            assert_eq!(editor.buffer_text(ctx), "wordword");
...
fn test_autocomplete_symbols() {
    ...
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            let mut editor =
                EditorView::new_with_base_text("word rook word", Default::default(), ctx);
            ...
            editor.add_next_occurrence(ctx);
            editor.user_insert("[", ctx);
            assert_eq!(editor.buffer_text(ctx), "[word] rook [word]");
    ...
    App::test((), |mut app| async move {
        ...
        app.add_window(WindowStyle::NotStealFocus, |ctx| {
            ...
            let mut editor = EditorView::new_with_base_text("word rook word", options, ctx);
...
fn test_paste_clipboard_with_text_only_should_paste_text_normally() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let clipboard_content = rookui::clipboard::ClipboardContent {
...
fn test_paste_clipboard_with_image_only_should_switch_to_agent_mode() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let png_image = rookui::clipboard::ImageData {
...
fn test_paste_clipboard_with_unsupported_image_and_text_should_show_error() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let bmp_image = rookui::clipboard::ImageData {
    ...
    App::test((), |mut app| async move {
        ...
        view.update(&mut app, |view, ctx| {
            ...
            view.set_drag_drop_path_transformer(Some(Box::new(
                rook_util::path::convert_windows_path_to_wsl,
            )));
            view.drag_and_drop_files(&paths(), ctx);
        ...
        view.update(&mut app, |view, ctx| {
            view.clear_buffer(ctx);
            view.set_drag_drop_path_transformer(Some(Box::new(
                rook_util::path::convert_windows_path_to_msys2,
            )));
            view.drag_and_drop_files(&paths(), ctx);
