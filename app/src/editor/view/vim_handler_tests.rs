...
fn test_vim_number_repeat_line_motion() {
    ...
    App::test((), |mut app| async move {
        ...
        let mut presenter = rookui::presenter::Presenter::new(window_id);

        let mut updated = EntityIdSet::default();
        updated.insert(app.root_view_id(window_id).unwrap());
        let invalidation = rookui::WindowInvalidation {
            updated,
            ..Default::default()
        };
...
fn test_vim_delete_word_dge() {
    App::test((), |mut app| async move {
        ...
        let editor = add_editor_vim_normal_mode("echo hello-hi rook-dev", &mut app);
        ...
        editor.read(&app, |view, app_ctx| {
            assert_eq!(view.buffer_text(app_ctx), "echo hello-hi rookev");
            assert_eq!(
                view.selected_ranges(app_ctx),
