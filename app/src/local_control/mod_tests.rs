...
fn tab_create_requires_active_window() {
    let active = rookui::WindowId::from_usize(1);
...
fn window_title_resolution_distinguishes_missing_and_ambiguous_targets() {
    ...
    let matches = [
        rookui::WindowId::from_usize(1),
        rookui::WindowId::from_usize(2),
    ];
...
fn feature_flag_disabled_denies_local_control() {
    let _flag = FeatureFlag::RookControlCli.override_enabled(false);
...
fn disabling_scripting_invalidates_existing_grant_and_prevents_new_grants() {
    let _flag = FeatureFlag::RookControlCli.override_enabled(true);
    rookui::App::test((), |mut app| async move {
