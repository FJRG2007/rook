//! Running app-side server for local Rook control requests.
//!
//! This module owns the in-process listener, discovery registration, credential
//! broker socket, and request handoff from Axum into the RookUI model graph.
...
//! Rook's secure storage provider.
//!
...
/// Process-local publisher, credential broker, and HTTP server for one Rook instance.
///
...
impl LocalControlServer {
    ...
    fn refresh_for_settings(&mut self, ctx: &mut ModelContext<Self>) -> Result<(), ControlError> {
        if !permissions::rook_control_cli_enabled() {
            self.stop(ctx);
            return Ok(());
        }
...
#[cfg(unix)]
/// Requires the kernel-reported peer UID to match Rook's effective UID.
///
/// This excludes other OS users but does not distinguish trusted Rook code from
