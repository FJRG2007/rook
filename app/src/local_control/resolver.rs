...
use rookui::{AppContext, ModelContext, TypedActionView, ViewHandle, WindowId};
...
pub(crate) fn require_active_window_id_for_action(
    ...
    active_window.ok_or_else(|| {
        ControlError::new(
            ...
            format!("{} requires an active Rook window", action.as_str()),
        )
    })
}
...
fn active_or_single_window_id(
    ...
    match window_ids.as_slice() {
        ...
        _ => Err(ControlError::new(
            ...
            format!(
                "{} requires an explicit window selector when no Rook window is active",
                action.as_str()
            ),
        )),
    }
