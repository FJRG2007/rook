...
use crate::drive::{OpenRookDriveObjectArgs, OpenRookDriveObjectSettings};
...
use crate::util::openable_file_type::{
    is_file_openable_in_rook, is_markdown_file, is_runnable_shell_script,
    renders_in_rook_notebook_viewer, starts_with_shebang,
};
...
/// Args for the `rook://settings` deeplink family, dispatched to the
/// `root_view:open_settings_in_{existing,new}_window` actions.
pub enum OpenSettingsArgs {
    /// `rook://settings` — open a settings tab on the default page.
    Default,
    /// `rook://settings?q=<query>` — open settings with the search bar pre-filled.
    Search { query: String },
    /// `rook://settings?widget=<widget_id>` — open settings scrolled to a widget.
    Widget {
...
pub enum UriHost {
    ...
    Launch,
    /// Supports joining shared sessions via a rook:// URI.
    SharedSession,
    /// Supports viewing AI conversations via a rook:// URI.
    Conversation,
    /// Supports WD object actions
    Drive,
    /// Supports opening rook's settings panel via URI
    Settings,
    ...
    fn handle(&self, primary_window_id: Option<WindowId>, url: &Url, ctx: &mut AppContext) {
        ...
        match self {
            ...
            UriHost::Conversation => {
                ...
                // e.g. rook://conversation/{conversation_id}
            ...
            UriHost::Drive => {
                ...
                // e.g. rook://drive/{object_type}?id={UID}
                ...
                if let Some((object_type, server_id)) = object_type.zip(object_server_id) {
                    ...
                    let args = OpenRookDriveObjectArgs {
                        object_type,
                        server_id,
                        settings: OpenRookDriveObjectSettings {
                            focused_folder_id,
                            invitee_email,
                        },
                    };
                    ...
                    if let Some((primary_window_id, root_view_id)) = primary_window_and_view {
                        ...
                        // debug representation to `rook.log` on non-dogfood
                        // release channels.
                        safe_info!(
                            safe: (
            ...
            UriHost::Settings => {
                ...
                // - rook://settings - opens a settings tab on the default page
                // - rook://settings?q={query} - opens settings with the search bar pre-filled
                // - rook://settings?widget={widget_id} - opens settings scrolled to a widget
                // - rook://settings/teams?invite={email} - opens team settings with invite modal
                // - rook://settings/billing_and_usage - opens billing and usage settings page
                // - rook://settings/environments - opens environments settings page
                // - rook://settings/mcp - opens MCP servers settings page
                // - rook://settings/platform - opens platform settings page
                // - rook://settings/appearance - opens appearance settings page (themes, fonts, etc.)
                // - rook://settings/rook_agent - opens the Rook Agent settings page (inference / API keys)
                let query_string: HashMap<_, _> = url.query_pairs().collect();
                // A bare `rook://settings` (or a trailing slash) yields an empty path
                ...
                match settings_sub_page.as_deref() {
                    ...
                    Some("mcp") => {
                        // rook://settings/mcp?autoinstall=<name> auto-installs a gallery MCP server.
                    ...
                    // rook_agent) resolved via `settings_section_for_simple_subpage`.
                    maybe_simple_subpage => {
                        ...
                        if let Some((page, widget_id)) = widget_target {
                            ...
                            // Bare `rook://settings` opens the default settings page.
...
/// Handles `rook://tab_config/<name>` deeplinks.
///
/// Resolution rules:
/// - `<name>` is matched case-insensitively against each tab config's file
///   stem, so both `rook://tab_config/my_tab` and
///   `rook://tab_config/my_tab.toml` work.
/// - When `?new_window=true` (or no Rook window is open) the tab config opens
    ...
    fn window_behavior_hint(&self) -> WindowBehaviorHint {
        ...
        match self {
            ...
            Self::NewTab => W::ShowPrimaryWindow(WindowActivationFallbackBehavior::Notify {
                ...
                description: "Go to Rook to see your new tab.".to_owned(),
            }),
...
enum OpenFileAction {
    ...
    Notebook,
    /// Open in Rook's code/text editor pane.
    Editor,
...
/// file URLs to Rook via the file type registration in `Info.plist`. Since Rook
...
/// Markdown file to the in-Rook notebook viewer. Other notebook viewer formats,
...
fn classify_open_file_action(path: &Path, prefer_markdown_viewer: bool) -> OpenFileAction {
    if renders_in_rook_notebook_viewer(path) && (!is_markdown_file(path) || prefer_markdown_viewer)
    {
        OpenFileAction::Notebook
    } else if is_runnable_shell_script(path) {
        OpenFileAction::ExecuteInSession
    } else if path.is_file()
        && (is_file_openable_in_rook(path).is_some() || starts_with_shebang(path))
    {
...
fn can_open_file_editor_path(path: &Path) -> bool {
    path.is_file() && is_file_openable_in_rook(path).is_some()
}
...
fn open_file(window_id: Option<WindowId>, path: PathBuf, ctx: &mut AppContext) {
    ...
    if action == OpenFileAction::Notebook {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            use crate::util::file::external_editor::EditorSettings;
            use crate::util::openable_file_type::resolve_file_target_to_open_in_rook;

            // Open text/code files in Rook's code editor, respecting the user's layout preference.
            let editor_settings = EditorSettings::as_ref(ctx);
            let target = resolve_file_target_to_open_in_rook(&path, editor_settings, None);
...
fn settings_section_for_simple_subpage(subpage: &str) -> Option<SettingsSection> {
    match subpage {
        ...
        "platform" => Some(SettingsSection::RookCloudAgentAPIKeys),
        "appearance" => Some(SettingsSection::Appearance),
        "rook_agent" => Some(SettingsSection::RookAgent),
        _ => None,
    }
}
