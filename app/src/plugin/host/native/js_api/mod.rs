...
cfg_if::cfg_if! {
    if #[cfg(feature = "completions_v2")] {
        ...
        use rook_completer::signatures::CommandSignature;
        use rook_js::FromRookJs;
    }
}

/// Returns a JS object representing the Rook Plugin API exposed to external JavaScript plugins.
///
...
/// Returns a JS object representing the Completions namespace for the Rook Plugin API.
///
/// API methods:
///
        ...
        Function::new(
            ...
            MutFn::from(move |val: Value<'js>| {
                if val.is_array() {
                    ...
                    match Vec::<CommandSignature>::from_rook_js(
                        ctx,
                        val,
                    ...
                    let mut plugin = plugin.get_mut();
                    match CommandSignature::from_rook_js(
                        ctx,
                        val,
                        plugin.js_function_registry_mut(),
                    ) {
