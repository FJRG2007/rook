...
pub fn run() -> Result<()> {
    rookui::r#async::block_on(async move {
        let executor = Arc::new(Background::default());

        // Initialize a client connection to the rook app process.
        ...
        // Initialize logging, which internally transmits logs from this process to the rook app
        ...
        // Send the connection address for the plugin host server back to the rook app.
...
///
/// This assumes that all plugins are located in ~/.rook/plugins.
fn plugin_paths() -> Vec<PathBuf> {
    const PLUGIN_PATH_SUFFIX: &str = ".rook/plugins";
...
/// Returns `true` if the directory at the given path is directory containing JS source for a Rook plugin.
