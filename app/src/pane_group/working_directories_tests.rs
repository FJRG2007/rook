...
fn refresh_working_directories_collapses_subroots_to_nearest_repo_root() {
    App::test((), |mut app| async move {
        ...
        // Use dunce::canonicalize to match the behavior of rook_util::standardized_path::StandardizedPath and normalize_cwd,
...
fn setup_repo(
    app: &mut rookui::App,
    detected_repos: &rookui::ModelHandle<DetectedRepositories>,
) -> (
    tempfile::TempDir,
    PathBuf,
    PathBuf,
    rookui::ModelHandle<WorkingDirectoriesModel>,
) {
    ...
    detected_repos.update(app, |repos, _ctx| {
        let canonical = rook_util::standardized_path::StandardizedPath::from_local_canonicalized(
