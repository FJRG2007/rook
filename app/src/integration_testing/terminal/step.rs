...
use rookui::{Event, SingletonEntity, async_assert};
...
fn switch_to_terminal_mode_and_type_command(
    ...
    command: String,
) -> impl Fn(&mut rookui::App, rookui::WindowId) -> Event + 'static {
    move |app, window_id| {
...
fn restore_ai_mode_if_needed(
    ...
    pane_idx: usize,
    was_ai_mode: Arc<AtomicBool>,
) -> impl Fn(&mut rookui::App, rookui::WindowId) + 'static {
    move |app, window_id| {
...
fn execute_command_step(
    ...
    pane_idx: usize,
    command: String,
    validate_output_fn: impl FnMut(&mut rookui::App, rookui::WindowId) -> AssertionOutcome + 'static,
) -> TestStep {
