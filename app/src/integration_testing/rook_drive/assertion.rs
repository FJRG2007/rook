...
pub fn assert_rook_drive_is_open() -> AssertionCallback {
    Box::new(move |app, window_id| {
        let workspace = workspace_view(app, window_id);

        workspace.read(app, |workspace, _| {
            async_assert!(
                workspace.is_rook_drive_open(),
                "Expected Rook Drive to be open, but it was closed"
            )
        })
    })
}

pub fn assert_rook_drive_is_closed() -> AssertionCallback {
    Box::new(move |app, window_id| {
        let workspace = workspace_view(app, window_id);
        ...
        workspace.read(app, |workspace, _| {
            async_assert!(
                !workspace.is_rook_drive_open(),
                "Expected Rook Drive to be closed, but it was open"
            )
        })
    })
}
