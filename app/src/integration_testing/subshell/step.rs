...
use rookui::{async_assert, async_assert_eq};
...
pub fn assert_subshell_banner_is_showing() -> TestStep {
    TestStep::new("Assert the Rookify banner is visible")
        .add_assertion(move |app, window_id| {
            ...
            terminal_view.read(app, |view, _ctx| {
                async_assert!(matches!(
                    view.model
                        ...
                        .active_block()
                        .block_banner(),
                    Some(WithinBlockBanner::RookifyBanner(..))
                ))
            })
        })
        ...
        move |app, window_id| {
            ...
            terminal_view.read(app, |view, _ctx| {
                ...
                match rich_content_type {
                    Some(RichContentType::RookifySuccessBlock) => {}
                    _ => {
                        return AssertionOutcome::failure(
                            "Rookify success block wasn't added to the blocklist".to_owned(),
                        );
                    }
                }
