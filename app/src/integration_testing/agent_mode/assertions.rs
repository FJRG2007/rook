...
use rookui::integration::{AssertionCallback, AssertionOutcome, TestStep};
use rookui::{EntityId, SingletonEntity as _, integration_assert};
...
/// This is private because `AIAgentActionType` is not public outside the rook app crate
...
/// by regular file tools pointed at `/tmp/rook_conversation_search`.
        ...
        BlocklistAIHistoryModel::handle(app).update(app, |history_model, _| {
            ...
            for (exchange_index, exchange) in conversation.all_exchanges().into_iter().enumerate() {
                ...
                for action in output.get().actions() {
                    ...
                    if action_text.contains("/tmp/rook_conversation_search") {
                        return AssertionOutcome::immediate_failure(format!(
