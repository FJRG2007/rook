...
fn poll_run_until_joinable_session(
    ...
    async_stream::stream! {
        ...
        let mut timeout_timer = FutureExt::fuse(match timeout {
            Some(d) => rookui::r#async::Timer::after(d),
            None => rookui::r#async::Timer::never(),
        });
        ...
        loop {
            let mut poll_timer = FutureExt::fuse(rookui::r#async::Timer::after(TASK_STATUS_POLL_INTERVAL));
