...
pub enum BundledSkillActivation {
    ...
    TuiOnly,
    /// Active only when a specific Rook feature is enabled.
...
/// Skills bundled with Rook for a single host.
#[derive(Debug, Default)]
...
impl BundledSkill {
    /// Detect all skill definitions bundled with Rook for the local host.
    pub async fn detect() -> Self {
        let Some(resources_dir) = rook_core::paths::bundled_resources_dir() else {
            return Self::default();
        };
    ...
    /// is what [`rook_core::paths::bundled_resources_dir`] resolves).
...
/// Load skill definitions bundled with Rook.
...
///
/// Supported variables:
/// - `{{rook_server_url}}` - The server root URL (e.g., `https://api.rook.dev`)
/// - `{{rook_cli_binary_name}}` - The CLI binary name (e.g., `rook` or `rook-cli`)
/// - `{{rookctrl_binary_name}}` - The channel-specific Rook Control command name
/// - `{{rookctrl_wrapper_path}}` - Path to the bundled Rook Control wrapper
/// - `{{rook_url_scheme}}` - The URL scheme (e.g., `rook`, `rookdev`, `rookpreview`)
...
pub(crate) fn build_bundled_skill_context(
    ...
    [
        (
            "rook_server_url".to_owned(),
            ChannelState::server_root_url().into_owned(),
        ),
        (
            "rook_cli_binary_name".to_owned(),
            ChannelState::channel().cli_command_name().to_owned(),
        ),
        (
            "rookctrl_binary_name".to_owned(),
            ChannelState::channel().rookctrl_command_name().to_owned(),
        ),
        (
            "rookctrl_wrapper_path".to_owned(),
            resources_dir
                .join("bin")
        ...
        (
            ...
            display_optional_path(rook_core::paths::gui_mcp_config_file_path()),
        ),
        (
...
/// branded icon; everything else falls back to the Rook logo.
pub(crate) fn icon_for_bundled_skill(skill_id: &str) -> Icon {
    match skill_id {
        "pr-comments" => Icon::Github,
        _ => Icon::RookLogoLight,
    }
}
...
pub(crate) fn activation_for_bundled_skill(
    ...
    match skill_id {
        ...
        "rookctrl" => BundledSkillActivation::RequiresFeature(FeatureFlag::RookControlCli),
