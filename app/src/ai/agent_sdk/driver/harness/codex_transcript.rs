...
//!   Codex conversation: the Rook server conversation id to reuse, the codex session
...
pub(crate) struct CodexResumeInfo {
    /// Rook server-side conversation id. Reused so subsequent transcript/block-snapshot
