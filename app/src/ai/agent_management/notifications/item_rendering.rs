...
use rook_core::ui::appearance::Appearance as CoreAppearance;
...
use rookui::{View, ViewContext, ViewHandle};
...
pub(crate) type OnExpandClick = Box<dyn Fn(&mut rookui::EventContext)>;
...
fn render_branch_label(branch: &str, appearance: &Appearance) -> Box<dyn Element> {
    ...
    Shrinkable::new(
        ...
        Flex::row()
            ...
            .with_spacing(2.)
            .with_child(
                ConstrainedBox::new(Icon::GitBranch.to_rookui_icon(color).finish())
                    .with_width(10.)
                    .with_height(10.)
                    .finish(),
            )
...
fn render_expand_chevron(
    ...
    let chevron = ConstrainedBox::new(
        icon.to_rookui_icon(theme.disabled_text_color(theme.surface_1()))
            .finish(),
    )
    .with_width(12.)
    .with_height(12.)
