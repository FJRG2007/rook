...
impl ReadDocumentsExecutor {
    ...
    pub(super) fn execute(
        ...
        // A requested plan may live in Rook Drive without being loaded into this conversation's
        ...
        for id in document_ids {
            ...
            if document.is_none() {
                AIDocumentModel::handle(ctx).update(ctx, |model, ctx| {
                    if let Err(error) =
                        model.hydrate_saved_plan_from_rook_drive(*id, conversation_id, ctx)
                    {
                        log::warn!(
                            "Failed to hydrate requested plan document {id} from Rook Drive: {error}"
                        );
                    }
                });
