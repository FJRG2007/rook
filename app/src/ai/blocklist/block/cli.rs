...
use rookui::keymap::{EditableBinding, Keystroke};
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CLISubagentAction::OpenFeedbackDocs => {
                ctx.open_url("https://docs.rook.dev/support-and-billing/sending-us-feedback");
            }
        }
    }
}
...
fn render_action(action: AIAgentActionType, app: &AppContext) -> Option<Box<dyn Element>> {
    ...
    let icon = Container::new(
        ConstrainedBox::new(
            rookui::elements::Icon::new(icon.into(), internal_colors::neutral_5(theme)).finish(),
        )
...
fn render_web_search(query: Option<String>, app: &AppContext) -> Box<dyn Element> {
    ...
    let icon = Container::new(
        ConstrainedBox::new(
            rookui::elements::Icon::new(Icon::Search.into(), internal_colors::neutral_5(theme))
                .finish(),
        )
        .with_width(icon_size(app))
        .with_height(icon_size(app))
