...
fn test_detect_secrets_no_regexes_configured() {
    ...
    let text = "foo rook-server-staging.firebaseapp.com bar";
...
fn test_detect_secrets_multiple_secrets() {
    ...
    let text = "ABCD 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 foo baz rook-server-staging.firebaseapp.com bar \n foo sk_live_4eC39HqLyjWDarjtT1zdp7dc qux foo";
    let detected_secrets = find_secrets_in_text(text);
    assert_eq!(
        detected_secrets,
        vec![
