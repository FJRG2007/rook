...
#[test]
fn internal_rook_error_is_error() {
    assert_update(
        classify_renderable_error(&RenderableAIError::InternalRookError),
        AgentTaskState::Error,
...
fn map_conversation_status_waiting_for_events_reports_in_progress_with_no_message() {
    App::test((), |mut app| async move {
        ...
        let terminal_view_id = rookui::EntityId::new();
...
fn map_conversation_status_error_classifies_status_error_other_as_error() {
    ...
    conversation.set_status_error_for_test(Some(RenderableAIError::other(
        "Out of credits. Upgrade your Rook plan to continue running cloud agents.",
        false,
    )));
    assert_update(
        map_conversation_status(&conversation),
...
async fn pump_spawned_tasks() {
    for _ in 0..5 {
        rookui::r#async::Timer::after(Duration::from_millis(2)).await;
    }
}
...
fn install_model_with_call_counter(
    ...
    rookui::ModelHandle<LocalAgentTaskSyncModel>,
    Arc<AtomicUsize>,
) {
...
fn install_model_with_constant_result(
    ...
    rookui::ModelHandle<LocalAgentTaskSyncModel>,
    rookui::ModelHandle<CLIAgentSessionsModel>,
) {
    app.add_singleton_model(|_| BlocklistAIHistoryModel::new(vec![], vec![], &[]));
    let cli_sessions_model = app.add_singleton_model(|_| CLIAgentSessionsModel::new());
    let mut mock = MockAIClient::new();
...
fn emit_cli_status(
    cli_sessions_model: &rookui::ModelHandle<CLIAgentSessionsModel>,
    app: &mut App,
    terminal_view_id: rookui::EntityId,
