...
pub(super) fn parse_single_theme_dir_entry(item: &DirEntry) -> Option<(ThemeKind, RookTheme)> {
    parse_single_item_file(item, |file_name, mut theme: RookTheme| {
        ...
        let theme_kind = if let Some(name) = theme.name() {
            RookThemeConfig::file_to_theme(name, item.path().into())
        } else {
