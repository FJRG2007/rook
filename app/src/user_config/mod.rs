...
# https://docs.rook.dev/terminal/sessions/launch-configurations
#
...
#  - tabs:
#      - layout:
#          cwd: /Users/rook-user/project
#          commands:
#            - exec: code .
",
        rook_core::paths::home_relative_path(&crate::user_config::launch_configs_dir())
    );
}

#[derive(Clone)]
pub enum RookConfigUpdateEvent {
    Themes,
...
pub struct RookConfig {
    ...
    theme_config: RookThemeConfig,
    local_user_workflows: Vec<Workflow>,
    /// User-defined custom model routers loaded from `~/.rook/custom_model_routers/`.
    #[cfg_attr(target_family = "wasm", allow(dead_code))]
...
/// Platform-independent parts of RookConfig.
///
...
impl RookConfig {
    ...
    pub fn mock(_ctx: &mut ModelContext<Self>) -> Self {
        Self {
            theme_config: RookThemeConfig::new(),
            ..Default::default()
        }
    }
    ...
    pub fn theme_config(&self) -> &RookThemeConfig {
        &self.theme_config
    }
    ...
    /// Saving the newly created launch configuration to the RookConfig that we currently
    /// have.
    pub fn append_launch_config(
        ...
        if !self.launch_configs.contains(launch_config) {
            ...
            ctx.emit(RookConfigUpdateEvent::LaunchConfigs);
        }
    }
    ...
    pub fn update_theme_config(
        ...
        ctx.emit(RookConfigUpdateEvent::Themes);
    }
    ...
    pub fn remove_tab_config_by_path(&mut self, path: &Path, ctx: &mut ModelContext<Self>) {
        ...
        if self.tab_configs.len() != before {
            ctx.emit(RookConfigUpdateEvent::TabConfigs);
        }
    }
}
...
/// configs (`~/.rook/custom_model_routers/`). Each file defines a single router.
#[cfg_attr(target_family = "wasm", expect(dead_code))]
...
/// These are shipped with Rook and user-editable (Rook does not overwrite modifications).
#[cfg_attr(target_family = "wasm", expect(dead_code))]
pub fn default_tab_configs_dir() -> PathBuf {
    base_dir().join("default_tab_configs")
}

/// Returns whether the path points to a tab config TOML file under one of Rook's
...
/// Ensures `~/.rook/default_tab_configs/worktree.toml` exists, creating it
...
impl Entity for RookConfig {
    type Event = RookConfigUpdateEvent;
}

impl SingletonEntity for RookConfig {}
