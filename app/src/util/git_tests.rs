...
fn repository_info_from_gh_output_parses_name_and_owner() {
    ...
    assert_eq!(
        super::repository_info_from_gh_output(
            r#"{"name":"rook-internal","owner":{"login":"warpdotdev"}}"#
        )
        .unwrap(),
        RepositoryInfo {
            name: "rook-internal".to_owned(),
...
fn repository_info_from_gh_output_parses_host_from_url() {
    assert_eq!(
        super::repository_info_from_gh_output(
            r#"{"name":"rook-internal","owner":{"login":"warpdotdev"},"url":"https://github.com/warpdotdev/warp-internal"}"#
        )
        .unwrap(),
        RepositoryInfo {
            name: "rook-internal".to_owned(),
...
fn repository_info_from_gh_output_rejects_missing_owner_login() {
    assert!(
        super::repository_info_from_gh_output(r#"{"name":"rook-internal","owner":{}}"#).is_err()
    );
}
...
fn repository_info_from_gh_output_rejects_empty_fields() {
    ...
    assert!(
        super::repository_info_from_gh_output(r#"{"name":"rook-internal","owner":{"login":""}}"#)
            .is_err()
    );
}
...
async fn get_repository_info_reads_gh_repo_view() {
    ...
    fs::write(
        &gh_path,
        "#!/bin/sh\nprintf '{\"name\":\"rook-internal\",\"owner\":{\"login\":\"warpdotdev\"}}\\n'\n",
    )
    .expect("failed to write fake gh");
    let mut permissions = fs::metadata(&gh_path).unwrap().permissions();
    permissions.set_mode(0o755);
...
async fn get_pr_for_branch_does_not_require_origin_remote() {
    ...
    fs::write(
        &gh_path,
        "#!/bin/sh\nprintf '{\"number\":123,\"url\":\"https://github.com/rook/rook/pull/123\",\"state\":\"OPEN\",\"isDraft\":true,\"baseRefName\":\"main\"}\\n'\n",
    )
    .expect("failed to write fake gh");
    let mut permissions = fs::metadata(&gh_path).unwrap().permissions();
    permissions.set_mode(0o755);
    ...
    assert_eq!(
        ...
        Some(PrInfo {
            number: 123,
            url: "https://github.com/rook/rook/pull/123".to_string(),
