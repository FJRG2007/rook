//! File type detection utilities for determining if files can be opened in Rook.
...
/// The type of file that can be opened in Rook. The in-product treatment for "opening" a file
/// depends on its type.
...
pub enum FileTarget {
    /// Open in Rook's Markdown viewer.
    MarkdownViewer(EditorLayout),
    /// Open in Rook's Code Editor.
...
/// Whether `path` renders in Rook's notebook viewer (with a Rendered/Raw
/// toggle): Markdown files always, Jupyter notebooks when the feature flag is
/// enabled.
pub fn renders_in_rook_notebook_viewer(path: impl AsRef<Path>) -> bool {
...
/// "Open with Rook" is invoked from Finder/another app via a `file://` URL.
///
...
pub fn is_runnable_shell_script(path: &Path) -> bool {
    ...
    // A file `chmod 070` belongs to a group, not to the user invoking Rook.
...
/// Determines if a file can be opened in Rook and returns its type.
/// Returns `None` if the file is binary and should not be opened.
pub fn is_file_openable_in_rook(path: &Path) -> Option<OpenableFileType> {
...
/// Only use this for UI elements that must explicitly open a file in Rook (i.e. "Open in New Tab").
/// Prefer `resolve_file_target` for all other cases to respect users' preferences.
/// This would also force any binary file to be opened in Rook's Code Editor, so you should likely check
/// `is_file_openable_in_rook` before rendering any such UI Elements.
#[cfg(feature = "local_fs")]
pub fn resolve_file_target_to_open_in_rook(
    path: &Path,
    settings: &EditorSettings,
    layout: Option<EditorLayout>,
) -> FileTarget {
    let openable_file_type = is_file_openable_in_rook(path);
    ...
    // Jupyter notebooks render in Rook's notebook viewer unconditionally when
...
pub fn resolve_file_target_with_editor_choice(
    ...
    let is_openable_in_rook = is_file_openable_in_rook(path);
    let is_markdown = matches!(is_openable_in_rook, Some(OpenableFileType::Markdown));
    let layout = layout.unwrap_or(default_layout);
    let is_openable_in_rook = is_openable_in_rook.is_some();

    // 0. Jupyter notebooks render in Rook's notebook viewer unconditionally
    ...
    // 2. Rook Code Editor (Explicit user preference)
    if is_openable_in_rook && matches!(editor_choice, EditorChoice::Rook) {
    ...
    match editor_choice {
        ...
        EditorChoice::Rook | EditorChoice::EnvEditor => unreachable!("Already matched above"),
    }
}
