...
//! depends on the platform. The Rook app must use this information to avoid rendering UI elements
//! underneath them.
...
mod windows_only {
    pub(super) use pathfinder_color::ColorU;
    pub(super) use pathfinder_geometry::vector::vec2f;
    pub(super) use rook_core::ui::theme;
    pub(super) use rookui::elements::{
...
use rookui::{AppContext, Element, WindowId};
#[cfg(target_os = "windows")]
use windows_only::*;
...
/// Data the Rook app needs to avoid rendering anything below the traffic lights.
#[derive(Clone, Debug)]
...
impl TrafficLightData {
    ...
    /// by rookui. However, native traffic light buttons (e.g. on macOS) don't scale with zoom, so
    ...
    fn render_windows_close_button_icon(icon_color: ColorU) -> Box<dyn Element> {
        ConstrainedBox::new(
            IconComponent::X
                .to_rookui_icon(theme::Fill::Solid(icon_color))
                .finish(),
        )
        .with_height(16.)
        .with_width(16.)
