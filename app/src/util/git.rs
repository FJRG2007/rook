...
use rook_util::git::run_git_command;
#[cfg(feature = "local_fs")]
use rook_util::git::run_git_command_with_env;
...
pub async fn get_diff_for_commit_message(
        ...
        .await
    {
        ...
        for file_name_bytes in untracked.as_bytes().split(|b| *b == 0) {
            ...
            if rook_util::file_type::is_buffer_binary(&bytes[..check_len]) {
                continue;
            }
...
fn count_lines_if_text_file(path: &Path) -> u32 {
    ...
    if rook_util::file_type::is_buffer_binary(&content[..check_len]) {
        return 0;
    }
