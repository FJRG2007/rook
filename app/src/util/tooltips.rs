    ...
    let tooltip_element = if is_enterprise_secret_redaction_enabled {
        let tooltip_column = Flex::column()
            ...
            .with_child(
                appearance
                    .ui_builder()
                    .span("*Secrets are not sent to Rook's server.")
                    .with_style(UiComponentStyles {
                        font_size: Some(12.),
...
/// Returns whether "Open in Rook" should be offered for the given file path.
///
/// This checks:
/// - Whether Rook is already the default editor (skip if so)
/// - Whether this file is openable in Rook (skips binary files and directories)
/// - Whether the file renders in Rook's notebook viewer, which is reached via a
///   different affordance (skips Markdown and, when enabled, Jupyter notebooks)
#[cfg(feature = "local_fs")]
pub fn should_show_open_in_rook_link(path: &Path, app: &AppContext) -> bool {
    use rookui::SingletonEntity;

    use crate::code::view::is_binary_file;
    use crate::notebooks::file::renders_in_rook_notebook_viewer;
    use crate::util::file::external_editor::EditorSettings;
    use crate::util::file::external_editor::settings::EditorChoice;

    let settings = EditorSettings::as_ref(app);

    if matches!(*settings.open_file_editor, EditorChoice::Rook) {
        return false;
    }

    !renders_in_rook_notebook_viewer(path) && !is_binary_file(path) && !path.is_dir()
}

#[cfg(not(feature = "local_fs"))]
pub fn should_show_open_in_rook_link(_path: &std::path::Path, _app: &AppContext) -> bool {
    false
}
