...
use super::{
    ...
    TuiMcpVariableValue, is_represented_by_global_rook_server, server_priority, sort_servers,
...
#[test]
fn global_rook_server_replaces_only_matching_personal_synced_template() {
    let local = template(
    ...
    assert!(is_represented_by_global_rook_server(
        &cloud,
        &personal,
        &identities
    ));
