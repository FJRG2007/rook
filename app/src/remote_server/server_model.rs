...
use rook_core::{SessionId, safe_error};
...
use rookui::r#async::{Spawnable, SpawnableOutput, SpawnedFutureHandle};
...
///
/// This deliberately does not use `rook_core::paths::bundled_resources_dir`,
...
pub struct ServerModel {
    ...
    /// per SSH session / Rook tab connecting to this host.  Each entry maps
...
impl ServerModel {
    ...
    fn refresh_remote_agent_context_snapshot(&mut self, ctx: &rookui::AppContext) {
        let revision = self
            .remote_agent_context_snapshot
            .revision
            .saturating_add(1);
    ...
    #[cfg(feature = "crash_reporting")]
    fn apply_sentry_user_id(&self, ctx: &mut rookui::AppContext) {
    ...
    /// diff locally, then calls the Rook server's code-review content endpoint
