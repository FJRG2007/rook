...
use crate::user_config::{RookConfig, util::file_name_to_human_readable_name};
...
pub enum LaunchConfigModalEvent {
    ...
    /// already, we may as well save it "manually" to the RookConfig, while waiting for the update
...
impl LaunchConfigSaveModal {
    ...
    fn try_save_launch_config(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(app_state) = &self.current_app_state {
            ...
            match RookConfig::save_new_launch_config(file_name_candidate, launch_config.clone()) {
                Ok(file_name) => {
...
impl TypedActionView for LaunchConfigSaveModal {
    ...
    fn handle_action(&mut self, action: &ActionRequest, ctx: &mut ViewContext<Self>) {
        ...
        // this will become moot when we put launch configs in Rook Drive.
