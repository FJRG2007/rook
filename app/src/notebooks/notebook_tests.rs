...
fn open_notebook(
    ...
    let load_future = handle.update(app, |view, ctx| {
        view.load(notebook, &OpenRookDriveObjectSettings::default(), ctx)
    });
...
fn test_not_eager_baton_grab_different_editor() {
    App::test((), |mut app| async move {
        ...
        let uid = "ian@rook.dev".to_string();
        let email = "ian@rook.dev".to_string();
...
/// while Rook was closed.
#[test]
fn test_baton_grab_editor_changed_offline() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);

        let other_uid = "ben@rook.dev";
        let other_email = "ben@rook.dev";
