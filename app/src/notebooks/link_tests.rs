...
use rookui::{App, ModelHandle, SingletonEntity, WindowId};
...
fn test_resolve_bare_url() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            resolve(&app, &links, "rook.dev").await,
            url("http://rook.dev")
        );
        assert_eq!(
...
fn test_open_extensionless_non_text_file_does_not_emit_open_event() {
    ...
    // binary by `is_file_openable_in_rook`, which previously routed it to `SystemGeneric` and
...
fn test_resolve_valid_url() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            resolve(&app, &links, "https://rook.dev").await,
            url("https://rook.dev")
        );
        assert_eq!(
            resolve(&app, &links, "mailto:test@rook.dev").await,
            url("mailto:test@rook.dev")
        );
    });
}
