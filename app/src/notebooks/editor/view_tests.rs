...
use rookui::assets::asset_cache::{AssetCache, AssetState};
...
fn rendered_mermaid_block_range(
    ...
    for block in content.block_items() {
        ...
        if matches!(
            block,
            rook_editor::render::model::BlockItem::MermaidDiagram { .. }
        ) {
...
fn test_appearance_changes() {
    App::test((), |mut app| async move {
        ...
        Appearance::handle(&app).update(&mut app, |appearance, ctx| {
            appearance.set_monospace_font_family(rookui::fonts::FamilyId(123), ctx);
            ctx.notify()
        });
    ...
    App::test((), |mut app| async move {
        ...
        editor_view.update(&mut app, |editor, ctx| {
            ...
            link_editor
                ...
                .update(ctx, |url_editor, ctx| {
                    url_editor.user_insert("https://rook.dev", ctx);
                });
        ...
        editor_view.read(&app, |editor, ctx| {
            assert_eq!(
                ...
                "<text>Some <a_https://rook.dev>text<a>"
            );
        });
        ...
        editor_view.read(&app, |editor, ctx| {
            assert_eq!(
                editor.model.as_ref(ctx).debug_buffer(ctx),
                "<text>Some <a_https://rook.dev>text<a><a_https://example.com>new link<a>"
            );
        });
    });
}
