...
use rook_editor::render::model::viewport::SizeInfo;
use rook_editor::render::model::{BlockItem, RenderEvent};
use rook_editor::selection::{TextDirection, TextUnit};
use rookui::r#async::{FutureId, Timer, block_on};
use rookui::elements::ListIndentLevel;
...
fn setup_editor_window(app: &mut App, should_initialize_cloud_model: bool) -> rookui::WindowId {
...
fn test_inline_markdown() {
    App::test((), |mut app| async move {
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            editor.user_insert("[abc](https://rook.dev", ctx);
        });

        editor.read(&app, |editor, ctx| {
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First[abc](https://rook.dev <b_s>bold<b_e>"
            );
        });
        ...
        editor.read(&app, |editor, ctx| {
            ...
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First<a_https://rook.dev>abc<a> <b_s>bold<b_e>"
            );
        });
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First<a_https://rook.dev>abc<a> <b_s>bold`abc<b_e>"
            );
        ...
        editor.read(&app, |editor, ctx| {
            ...
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First<a_https://rook.dev>abc<a> <b_s>bold<b_e><c_s>abc<c_e>"
            );
        });
    })
}
...
fn test_pasting_link_on_selected_text() {
    App::test((), |mut app| async move {
        initialize_deps(&mut app);
        let editor = model_from_markdown("First text\nSecond line", &mut app, true);
        let clipboard_content = "https://rook.dev";
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                ...
                "<text><a_https://rook.dev>First text<a>\\nSecond line"
            );
        });
    });
}
...
fn test_cut_code_block() {
    App::test((), |mut app| async move {
        ...
        model_handle.update(&mut app, |model, ctx| {
            ...
            assert_eq!(
                clipboard.html.unwrap(),
                r#"<pre><code class="language-rook-runnable-command">command</code></pre>"#
            );
        })
    });
}
...
fn test_multiselect_pasting() {
    App::test((), |mut app| async move {
        ...
        let url_clipboard_content = "https://rook.dev";
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                editor.debug_buffer(ctx),
                "<text><a_https://rook.dev>https://rook.dev<a>\\nSecond line\\n<a_https://rook.dev>https://rook.dev<a><code:Shell><c_#b4fa72>code<c><text>"
            );
        });
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                editor.debug_buffer(ctx),
                "<text>echo test<a_https://rook.dev>https://rook.dev<a>\\nSecond line\\n<a_https://rook.dev>https://rook.dev<a><code:Shell><c_#b4fa72>coecho testde<c><text>"
            );
        });
    });
}
