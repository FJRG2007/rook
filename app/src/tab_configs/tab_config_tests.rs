...
directory = "{{repo}}"
commands = [
  "git worktree add -b {{worktree_branch_name}} $HOME/.rook/worktrees/$(basename {{repo}})/{{worktree_branch_name}} {{branch}}",
  "cd $HOME/.rook/worktrees/$(basename {{repo}})/{{worktree_branch_name}}",
]
...
fn test_render_tab_config_substitutes_values() {
    ...
    } = pane_template
    {
        ...
        assert_eq!(
            commands[0].exec,
            "git worktree add -b my-feature $HOME/.rook/worktrees/$(basename /Users/me/repo)/my-feature main"
        );
        assert_eq!(
            commands[1].exec,
            "cd $HOME/.rook/worktrees/$(basename /Users/me/repo)/my-feature"
        );
    } else {
