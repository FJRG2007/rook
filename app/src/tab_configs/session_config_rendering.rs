    ...
    F: Fn(usize, &mut EventContext, Vector2F) + 'static,
{
    ...
    for (i, session_type) in session_types.iter().enumerate() {
        ...
        let icon = ConstrainedBox::new(
            session_type
                .icon()
                .to_rookui_icon(item_color.into())
                .finish(),
        )
        .with_width(14.)
        .with_height(14.)
    ...
    F: Fn(&mut EventContext, Vector2F) + 'static,
{
    ...
    let dir_display =
        rook_util::path::user_friendly_path(&raw_path, home_dir.as_ref().and_then(|h| h.to_str()))
            .into_owned();
...
where
    F: Fn(&mut rookui::EventContext, rookui::geometry::vector::Vector2F) + 'static,
{
