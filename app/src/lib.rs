...
//
// Any modules which we make public outside of the `rook` crate lose dead code
...
// in the rook::integration_testing::assertions module (or a sub-module).  These
...
pub use rook_core::r#async::debounce;
...
pub use rook_core::send_telemetry_from_app_ctx;
pub use rook_core::send_telemetry_from_ctx;
// Re-export the safe logging macros at the crate root level for backwards compatibility
pub use rook_core::{safe_debug, safe_error, safe_info, safe_warn};
use rook_errors::{report_error, report_if_error};
#[cfg(feature = "local_fs")]
use rook_files::FileModel;
use rook_logging::{LogDestination, LogFrontend};
use rook_managed_secrets::ManagedSecretManager;
use rook_server_client::iap::{IapManager, IapManagerEvent, IapState, ManagedIapMint};
use rook_server_client::network_logging::NetworkLogModel;
use rookui::integration::TestDriver;
use rookui::modals::{AlertDialogWithCallbacks, AppModalCallback};
use rookui::platform::TerminationMode;
use rookui::platform::app::{ApproveTerminateResult, TerminationRequestSource};
use rookui::windowing::state::ApplicationStage;
use rookui::{App, AppContext, Event, SingletonEntity, WindowId};
...
pub static ASSETS: rook_assets::Assets = rook_assets::Assets;
...
/// Launch mode for how to start up Rook.
#[allow(clippy::large_enum_variant)]
pub(crate) enum LaunchMode {
    /// Run the regular GUI application.
    App {
        args: rook_cli::AppArgs,
        /// API key for server authentication, if provided via `--api-key` or `ROOK_API_KEY`.
        api_key: Option<String>,
    },

    /// Run the Rook command-line SDK.
    CommandLine {
        command: rook_cli::CliCommand,
...
enum TuiEntryPoint {
    ...
    Interactive {
        mount: TuiMountFn,
        /// API key for non-interactive Rook authentication.
        api_key: Option<String>,
    },
    /// Execute a CLI command after TUI-scoped app initialization, then exit.
    CliCommand {
        execute: Box<dyn FnOnce(&mut rookui::AppContext)>,
    },
}
...
impl LaunchMode {
    fn args(&self) -> Cow<'_, rook_cli::AppArgs> {
        match self {
            ...
            | LaunchMode::RemoteServerProxy
            | LaunchMode::RemoteServerDaemon { .. }
            | LaunchMode::Tui { .. } => Cow::Owned(rook_cli::AppArgs::default()),
        }
    }
    ...
    /// Returns `true` if Rook should run headlessly, without a visible UI.
...
/// Runs the shared Rook executable as the app or as one of its command-line modes.
///
/// The bundled Rook Control wrapper injects `--rookctrl`, which is dispatched
/// before the normal Rook/Oz parser. Oz subcommands are part of that normal
...
pub fn run() -> Result<()> {
    ...
    if let Some(args) = rook_cli::local_control::ControlArgs::from_control_mode_env() {
        #[cfg(windows)]
        rook_util::windows::attach_to_parent_console();
        rook_cli::local_control::run_and_exit(args);
    }

    // Parse command-line arguments.
    let args = rook_cli::Args::from_env();
    ...
    // `ROOK_*` env-var equivalents) so shipped builds can't be redirected away from their
    ...
    if let Some(command) = args.command() {
        ...
        match command {
            rook_cli::Command::Worker(worker) => return run_worker_command(worker),
            rook_cli::Command::Completions { shell } => {
                return rook_cli::completions::generate_to_stdout(*shell);
            }
            rook_cli::Command::CommandLine(cmd) => {
                let (is_sandboxed, computer_use_override) = match cmd.as_ref() {
                    rook_cli::CliCommand::Agent(rook_cli::agent::AgentCommand::Run(run_args)) => (
                        run_args.sandboxed,
            ...
            rook_cli::Command::DumpDebugInfo => {
                return debug_dump::run();
            }
            #[cfg(not(target_family = "wasm"))]
            rook_cli::Command::DumpSettingsSchema { output_path } => {
                return settings::schema_generation::dump_settings_schema(output_path.as_deref());
            }
            #[cfg(not(target_family = "wasm"))]
            rook_cli::Command::PrintTelemetryEvents => {
    ...
    let is_cli_binary = cfg!(feature = "standalone")
        || rook_cli::binary_name().is_some_and(|name| name.starts_with("oz"))
        || std::env::var_os("ROOK_CLI_MODE").is_some();
    if is_cli_binary {
        rook_cli::Args::clap_command().print_help()?;
        return Ok(());
    }
...
/// Runs a parsed Rook worker command.
fn run_worker_command(worker: &rook_cli::WorkerCommand) -> Result<()> {
    match worker {
        #[cfg(all(feature = "local_tty", unix))]
        rook_cli::WorkerCommand::TerminalServer(args) => {
            crate::terminal::local_tty::run_terminal_server(args);
            Ok(())
        }
        #[cfg(feature = "plugin_host")]
        rook_cli::WorkerCommand::PluginHost { .. } => crate::run_plugin_host(),
        #[cfg(feature = "local_tty")]
        rook_cli::WorkerCommand::MinidumpServer { socket_name } => {
            cfg_if::cfg_if! {
        ...
        #[cfg(not(target_family = "wasm"))]
        rook_cli::WorkerCommand::RemoteServerProxy(args) => {
            ...
            rook_logging::init(rook_logging::LogConfig {
        ...
        #[cfg(not(target_family = "wasm"))]
        rook_cli::WorkerCommand::RemoteServerDaemon(args) => {
        ...
        #[cfg(not(target_family = "wasm"))]
        rook_cli::WorkerCommand::RipgrepSearch {
            parent,
            ignore_case,
            multiline,
            pattern,
            ...
            rook_ripgrep::search::run_search_subprocess(
...
pub fn run_integration_test(driver: TestDriver) -> Result<()> {
    let is_integration_test = std::env::var("ROOK_INTEGRATION").is_ok();
...
/// Runs the headless TUI front-end (the `rook-tui` binary in the `rook_tui`
...
///
/// `mount` is supplied by the `rook_tui` crate (which owns the concrete root
/// view plus the window/driver bootstrap), so `rook` never has to depend on
/// `rook_tui`.
#[cfg(feature = "tui")]
...
#[cfg(feature = "tui")]
pub fn run_tui_cli_command(execute: Box<dyn FnOnce(&mut rookui::AppContext)>) -> Result<()> {
    run_internal(LaunchMode::Tui {
...
pub fn run_tui_worker_if_requested() -> Option<Result<()>> {
    ...
    let is_worker = std::env::args()
        .nth(1)
        .is_some_and(|arg| rook_cli::is_worker_invocation(&arg));
    if !is_worker {
        return None;
    }
    ...
    features::init_feature_flags();
    let args = rook_cli::Args::from_env();
    let Some(rook_cli::Command::Worker(worker)) = args.command() else {
        return Some(Err(anyhow!(
            "Recognized a Rook worker invocation, but failed to parse its worker command"
        )));
    };
...
/// Supplied to [`run_tui`] by the `rook_tui` crate; it runs after
/// `initialize_app` to build the root TUI view and start the TUI driver.
pub type TuiMountFn = Box<dyn FnOnce(&mut rookui::AppContext)>;
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    cfg_if::cfg_if! {
        if #[cfg(enable_crash_recovery)] {
            if crash_recovery::is_crash_recovery_process(launch_mode.args().as_ref()) {
                rook_logging::init_for_crash_recovery_process()?;
            } else {
                rook_logging::init(rook_logging::LogConfig {
                    frontend: launch_mode.log_frontend(),
                    log_destination,
    ...
    if let LaunchMode::App { .. } = launch_mode {
        match app_services::linux::pass_startup_args_to_existing_instance(
            ...
            // we only want to run a single instance of Rook at a time.
            Ok(_) => std::process::exit(0),
            // If Rook isn't already running, we're good to go.
            ...
            // state where Rook refuses to run even a first instance.
            Err(err) => {
    ...
    // Sets up a Job Object that we associate with the Rook process to handle
    ...
    let prefs_for_public_settings: &dyn rookui_extras::user_preferences::UserPreferences =
    ...
    let mut app_builder = if launch_mode.is_headless() {
        rookui::platform::AppBuilder::new_headless(
            callbacks,
            Box::new(ASSETS),
            launch_mode.take_test_driver(),
        )
    } else {
        rookui::platform::AppBuilder::new(
            callbacks,
            Box::new(ASSETS),
            launch_mode.take_test_driver(),
        )
    ...
    #[cfg(target_os = "macos")]
    if !launch_mode.is_headless() {
        use rookui::AssetProvider as _;
        use rookui::platform::mac::AppExt;

        let activate_on_launch = !launch_mode.is_integration_test()
            || std::env::var("ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS").is_ok();
    ...
    {
        use rookui::platform::linux::{self, AppBuilderExt};
        ...
        // Force use of wayland if the user has passed the `ROOK_ENABLE_WAYLAND` env var.
    ...
    {
        use rookui::platform::windows::AppBuilderExt;
        ...
        if !is_parallels_vm {
            ...
            use rookui::platform::windows::DXCPath;
    ...
    app_builder.run(move |ctx| {
        ...
        ctx.background_executor()
            .spawn(rook_logging::rotate_log_files())
            .detach();
...
pub(crate) fn initialize_app(
    ...
    startup_toml_parse_error: Option<rookui_extras::user_preferences::Error>,
    ctx: &mut rookui::AppContext,
    ...
    if matches!(launch_mode, LaunchMode::RemoteServerDaemon { .. }) {
        rookui_extras::secure_storage::register_unavailable(ctx);
    } else {
        // Register an implementation of the secure storage service.
        cfg_if::cfg_if! {
            if #[cfg(feature = "integration_tests")] {
                rookui_extras::secure_storage::register_noop(&secure_storage_service_name, ctx);
            } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
                rookui_extras::secure_storage::register_with_fallback(&secure_storage_service_name, rook_core::paths::state_dir(), ctx)
            } else if #[cfg(target_os = "windows")] {
                rookui_extras::secure_storage::register_with_dir(&secure_storage_service_name, rook_core::paths::state_dir(), ctx)
            } else {
                rookui_extras::secure_storage::register(&secure_storage_service_name, ctx);
            }
        }
    }

    // One-time migration: give Preview its own config directory by
    // symlinking contents from the shared ~/.rook location. Must run
    // before ensure_rook_watch_roots_exist() creates the new directory.
    #[cfg(target_os = "macos")]
    preview_config_migration::migrate_preview_config_dir_if_needed();

    ensure_rook_watch_roots_exist();
    ctx.add_singleton_model(RookManagedPathsWatcher::new);

    ctx.add_singleton_model(RookConfig::new);
    ...
    #[cfg(not(target_family = "wasm"))]
    let ambient_agent_task_id: Option<AmbientAgentTaskId> = std::env::var(rook_cli::OZ_RUN_ID_ENV)
        .ok()
        .and_then(|run_id| match run_id.parse() {
            Ok(task_id) => Some(task_id),
            Err(err) => {
                log::warn!("Ignoring invalid {}: {err}", rook_cli::OZ_RUN_ID_ENV);
                None
            }
        });
    #[cfg(not(target_family = "wasm"))]
    ...
    // inside `Rook.app`, so without this it would rewrite a bundle it does not
    // own. See APP-2946.
    ...
    log::info!(
        "Starting rook with channel state {} and version {:?}",
    ...
    // Rewrite recognized Rook web URLs (sessions, Drive, settings, home) into local
    ...
    #[cfg(feature = "local_fs")]
    {
        ...
        if ChannelState::channel() != rook_core::channel::Channel::Integration {
    ...
    ctx.add_singleton_model(|ctx| {
        rook_core::sync_queue::SyncQueue::<SyncTask>::new_with_rate_limit(
            &ctx.background_executor(),
    ...
    // on IAP here, since the request itself calls the IAP-gated rook-server — except the TUI,
    ...
    });
    {
        ...
        // the org level). Billing metadata — including `rook_ai_policy.is_voice_enabled`
    ...
    if matches!(
        launch_mode,
        LaunchMode::App { .. } | LaunchMode::Test { .. }
    ) && FeatureFlag::RookControlCli.is_enabled()
    {
...
pub(crate) fn app_callbacks(
    ...
    rookui::platform::AppCallbacks {
        ...
        on_will_terminate: Some(Box::new(move |ctx| {
            ...
            // No snapshot here. By the time this runs the windows are gone and
            // the workspace has been taken apart, so what it would capture is the
            // teardown rather than the session. The saves that matter happen
            // earlier, in `on_should_terminate_app` and `on_should_close_window`,
            ...
            // autoupdate, to ensure we're not running any extra Rook processes
        ...
        on_should_terminate_app: Some(Box::new(move |source, ctx| {
            ...
            // `ApproveTerminateResult::Cancel`, which macOS interprets as Rook
            ...
            // Rook waiting on a prompt nobody can see (#12441). Skipping
...
#[::tracing::instrument(skip_all, fields(tags.cloud_agent = true))]
fn launch(ctx: &mut rookui::AppContext, app_state: Option<AppState>, launch_mode: LaunchMode) {
...
fn init_logging_for_unit_tests_glue() {
    ...
    rook_logging::init_logging_for_unit_tests();
}
