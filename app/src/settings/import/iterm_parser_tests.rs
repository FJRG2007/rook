...
#[test]
fn test_into_rook_theme_valid() {
    let theme: RookTheme = solarized_dark_theme()
        .into_rook_theme("", &default_dark_theme())
        .expect("Should be able to convert into RookTheme");
    assert_eq!(
        theme.accent(),
        Fill::Solid(ColorU {
            r: 229,
...
#[test]
fn test_into_rook_theme_invalid() {
    default_dark_theme()
        .into_rook_theme("", &default_dark_theme())
...
<key>PrefsCustomFolder</key><string>{}</string>
        ...
        let ThemeType::Single(rook_theme) =
...
fn test_parse_font_with_default_size() {
    let rook_default_profile = ITermProfile {
        font_name: Some("CourierNewPSMT".to_string()),
