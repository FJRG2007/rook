...
define_settings_group!(TuiAutoupdateSettings, settings: [
    // Whether the `rook-tui` background update check is enabled.
    //
    ...
    // settings file. Read once at TUI startup; the `ROOK_TUI_DISABLE_AUTOUPDATE`
    ...
    autoupdate_enabled: TuiAutoupdateEnabled {
        ...
        description: "Whether Rook Agent CLI checks for updates and automatically installs them when supported by the installation method.",
    },
]);
