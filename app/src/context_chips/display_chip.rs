...
use rookui::fonts::{Cache, FamilyId, Properties, Weight};
...
fn git_branch_status_icon(icon: Icon, color: ColorU, icon_size: f32) -> Box<dyn Element> {
    Container::new(
        ConstrainedBox::new(icon.to_rookui_icon(Fill::Solid(color)).finish())
            .with_height(icon_size)
            .with_width(icon_size)
            .finish(),
    )
    ...
    fn prompt_chip_command(&self) -> PromptChipShellCommand {
        ...
        if branch.is_linked_worktree {
            return PromptChipShellCommand::Echo {
                message: "The branch is already checked out in another worktree, but Rook couldn't find its path.",
            };
        }
    ...
    fn git_diff_stats_chip(
        ...
        let supports_code_review = self
            ...
            .map(|ctx| match ctx.session.session_type() {
                ...
                SessionType::RookifiedRemote { host_id: None } => false,
            })
            .unwrap_or(false);
...
pub(crate) fn render_udi_chip(config: UdiChipConfig, appearance: &Appearance) -> Box<dyn Element> {
    ...
    if let Some(icon) = config.icon {
        content.add_child(
            Container::new(
                ConstrainedBox::new(icon.to_rookui_icon(Fill::Solid(config.color)).finish())
                    .with_height(icon_size)
                    .with_width(icon_size)
                    .finish(),
            )
