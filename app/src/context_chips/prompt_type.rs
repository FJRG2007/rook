...
use crate::settings::RookPromptSeparator;
...
/// The type of rook prompt being used
#[derive(Clone)]
pub enum PromptType {
    /// A rook prompt that refreshes chip values on its own. Typical for local sessions.
    Dynamic { prompt: ModelHandle<CurrentPrompt> },
    /// A rook prompt that does not change unless explicitly overwritten. Used for viewers of shared sessions.
...
impl PromptType {
    ...
    /// The separator for the Rook prompt.
    pub fn separator(&self, ctx: &AppContext) -> RookPromptSeparator {
        match self {
