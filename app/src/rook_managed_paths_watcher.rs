...
#[cfg(not(target_family = "wasm"))]
use rookui::ModelHandle;
use rookui::{Entity, ModelContext, SingletonEntity};
#[cfg(not(target_family = "wasm"))]
use watcher::{BulkFilesystemWatcher, BulkFilesystemWatcherEvent};

/// Duration between filesystem watch events for the Rook managed paths watcher, in milliseconds.
#[cfg(not(target_family = "wasm"))]
const ROOK_MANAGED_PATHS_WATCHER_DEBOUNCE_MILLI_SECS: u64 = 500;

pub(crate) fn rook_data_dir() -> PathBuf {
    rook_core::paths::data_dir()
}

#[cfg(target_family = "wasm")]
pub(crate) fn ensure_rook_watch_roots_exist() {}

#[cfg(not(target_family = "wasm"))]
pub(crate) fn ensure_rook_watch_roots_exist() {
    let data_dir = rook_data_dir();
    if let Err(err) = fs::create_dir_all(&data_dir) {
        log::warn!(
            "Failed to create Rook data directory {}: {err}",
            data_dir.display()
        );
    }

    let config_local_dir = rook_core::paths::config_local_dir();
    if config_local_dir != data_dir
        && let Err(err) = fs::create_dir_all(&config_local_dir)
    {
        log::warn!(
            "Failed to create Rook config directory {}: {err}",
            config_local_dir.display()
        );
    }

    // The TUI surface stores its settings in a separate config directory
    // (see `rook_core::paths::tui_config_local_dir`). Create it up front — only
    ...
    if settings::settings_mode() == settings::SettingsMode::Tui {
        let tui_config_local_dir = rook_core::paths::tui_config_local_dir();
        if let Err(err) = fs::create_dir_all(&tui_config_local_dir) {
            log::warn!(
                "Failed to create Rook TUI config directory {}: {err}",
...
pub(crate) fn rook_home_skills_dir() -> Option<PathBuf> {
    rook_core::paths::rook_home_skills_dir()
}

#[cfg_attr(target_family = "wasm", allow(dead_code))]
pub(crate) fn rook_home_mcp_config_file_path() -> Option<PathBuf> {
    rook_core::paths::rook_home_mcp_config_file_path()
}
#[cfg_attr(target_family = "wasm", allow(dead_code))]
pub(crate) fn active_mcp_config_file_path() -> Option<PathBuf> {
    match settings::settings_mode() {
        settings::SettingsMode::Gui => rook_home_mcp_config_file_path(),
        settings::SettingsMode::Tui => Some(rook_core::paths::tui_mcp_config_file_path()),
    }
}

#[cfg_attr(target_family = "wasm", allow(dead_code))]
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct RookMcpConfigPath {
    pub(crate) root_path: PathBuf,
    pub(crate) config_path: PathBuf,
}

#[cfg_attr(target_family = "wasm", allow(dead_code))]
pub(crate) fn rook_managed_skill_dirs() -> Vec<PathBuf> {
    rook_home_skills_dir().into_iter().collect()
}

#[cfg_attr(target_family = "wasm", allow(dead_code))]
pub(crate) fn rook_managed_mcp_config_path() -> Option<RookMcpConfigPath> {
    Some(RookMcpConfigPath {
        root_path: home_dir()?,
...
#[cfg(target_family = "wasm")]
#[allow(dead_code)]
pub(crate) enum RookManagedPathsWatcherEvent {}
...
#[cfg(not(target_family = "wasm"))]
pub(crate) struct RookManagedPathsWatcher {
...
impl RookManagedPathsWatcher {
        ...
        let watcher = if should_register_watcher {
            ctx.add_model(|ctx| {
                BulkFilesystemWatcher::new(
                    Duration::from_millis(ROOK_MANAGED_PATHS_WATCHER_DEBOUNCE_MILLI_SECS),
                    ctx,
                )
            })
        } else {
        ...
        if should_register_watcher {
            ...
            if should_register_config_local_dir {
                Self::register_path(
                    ...
                    RecursiveMode::Recursive,
                    "Rook config directory",
                );
            }
            // Watch the TUI settings directory for that surface. On macOS it's
            // a sibling `.rook_cli*` directory outside `config_local_dir`; on
            ...
            if settings::settings_mode() == settings::SettingsMode::Tui {
                        ...
                        || !tui_config_local_dir.starts_with(&config_local_dir))
                {
                    Self::register_path(
                        ...
                        WatchFilter::accept_all(),
                        RecursiveMode::Recursive,
                        "Rook TUI config directory",
                    );
                }
            }
            if let Some(rook_home_skills_dir) = rook_home_skills_dir()
                && rook_home_skills_dir.exists()
                && !rook_home_skills_dir.starts_with(&data_dir)
                && (!should_register_config_local_dir
                    || !rook_home_skills_dir.starts_with(&config_local_dir))
            {
                Self::register_path(
                    ctx,
                    &watcher,
                    rook_home_skills_dir,
                    WatchFilter::accept_all(),
                    RecursiveMode::Recursive,
                    "Rook home skills directory",
                );
            }
            ...
            let is_covered_by_tui_config_watcher = settings::settings_mode()
                ...
                && active_mcp_config_dir.as_deref()
                    == Some(rook_core::paths::tui_config_local_dir().as_path());
    ...
    fn handle_fs_event(
        ...
        if !update.is_empty() {
            ctx.emit(RookManagedPathsWatcherEvent::FilesChanged(update));
        }
    }
}
...
impl Entity for RookManagedPathsWatcher {
    type Event = RookManagedPathsWatcherEvent;
}

impl SingletonEntity for RookManagedPathsWatcher {}

#[cfg(test)]
#[path = "rook_managed_paths_watcher_tests.rs"]
mod tests;
