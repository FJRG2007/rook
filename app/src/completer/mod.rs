...
pub struct SessionContext {
    ...
    /// Snapshot of all Rook workflow aliases.
...
impl GeneratorContext for SessionContext {
    async fn execute_command_at_pwd(
        ...
        // because if the user opened Rook through a parent process that didn't have the PATH var set
...
impl CompletionContext for SessionContext {
    ...
    #[cfg(feature = "completions_v2")]
    fn js_context(&self) -> Option<&dyn rook_completer::completer::JsExecutionContext> {
        self.js_ctx
            .as_ref()
            .map(|ctx| -> &dyn rook_completer::completer::JsExecutionContext { ctx })
    }
...
fn ls_script_for_dir(directory: &TypedPath) -> Option<String> {
    ...
    let escaped_dir = rook_util::path::ShellFamily::Posix.shell_escape(dir_str);
