    ...
    App::test((), |app| async move {
        VirtualFS::test(
            ...
            |dirs, mut sandbox| {
                ...
                let entries = rookui::r#async::block_on(super::list_entries(&ctx, &cwd.to_path()))
...
fn test_list_entries_returns_none_on_guest_failure() {
    App::test((), |app| async move {
        ...
        let result = rookui::r#async::block_on(super::list_entries(&ctx, &directory));
...
fn test_run_guest_listing_returns_none_on_timeout() {
    App::test((), |app| async move {
        ...
        let result = rookui::r#async::block_on(super::run_guest_listing(
            &ctx,
            "sleep 5",
