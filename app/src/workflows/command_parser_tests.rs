...
lazy_static! {
    static ref WORKFLOW: Workflow = Workflow::Command {
        ...
        command:
            "RUST_BACKTRACE=full ROOK_SHELL_PATH={{shell_path}} cargo run -p integration --bin \
...
fn test_compute_workflow_display_data_for_linked_history_command() {
    ...
    let linked_history_command = "RUST_BACKTRACE=full ROOK_SHELL_PATH=/opt/homebrew/bin/fish cargo run -p integration --bin \
...
fn test_compute_workflow_display_data_for_linked_history_command_with_multiple_instances_same_parameter()
 {
    ...
    let linked_history_command = r#"echo rook rook rook"#;
...
fn test_compute_workflow_display_data_for_similar_but_unlinked_history_command() {
    ...
    // command.
    let similar_but_unlinked_history_command = "RUST_BACKTRACE=full ROOK_SHELL_PATH=/opt/homebrew/bin/fish cargo run integration --bin \
        integration --features=with_real_display_in_integration_tests -- \
        test_command_search_loads_history";
    assert!(
        compute_workflow_display_data_for_history_command(
