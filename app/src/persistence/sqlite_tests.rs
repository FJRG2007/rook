...
fn tui_scope_database_path_is_tui_subdirectory_of_app_database_dir() {
    ...
    assert_eq!(
        tui_path,
        rook_core::paths::tui_state_dir().join("rook.sqlite")
    );
...
fn remote_server_daemon_scope_database_path_uses_identity_data_dir() {
    ...
    assert_eq!(
        path,
        PathBuf::from(shellexpand::tilde(&expected_data_dir).into_owned()).join("rook.sqlite")
    );
}
...
fn remote_server_daemon_database_permissions_are_owner_only() {
    ...
    let database_path = daemon_dir.join("rook.sqlite");
...
fn sqlite_read_restores_app_state_and_codebase_metadata() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let database_path = tempdir.path().join("rook.sqlite");
...
fn tui_database_in_tui_subdirectory_round_trips_data() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let database_path = tempdir.path().join("tui").join("rook.sqlite");
...
// Regression: GH#10083. Users whose rook.sqlite already contains a 1px row
