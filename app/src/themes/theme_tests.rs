...
fn custom_theme_path_under_theme_root_storage_helper_returns_relative_path() {
    let root = PathBuf::from("/home/user/.local/share/rook-terminal/themes");
...
fn custom_theme_relative_path_resolves_under_local_theme_root() {
    let root = PathBuf::from("/Users/example/.rook/themes");
...
fn custom_theme_legacy_macos_path_is_preserved_even_when_local_file_exists() {
    ...
    let root = temp.path().join("rook-terminal/themes");
    let local = root.join("catppuccin/catppuccin_mocha.yml");
    std::fs::create_dir_all(local.parent().unwrap()).unwrap();
    std::fs::write(&local, "").unwrap();

    let stored = PathBuf::from("/Users/example/.rook/themes/catppuccin/catppuccin_mocha.yml");
...
fn custom_theme_legacy_parent_dir_path_is_preserved() {
    ...
    let stored = PathBuf::from("/Users/example/.rook/themes/../outside.yml");
...
fn custom_theme_legacy_linux_path_is_preserved_even_when_local_file_exists() {
    let temp = tempfile::tempdir().unwrap();
    let root = temp.path().join(".rook/themes");
    let local = root.join("catppuccin/catppuccin_latte.yml");
    std::fs::create_dir_all(local.parent().unwrap()).unwrap();
    std::fs::write(&local, "").unwrap();

    let stored = PathBuf::from(
        "/home/user/.local/share/rook-terminal/themes/catppuccin/catppuccin_latte.yml",
    );
...
fn custom_theme_unmatched_legacy_absolute_path_is_preserved() {
    let temp = tempfile::tempdir().unwrap();
    let root = temp.path().join(".rook/themes");
    let stored = PathBuf::from("/Users/example/.rook/themes/missing.yml");
...
fn custom_theme_windows_absolute_path_string_is_preserved() {
    let root = PathBuf::from("/Users/example/.rook/themes");
    let stored = PathBuf::from(r"C:\Users\example\AppData\Roaming\rook\Rook\data\themes\mocha.yml");
...
fn custom_theme_windows_unc_path_string_is_preserved() {
    let root = PathBuf::from("/Users/example/.rook/themes");
    let stored = PathBuf::from(r"\\server\share\rook\themes\mocha.yml");
...
fn custom_theme_serde_preserves_unportable_raw_paths() {
    for raw_path in [
        ...
        "C:/Users/example/AppData/Roaming/rook/Rook/data/themes/mocha.yml",
        "C:themes/mocha.yml",
    ] {
...
mod windows_custom_theme_path_tests {
    ...
    fn windows_theme_root() -> PathBuf {
        PathBuf::from(r"C:\Users\example\AppData\Roaming\rook\Rook\data\themes")
    }
...
fn in_memory_theme_generation_test() {
    ...
    let mut in_memory_theme = rookui::r#async::block_on(InMemoryThemeOptions::new(
        "mountains".to_string(),
