...
| Autoupdate | Off. Upstream polls a release host every ten minutes; Rook has none and publishes to GitHub releases instead. `ContextFlag::PromptForVersionUpdates` follows the flag so the update controls disappear with it. | `crates/rook_features/src/lib.rs`, `crates/rook_core/src/context_flag.rs` |
