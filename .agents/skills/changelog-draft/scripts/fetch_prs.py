...
Usage:
    python3 fetch_prs.py --repo FJRG2007/rook --base-ref <prev_tag> --head-ref <release_tag>
...
PUBLIC_PR_URL_RE = re.compile(r"https://github\.com/FJRG2007/rook/pull/(\d+)")
...
def should_include_pr(repo: str, data: dict) -> bool:
    ...
    Releases are cut from rook-internal, but non-sync-bot PRs merged there are
...
def extract_public_pr_number(text: str) -> int | None:
    """Extract a public FJRG2007/rook PR number from text."""
    if not text:
        return None
...
def resolve_public_pr_number(repo: str, pr_number: int, data: dict) -> int | None:
    """Resolve a repo-sync PR back to its original public FJRG2007/rook PR."""
...
def normalize_pr_data(repo: str, pr_number: int, data: dict) -> tuple[str, dict, dict | None]:
    ...
    The release workflow runs from rook-internal, where public PRs are mirrored
    as rook-repo-sync[bot] PRs with different PR numbers. For changelog output
