...
description: Remove a feature flag after it has been rolled out and stabilized in the Rook codebase.
---

# remove-feature-flag

Remove a feature flag after it has been rolled out and stabilized in the Rook codebase.
...
Remove the variant from the `FeatureFlag` enum in `rook_core/src/features.rs`:
...
Use ripgrep to find all occurrences. The shared `FeatureFlag` may be used from the headless TUI, so search `crates/rook_tui/` (and other non-`app/` crates), not just `app/` and `rook_core/`:
```bash
rg "YourFeatureName" app/ rook_core/ crates/rook_tui/
```
...
rg "FeatureFlag::YourFeatureName" app/ crates/rook_tui/
