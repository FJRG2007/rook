...
description: Verify a change to Rook's headless TUI front-end (crates/rook_tui) by running it — locally via ./script/run-tui, or in a headless cloud runner via a ROOK_API_KEY dogfood build — and reading the rendered screen back (under tmux when it's installed, otherwise directly). Use whenever you change TUI UI, rendering, input, or behavior and need to confirm the real on-screen result.
---

# tui-verify-change

Verify a change to Rook's **headless TUI** front-end (`crates/rook_tui`, and the
cell-grid element library in `crates/rookui_core/src/elements/tui`) by running it
...
- You changed anything a TUI user sees or interacts with: an element/layout in
  `crates/rookui_core/src/elements/tui`, a TUI view/screen in `crates/rook_tui`
...
If your change is GUI-only (`app/`, RookUI pixel `Element`/`View`), this is the
...
up front — it determines whether you use `./script/run-tui` or the `ROOK_API_KEY`
path below.
...
- **Local context** — you're working in a local dev checkout, typically
  ...
  `rook_tui` and runs it, selecting the internal `local` channel when the
  `rook-channel-config` generator is available and falling back to
  `rook-tui-oss` otherwise. **Do not reach for `ROOK_API_KEY`** here — it's
  ...
  non-interactive `ROOK_API_KEY` login below is a *cloud-runner* affordance, not
...
- **Cloud context** — you're a headless cloud agent (e.g. the factory-client
  ...
  relies on the non-interactive `ROOK_API_KEY` already in the environment. That
  does **not** mean bypassing `./script/run-tui`: when the `rook-channel-config`
  generator is available, `./script/run-tui` selects the internal `local`
  binary; otherwise it falls back to `rook-tui-oss`. Both accept the inherited
  `ROOK_API_KEY`, so prefer the maintained runner unless you need a specific
...
Always build with a small job count so the large `rook` dependency tree doesn't
OOM the machine:
...
- `rook-tui-oss` is the OSS channel binary and the safest default (no internal
  `rook-channel-config` generator required). `./script/run-tui` does the
  equivalent, selecting the internal `local` binary when the generator is
  available and falling back to `rook-tui-oss` otherwise.
...
- `LoggedIn` → briefly `Starting terminal…`, then the **zero state** (`Rook
...
(Exact strings live in `crates/rook_tui/src/ui.rs` — verify against it if you're
...
### Logging in non-interactively (`ROOK_API_KEY`) — cloud context
...
`ROOK_API_KEY` usually isn't set locally. Use the flow here when you're a
...
device-auth flow — by launching any TUI channel binary with a `ROOK_API_KEY` in
the environment. This is the fast way to verify live terminal/transcript/input
changes.

Key constraints:

- **All channels are supported.** API-key login works with `rook-tui-oss` as
  well as the Preview, Dev, Local, and Stable binaries.
- **The key must already be in the environment.** In a sandbox where
  `ROOK_API_KEY` is set, a freshly started `tmux` server inherits it. Never echo,
...
# ROOK_API_KEY is inherited from the environment by the new tmux server.
tmux new-session -d -s tuicheck -x 120 -y 40 './target/debug/rook-tui-oss'
sleep 20                                      # login + session start
tmux capture-pane -t tuicheck -p              # expect the logged-in zero state
```

When it works you'll see the **zero state** (`Rook Agent` + input view + model
...
- **Cloud / no-tmux context:** run the built binary inside another PTY wrapper so
  ...
  `script -qe -c './target/debug/rook-tui-oss' /tmp/tui.log`, then read
...
`cargo build -p rook_tui --bin rook-tui-oss` → `tmux kill-session` + restart the
...
- **If it exits (code 101) right after the first frame instead of staying up:**
  ...
  `tail -40 ~/.local/state/rook-terminal-tui/oz/rook-tui.log`. One cause seen in
  the headless OSS/logged-out sandbox build is a debug-only binding-validation
  panic: `crates/rookui_core/src/keymap/matcher.rs` (`validate_bindings`, gated on
  ...
  *keystroke* binding matches a TUI keymap context without being TUI-owned (it was
  ...
  - **Build `--release`** — the validator is compiled out, so the TUI stays up
    ...
    `cargo build --release -p rook_tui --bin rook-tui-oss` then run
    `./target/release/rook-tui-oss`.
...
transcoding it to an **MP4** (the same format Rook's `computer_use` screen
...
tmux new-session -d -s tuicap -x 120 -y 40 \
  'asciinema rec --overwrite -c "./target/debug/rook-tui-oss" /tmp/tui.cast'
...
For a **logged-in** capture, build/run `rook-tui-dev` with `ROOK_API_KEY` per
Step 1 instead of `rook-tui-oss`.

**Render the video (MP4).** Match the format Rook's `computer_use` screen
...
- **Don't take the still from the first or last video frame.** The first frame
  ...
  OSS `ROOK_API_KEY ... IGNORED` startup warning), **not** the TUI surface.
...
test (`rookui_core::elements::tui::test_support::render_to_lines` /
`TuiBuffer::to_lines`) per `tui-testing`, and run:

```bash
cargo nextest run -p rook_tui
cargo nextest run -p rookui_core
```
