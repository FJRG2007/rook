...
Use this skill when working with Rook's integration test recording pipeline on this branch.
...
Rook app or a live UI flow. For those, use the **computer use** tool, which
...
```bash
ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
...
If you want the driver to auto-record a test or set of tests, add `ROOK_INTEGRATION_TEST_VIDEO`:

```bash
ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
ROOK_INTEGRATION_TEST_VIDEO=test_video_recording \
cargo run -p integration --bin integration -- test_video_recording
```
...
```bash
ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
ROOK_INTEGRATION_TEST_VIDEO=test_foo,test_bar \
cargo nextest run --no-fail-fast --workspace test_foo
```

## Environment variables

### `ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS`
...
ROOK_INTEGRATION_TEST_VIDEO=all
```
...
Important nuance:
- You do not need `ROOK_INTEGRATION_TEST_VIDEO` if the test itself explicitly calls `with_start_recording()` and `with_stop_recording()`.
- Use the env var when you want whole-test recording without changing the test code.

### `ROOK_INTEGRATION_TEST_ARTIFACTS_DIR`
...
```text
$TMPDIR/rook_integration_test_artifacts
```
...
### `ROOK_INTEGRATION_TEST_VIDEO_DIR`
This env var exists in `ui/src/integration/video_recorder.rs` as the lower-level recorder output root helper, defaulting to:

```text
$TMPDIR/rook_integration_video_captures
```

On this branch, the normal integration driver flow writes the finalized video into the test artifacts directory instead, so `ROOK_INTEGRATION_TEST_ARTIFACTS_DIR` is the one you usually care about when reviewing results.
...
Set `ROOK_INTEGRATION_TEST_VIDEO` to:
- `all`
- `1`
...
```text
${ROOK_INTEGRATION_TEST_ARTIFACTS_DIR:-$TMPDIR/rook_integration_test_artifacts}/<test_name>/<timestamp>/
```
...
2. Decide whether recording should be explicit in the test or enabled via `ROOK_INTEGRATION_TEST_VIDEO`.
3. Ensure the run uses `ROOKUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1`.
