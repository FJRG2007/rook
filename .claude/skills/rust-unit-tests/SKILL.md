...
description: Write, improve, and run Rust unit tests in the rook Rust codebase.
---

# Rust Unit Tests in rook
...
- Prefer `rookui::App::test` for deterministic unit tests around views/models.
- Initialize app models once, then mutate via `update` and assert via `read`.

```rust
use rookui::App;
// In app crate tests prefer `crate::test_util::...`; from other crates use `rook::test_util::...`.
use rook::test_util::{terminal::initialize_app_for_terminal_view, add_window_with_terminal};
...
## TUI element tests
Tests for the headless TUI render an element tree to text lines rather than drawing pixels. Use `rookui_core::elements::tui::test_support::render_to_lines` and `TuiBuffer::to_lines`, and keep them in `*_tests.rs` files next to the source in `crates/rook_tui` and `crates/rookui_core/src/elements/tui`. They are plain unit tests and do NOT use the GUI integration / real-display / `computer_use` framework. See the `tui-testing` skill for details. The `rookui::App::test` harness above still applies to shared model logic that both front-ends use.
...
```rust
use rook::features::FeatureFlag; // or `use crate::features::FeatureFlag;` inside the app crate
