...
- **TUI** — User-visible changes that impact the Rook Agent CLI/headless TUI, including shared Agent capabilities.
...
- For stable releases, also use the PR title/body and TUI-owned paths such as `crates/rook_tui` or `crates/rookui_core/src/elements/tui`. Shared capabilities such as Agent tool-call or edit-file behavior also impact TUI when Rook Agent CLI users observe the change. If metadata is ambiguous, inspect the commit diff.
- Do not infer TUI impact from file paths alone; confirm that Rook Agent CLI users observe the changed behavior.
